...
sealed class SegmentedAllByteBufReader implements ByteBufReader permits SegmentedCommitsByteBufReader {
    ...
    public <T> T tryNext(final EntryMapper<T> mapper) {
        ...
        return bytes == null ? null : mapper.mapEntry(index, bytes);
    }

    // Attempts to read the next extry. 'index' here is really 'nextIndex' passed down by callers, which already check
    // it for invariants. If non-null is returned, 'nextIndex' has already been set to 'index' + 1.
    private ByteBuf tryAdvance(final long index) {
