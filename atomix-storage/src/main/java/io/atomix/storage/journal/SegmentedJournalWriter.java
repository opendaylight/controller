...
final class SegmentedJournalWriter<E> implements JournalWriter<E> {
    ...
    public Indexed<E> getLastEntry() {
        final var bytes = byteJournalWriter.lastWritten();
        return bytes == null ? null
            : new Indexed<>(byteJournalWriter.lastIndex(), serializer.deserialize(bytes), bytes.length);
    }
    ...
    public <T extends E> Indexed<T> append(T entry) {
        final var bytes = serializer.serialize(entry);
        return new Indexed<>(byteJournalWriter.append(bytes), entry, bytes.length);
    }
