...
final class JournalSegmentWriter {
    ...
    <T> @Nullable Integer append(final ByteBufMapper<T> mapper, final T entry) {
        ...
        // Allocate entry space and attempt to serialize.
        final var diskEntry = fileWriter.startWrite(position, writeLimit + HEADER_BYTES);
        final var bytes = Unpooled.wrappedBuffer(diskEntry.slice(HEADER_BYTES, writeLimit));
        try {
        ...
        // Determine length and compute the corresponding checksum
        final var length = bytes.readableBytes();
        final var checksum = SegmentEntry.computeChecksum(diskEntry.slice(HEADER_BYTES, length));

        // update the header, adjust limit and commit entry to file
        fileWriter.commitWrite(position,
            diskEntry.putInt(0, length).putInt(Integer.BYTES, checksum).limit(HEADER_BYTES + length));
