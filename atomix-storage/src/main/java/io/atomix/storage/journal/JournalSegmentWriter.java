...
final class JournalSegmentWriter {
    ...
    private void reset(final long maxNextIndex, final @Nullable Position position) {
        ...
        final var fileReader = fileWriter.reader();
        try {
            reset(fileReader, fileWriter.maxEntrySize(), maxNextIndex, position);
        } finally {
            // Make sure reader does not see anything we've done
            fileReader.invalidateCache();
        }
    ...
    private void reset(final FileReader fileReader, final int maxEntrySize, final long maxNextIndex,
            final @Nullable Position position) {
        long nextIndex;
        if (position != null) {
