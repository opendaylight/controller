...
final class SegmentedJournalReader<E> implements JournalReader<E> {
    ...
    public final Indexed<E> getCurrentEntry() {
        final var bytes =  byteJournalReader.lastRead();
        return bytes == null ? null : indexed(byteJournalReader.lastIndex(), bytes);
    }
    ...
    public Indexed<E> tryNext() {
        final var bytes = byteJournalReader.tryNext();
        return bytes == null ? null : indexed(byteJournalReader.lastIndex(), bytes);
    }
    ...
    private Indexed<E> indexed(final long index, final byte[] bytes) {
        return new Indexed<>(index, serializer.deserialize(bytes), bytes.length);
    }
}
