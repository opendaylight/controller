 ...
 */
final class MappedByteBuf extends AbstractReferenceCountedByteBuf {
    private final ByteBufAllocator alloc;

    private ByteBuffer byteBuffer;
    private ByteBuffer internalNio;

    MappedByteBuf(final ByteBufAllocator alloc, final ByteBuffer byteBuffer) {
        super(byteBuffer.limit());
        this.alloc = requireNonNull(alloc);
        this.byteBuffer = requireNonNull(byteBuffer);
    }

    static MappedByteBuf fromFileChannel(final FileChannel fileChannel, final int capacity) throws IOException {
        return new MappedByteBuf(UnpooledByteBufAllocator.DEFAULT,
            fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, capacity));
    }
    ...
    public void force() {
        if (byteBuffer instanceof MappedByteBuffer mappedByteBuffer){
