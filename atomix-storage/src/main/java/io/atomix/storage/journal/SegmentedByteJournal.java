...
/**
 * A {@link ByteJournal} Implementation.
 */
public final class SegmentedByteJournal implements ByteJournal {

    private static final Logger LOG = LoggerFactory.getLogger(SegmentedByteJournal.class);
    ...
    private final ByteJournalWriter writer;
    private volatile long commitIndex;

    private final ConcurrentNavigableMap<Long, JournalSegment> segments = new ConcurrentSkipListMap<>();
    private final Collection<ByteJournalReader> readers = ConcurrentHashMap.newKeySet();

    private JournalSegment currentSegment;
    private volatile boolean open = true;

    public SegmentedByteJournal(final String name, final StorageLevel storageLevel, final File directory,
            final int maxSegmentSize, final int maxEntrySize, final double indexDensity, boolean flushOnCommit) {
        ...
        open();
        writer = new SegmentedByteJournalWriter(this);
    }
     ...
     * <p>
     * The storage directory is the directory to which all segment files are written. Segment files for multiple
     * journals can be stored in same directory, identified by journal name being used as filename prefix.
     *
    ...
    @Override
    public ByteJournalWriter writer() {
        return writer;
    }

    @Override
    public ByteJournalReader newReader(final long index) {
        return newReader(index, JournalReader.Mode.ALL);
    }
     ...
     */
    @Override
    public ByteJournalReader newReader(final long index, final JournalReader.Mode mode) {
        final var segment = segment(index);
        final var reader = switch (mode) {
            case ALL -> new SegmentedByteJournalReader(this, segment);
            case COMMITS -> new SegmentedByteJournalReader.Commits(this, segment);
        };

        while (index > reader.nextIndex() && reader.tryNext() != null) {
            // repeat till reader index matches requested
        }
        readers.add(reader);
        return reader;
    }
    ...
    private synchronized void open() {
        ...
        if (!segments.isEmpty()) {
            ...
            final var descriptor = JournalSegmentDescriptor.builder()
                .withId(1).withIndex(1).withMaxSegmentSize(maxSegmentSize).build();
    ...
    JournalSegment resetSegments(long index) {
        ...
        final var descriptor = JournalSegmentDescriptor.builder()
            .withId(1).withIndex(index).withMaxSegmentSize(maxSegmentSize).build();
        currentSegment = createSegment(descriptor);
    ...
    synchronized JournalSegment nextSegment() {
        ...
        final var descriptor = JournalSegmentDescriptor.builder()
            ...
            .withMaxSegmentSize(maxSegmentSize).build();
    ...
    synchronized JournalSegment segment(final long index) {
        ...
        final var floorEntry = segments.floorEntry(index);
        if (floorEntry != null) {
    ...
    JournalSegment createSegment(final JournalSegmentDescriptor descriptor) {
        ...
        try (var raf = new RandomAccessFile(segmentFile, "rw"); var channel = raf.getChannel()) {
     ...
     */
    protected JournalSegment newSegment(final JournalSegmentFile segmentFile,
    ...
    private JournalSegment loadSegment(final long segmentId) {
        final var segmentFile = JournalSegmentFile.createSegmentFile(name, directory, segmentId);
        final var buffer = ByteBuffer.allocate(JournalSegmentDescriptor.BYTES);
        try (var channel = openChannel(segmentFile)) {
            channel.read(buffer);
            buffer.flip();
            final var descriptor = new JournalSegmentDescriptor(buffer);
            final var segment = newSegment(new JournalSegmentFile(segmentFile), descriptor);
    ...
    void closeReader(final SegmentedByteJournalReader reader) {
    ...
    public static final class Builder {
        ...
        /**
         * Build the {@link SegmentedByteJournal}.
         *
         * @return A new {@link SegmentedByteJournal}.
         */
        public SegmentedByteJournal build() {
            return new SegmentedByteJournal(name, storageLevel, directory, maxSegmentSize, maxEntrySize,
