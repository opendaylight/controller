...
public class JournalSerdesImpl implements JournalSerdes {
    ...
    public <T> T deserialize(ByteBuffer buffer) {
        byte id = buffer.get();
        var type = findRegisteredType(id);
        byte legacyTag = buffer.get();
        ...
        if (legacyTag == 2) {
            try (var byteBufferBackedInputStream = new ByteBufferBackedInputStream(buffer)) {
                try (ObjectInputStream o = new ObjectInputStream(byteBufferBackedInputStream)) {
                    var tmp = (T) type.serializer().read(new EntryInputImpl(o));
                    return tmp;
                }
            } catch (IOException e) {
                throw new IllegalStateException(e);
    ...
    public <T> T deserialize(InputStream stream, int bufferSize) {
        ...
        try {
            stream.read(header);
            var type = findRegisteredType(header[0]);
    ...
    private RegisteredType findRegisteredType(byte id) {
        for (var type : registeredTypes) {
            if (type.id() == id) {
                return type;
            }
