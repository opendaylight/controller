...
/**
 * A {@link ClassLoader} which delegates to a set of {@link PekkoAccessibleClasses}.
 */
@Component(service = PekkoAccessibleClassLoader.class, immediate = true)
public final class PekkoAccessibleClassLoader extends ClassLoader {
    ...
    private PekkoAccessibleClassLoader(final Stream<PekkoAccessibleClasses> accessibleClasses) {
        super(UUID.randomUUID().toString(), ActorSystem.class.getClassLoader());
    ...
    public PekkoAccessibleClassLoader(
        ...
        LOG.info("Actor class loader {} started with {} delegate loader(s)", getName(), loaders.size());
    }
    ...
    @Deactivate
    void deactivate() {
        LOG.info("Actor class loader {} stopped", getName());
    }
