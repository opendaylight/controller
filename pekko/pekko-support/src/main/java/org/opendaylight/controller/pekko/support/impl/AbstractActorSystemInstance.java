 ...
 */
public abstract sealed class AbstractActorSystemInstance implements ActorSystemInstance.WithShutdown
        permits DefaultActorSystemInstance, OSGiActorSystemInstance {
    private static final Logger LOG = LoggerFactory.getLogger(AbstractActorSystemInstance.class);
    private static final VarHandle VH;

    static {
        try {
            VH = MethodHandles.lookup()
                .findVarHandle(AbstractActorSystemInstance.class, "terminationMonitor", ActorRef.class);
     ...
     */
    @NonNullByDefault
    AbstractActorSystemInstance(final String name, final Config rawConfig, final ActorSystemCreator.Callbacks callbacks,
            final PekkoAccessibleClassLoader classLoader) {
        if (name.isBlank()) {
            throw new IllegalArgumentException("blank name");
        }
        requireNonNull(callbacks);
        final var config = ConfigFactory.load(ActorSystem.class.getClassLoader(),
            rawConfig.getConfig("odl-cluster-data"));
    ...
    @Override
    public final void shutdownAndWait(final Duration atMost) throws InterruptedException, TimeoutException {
