...
public abstract non-sealed class Request<T extends WritableIdentifier, C extends Request<T, C>> extends Message<T, C> {
    protected non-sealed interface SerialForm<T extends WritableIdentifier, C extends Request<T, C>>
        ...
        default void writeExternal(final ObjectOutput out, final C msg) throws IOException {
            out.writeObject(Serialization.serializedActorPath(msg.replyTo()));
        }
    }
    ...
    protected Request(final @NonNull C request) {
        super(request);
        replyTo = requireNonNull(request.replyTo());
    }
     ...
     */
    public final @NonNull ActorRef replyTo() {
        return replyTo;
    }
