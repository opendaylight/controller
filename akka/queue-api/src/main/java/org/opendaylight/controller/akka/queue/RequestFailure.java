...
public abstract non-sealed class RequestFailure<T extends WritableIdentifier, C extends RequestFailure<T, C>>
     ...
     * @param <T> Target identifier type
     */
    protected non-sealed interface SerialForm<T extends WritableIdentifier, C extends RequestFailure<T, C>>
    ...
    protected RequestFailure(final @NonNull C failure) {
        super(failure);
        cause = requireNonNull(failure.cause());
    }
     ...
     */
    public final @NonNull RequestException cause() {
        return cause;
    }
