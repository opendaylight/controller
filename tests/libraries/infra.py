...
def shell(
    ...
    Args:
        ...
        run_in_background (bool): If the command should be started as background
            process without tty.
        timeout (int): Timeout in seconds for the foreground command.

    Returns:
        tuple[int, str] | subprocess.Popen :
            For foreground process it returns final return code and stdout,
            for background process it returns process handler.
    """
...
def execute_karaf_command(command: str) -> tuple[str, str]:
    """Executes specific command using ODL karaf CLI console

    It uses ssh connection to connect to karaf CLI.
...
def log_message_to_karaf(message: str):
    """Log specific message to ODL karaf
