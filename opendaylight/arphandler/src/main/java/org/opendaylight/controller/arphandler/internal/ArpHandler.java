...
public class ArpHandler implements IHostFinder, IListenDataPacket {
    ...
    protected void handleARPPacket(Ethernet eHeader, ARP pkt, NodeConnector p) {
		...
		/*
		 * Gratuitous ARP. If there are hosts (in arpRequestors) waiting for the ARP reply for this
		 * sourceIP, it's time to generate the reply and it to these hosts
		 */
        ...
        /*
         * ARP Reply. If there are hosts (in arpRequesttors) waiting for the ARP reply for this
		 * sourceIP, it's time to generate the reply and it to these hosts
         */
        ...
        if (host == null) {
        	// add the requestor to the list so that we can replay the reply when the host responds
        	if (requestor != null) {
        		...
        		if ((requestorSet == null) || requestorSet.isEmpty()) {
        			...
        			countDownTimers.put(targetIP, (short)2); // set max timeout to 2sec
        		}
