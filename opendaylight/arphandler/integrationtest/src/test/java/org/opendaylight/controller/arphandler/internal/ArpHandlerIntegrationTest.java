...
public class ArpHandlerIntegrationTest {
    private Logger log = LoggerFactory.getLogger(ArpHandlerIntegrationTest.class);
    ...
    public Option[] config() {
        return options(
                ...
                mavenBundle("org.opendaylight.controller", "arphandler",
                        "0.4.0-SNAPSHOT"),
                mavenBundle("org.opendaylight.controller", "arphandler.implementation",
                       "0.4.0-SNAPSHOT"),
    ...
    public void areWeReady() {
        ...
        assertFalse(debugit);
         ...
         // If ARPHandler is not found, we can't proceed
         s = bc.getServiceReference(IArpHandler.class.getName());
         if (s == null) {
        	 // as instance of IHostFinder?
        	 s = bc.getServiceReference(IHostFinder.class.getName());
         }
         if (s != null) {
    ...
    @Test
    public void testArphandlerBasic() {
        
    	IfIptoHost origHt = arpHandler.getHostTracker();
    	arpHandler.unsetHostTracker(origHt);
    	assertTrue("ArpHandler failed unsetHostTracker()", arpHandler.getHostTracker() == null);
    	arpHandler.setHostTracker(hostTracker);
        assertNotNull("IArpHandler setHostTracker() returns null", arpHandler.getHostTracker());
    	assertEquals(hostTracker, arpHandler.getHostTracker());

		
    	arpHandler.setHostListener(hostListener);
    	arpHandler.unsetHostListener(hostListener);
		
		arpHandler.setSwitchManager(switchManager);
		arpHandler.unsetSwitchManager(switchManager);
    ...
    public void testFindProbe() {
        ...
        // The non-getter/setter methods fit for integration test are
    	// as follows:
    	try {
    	...
    	// The functions below are believed to be tested within the
    	// HostTracker already.
        // IArpHandler.probe(): HostTracker->ARPRefreshHandler.run().
        // IArpHandler.find():  HostTracker->findHost()
