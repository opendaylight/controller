...
public class FeaturesServiceNorthbound {
    ...
    @Path("/installed")
    @Produces(MediaType.APPLICATION_JSON)
    public List<Feature> installedFeatures() throws Exception{
        List<Feature> list = new ArrayList<Feature>();
        for(org.apache.karaf.features.Feature f : featuresService.listInstalledFeatures()) {
            list.add(toFeature(f));
        }
        return list;
    }
    ...
    private Feature toFeature(org.apache.karaf.features.Feature feature) {
