...
/*package*/ class TypeInfo {
    ...
    public final String getTypeName(Field f) {
        ...
        //check accessType
        if (_accessType == XmlAccessType.NONE) {
          return name;
        }
        // name is null and element is not transient
        if(name == null && f.getAnnotation(XmlTransient.class)==null) {
          //for public and field level access
          if (_accessType == XmlAccessType.FIELD) {
            return f.getName();
          }else if (_accessType == XmlAccessType.PUBLIC_MEMBER && Modifier.isPublic(f.getModifiers())) {
            return f.getName();
          }
        }
        return name;
    ...
    public final String getTypeName(Method m) {
        ...
        // name is null and element is not transient
        if(name == null && m.getAnnotation(XmlTransient.class)==null) {
          //for public and field level access
          if (_accessType == XmlAccessType.PROPERTY) {
            return m.getName();
          }else if (_accessType == XmlAccessType.PUBLIC_MEMBER && Modifier.isPublic(m.getModifiers())) {
            return m.getName();
          }
        }
        return name;
