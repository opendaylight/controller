...
public class ExpresssionTest {
  ...
  private static final List<BookBean> books = new ArrayList<BookBean>();
  ...
  public Expression parseQuery(String query) throws Exception {
    ...
    Expression exp = FiqlParser.parse(query, null);
    p(exp.toString());
    return exp;
  }
  ...
  public int find(String query) throws Exception {
    ...
    TypeInfo personType = TypeInfo.createRoot("person", PersonBean.class);
  ...
  public void testTypeTree() throws Exception {
    ...
    Assert.assertEquals("cool", bookType.retrieve(books.get(0),
  ...
  public void testQueryAPI() throws Exception {
    ...
    Collection<BookBean> r1 = q1.filter(books);
    p("Filtered books: " + r1.size());
    Assert.assertEquals(1, r1.size());

    // find all books reviewed by people in a city "San*"
    Query q2 = qc.createQuery("book.reviews.reviewer.city=San.*", BookBean.class);
    Collection<BookBean> r2 = q2.filter(books);
