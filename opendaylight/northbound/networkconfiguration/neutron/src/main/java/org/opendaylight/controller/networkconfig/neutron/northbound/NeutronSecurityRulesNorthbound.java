...
public class NeutronSecurityRulesNorthbound {
    ...
    public Response listRules(
        ...
        //TODO: pagination, see Giovannis recent commit
    ...
    public Response createSecurityRules(final NeutronSecurityRuleRequest input) {
        ...
        /*
         * Security Rule are checked for the following:
         * 1. ID can't already exist.
         * 2. The Security GroupID exists to be associated to.
        */

        if (input.isSingleton()) {
            NeutronSecurityRule singleton = input.getSingleton();

            String securityRuleGroupID = singleton.getSecurityRuleGroupID();
            ...
            if (securityRuleGroupID != null  ) {
                throw new BadRequestException("The Security Rule does not contain " +
                        "a Security Group ID to associate with");
            }
            if (!securityGroupInterface.neutronSecurityGroupExists(securityRuleGroupID))
                throw new BadRequestException("The Security Rule's GroupID does not Exist");
