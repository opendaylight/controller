...
public class NeutronLoadBalancerPoolNorthbound {
    ...
    public Response deleteLoadBalancerPool(
        ...
        if (loadBalancerPoolInterface.neutronLoadBalancerPoolInUse(loadBalancerPoolID)) {
            return Response.status(409).build();
        }
        NeutronLoadBalancerPool singleton = loadBalancerPoolInterface.getNeutronLoadBalancerPool(loadBalancerPoolID);
        Object[] instances = ServiceHelper.getGlobalInstances(INeutronLoadBalancerPoolAware.class, this, null);
        if (instances != null) {
            for (Object instance : instances) {
                INeutronLoadBalancerPoolAware service = (INeutronLoadBalancerPoolAware) instance;
        ...
        loadBalancerPoolInterface.removeNeutronLoadBalancerPool(loadBalancerPoolID);
        if (instances != null) {
            for (Object instance : instances) {
                INeutronLoadBalancerPoolAware service = (INeutronLoadBalancerPoolAware) instance;
    ...
    /**
     * Adds a Member to an LBaaS Pool
     */
    @Path("{loadBalancerPoolID}/members")
    @PUT
    @Produces({ MediaType.APPLICATION_JSON })
    @Consumes({ MediaType.APPLICATION_JSON })
    @StatusCodes({
    ...
    public Response addPoolMember(
            @PathParam("loadBalancerPoolID") String poolUUID,
            NeutronLoadBalancerPoolMember input
    ) {
        // TODO: Complete Add Member to Pool
        return Response.status(200).entity(input).build();

    }

    /**
     * Removes a member from an LBaaS pool
     */
    @Path("{loadBalancerPoolID}/members")
    @PUT
    @Produces({ MediaType.APPLICATION_JSON })
