 ...
 *
 * NOTE: These API's need to be synchronized due to bug 1198. Thread access to the SchemaContext
 * is not synchronized properly and thus you can end up with missing definitions without this
 * synchronization. There are likely otherways to work around this limitation, but given that this
 * API is a dev only tool and not dependent UI, this was the fastest work around.
 *
 */
public class ApiDocServiceImpl implements ApiDocService {
    ...
    public synchronized  Response getListOfMounts(UriInfo uriInfo) {
        ...
        try( OutputStreamWriter streamWriter = new OutputStreamWriter( baos ) ){
            ...
            for( Entry<String, Long> entry :
                                MountPointSwagger.getInstance().getInstanceIdentifiers().entrySet() ){
                writer.object();
    ...
    public synchronized  Response getMountRootDoc(String instanceNum, UriInfo uriInfo) {
        ResourceList resourceList =
                MountPointSwagger.getInstance().getResourceList( uriInfo, Long.parseLong( instanceNum ) );
        return Response.ok( resourceList ).build();
    }

    @Override
    public synchronized  Response getMountDocByModule(String instanceNum, String module, String revision,
            UriInfo uriInfo) {
        ApiDeclaration api = MountPointSwagger.getInstance().
                               getMountPointApi( uriInfo,Long.parseLong( instanceNum ), module, revision );
