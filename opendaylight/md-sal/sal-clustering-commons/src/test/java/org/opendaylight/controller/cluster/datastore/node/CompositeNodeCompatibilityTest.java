...
public class CompositeNodeCompatibilityTest extends TestCase {
    ...
    public void printSchema(DataSchemaNode schemaNode, int level) {
        if (schemaNode instanceof DataNodeContainer) {
            DataNodeContainer container = (DataNodeContainer) schemaNode;
            for (DataSchemaNode data : container.getChildNodes()) {
                printSchema(data, level + 1);
            }
        } else if (schemaNode instanceof LeafListSchemaNode) {
            System.out.println(
                spaces(level) + " + " + ((LeafListSchemaNode) schemaNode)
                    .getType());
        } else if (schemaNode instanceof LeafSchemaNode) {
            System.out.println(
                spaces(level) + " + " + ((LeafSchemaNode) schemaNode)
                    .getType());
        }
