...
public class NormalizedNodeInputStreamReader implements NormalizedNodeDataInput {
    ...
    private void streamNormalizedNode(final NormalizedNodeStreamWriter writer, final byte nodeType) throws IOException {
        switch (nodeType) {
            ...
            case NodeTypes.UNKEYED_LIST_ITEM:
                ...
                break;
            default:
                throw new InvalidNormalizedNodeStreamException("Unexpected node %d", nodeType);
        }
    }
    ...
    private void streamAugmentation(final NormalizedNodeStreamWriter writer) throws IOException {
        final AugmentationIdentifier augIdentifier = new AugmentationIdentifier(readQNameSet());
    ...
    private void readSignatureMarkerAndVersionIfNeeded() throws IOException {
        if (readSignatureMarker) {
            ...
            if (marker != TokenTypes.SIGNATURE_MARKER) {
                throw new InvalidNormalizedNodeStreamException("Invalid signature marker: %d", marker);
            }

            final short version = input.readShort();
            if (version != TokenTypes.LITHIUM_VERSION) {
                throw new InvalidNormalizedNodeStreamException("Unhandled stream version %s", version);
            }
        }
    }
