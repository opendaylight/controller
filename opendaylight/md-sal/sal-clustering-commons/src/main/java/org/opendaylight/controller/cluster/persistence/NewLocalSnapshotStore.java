...
public class NewLocalSnapshotStore {
    ...
    public ListenableFuture<Optional<SnapshotOffer>> doLoadAsync(final String persistenceId,
        ...
        final var metadatas = getSnapshotMetadatas(persistenceId, criteria).stream()
    ...
    private Optional<SnapshotOffer> doLoad(final Deque<SnapshotMetadata> metadatas) throws IOException {
        final var snapshotMetadata = metadatas.removeFirst();
        final var file = toSnapshotFile(snapshotMetadata);
        LOG.debug("doLoad {}", file);

        try {
            final var data = deserialize(file);
            LOG.debug("deserialized data: {}", data);
            return Optional.of(new SnapshotOffer(snapshotMetadata, data));
        } catch (IOException e) {
    ...
    private Object deserialize(final File file) throws IOException {
        try (var in = new ObjectInputStream(streamFactory.createInputStream(file))) {
    ...
    private SnapshotMetadata doSave(final SnapshotMetadata metadata, final Object snapshot) throws IOException {
        final var actual = toSnapshotFile(metadata);
        final var temp = File.createTempFile(actual.getName(), null, snapshotDir);
    ...
    private static @Nullable SnapshotMetadata extractMetadata(final File file) {
        ...
        final int sequenceNumberEndIndex = name.lastIndexOf('-');
        final int persistenceIdEndIndex = name.lastIndexOf('-', sequenceNumberEndIndex - 1);
        ...
        try {
            ...
            final var persistenceId = decode(name.substring(PERSISTENCE_ID_START_INDEX, persistenceIdEndIndex));
            final long sequenceNumber = Long.parseLong(name
                .substring(persistenceIdEndIndex + 1, sequenceNumberEndIndex));
            final long timestamp = Long.parseLong(name.substring(sequenceNumberEndIndex + 1));
