...
/**
 * Dictionary for use with {@link DictionaryNormalizedNodeDataInput}s. An instance can be owned by a single input at
 * any given time.
 */
@NotThreadSafe
public final class NormalizedNodeInputDictionary extends AbstractNormalizedNodeDictionary {
    private final Map<Integer, String> strings = new HashMap<>();
    private final Map<Integer, QName> qnames = new HashMap<>();

    NormalizedNodeInputDictionary() {
        // Hidden on purpose
    }
    ...
    void storeQName(final QName qname) {
        ...
        final QName prev = qnames.put(code, qname.intern());
