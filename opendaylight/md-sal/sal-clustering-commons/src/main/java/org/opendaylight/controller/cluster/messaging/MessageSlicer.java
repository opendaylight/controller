...
public class MessageSlicer {
    ...
    public <T extends Serializable> void slice(@Nonnull final Identifier identifier, @Nonnull final T message,
        ...
        try {
            ...
            LOG.error("Error initializing SlicedMessageState for {}", identifier, e);
            if (state != null) {
                state.close();
            } else {
                fileBackedStream.cleanup();
        ...
        synchronized (state) {
            try {
                ...
                if (state.getCurrentSliceIndex() != reply.getSliceIndex()) {
                    LOG.warn("Slice index {} in {} does not match expected index {}", reply.getSliceIndex(), reply,
                ...
                if (state.isLastSlice(reply.getSliceIndex())) {
                    LOG.debug("Received last slice reply for {}", identifier);
    ...
    private void removeState(final Identifier identifier) {
        LOG.debug("Removing state for {}", identifier);
