...
public class MessageSlicer implements AutoCloseable {
    ...
    /**
     * Serializes the given message and slices it into chunks based on the serialized size and the maximum message
     * slice size. If the message doesn't need to be sliced, ie its serialized size is less than the maximum message
     * slice size, then the original message is sent. Otherwise the first message slice is sent.
     *
     * @param identifier the identifier of the message
     * @param message the message to slice
     * @param sendTo the reference of the actor to which to send the message slices
     * @param replyTo the reference of the actor to which to message slice replies should be sent. The actor should
     *                forward the replies to the {@link #handleMessage(Object)} method
     * @param onFailureCallback the callback to be notified of failure
     */
    public <T extends Serializable> void slice(@Nonnull final Identifier identifier, @Nonnull final T message,
        ...
        sliceInternal(identifier, fileBackedStream, Optional.of(message), sendTo, replyTo, onFailureCallback);
    }

    /**
     * Slices the serialized data from the given output stream into chunks based on the serialized size and the maximum
     * message slice size.
     *
     * @param identifier the identifier of the message
     * @param fileBackedStream the {@link FileBackedOutputStream} containing the data to slice
     * @param sendTo the reference of the actor to which to send the message slices
     * @param replyTo the reference of the actor to which to message slice replies should be sent. The actor should
     *                forward the replies to the {@link #handleMessage(Object)} method
     * @param onFailureCallback the callback to be notified of failure
     */
    public <T extends Serializable> void slice(@Nonnull final Identifier identifier,
            @Nonnull final FileBackedOutputStream fileBackedStream, @Nonnull final ActorRef sendTo,
            @Nonnull final ActorRef replyTo, @Nonnull final Consumer<Throwable> onFailureCallback) {
        LOG.debug("{}: slice: identifier: {}", logContext, identifier);
        sliceInternal(identifier, fileBackedStream, Optional.empty(), sendTo, replyTo, onFailureCallback);
    }

    private void sliceInternal(final Identifier identifier, final FileBackedOutputStream fileBackedStream,
            final Optional<Serializable> originalMessage, final ActorRef sendTo, final ActorRef replyTo,
            final Consumer<Throwable> onFailureCallback) {
        ...
        try {
            state = new SlicedMessageState<>(messageSliceId, fileBackedStream, messageSliceSize, maxSlicingTries,
                    replyTo, onFailureCallback, logContext);

            if (state.getTotalSlices() == 1 && originalMessage.isPresent()) {
                LOG.debug("{}: Message does not need to be sliced - sending original message", logContext);
                state.close();
                sendTo.tell(originalMessage.get(), replyTo);
                return;
            }
    ...
    public static class Builder {
         ...
         * is used by the {@link MessageSlicer#slice(Identifier, Serializable, ActorRef, ActorRef, Consumer)} method.
         * If this method isn't called then the factory need not be set.
         *
