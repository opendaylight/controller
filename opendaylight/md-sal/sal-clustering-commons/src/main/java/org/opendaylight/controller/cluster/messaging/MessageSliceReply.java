...
class MessageSliceReply implements Serializable {
    ...
    private MessageSliceReply(final Identifier identifier, final int sliceIndex, final ActorRef sendTo,
            final Throwable failure, final boolean isRetriable) {
    ...
    static MessageSliceReply success(final Identifier identifier, final int sliceIndex, final ActorRef sendTo) {
        return new MessageSliceReply(identifier, sliceIndex, sendTo, null, false);
    }

    static MessageSliceReply failed(final Identifier identifier, final ActorRef sendTo,
            final Throwable failure, final boolean isRetriable) {
        return new MessageSliceReply(identifier, -1, sendTo, failure, isRetriable);
    }
    ...
    public String toString() {
        return "MessageSliceReply [identifier=" + identifier + ", sliceIndex=" + sliceIndex + ", failure=" + failure
                + ", isRetriable=" + isRetriable + ", sendTo=" + sendTo + "]";
    }
    ...
    private static class Proxy implements Externalizable {
        ...
        public void writeExternal(ObjectOutput out) throws IOException {
            ...
            out.writeBoolean(messageSliceReply.isRetriable);
        ...
        public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
            ...
            final Throwable failure = (Throwable) in.readObject();
            final boolean isRetriable = in.readBoolean();
            ActorRef sendTo = JavaSerializer.currentSystem().value().provider()
                    .resolveActorRef((String) in.readObject());

            messageSliceReply = new MessageSliceReply(identifier, sliceIndex, sendTo, failure, isRetriable);
        }
