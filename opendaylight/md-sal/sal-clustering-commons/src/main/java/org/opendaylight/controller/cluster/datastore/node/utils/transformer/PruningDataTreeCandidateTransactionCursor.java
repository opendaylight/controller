...
public final class PruningDataTreeCandidateTransactionCursor implements DataTreeCandidateTransactionCursor {
    ...
    public PruningDataTreeCandidateTransactionCursor(final DataSchemaContextTree tree) {
        this.pruner = ReusableNormalizedNodePruner.forDataSchemaContext(tree);
    }
    ...
    public void merge(final PathArgument child, final NormalizedNodeStream dataStream) throws IOException {
        ...
        if (schema != null) {
            cursor.merge(child, streamData(schema, dataStream));
        } else {
            skipData(dataStream);
        }
    }
    ...
    public void write(final PathArgument child, final NormalizedNodeStream dataStream) throws IOException {
        final DataSchemaContextNode<?> schema = findChild(child);
        if (schema != null) {
            cursor.write(child, streamData(schema, dataStream));
        } else {
            skipData(dataStream);
        }
    }
    ...
    private @Nullable DataSchemaContextNode<?> findChild(final PathArgument id) {
        ...
        if (current == null) {
            current = pruner.getTree().getRoot();
        }
        return current.getChild(id);
    }

    private NormalizedNode<?, ?> streamData(final DataSchemaContextNode<?> schema,
            final NormalizedNodeStream dataStream) throws IOException {
        pruner.initialize(schema);
        try {
            dataStream.writeTo(pruner);
    ...
    private static void skipData(final NormalizedNodeStream dataStream) throws IOException {
