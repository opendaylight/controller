 ...
 * can prevent OOM in case the actor can't process messages quicker than they arrive. If a queue reaches it's limit, all
 * further messages are sent to DeadLetters (if possible)
 */
public final class SelectivelyBoundedDequeBasedControlAwareMailbox implements MailboxType,
    ...
    private static final String LIMIT_NORMAL_QUEUE_CAPACITY_PROPERTY = "limit-normal-queue-capacity";
    private static final String LIMIT_CONTROL_QUEUE_CAPACITY_PROPERTY = "limit-control-queue-capacity";
    private final Config config;

    public SelectivelyBoundedDequeBasedControlAwareMailbox(final ActorSystem.Settings settings, final Config config) {
        this.config = config;
    }
    ...
    public MessageQueue create(final Option<ActorRef> owner, final Option<ActorSystem> system) {
        ...
        final MessageQueue queue = new MessageQueue(config, system);
    ...
    static class MessageQueue extends UnboundedControlAwareMailbox.MessageQueue
        ...
        MessageQueue(final Config config, final Option<ActorSystem> system) {
            if (system.nonEmpty()) {
                deadLetters = system.get().deadLetters();
            } else {
                deadLetters = null;
            }
            if (config.hasPath(LIMIT_CONTROL_QUEUE_CAPACITY_PROPERTY)) {
                int capacity = config.getInt(LIMIT_CONTROL_QUEUE_CAPACITY_PROPERTY);
                this.maxControlQueueCapacity = capacity > 0 ? capacity : NO_CAPACITY_LIMIT;
            } else {
                this.maxControlQueueCapacity = NO_CAPACITY_LIMIT;
            }
            if (config.hasPath(LIMIT_NORMAL_QUEUE_CAPACITY_PROPERTY)) {
                int capacity = config.getInt(LIMIT_NORMAL_QUEUE_CAPACITY_PROPERTY);
        ...
        public void enqueueFirst(final ActorRef actor, final Envelope envelope) {
            ...
            if (message instanceof ControlMessage) {
                if (maxControlQueueCapacity > NO_CAPACITY_LIMIT) {
                    if (controlQueue.size() >= maxControlQueueCapacity) {
                        LOG.warn("Control MessageQueue full(reached {}), dropping message {}", controlQueue.size(),
                            message.getClass());
                        toDeadLettersIfPossible(actor, envelope);
                        return;
                    }
                }
                controlQueue.addFirst(envelope);
            } else {
                if (maxNormalQueueCapacity > NO_CAPACITY_LIMIT) {
                    if (queue.size() >= maxNormalQueueCapacity) {
                        LOG.warn("Normal MessageQueue full(reached {}), dropping message {}", queue.size(),
                            message.getClass());
                        toDeadLettersIfPossible(actor, envelope);
                        return;
                    }
        ...
        private void toDeadLettersIfPossible(final ActorRef actor, final Envelope envelope) {
            if (deadLetters != null) {
                deadLetters.tell(envelope, actor);
            }
        }
    }
}
