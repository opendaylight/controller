...
import org.opendaylight.controller.cluster.persistence.SegmentedJournalActor.AsyncMessage;
import org.opendaylight.controller.cluster.persistence.SegmentedJournalActor.WriteMessages;
 ...
 * of multiple journals and performs a receptionist job between Akka and invididual per-persistenceId actors. See
...
public class SegmentedFileJournal extends AsyncWriteJournal {
    ...
    private final BiMap<String, ActorRef> handlers = HashBiMap.create();
    ...
    public Future<Iterable<Optional<Exception>>> doAsyncWriteMessages(final Iterable<AtomicWrite> messages) {
        ...
        for (AtomicWrite message : messages) {
            final Iterator<PersistentRepr> it = message.payload().toList().iterator();
            final PersistentRepr first = it.next();
            if (it.hasNext()) {
                result.add(Futures.successful(Optional.of(
                    new UnsupportedOperationException("Cannot write multiple events atomically"))));
                continue;
            }

            final String persistenceId = first.persistenceId();
            final ActorRef handler = handlers.computeIfAbsent(persistenceId, this::createHandler);
            result.add(map.computeIfAbsent(handler, key -> new WriteMessages()).add(first));
        }
    ...
    public Future<Long> doAsyncReadHighestSequenceNr(final String persistenceId, final long fromSequenceNr) {
        ...
        return handler == null ? Futures.successful(0L) : delegateMessage(handler,
