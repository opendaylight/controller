 ...
 */
public abstract class AbstractActorWithPersistenceFailureTracking extends UntypedPersistentActor {
    private static final Logger LOG = LoggerFactory.getLogger(AbstractActorWithPersistenceFailureTracking.class);

    protected final AtomicBoolean backoffSupervised = new AtomicBoolean(false);
    private final AtomicBoolean persistFailed = new AtomicBoolean(false);

    @VisibleForTesting
    void setPersistFailed(final boolean persistFailed) {
        this.persistFailed.set(persistFailed);
    }

    @Override
    public void onRecoveryFailure(final Throwable cause, final Option<Object> event) {
        if (event.isEmpty()) {
            persistFailed.set(true);
        }
