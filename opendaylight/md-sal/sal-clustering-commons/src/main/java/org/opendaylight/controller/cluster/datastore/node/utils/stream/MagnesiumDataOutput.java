...
final class MagnesiumDataOutput extends AbstractNormalizedNodeDataOutput {
    ...
    // Marker for simple values
    private static final Object SIMPLE_STATE = new Object();
    ...
    public void startLeafNode(final NodeIdentifier name) throws IOException {
        ...
        startQNameNode(MagnesiumNode.NODE_LEAF, name);
    }
    ...
    public void startUnkeyedListItem(final NodeIdentifier name, final int childSizeHint) throws IOException {
        // FIXME: improve to take parent into account, skipping encoding if need be
        startQNameNode(MagnesiumNode.NODE_LIST_ENTRY, name);
    }
    ...
    public void startMapEntryNode(final NodeIdentifierWithPredicates identifier, final int childSizeHint)
        ...
        if (size == 1) {
            startQNameNode((byte) (MagnesiumNode.NODE_MAP_ENTRY | MagnesiumNode.PREDICATE_ONE), identifier);
        } else if (size == 0) {
            startQNameNode(MagnesiumNode.NODE_MAP_ENTRY, identifier);
        } else if (size < 256) {
            startQNameNode((byte) (MagnesiumNode.NODE_MAP_ENTRY | MagnesiumNode.PREDICATE_1B), identifier);
            output.writeByte(size);
        } else {
            startQNameNode((byte) (MagnesiumNode.NODE_MAP_ENTRY | MagnesiumNode.PREDICATE_4B), identifier);
    ...
    public void startAnyxmlNode(final NodeIdentifier name) throws IOException {
        startQNameNode(MagnesiumNode.NODE_ANYXML, name);
    }
    ...
    public void endNode() throws IOException {
        final Object finished = stack.pop();
    ...
    private void writePathArgumentQName(final QName qname, final byte typeHeader) throws IOException {
        ...
        if (code != null) {
            ...
            output.writeByte(typeHeader | MagnesiumPathArgument.QNAME_DEF);
            encodeQName(qname);
        }
    }
    ...
    private void writeValue(final byte value) throws IOException {
        switch (value) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.INT8_1);
                break;
            case Byte.MIN_VALUE:
                output.writeByte(MagnesiumValue.INT8_MIN);
                break;
            case Byte.MAX_VALUE:
                output.writeByte(MagnesiumValue.INT8_MAX);
                break;
            default:
    ...
    private void writeValue(final short value) throws IOException {
        switch (value) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.INT16_1);
                break;
            case Short.MIN_VALUE:
                output.writeByte(MagnesiumValue.INT16_MIN);
                break;
            case Short.MAX_VALUE:
                output.writeByte(MagnesiumValue.INT16_MAX);
                break;
            default:
    ...
    private void writeValue(final int value) throws IOException {
        switch (value) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.INT32_1);
                break;
            case Integer.MIN_VALUE:
                output.writeByte(MagnesiumValue.INT32_MIN);
                break;
            case Integer.MAX_VALUE:
                output.writeByte(MagnesiumValue.INT32_MAX);
                break;
            default:
    ...
    private void writeValue(final long value) throws IOException {
        ...
        if (value == 0) {
            ...
            output.writeByte(MagnesiumValue.INT64_1);
        } else if (value == Long.MIN_VALUE) {
            output.writeByte(MagnesiumValue.INT64_MIN);
        } else if (value == Long.MAX_VALUE) {
            output.writeByte(MagnesiumValue.INT64_MAX);
        } else {
    ...
    private void writeValue(final Uint8 value) throws IOException {
        ...
        switch (b) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.UINT8_1);
                break;
            case -1:
                output.writeByte(MagnesiumValue.UINT8_MAX);
                break;
            default:
    ...
    private void writeValue(final Uint16 value) throws IOException {
        ...
        switch (s) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.UINT16_1);
                break;
            case -1:
                output.writeByte(MagnesiumValue.UINT16_MAX);
                break;
            default:
    ...
    private void writeValue(final Uint32 value) throws IOException {
        ...
        switch (i) {
            case 0:
                ...
                break;
            case 1:
                output.writeByte(MagnesiumValue.UINT32_1);
                break;
            case -1:
                output.writeByte(MagnesiumValue.UINT32_MAX);
                break;
            default:
    ...
    private void writeValue(final Uint64 value) throws IOException {
        ...
        if (l == 0) {
            ...
            output.writeByte(MagnesiumValue.UINT64_1);
        } else if (l == -1) {
            output.writeByte(MagnesiumValue.UINT64_MAX);
        } else {
    ...
    private void writeValue(final BigDecimal value) throws IOException {
        output.writeByte(MagnesiumValue.DECIMAL64);
    ...
    private void writeValue(final Set<?> value) throws IOException {
        ...
        output.writeByte(size < 32 ? MagnesiumValue.BITS_0 + size : MagnesiumValue.BITS_32);
    ...
    private void encodeQName(final @NonNull QName qname) throws IOException {
        ...
        if (code == null) {
            ...
            encodeString(module.getRevision().map(Revision::toString).orElse(""));
        } else {
            writeRef(code);
        }
    ...
    private void writeRef(final int code) throws IOException {
        final int val = code;
        if (val < 256) {
            output.writeByte(MagnesiumValue.REF_1B);
            output.writeByte(val);
        } else if (val < 65536) {
            output.writeByte(MagnesiumValue.REF_2B);
            output.writeShort(val);
        } else {
            output.writeByte(MagnesiumValue.REF_4B);
            output.writeInt(val);
        }
    }
}
