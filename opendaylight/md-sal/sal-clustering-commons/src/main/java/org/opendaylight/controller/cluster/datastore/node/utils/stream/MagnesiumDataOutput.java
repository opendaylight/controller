...
final class MagnesiumDataOutput extends AbstractNormalizedNodeDataOutput {
    ...
    // FIXME: do not use Integers here?
    ...
    private void encodeQName(final @NonNull QName qname) {
        // FIXME: interning table and all that
    }

    private void encodeString(final @NonNull String str) {
        final Integer code = stringCodeMap.get(str);
        if (code == null) {
            // FIXME: emit a full string
            return;
        }
