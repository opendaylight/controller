...
public class DDSNormalizedNodeStreamWriter implements NormalizedNodeStreamWriter{
    ...
    private void startNode(final YangInstanceIdentifier.NodeIdentifier name, NodeTypes nodeType) throws IOException {
        Preconditions.checkNotNull(name, "Node identifier should not be null");
        Preconditions.checkNotNull(name.getNodeType(), "Node Type should not be null");
        // First write the type of node
        writer.writeObject(nodeType);
        // Write Start Tag
        writer.writeObject(name.getNodeType());
    }
    ...
    public void leafSetEntryNode(YangInstanceIdentifier.NodeWithValue name, Object value) throws IOException, IllegalArgumentException {
        ...
        writer.writeObject(NodeTypes.LEAF_SET_ENTRY_NODE);
        writer.writeObject(name.getNodeType());
        writer.writeObject(value);
    }
    ...
    public void startMapEntryNode(YangInstanceIdentifier.NodeIdentifierWithPredicates identifier, int childSizeHint) throws IOException, IllegalArgumentException {
        ...
        Preconditions.checkNotNull(identifier.getNodeType(), "Node Type should not be null");

        LOG.debug("Starting a new map entry node");

        writer.writeObject(NodeTypes.MAP_ENTRY_NODE);

        writer.writeObject(identifier.getNodeType());
