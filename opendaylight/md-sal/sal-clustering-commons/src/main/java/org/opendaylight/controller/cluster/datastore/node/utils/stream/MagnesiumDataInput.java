...
final class MagnesiumDataInput extends AbstractNormalizedNodeDataInput {
    ...
    private void streamLeaf(final NormalizedNodeStreamWriter writer, final PathArgument parent, final byte nodeHeader)
        ...
        final Object value;
        if ((nodeHeader & MagnesiumNode.PREDICATE_ONE) != 0) {
    ...
    private void streamLeafsetEntry(final NormalizedNodeStreamWriter writer, final PathArgument parent,
        ...
        if ((nodeHeader & 0x30) == MagnesiumNode.ADDR_PARENT) {
            if (!(parent instanceof NodeIdentifier)) {
                throw new InvalidNormalizedNodeStreamException("Invalid leaf set entry header " + nodeHeader
                    + " in parent " + parent);
            }
            nodeId = (NodeIdentifier) parent;
        } else {
            nodeId = decodeNodeIdentifier(nodeHeader);
        }
    ...
    private void streamMapEntry(final NormalizedNodeStreamWriter writer, final PathArgument parent,
        ...
        if ((nodeHeader & 0x30) == MagnesiumNode.ADDR_PARENT) {
            if (!(parent instanceof NodeIdentifier)) {
                throw new InvalidNormalizedNodeStreamException("Invalid map entry header " + nodeHeader + " in parent "
                        + parent);
            }
            nodeId = (NodeIdentifier) parent;
        } else {
        ...
        final int size;
        switch (nodeHeader & MagnesiumNode.PREDICATE_4B) {
            case 0:
                size = 0;
                break;
    ...
    private NodeIdentifier readNodeIdentifier() throws IOException {
        final NodeIdentifier ret = NodeIdentifier.create(decodeQName());
        // FIXME: okay: unify cache?
        return ret;
    }
    ...
    public YangInstanceIdentifier readYangInstanceIdentifier() throws IOException {
        ...
        } else if (type >= MagnesiumValue.YIID_0 && type <= MagnesiumValue.YIID_31) {
    ...
    private AugmentationIdentifier readAugmentationIdentifier(final byte header) throws IOException {
        final int count = header & MagnesiumPathArgument.AID_COUNT_4B;
        switch (count) {
            ...
            default:
                return readAugmentationIdentifier(count >>> 3);
        }
    }
    ...
    private NodeIdentifier readNodeIdentifier(final byte header) throws IOException {
        switch (header & MagnesiumPathArgument.QNAME_REF4) {
            case MagnesiumPathArgument.QNAME_DEF:
                // FIXME: use cache
                return NodeIdentifier.create(decodeQName());
            case MagnesiumPathArgument.QNAME_REF1:
                // FIXME: use cache
                return NodeIdentifier.create(decodeQNameRef1());
            case MagnesiumPathArgument.QNAME_REF2:
                // FIXME: use cache
                return NodeIdentifier.create(decodeQNameRef2());
            case MagnesiumPathArgument.QNAME_REF4:
                // FIXME: use cache
                return NodeIdentifier.create(decodeQNameRef4());
            default:
    ...
    private NodeIdentifierWithPredicates readNodeIdentifierWithPredicates(final byte header) throws IOException {
        ...
        switch (header & MagnesiumPathArgument.SIZE_4B) {
            ...
            default:
                return readNodeIdentifierWithPredicates(qname, header >>> 5);
        }
    }
    ...
    private @NonNull QName decodeQName() throws IOException {
        ...
        final QName qname;
        try {
            qname = QNameFactory.create(module, readRefString());
    ...
    private @NonNull QName decodeQNameRef2() throws IOException {
        return lookupQName(input.readUnsignedShort());
    }
    ...
    private @NonNull QNameModule decodeQNameModule() throws IOException {
        ...
        final int index;
        switch (type) {
            case MagnesiumValue.REF_1B:
                index = input.readUnsignedByte();
                break;
            case MagnesiumValue.REF_2B:
                index = input.readUnsignedShort();
                break;
            case MagnesiumValue.REF_4B:
                index = input.readInt();
                break;
            default:
    ...
    private @NonNull QNameModule decodeQNameModuleDef(final byte type) throws IOException {
        ...
        try {
            module = QNameFactory.createModule(namespace, revision.isEmpty() ? null : revision);
        } catch (UncheckedExecutionException e) {
    ...
    private @NonNull String readRefString(final byte type) throws IOException {
        ...
        switch (type) {
            ...
            case MagnesiumValue.REF_2B:
                return lookupString(input.readUnsignedShort());
            case MagnesiumValue.REF_4B:
    ...
    private @NonNull Object readLeafValue() throws IOException {
        ...
        switch (type) {
            ...
            case MagnesiumValue.INT8_0:
                return INT8_0;
            case MagnesiumValue.INT8_1:
                return INT8_1;
            case MagnesiumValue.INT8_MIN:
                return INT8_MIN;
            case MagnesiumValue.INT8_MAX:
                return INT8_MAX;
            case MagnesiumValue.INT16:
                return input.readShort();
            case MagnesiumValue.INT16_0:
                return INT16_0;
            case MagnesiumValue.INT16_1:
                return INT16_1;
            case MagnesiumValue.INT16_MIN:
                return INT16_MIN;
            case MagnesiumValue.INT16_MAX:
                return INT16_MAX;
            case MagnesiumValue.INT32:
                return input.readInt();
            case MagnesiumValue.INT32_0:
                return INT32_0;
            case MagnesiumValue.INT32_1:
                return INT32_1;
            case MagnesiumValue.INT32_MIN:
                return INT32_MIN;
            case MagnesiumValue.INT32_MAX:
                return INT32_MAX;
            case MagnesiumValue.INT64:
                return input.readLong();
            case MagnesiumValue.INT64_0:
                return INT64_0;
            case MagnesiumValue.INT64_1:
                return INT64_1;
            case MagnesiumValue.INT64_MIN:
                return INT64_MIN;
            case MagnesiumValue.INT64_MAX:
                return INT64_MAX;
            case MagnesiumValue.UINT8:
                return Uint8.fromByteBits(input.readByte());
            case MagnesiumValue.UINT8_0:
                return UINT8_0;
            case MagnesiumValue.UINT8_1:
                return UINT8_1;
            case MagnesiumValue.UINT8_MAX:
                return UINT8_MAX;
            case MagnesiumValue.UINT16:
                return Uint16.fromShortBits(input.readShort());
            case MagnesiumValue.UINT16_0:
                return UINT16_0;
            case MagnesiumValue.UINT16_1:
                return UINT16_1;
            case MagnesiumValue.UINT16_MAX:
                return UINT16_MAX;
            case MagnesiumValue.UINT32:
                return Uint32.fromIntBits(input.readInt());
            case MagnesiumValue.UINT32_0:
                return UINT32_0;
            case MagnesiumValue.UINT32_1:
                return UINT32_1;
            case MagnesiumValue.UINT32_MAX:
                return UINT32_MAX;
            case MagnesiumValue.UINT64:
                return Uint64.fromLongBits(input.readLong());
            case MagnesiumValue.UINT64_0:
                return UINT64_0;
            case MagnesiumValue.UINT64_1:
                return UINT64_1;
            case MagnesiumValue.UINT64_MAX:
                return UINT64_MAX;
            case MagnesiumValue.DECIMAL64:
