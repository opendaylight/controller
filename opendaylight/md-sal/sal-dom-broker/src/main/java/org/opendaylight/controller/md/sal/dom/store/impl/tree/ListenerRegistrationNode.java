...
public class ListenerRegistrationNode implements StoreTreeNode<ListenerRegistrationNode>, Identifiable<PathArgument> {
    ...
    /**
     * Return the list of current listeners. Any caller wishing to use this
     * method has to make sure the collection remains unchanged while it's
     * executing. This means the caller has to synchronize externally both the
     * registration and unregistration process.
     *
     ...
     *
     * @param path
     *            Full path on which listener is registered.
     * @param listener
     *            Listener
     * @param scope
     *            Scope of triggering event.
     * @return
     */
    public synchronized <L extends AsyncDataChangeListener<InstanceIdentifier, NormalizedNode<?, ?>>> DataChangeListenerRegistration<L> registerDataChangeListener(
            final InstanceIdentifier path, final L listener, final DataChangeScope scope) {

        DataChangeListenerRegistration<L> listenerReg = new DataChangeListenerRegistration<L>(path, listener, scope,
                this);
    ...
    public static class DataChangeListenerRegistration<T extends AsyncDataChangeListener<InstanceIdentifier, NormalizedNode<?, ?>>>
        ...
        public DataChangeListenerRegistration(final InstanceIdentifier path, final T listener,
                final DataChangeScope scope, final ListenerRegistrationNode node) {
            super(listener);
            this.path = path;
            this.scope = scope;
            this.node = node;
    ...
    public static Collection<org.opendaylight.controller.md.sal.dom.store.impl.DataChangeListenerRegistration<?>> getRegistrations(
            final Collection<ListenerRegistrationNode> listeners) {
        if (listeners.isEmpty()) {
            return Collections.emptyList();
        }
        ImmutableList.Builder<org.opendaylight.controller.md.sal.dom.store.impl.DataChangeListenerRegistration<?>> result = ImmutableList
                .builder();
        for (ListenerRegistrationNode listenerNode : listeners) {
            result.addAll(listenerNode.getListeners());
        }
        return result.build();
    }
    ...
    public String toString() {
        return "ListenerRegistrationNode [identifier=" + identifier + ", listeners=" + listeners.size() + "]";
    }
