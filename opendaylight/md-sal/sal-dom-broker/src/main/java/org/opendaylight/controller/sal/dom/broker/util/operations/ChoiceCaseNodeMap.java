...
final class ChoiceCaseNodeMap {
    ...
    static ChoiceCaseNodeMap fromUnknownNodes(DataNodeContainer schema, List<QName> unknownChildNodes) {
        ...
        for (QName unknownChildNode : unknownChildNodes) {
            ...
            // TODO Throw DataModificationEx
            Preconditions.checkArgument(inChoiceChild != null, "Unknown child node: %s in: %s", unknownChildNode, schema);
    ...
    static final class ChoiceMap {
        ...
        private void add(DataSchemaNode inChoiceChild) {
            List<QName> childPath = inChoiceChild.getPath().getPath();
            QName caseQName = childPath.get(CASE_QNAME_POSITION);
            QName nodeQName = childPath.get(CASE_QNAME_POSITION + 1);

            // TODO DataModificationEx
            Preconditions.checkArgument(nodeToCase.containsKey(nodeQName) == false, "Duplicate child node in choice " + caseQName);
            nodeToCase.put(nodeQName, caseQName);
            nodeToSchema.put(nodeQName, inChoiceChild);
            caseToNodes.put(caseQName, nodeQName);
            // TODO DataModificationEx
            Preconditions.checkState(caseToNodes.keySet().size()<=2, "Nodes present from multiple cases within one choice: %s", caseToNodes);
        }
