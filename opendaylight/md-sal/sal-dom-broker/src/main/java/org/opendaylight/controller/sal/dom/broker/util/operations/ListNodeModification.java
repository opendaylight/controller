...
public class ListNodeModification implements Modification<ListSchemaNode, Modification.ListNodeWrapper> {
    ...
    public ListNodeWrapper modify(ListSchemaNode schemaNode, ListNodeWrapper actualNodes,
        ...
        Map<Modification.ListNodeKey, Node<?>> modified = Maps.newLinkedHashMap(actualNodes.getMappedNodes());
        ...
        for (Node<?> listModification : modificationNodes.getNodes()) {
            ...
            case MERGE: {
                ...
                modified.put(NodeWrappers.getKeyForListNode(schemaNode, mergedListNode), mergedListNode);
                break;
            }
            case CREATE: {
            ...
            case REPLACE: {
                modified.put(modificationNodeKey, listModification);
                break;
            }
            case DELETE: {
            ...
            case REMOVE: {
                if (modified.containsKey(modificationNodeKey)) {
                    modified.remove(modificationNodeKey);
                }
                break;
            }
            default:
        ...
        return NodeWrappers.wrapListNodes(schemaNode, Lists.newArrayList(modified.values()));
    }
