package org.opendaylight.controller.md.sal.dom.store.impl.xsql.jdbc;
...
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.XSQLBluePrintNode;
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.XSQLColumn;
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.XSQLCriteria;
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.XSQLODLUtils;
...
public class JDBCResultSet implements Serializable,ResultSet,ResultSetMetaData{
	...
	public int isObjectFitCriteria(Object element,Class cls){
		Map<XSQLColumn,List<XSQLCriteria>> tblCriteria = criteria.get(cls.getName());
		if(tblCriteria==null)
			return 1;
		for(Map.Entry<XSQLColumn, List<XSQLCriteria>> cc:tblCriteria.entrySet()){
			for(XSQLCriteria c:cc.getValue()){
				int result = c.isObjectFitCriteria(element,cc.getKey().getName());
				if(result==0)
					return 0;
			}
		}
	...
	public void addTableToQuery(XSQLBluePrintNode node){
		if(this.tablesInQueryMap.containsKey(node.getODLNodeName()))
			return;
		this.tablesInQuery.add(node);
		this.tablesInQueryMap.put(node.getODLNodeName(),node);
	}
	...
	public XSQLBluePrintNode getMainTable(){
		...
		for(XSQLBluePrintNode node:tablesInQuery){
			...
			if(result.getLevel()<node.getLevel()){
				result = node;			
			}
		}
		return result;
		...
		for(BluePrintNode tbl:tablesInQuery){
			cls.add(tbl.getInterface());
		}
	
		List<BluePrintNode> tables = new LinkedList<BluePrintNode>();
		tables.addAll(tablesInQuery);
		for(Class p:cls){
			for(Iterator<BluePrintNode> iter=tables.iterator(); iter.hasNext();){
				BluePrintNode tbl = iter.next();
				if(!p.equals(tbl.getInterface())){
					if(!tbl.isModelChild(p)){
						iter.remove();
					}
				}
			}
	...
	public boolean next(){
		...
		while(!finished || records.size()>0){
			synchronized(this){			
				if(records.size()==0){
					try{this.wait(1000);}catch(Exception err){}
					if(records.size()>0){
						try{
	...
	private void createRecord(Object data,XSQLBluePrintNode node){
		...
		for(XSQLColumn c:this.fieldsInQuery){
			if(c.getTableName().equals(node.getName())){
				try{
	...
	private Map collectColumnValues(Object node,XSQLBluePrintNode bpn){
		...
		for(Object stc:subChildren.values()){
			/*
			if(sc.getClass().getName().endsWith("ImmutableMapNode")){
				result.put("_children", Adapter.getChildren(sc));
			}else*/
			if(stc.getClass().getName().endsWith("ImmutableAugmentationNode")){
				...
				for(Object key:values.keySet()){
					...
					if(val.getClass().getName().endsWith("ImmutableLeafNode")){
						...
						if(value!=null){
							result.put(bpn.getTableName()+"."+k, value.toString());						
						}
					}
				}
			}else
			if(stc.getClass().getName().endsWith("ImmutableLeafNode")){
	...
	private void addToData(Record rec,XSQLBluePrintNode bpn,Map fullRecord){		
		if(bpn!=null){ 			
			for(XSQLColumn c:fieldsInQuery){
				if(((XSQLBluePrintNode)c.getBluePrintNode()).getODLNodeName().equals(XSQLODLUtils.getNodeIdentiofier(rec.element))){
			...
			if(child.getClass().getName().endsWith("ImmutableAugmentationNode")){
				...
				for(Object c:_children){
					if(c.getClass().getName().endsWith("ImmutableContainerNode")){
						if(tableName.indexOf(XSQLODLUtils.getNodeIdentiofier(c).toString())!=-1){
							result.add(c);
						}
					}			
				}
	...
	public List<Record> addRecords(Object element,XSQLBluePrintNode node,boolean root,String tableName){
	...
	public List<Record> addRecords(Object element,BluePrintNode node,Set<BluePrintRelation> beenHereNode,Set<String> beenHereElement,boolean root){
		List<Record> result = new LinkedList<Record>();
		
		if(root && node.getInterface().equals(element.getClass())){
			Record rec = new Record();
			rec.element = element;
			addToData(rec);
			addRecord(rec.data);
		...
		for(BluePrintRelation r:node.getRelations()){
			
			if(root){
				beenHereNode = new HashSet<BluePrintRelation>();
			}			
			
			if(beenHereNode.contains(r)) 
				continue;						
			
			beenHereNode.add(r);
			
			if(r.getParent().getInterface().equals(element.getClass()) && isObjectFitCriteria(element,element.getClass())==1){
				Record neRec = new Record();
				neRec.element = element;
				addToData(neRec);
				
				List<Object> data = r.execute(element);
				if(data!=null){ 
					for(Object d:data){
						
						if(beenHere(beenHereElement, d))
							continue;
						
						if(!(isObjectFitCriteria(d, d.getClass())==1)){
							continue;
						}
				...
				List<Record> subRecords = addRecords(element,r.getParent(),beenHereNode,beenHereElement,false);
				for(Record subRec:subRecords){
					List subO = r.execute(subRec.element);
					if(subO!=null){
						for(Object subData:subO){
							
							if(beenHere(beenHereElement, subData))
								continue;
							
							if(!(isObjectFitCriteria(subData, subData.getClass())==1)){
								continue;
							}							
							Record rec = new Record();
							rec.element = subData;
		...
		if(root){
			if(beenHereElement!=null)
				beenHereElement.clear();
			
			if(beenHereNode!=null)
				beenHereNode.clear();
		}
	...
	public Object getObject(int columnIndex) throws SQLException {
		return currentRecord.get(this.fieldsInQuery.get(columnIndex-1).toString());
	}
