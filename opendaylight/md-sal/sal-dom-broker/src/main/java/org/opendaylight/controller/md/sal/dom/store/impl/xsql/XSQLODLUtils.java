...
public class XSQLODLUtils {
	...
	public static String extractTableName(SchemaPath path){
		...
		//name.append(lst.get(lst.size()-1).getLocalName());
	...
	public static String getCacheKey(Object odlNode){
		if(odlNode instanceof Module){
			return ((Module)odlNode).getNamespace().toString();
		}else
		if(odlNode instanceof DataSchemaNode){
	...
	public static boolean createOpenDaylightCache(XSQLBluePrint bluePrint,Object module){
		XSQLBluePrintNode node = new XSQLBluePrintNode(module,0);
		if(node.getCacheKey().indexOf("config")!=-1)
			return false; 
		bluePrint.addToBluePrintCache(node.getCacheKey(), node);
		collectODL(bluePrint,node,((Module)module).getChildNodes(),1);
		return true;
	}
		
	private static void collectODL(XSQLBluePrint bluePrint,XSQLBluePrintNode parent,Set<DataSchemaNode> nodes,int level){
		...
		for(DataSchemaNode n:nodes){
			if(n instanceof DataNodeContainer /*|| n instanceof LeafListSchemaNode*/ || n instanceof ListSchemaNode){			
				XSQLBluePrintNode bn = new XSQLBluePrintNode(n,level);	
				bluePrint.addToBluePrintCache(bn.getCacheKey(), bn);
				parent.AddChild(bn);
				bn.setParent(parent);
				...
				if(parent!=null){
					parent.addColumn(n,parent.getParent().getTableName());
				}else{
