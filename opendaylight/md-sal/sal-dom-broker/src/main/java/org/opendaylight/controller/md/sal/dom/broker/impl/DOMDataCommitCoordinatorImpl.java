...
public class DOMDataCommitCoordinatorImpl implements DOMDataCommitExecutor {
    ...
    private static class CommitCoordinationTask implements Callable<Void> {
        ...
        private void canCommitBlocking() throws TransactionCommitFailedException {
            final Iterable<ListenableFuture<Boolean>> canCommitFutures = canCommitAll();
            for(ListenableFuture<Boolean> canCommit : canCommitFutures) {
                Boolean result;
                try {
        ...
        private List<ListenableFuture<Void>> commitAll() {
            ...
            ArrayList<ListenableFuture<Void>> ops = new ArrayList<>();
            for (DOMStoreThreePhaseCommitCohort cohort : cohorts) {
