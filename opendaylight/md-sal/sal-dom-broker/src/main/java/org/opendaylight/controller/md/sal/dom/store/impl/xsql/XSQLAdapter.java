...
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.jdbc.JDBCResultSet;
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.jdbc.JDBCServer;
...
public class XSQLAdapter extends Thread implements SchemaContextListener{
	...
	public void addDOMStroe(DOMStore s){
		this.stores.add(s);
	}
		...
		if(table.getParent().isModule()){			
			try{
				...
				for(DOMStore store:stores){
					...
					DOMStoreReadTransaction t = store.newReadOnlyTransaction();
					Object node = t.read(instanceIdentifier).get();
					node = XSQLODLUtils.get(node,"reference");							
					if(node==null)
						continue;
					PrintStream out = new PrintStream(new FileOutputStream("/tmp/node-"+j));
					j++;
					//printTree((StoreMetadataNode)node, 0, out);
					out.close();
