...
public class DOMMountPointServiceImpl implements DOMMountPointService {
    ...
    ObjectRegistration<DOMMountPoint> doRegisterMountPoint(final SimpleDOMMountPoint mountPoint) {
        ...
        for (Entry<Class<? extends DOMService>, DOMService> entry : myServices.entrySet()) {
            delegateBuilder.addService(entry.getKey().asSubclass(DOMService.class), entry.getValue());

            final CompatFactory<?, ?> compat = KNOWN_SERVICES.get(entry.getKey());
            if (compat != null && !myServices.containsKey(compat.mdsalClass)) {
