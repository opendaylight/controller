...
public class XSQLThreadPool {
    ...
    private class WorkerThread extends Thread{
          ...
          public WorkerThread(int threadNumber){
                super("Thread #"+threadNumber+" Of Threadpool "+threadPoolName);              
          }
          ...
          public void run(){
                ...
                while(true){
                      ...
                      synchronized(tasks){
                            if(tasks.isEmpty()){
                                  try{tasks.wait(2000);}catch(Exception err){}
                            }
                      ...
                      if(System.currentTimeMillis()-lastTimeExecuted>waitTimeForIdle){
                            break;
                      }
                }
                synchronized(tasks){
    ...
    public void waitForSlot(){
    	if(tasks.size()>maxQueueSize){
	    	synchronized(waitForSlotSync){
	    		try{waitForSlotSync.wait();}catch(Exception err){};
	    	}
    	}
    }
