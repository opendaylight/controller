package org.opendaylight.controller.md.sal.dom.store.impl.xsql;
...
public class XSQLCriteria implements Serializable{
	...
	private void parse(String data,int parentOperation){
		...
		if(index1!=-1){
			...
			if(leftCondition.trim().equals("")){
				...
				if(index2<data.length()-1){
					...
					if(!rtValue.equals("")){
						left = new XSQLCriteria(data.substring(index1+1,index2),parentOperation);
				...
				right = new XSQLCriteria(data.substring(index1+1,data.length()-1),parentOperation);				
		...
		for(int i=0;i<operators.length;i++){
			...
			if(index1!=-1){				
				this.operation = i;
				if(left==null)
					left = new XSQLCriteria(data.substring(0,index1),this.operation);
				if(left.leftValue!=null && left.rightValue==null && left.right==null){
					leftValue = left.leftValue;
					left = null;
				}
				if(right==null)
					right = new XSQLCriteria(data.substring(index1+operators[i].length()),this.operation);
				if(right.leftValue!=null && right.rightValue==null && right.right==null){
		...
		if(parentOperation==OP_CODE_LIKE && data.startsWith("%") && data.endsWith("%") && data.substring(1,data.length()-1).indexOf("%")==-1){
			data = data.substring(1,data.length()-1);			
		}
		...
		try{
			...
			if(m==null){
				...
				for(Method mm:methods){
					if(mm.getName().equals(propertyName) || mm.getName().equals("get"+propertyName) || mm.getName().equals("is"+propertyName)){
						m = mm;
	...
	public String getCriteriaForProperty(XSQLColumn col){
		...
		/*
		if(criteria.toLowerCase().indexOf(propertyName.toLowerCase())==-1){
				return "";
		}*/
		...
		if(left!=null && right!=null){
			...
			if(!leftString.equals("") && !rightString.equals("")){
				return leftString+" "+operators[operation]+" "+rightString;
			}else
		...
		if(leftValue!=null && leftValue.toString().toLowerCase().equals(col.toString().toLowerCase()) && right!=null){
			return "? "+operators[operation]+" ("+right.getCriteriaForProperty(col)+")";
		}else
		if(rightValue!=null && rightValue.toString().toLowerCase().equals(col.toString().toLowerCase()) && left!=null){
			return "("+left.getCriteriaForProperty(col)+") "+operators[operation]+" ?";
		}		
		return "";		
	}
