...
public final class PingPongTransactionChain implements DOMTransactionChain {
    ...
    private void processBuffer() {
        ...
        if (tx != null) {
            if (!failed) {
                LOG.debug("Submitting transaction {}", bufferTransaction);
                ...
                LOG.debug("Cancelling transaction {}", bufferTransaction);
                bufferTransaction.getTransaction().cancel();
            }
    ...
    private void transactionSuccessful(final PingPongTransaction tx, final Void result) {
        ...
        synchronized (this) {
            Preconditions.checkState(inflightTransaction == tx, "Successful transaction {} while {} was submitted", tx, inflightTransaction);
    ...
    private void transactionFailed(final PingPongTransaction tx, final Throwable t) {
        ...
        synchronized (this) {
            Preconditions.checkState(inflightTransaction == tx, "Failed transaction {} while {} was submitted", tx, inflightTransaction);
            inflightTransaction = null;
        }
