...
public class DOMDataBrokerImpl extends AbstractDOMForwardedTransactionFactory<DOMStore> implements DOMDataBroker, AutoCloseable {
    ...
    public DOMTransactionChain createTransactionChain(final TransactionChainListener listener) {
        ImmutableMap.Builder<LogicalDatastoreType, DOMStoreTransactionChain> backingChains = ImmutableMap.builder();
        for(Entry<LogicalDatastoreType, DOMStore> entry : datastores.entrySet()) {
            backingChains.put(entry.getKey(), entry.getValue().createTransactionChain());
        }
        long chainId = chainNum.getAndIncrement();
        return new DOMBrokerTransactionChainImpl(chainId,backingChains.build(),coordinator,listener);
