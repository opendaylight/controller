...
public class ProxyFactory {
    ...
    private static Object _createProxyImpl(final ServiceReference<?> ref,
            final DOMNotificationPublishService service) {

        return new DOMNotificationPublishServiceProxy(
                ((ServiceReference<DOMNotificationPublishService>) ref), service);
    }

    private static Object _createProxyImpl(final ServiceReference<?> ref,
            final DOMNotificationService service) {

        return new DOMNotificationServiceProxy(
                ((ServiceReference<DOMNotificationService>) ref), service);
    }
    ...
    private static Object createProxyImpl(final ServiceReference<?> ref,
        ...
        } else if (service instanceof DOMNotificationPublishService) {
            return _createProxyImpl(ref, (DOMNotificationPublishService) service);
        } else if (service instanceof SchemaService) {
            return _createProxyImpl(ref, (SchemaService) service);
        } else if (service instanceof DOMNotificationService) {
            return _createProxyImpl(ref, (DOMNotificationService) service);
