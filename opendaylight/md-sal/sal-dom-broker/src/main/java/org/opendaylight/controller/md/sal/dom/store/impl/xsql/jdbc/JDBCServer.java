...
import org.opendaylight.controller.md.sal.dom.store.impl.xsql.XSQLBluePrint;
...
public class JDBCServer extends Thread{		
	...
	public static void parseTables(JDBCResultSet rs,XSQLBluePrint bp) throws SQLException{
		...
		while(tokens.hasMoreTokens()){
			...
			XSQLBluePrintNode table = bp.getBluePrintNode(tableName);
			if(table==null){
	...
	public static void addCriteria(XSQLColumn col,XSQLCriteria c,JDBCResultSet rs){
		Map<XSQLColumn,List<XSQLCriteria>> tblCriteria = rs.getCriteria().get(col.getTableName());
		if(tblCriteria==null){
	...
	public static void parseFields(JDBCResultSet rs,XSQLBluePrint bp) throws SQLException{
		String lowSQL = rs.getSQL().toLowerCase();
		...
		while(tokens.hasMoreTokens()){
			...
			if(token.indexOf(".")!=-1){
				XSQLBluePrintNode tbl = bp.getBluePrintNode(token.substring(0,token.indexOf(".")).trim());
				...
				XSQLColumn col = null;				
				for(XSQLBluePrintNode table:rs.getTables()){
					try{col=table.findColumnByName(token);}catch(Exception err){}
					if(col!=null){
						break;
					}						
				}				
	...
	public static void parseCriteria(JDBCResultSet rs,XSQLBluePrint bp){
		...
		String whereStatement = rs.getSQL().substring(where+5,whereTo).trim();
