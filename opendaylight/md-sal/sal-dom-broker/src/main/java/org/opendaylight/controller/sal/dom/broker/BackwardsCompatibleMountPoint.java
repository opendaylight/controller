...
public class BackwardsCompatibleMountPoint implements MountProvisionInstance, SchemaContextProvider {
    ...
    public BackwardsCompatibleMountPoint(final InstanceIdentifier path, final DOMMountPoint mount) {
        ...
        final Optional<DOMDataBroker> domBroker = mount.getService(DOMDataBroker.class);
        dataReader = new BackwardsCompatibleDataBroker(domBroker.get(), getStaticSchemaService(mount));
        readWrapper = new ReadWrapper();

        final Optional<NotificationPublishService> notificationService = mount.getService(NotificationPublishService.class);
        notificationPublishService = notificationService.get();
        final Optional<RpcProvisionRegistry> rpcService = mount.getService(RpcProvisionRegistry.class);
        rpcs = rpcService.get();

    }

    private static SchemaService getStaticSchemaService(final DOMMountPoint mount) {
        return new SchemaService() {
            ...
            public ListenerRegistration<SchemaServiceListener> registerSchemaServiceListener(final SchemaServiceListener listener) {
                listener.onGlobalContextUpdated(mount.getSchemaContext());
                return new ListenerRegistration<SchemaServiceListener>() {
                    @Override
                    public void close() {
                        // NOOP
                    }
                    ...
                    @Override
                    public SchemaServiceListener getInstance() {
                        return listener;
                    }
                };
            }
    ...
    private static final class BackwardsCompatibleDomStore implements DOMDataBroker {
        private final DataProviderService dataReader;
        ...
        private static final class BackwardsCompatibleReadTransaction implements DOMDataReadOnlyTransaction {
            private final DataProviderService dataReader;
        ...
        private static final class BackwardsCompatibleWriteTransaction implements DOMDataWriteTransaction {
        ...
        private class BackwardsCompatibleReadWriteTransaction implements DOMDataReadWriteTransaction {
