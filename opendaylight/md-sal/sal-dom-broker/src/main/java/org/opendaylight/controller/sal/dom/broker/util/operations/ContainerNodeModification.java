...
class ContainerNodeModification implements Modification<ContainerSchemaNode, Modification.SingleNodeWrapper> {
    ...
    private List<Node<?>> mergeContainersChildNodes(DataNodeContainer schema, OperationStack operationStack,
        ...
        for (QName childToProcessQName : toProcess) {
            ...
            Preconditions.checkState(schemaOfChildToProcess != null,
                    "Unable to modify child node: %s, missing in schema: %s", childToProcessQName, schema);
        ...
        // TODO keep order of xml nodes somehow
        ...
        for (QName choiceQName : mappedChoices.keySet()) {
            ...
            for (QName nodeFromCase : mappedCases.getAllNodes()) {
                if(modification.containsKey(nodeFromCase)) {
                    if(caseToProcessFromModification!=null) {
                        ...
                        throw new IllegalStateException(String.format(
                                "Child nodes from multiple cases present in modification, choice: %s, case1: %s, case2: %s",
                                choiceQName, caseToProcessFromModification, mappedCases.getCaseQNameForNodeQName(nodeFromCase)));
                    }
                ...
                if(actual.containsKey(nodeFromCase)) {
                    if(caseToProcessFromActual!=null) {
                        ...
                        // check for nodes from different cases
                        throw new IllegalStateException(String.format(
                                "Child nodes from multiple cases present in actual state, choice: %s, case1: %s, case2: %s",
                                choiceQName, caseToProcessFromModification, mappedCases.getCaseQNameForNodeQName(nodeFromCase)));
                    }
            ...
            // TODO more information to exception
            Preconditions.checkState(caseToProcess!=null, "Unable to find case statement to process");
