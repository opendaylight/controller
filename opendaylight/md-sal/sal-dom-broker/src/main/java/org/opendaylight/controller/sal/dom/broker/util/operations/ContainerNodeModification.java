...
class ContainerNodeModification implements Modification<ContainerSchemaNode, Modification.SingleNodeWrapper> {
    ...
    Modification.SingleNodeWrapper modify(DataNodeContainer schema, Modification.SingleNodeWrapper actualNodes,
        ...
        switch (operationStack.getCurrentOperation()) {
            ...
            case MERGE: {
                ...
                modified = modifyContainer(schema, actualNodes, modificationNodes, operationStack);
                break;
            }
        }
    ...
    private SingleNodeWrapper modifyContainer(DataNodeContainer schema, SingleNodeWrapper actualNodes, SingleNodeWrapper modificationNodes, OperationStack operationStack) throws DataModificationException {
        ...
        List<Node<?>> merged = modifyCOntainerChildNodes(schema, operationStack, actual, modification, toProcess);
        return NodeWrappers.wrapNode(ImmutableCompositeNode.create(actual.getNodeType(), merged));
    }

    private List<Node<?>> modifyCOntainerChildNodes(DataNodeContainer schema, OperationStack operationStack,
        ...
        for (QName childToProcessQName : toProcess) {
            ...
            NodeWrapper modifiedValues = modifyContainerNode(operationStack, actual, modification,
                    childToProcessQName, schemaOfChildToProcess);

            if (modifiedValues.isEmpty() == false) {
                result.addAll(modifiedValues.getNodes());
            }
        }

        // Try to figure out unknown child nodes
        List<Node<?>> modifiedUnknownNodes = tryToModifyUnknownNodes(schema, operationStack, actual, modification, unknownChildNodes);
        result.addAll(modifiedUnknownNodes);

        return result;
    }

    private List<Node<?>> tryToModifyUnknownNodes(DataNodeContainer schema, OperationStack operationStack, CompositeNode actual, CompositeNode modification, List<QName> unknownChildNodes) throws DataModificationException {
        // TODO preserve order of xml nodes somehow
        List<Node<?>> modifiedChoiceCaseNodes = Lists.newArrayListWithCapacity(unknownChildNodes.size());

        // Unknown nodes are expected to belong to a choice/case statement
        ChoiceCaseMap choiceCaseNodeMap = ChoiceCaseMap.createFromUnknownNodes(schema, unknownChildNodes);
        Map<QName/*Choice QNAme*/, ChoiceCaseMap.ChoiceMap> mappedChoices = choiceCaseNodeMap.getMappedChoices();

        for (QName choiceQName : mappedChoices.keySet()) {
            ChoiceCaseMap.ChoiceMap mappedCases = mappedChoices.get(choiceQName);

            // Find case statements used in actual data and in modification data
            ...
            // Find out which case node to be present in the resulting data
            // If a case is used in modification, only it should be used in result
            // Nodes from other cases from actual data should be deleted
            ...
            // Recursively modify children from case
            modifyContainerCaseNode(operationStack, actual, modification, modifiedChoiceCaseNodes, mappedCases, caseToProcess);
        }
        return modifiedChoiceCaseNodes;
    }

    private void modifyContainerCaseNode(OperationStack operationStack, CompositeNode actual, CompositeNode modification, List<Node<?>> modifiedChoiceCaseNodes, ChoiceCaseMap.ChoiceMap mappedCases, QName caseToProcess) throws DataModificationException {
        for (QName childToProcessQName :  mappedCases.getChildNodesForCaseQName(caseToProcess)) {
            ...
            if (modifiedValues.isEmpty() == false) {
                modifiedChoiceCaseNodes.addAll(modifiedValues.getNodes());
            }
        }
    }

    private NodeWrapper modifyContainerNode(OperationStack operationStack, CompositeNode actual, CompositeNode modification, QName childToProcessQName, DataSchemaNode schemaChild) throws DataModificationException {
    ...
    private NodeWrapper dispatchChildModification(DataSchemaNode schemaChild, List<Node<?>> actualNodes,
