...
public class BrokerImpl implements Broker, RpcProvisionRegistry, AutoCloseable {
    ...
    public  BrokerImpl(final RpcRouter router,final ClassToInstanceMap<BrokerService> services) {
        this.router = Preconditions.checkNonNull(router, "RPC Router must not be null");
    ...
    protected <T extends BrokerService> T getGlobalService(final Class<T> service) {
        return services.getInstance(service);
    }
