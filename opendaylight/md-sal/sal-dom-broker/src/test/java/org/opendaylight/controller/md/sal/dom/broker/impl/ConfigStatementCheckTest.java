...
public class ConfigStatementCheckTest {
    ...
    private static final QName CONFIG_CHOICE_1 = QName.create(TEST_MODEL_NAME, "config-choice-1");
    private static final QName OPER_LEAF_IN_CASE = QName.create(TEST_MODEL_NAME, "oper-leaf-in-case");
    private static final QName CONFIG_LEAF_IN_CASE = QName.create(TEST_MODEL_NAME, "config-leaf-in-case");
    private static final QName CONFIG_AUGMENTED_CONTAINER = QName.create(TEST_MODEL_NAME, "config-augmented-cont-1");
    private static final QName OPER_AUGMENT_LEAF = QName.create(TEST_MODEL_NAME, "oper-augment-leaf");
    ...
    private static final YangInstanceIdentifier CONFIG_CHOICE_1_PATH = YangInstanceIdentifier.of(CONFIG_CHOICE_1);
    private static final YangInstanceIdentifier OPER_LEAF_IN_CASE_PATH = CONFIG_CHOICE_1_PATH.node(OPER_LEAF_IN_CASE);
    private static final YangInstanceIdentifier CONFIG_LEAF_IN_CASE_PATH = CONFIG_CHOICE_1_PATH.node(CONFIG_LEAF_IN_CASE);
    private static final YangInstanceIdentifier CONFIG_AUGMENTED_CONTAINER_PATH = YangInstanceIdentifier.of(CONFIG_AUGMENTED_CONTAINER);
    private static final YangInstanceIdentifier OPER_AUGMENT_LEAF_PATH = CONFIG_AUGMENTED_CONTAINER_PATH.node(OPER_AUGMENT_LEAF);
    ...
    @Test
    public void insertIntoOperLeafInCaseConfDS() throws ExecutionException, InterruptedException, ReadFailedException {
        assertNotNull(domBroker);
        ...
        writeTx = domBroker.newReadWriteTransaction();
        writeTx.put(CONFIGURATION, CONFIG_CHOICE_1_PATH, Builders.choiceBuilder().withNodeIdentifier(
                new YangInstanceIdentifier.NodeIdentifier(CONFIG_CHOICE_1)).build());
        try {
            writeTx.put(CONFIGURATION, OPER_LEAF_IN_CASE_PATH, ImmutableNodes.leafNode(OPER_LEAF_IN_CASE, "clic"));
        } catch (Exception e) {
            return;
        }
        fail("Exception should have been thrown.");
    ...
    @Test
    public void insertIntoOperLeafInCaseOperDS() throws ExecutionException, InterruptedException, ReadFailedException {
        assertNotNull(domBroker);
        ...
        writeTx = domBroker.newReadWriteTransaction();
        writeTx.put(OPERATIONAL, CONFIG_CHOICE_1_PATH, Builders.choiceBuilder().withNodeIdentifier(
                new YangInstanceIdentifier.NodeIdentifier(CONFIG_CHOICE_1)).build());
        writeTx.put(OPERATIONAL, OPER_LEAF_IN_CASE_PATH, ImmutableNodes.leafNode(OPER_LEAF_IN_CASE, "clic"));
        writeTx.submit().get();

        DOMDataReadTransaction readTx = domBroker.newReadOnlyTransaction();
        Optional<NormalizedNode<?, ?>> read = readTx.read(OPERATIONAL, OPER_LEAF_IN_CASE_PATH).checkedGet();
        assertTrue(read.isPresent());
    }

    @Test
    public void insertIntoOperAugmentedLeafConfDS() throws ExecutionException, InterruptedException, ReadFailedException {
        assertNotNull(domBroker);
        ...
        writeTx = domBroker.newReadWriteTransaction();
        writeTx.put(CONFIGURATION, CONFIG_AUGMENTED_CONTAINER_PATH, ImmutableNodes.containerNode(CONFIG_AUGMENTED_CONTAINER));
        try {
            writeTx.put(CONFIGURATION, OPER_AUGMENT_LEAF_PATH, ImmutableNodes.leafNode(OPER_AUGMENT_LEAF, "clic"));
        } catch (Exception e) {
            return;
        }
        fail("Exception should have been thrown.");
    ...
    @Test
    public void insertIntoOperAugmentedLeafOperDS() throws ExecutionException, InterruptedException, ReadFailedException {
        assertNotNull(domBroker);
        ...
        writeTx = domBroker.newReadWriteTransaction();
        writeTx.put(OPERATIONAL, CONFIG_AUGMENTED_CONTAINER_PATH, ImmutableNodes.containerNode(CONFIG_AUGMENTED_CONTAINER));
        writeTx.put(OPERATIONAL, OPER_AUGMENT_LEAF_PATH, ImmutableNodes.leafNode(OPER_AUGMENT_LEAF, "clic"));
        writeTx.submit().get();

        DOMDataReadTransaction readTx = domBroker.newReadOnlyTransaction();
        Optional<NormalizedNode<?, ?>> read = readTx.read(OPERATIONAL, OPER_AUGMENT_LEAF_PATH).checkedGet();
        assertTrue(read.isPresent());
    }
