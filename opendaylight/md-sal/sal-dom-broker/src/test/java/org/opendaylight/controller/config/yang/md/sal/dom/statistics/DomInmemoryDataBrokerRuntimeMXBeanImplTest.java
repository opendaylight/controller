...
public class DomInmemoryDataBrokerRuntimeMXBeanImplTest {
    ...
    @Test
    public void getCommitStatsTest() {

        CommitStatsTracker commitStatsTracker = new CommitStatsTracker();
        DomInmemoryDataBrokerRuntimeMXBeanImpl bean =
                new DomInmemoryDataBrokerRuntimeMXBeanImpl(null, null, commitStatsTracker);

        commitStatsTracker.addCommitStats(100);

        CommitStats commitStats = bean.getCommitStats();
        String prefix = "100.0 ns";
        assertEquals("getTotalCommits", Long.valueOf(1), commitStats.getTotalCommits());
        assertEquals("getLongestCommitTime starts with \"" + prefix + "\"", true,
                      commitStats.getLongestCommitTime().startsWith("100.0 ns"));
        assertEquals("getShortestCommitTime starts with \"" + prefix + "\"", true,
                      commitStats.getShortestCommitTime().startsWith(prefix));
        assertEquals("getAverageCommitTime starts with \"" + prefix + "\"", true,
                     commitStats.getAverageCommitTime().startsWith(prefix));
    }
}
