...
public class DataPersister {
    ...
    private EncodeDataContainer buffer = new EncodeDataContainer(1024);
    private static final long MAX_BUFFER_SIZE = 1024*1024*10;
    public static boolean USE_BUFFERING = true;
    ...
    private void writeToBuffer(byte data[],int parentRecordIndex) throws IOException{
        buffer.adjustSize(HEADER_SIZE+data.length);
        System.arraycopy(getHeader(data.length,parentRecordIndex),0,buffer.getBytes(),buffer.getLocation(),HEADER_SIZE);
        buffer.advance(HEADER_SIZE);
        System.arraycopy(data,0, buffer.getBytes(), buffer.getLocation(), data.length);
        buffer.advance(data.length);
    }

    private void flushBuffer(){
        if(!USE_BUFFERING) return;
        try{
            if(buffer.getLocation()>0){
                byte data[] = buffer.getData();
                if(file.length()!=fileSize || lastLocation!=fileSize){
                    randomAccessFile.seek(file.length());
                }
                randomAccessFile.write(data);
                buffer.resetLocation();
    ...
    private int writeNew(byte data[],MD5Identifier x,int parentRecordIndex){
        ...
        try {
            if (lastLocation != fileSize && !USE_BUFFERING) {
        ...
        if(buffer.getLocation()>=MAX_BUFFER_SIZE){
            flushBuffer();
        }
        return locationsByIndex.size()-1;
    }
        ...
        synchronized (locations) {
            ...
            if (loc != null) {
                try {
                    ...
                    randomAccessFile.seek(loc.getStartPosition()+HEADER_SIZE);
                    byte data[] = new byte[loc.getLength()-HEADER_SIZE];
            ...
            if (recordIndexs != null) {
                try {
                    ...
                    for (int recIndex : recordIndexs) {
                        ...
                        randomAccessFile.seek(l.getStartPosition()+HEADER_SIZE);
                        data[index] = new byte[l.getLength()-HEADER_SIZE];
