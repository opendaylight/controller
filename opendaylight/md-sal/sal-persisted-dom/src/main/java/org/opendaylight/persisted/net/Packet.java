...
public class Packet implements ISerializer{
	...
	public Packet(NetworkID _source, NetworkID _destination,byte[] _data,int _id,boolean _multiPart){
		if(_id==-1){
	...
	public void encode(Object value, byte[] byteArray, int location) {
		...
		EncodeUtils.encodeInt16(p.packetID, byteArray, location+PACKET_ID_LOCATION);
		if(p.multiPart){
		...
		System.arraycopy(p.data,0, byteArray, PACKET_DATA_LOCATION, p.data.length);				
	}
	...
	public Object decode(byte[] byteArray, int start,int length) {
		Packet m = new Packet();
		m.source = (NetworkID)EncodeUtils.getSerializer(NetworkID.class).decode(byteArray, start+PACKET_SOURCE_LOCATION,8);
		m.destination = (NetworkID)EncodeUtils.getSerializer(NetworkID.class).decode(byteArray, start+PACKET_DEST_LOCATION,8);
		m.packetID = EncodeUtils.decodeInt16(byteArray,start+PACKET_ID_LOCATION);
		m.priority = ((int)byteArray[start+PACKET_MULTIPART_AND_PRIORITY_LOCATION])/2;
		m.multiPart = byteArray[start+PACKET_MULTIPART_AND_PRIORITY_LOCATION]%2==1;		
		m.data = new byte[length-PACKET_DATA_LOCATION];
		System.arraycopy(byteArray, start+PACKET_DATA_LOCATION, m.data,0, length-PACKET_DATA_LOCATION);
		return m;
	}
	...
	@Override
	public int hashCode() {
		return source.getIPv4Address()^destination.getIPv4Address()^source.getPort()^destination.getPort()^source.getSubSystemID()^destination.getSubSystemID();
	}
	
	@Override
	public boolean equals(Object obj) {
		Packet other = (Packet)obj;
		if(source.equals(other.source) && destination.equals(other.destination) && packetID==other.packetID)
			return true;
		return false;
	}
	...
	public String toString(){
		...
		buff.append("ID=").append(this.packetID).append(" Source=").append(this.source).append(" Dest=").append(this.destination);
	...
	public void decode(){
		if(this.decodedObject==null){
			...
			decodedObject = EncodeUtils.decodeObject(ba);
		}
	}
