...
public class NetworkNodeConnection extends Thread{
	...
	public void shutdown(){
	    this.running = false;
	    try{this.socket.close();}catch(Exception err){}
	}
	
	public void run(){
		NetworkID id = null;
		synchronized(networkNode.incomingConnections){
			if(!valideConnection) return;				
	...
	private class Switch extends Thread {
		...
		public void run(){
			while(running){
				...
				synchronized(incoming){
					if(incoming.size()==0){
						try{incoming.wait(5000);}catch(Exception err){}
					}
					if(incoming.size()>0){
						data = incoming.next();
					}					
				...
				if(data!=null && data.length>0){
					int destAddr = EncodeUtils.decodeInt32(data, Packet.PACKET_DEST_LOCATION);
					int destPort = EncodeUtils.decodeInt16(data, Packet.PACKET_DEST_LOCATION+4);
					if(destAddr==0){
					...
					if(destAddr==networkNode.getLocalHost().getIPv4Address() && destPort==networkNode.getLocalHost().getPort()){
						networkNode.receivedPacket(data);
					}else
					if(destAddr==networkNode.getLocalHost().getIPv4Address() && networkNode.getLocalHost().getPort()==50000 && destPort!=50000){						
						NetworkNodeConnection other = networkNode.getNodeConnection(destAddr, destPort);
						if(other!=null){							
							try{other.sendPacket(data);}catch(Exception err){err.printStackTrace();}
						}else{							
							data = markAsUnreachable(data);							
							destAddr = EncodeUtils.decodeInt32(data, Packet.PACKET_DEST_LOCATION);
							destPort = EncodeUtils.decodeInt16(data, Packet.PACKET_DEST_LOCATION+4);							
							NetworkNodeConnection source = networkNode.getNodeConnection(destAddr, destPort);
							if(source!=null){
								try{source.sendPacket(data);}catch(Exception err){err.printStackTrace();}								
							}else
								System.err.println("Source unreachable:"+new NetworkID(destAddr, destPort,EncodeUtils.decodeInt16(data, 16)));
						}
					}else
					if(destAddr!=networkNode.getLocalHost().getIPv4Address() && networkNode.getLocalHost().getPort()==50000){												
