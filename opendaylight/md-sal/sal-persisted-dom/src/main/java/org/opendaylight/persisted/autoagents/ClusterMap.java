...
public class ClusterMap<K, V> extends AutonomousAgent implements Map<K, V> {
    ...
    private Map<ChangeID,CCommandEntry> jurnal = new LinkedHashMap<ChangeID, ClusterMap.CCommandEntry>();
    ...
    private void checkForTimeout(){
        for(Iterator<CCommandEntry> iter=jurnal.values().iterator();iter.hasNext();){
            CCommandEntry e = iter.next();
            if(e.ccommand.getOperation()==C_SYNC_FLUSH && System.currentTimeMillis()-e.insertTime>5000){
                iter.remove();
                handleTimeout(e.ccommand);
            }
        }
    }

    private void handleTimeout(CCommand ccommand){
        if(ccommand.getOperation()==C_SYNC_FLUSH){
