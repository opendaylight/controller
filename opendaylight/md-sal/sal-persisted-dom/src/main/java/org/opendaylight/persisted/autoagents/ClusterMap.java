...
public class ClusterMap<K, V> extends AutonomousAgent implements Map<K, V> {
    ...
    private Object sendPingObject = new Object();
    private Map<ChangeID,CCommandEntry> jurnal = new HashMap<ChangeID, ClusterMap.CCommandEntry>();
    private List<ChangeID> jurnalList = new LinkedList<ChangeID>();
    ...
    public ClusterMap(int subSystemId, NetworkID localhost,AutonomousAgentManager m) {
        ...
        Packet sendPing = new Packet(sendPingObject,this.getAgentID(), this.getAgentID());
        this.lastChangeID = new ChangeID(this.getAgentID());
        this.addRepetitiveFrame(sendPing, 15000, 0, 0);
    }
    ...
    static {
        EncodeUtils.registerSerializer(CCommand.class, new CCommand(), ClassCodes.CCommand.getCode());
        EncodeUtils.registerSerializer(ChangeID.class, new ChangeID(), ClassCodes.ChangeID.getCode());
    }
    ...
    private void addJurnal(CCommand command){
        if(this.peers.size()>0){
            CCommandEntry entry = new CCommandEntry();
            entry.ccommand = command;
            jurnal.put(entry.ccommand.getChangeID(), entry);
            entry.expectedReplyFrom.addAll(this.peers.keySet());
            if(command.getOperation()==C_PUT || command.getOperation()==C_REMOVE){
                jurnalList.add(command.getChangeID());
            }
        }
    }
    ...
    private boolean checkForUnreachable(boolean isUnreachable,CCommand ccommand,NetworkID source,NetworkID unreachableOriginalSource){
        ...
        if(isUnreachable){
            ...
            CCommandEntry entry = jurnal.get(ccommand.getChangeID());
            if(entry!=null){
    ...
    public void handleStartSyncMode(){
        if(!(this.syncModeFlag==SYNC_MODE_ON)){
            ...
            addJurnal(syncModeCMD);
            send(syncModeCMD,MULTICAST);
        }
    }

    public void handleSyncFlush(CCommand ccommand,boolean isUnreachable){
        ...
        if(pl.size()==1){
            ...
            if(isUnreachable){
                ...
                for(ChangeID cid:jurnalList){
                    CCommandEntry entry = jurnal.get(cid);
                    if(entry!=null){
    ...
    private void processCCommand(CCommand ccommand, NetworkID source,NetworkID unreachableOriginalSource) {
        ...
        case C_CHANGEID:
            ...
            break;
        case C_SYNC_FLUSH:
            handleSyncFlush(ccommand, isUnreachableLast);
            break;
        case C_SYNC_PUT:
            if(!isUnreachable){
