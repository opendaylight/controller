...
public class TypeDescriptor {
    ...
    private static List<IChildAttributeRule> childIdentifierRules = new ArrayList<IChildAttributeRule>();
    private static List<ITypeAttributeRule> typeIdentifierRules = new ArrayList<ITypeAttributeRule>();
    private static IClassExtractor classExtractor = null;
    private ThreadLocal<Class<?>> rootClass = new ThreadLocal<Class<?>>();
    ...
    public TypeDescriptor(Class<?> cls, ISerializer _serializer, int _hierarchyLevel) {
        synchronized(TypeDesciptorRepository.getInstance()){
            if(rootClass.get()==null){
                rootClass.set(cls);
            }
            ...
            this.typeClass = cls;
            TypeDesciptorRepository.getInstance().registerClass(this);
            ...
            for (Method m : mth) {
                ...
                if (m.getName().startsWith("get") || m.getName().startsWith("is")) {
                    ...
                    if (isChildAttribute(p)) {
                        children.put(p, p);
                        TypeDescriptor child = TypeDesciptorRepository.getInstance().getTypeDescriptorClass(p.getReturnType(),this.hierarchyLevel+1);
                        child.parents.put(p, p);
                    } else {
                        if (isTypeAttribute(p)) {
                            TypeDescriptor typedef = TypeDesciptorRepository.getInstance().getTypeDescriptorByClass(p.getReturnType());
                            if (typedef == null) {
                                typedef = new TypeDescriptor((Class<?>) p.getReturnType(), null,this.hierarchyLevel + 1);
                                TypeDesciptorRepository.getInstance().registerClass(typedef);
                            }
                        }
                        pList.add(p);
                    }
        ...
        if(rootClass.get().equals(cls)){
            TypeDesciptorRepository.getInstance().save();
        }
    }
    ...
    private void initMaps() {
        for (int i = 0; i < this.attributes.length; i++) {
            attributeNameToAttribute.put(this.attributes[i].getColumnName(),
                    this.attributes[i]);
            methodNameToAttribute.put(this.attributes[i].getMethodName(),
                    this.attributes[i]);
        ...
        if (this.serializer == null) {
            try {
                this.serializer = (ISerializer) getClass().getClassLoader()
                        .loadClass(this.typeSerializerClassName).newInstance();
            } catch (Exception err) {
            }
            try {
                String directory = "serializers/"
                        + typeClass.getPackage().getName();
    ...
    public static ISerializer createSerializer(TypeDescriptor type) {
        ...
        String directory = "/src/main/yang-gen-sal/"
                + type.getTypeClass().getPackage().getName();
        directory = "." + replaceAll(directory, ".", "/");
        ...
        file = new File(file.getPath() + "/"
                + type.getTypeClass().getSimpleName() + "Serializer.java");
        try {
    ...
    public static String generateSerializerClass(TypeDescriptor type) {
        ...
        if (type.getTypeClass().isInterface())
            append("import " + type.getTypeClassName() + "Builder;", 0, buff);
        append("", 0, buff);
        ...
        append(type.getTypeClass().getSimpleName() + " element = ("
                + type.getTypeClass().getSimpleName() + ") value;", 8, buff);
        ...
        if (isChildAttribute(type)) {
            ...
            for (AttributeDescriptor child : type.getChildren().keySet()) {
                TypeDescriptor subTable = TypeDesciptorRepository.getInstance()
                        .getTypeDescriptorByClass(child.getReturnType());
        ...
        if (type.getTypeClass().isInterface()) {
            append(type.getTypeClass().getSimpleName() + "Builder builder = new "
                    + type.getTypeClass().getSimpleName() + "Builder();", 8, buff);
            for (AttributeDescriptor p : type.attributes) {
                if (p.getReturnType().equals(short.class) || p.getReturnType().equals(Short.class)) {
                    append("builder.set" + p.getColumnName()
                            + "(EncodeUtils.decodeShort(ba));", 8, buff);
                }else
                if (p.getReturnType().equals(boolean.class) || p.getReturnType().equals(Boolean.class)) {
                    append("builder.set" + p.getColumnName()
            ...
            if (isChildAttribute(type)) {
                append("EncodeUtils.decodeAugmentations(builder, ba,"
                        + type.getTypeClass().getSimpleName() + ".class);", 8,
                        buff);
                for (AttributeDescriptor child : type.getChildren().keySet()) {
                    TypeDescriptor subTable = TypeDesciptorRepository.getInstance()
                            .getTypeDescriptorByClass(child.getReturnType());
                    String shortName = subTable.getTypeClassShortName()
                            .toLowerCase();
    ...
    public static void setClassExtractor(IClassExtractor ce){
        classExtractor = ce;
    }

    public static void addChildAttributeRule(IChildAttributeRule rule){
        childIdentifierRules.add(rule);
    }

    public static void addTypeAttributeRule(ITypeAttributeRule rule){
        typeIdentifierRules.add(rule);
    }

    private static boolean isChildAttribute(AttributeDescriptor ad){
        for(IChildAttributeRule rule:childIdentifierRules){
    ...
    private static boolean isTypeAttribute(AttributeDescriptor ad){
        for(ITypeAttributeRule rule:typeIdentifierRules){
