...
import org.opendaylight.persisted.autoagents.IClusterMapListener;
...
public class TypeDescriptorsContainer {
    ...
    private class PeerListener implements IClusterMapListener<String, TypeDescriptor> {

        @Override
        public void peerPut(String key, TypeDescriptor value) {
            save();
        }

        @Override
        public void peerRemove(String key) {
            save();
        }
    }

    public void initClustering(AutonomousAgentManager m){
        Map<String, TypeDescriptor> map = new ClusterMap<String, TypeDescriptor>(6,m,new PeerListener());
    ...
    public byte[] getMDSALTableRepositoryData() {
        EncodeDataContainer ba = new EncodeDataContainer(1024,this);
        ...
        try {
            ...
            if (f.exists() && f.length() > 0) {
                try {
                    ...
                    EncodeDataContainer ba = new EncodeDataContainer(data,this);
