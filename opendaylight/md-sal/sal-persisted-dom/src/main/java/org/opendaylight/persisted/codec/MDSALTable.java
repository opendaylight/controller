...
public class MDSALTable {
    ...
    public static String generateSerializerClass(MDSALTable type) {
            ...
            for (MDSALColumn child : type.getChildren().keySet()) {
                ...
                if(child.isCollection()){
                    ...
                    append("MDSALTable subTable = MDSALTableRepository.getInstance().getCtypeByClass("
                            + subTable.getMyClassName() + ".class);", 12, buff);
                    append("BytesArray subBA = new BytesArray(1024);", 12, buff);
                    ...
                    append("MDSALTable subTable = MDSALTableRepository.getInstance().getCtypeByClass("
                            + subTable.getMyClassName() + ".class);", 12, buff);
                    append("BytesArray subBA = new BytesArray(1024);", 12, buff);
                    append("subTable.getSerializer().encode(element."+child.getMethodName()+"(), subBA);",
                            12, buff);
                    append("ba.addSubElementData(subTable.getClassCode(),subBA);",
                            12, buff);
                }
        ...
        append("if(MDSALEncoder.isNULL(ba)){return null;}",8,buff);
        if (type.getMyClass().isInterface()) {
                ...
                for (MDSALColumn child : type.getChildren().keySet()) {
                    ...
                    if(child.isCollection()){
                        append("List<" + subTable.getMyClassName()
                                + "> list = new ArrayList<"
                                + subTable.getMyClassName() + ">();", 8, buff);
                        append("for(BytesArray subBA:" + shortName + "List){", 8,
                                buff);
                        append("list.add((" + subTable.getMyClassName() + ")"
                                + shortName + "Serializer.decode(subBA, 0));", 12,
                                buff);
                        append("}", 8, buff);
                        append("builder.set" + child.getColumnName() + "(list);",
                                8, buff);
                    }else{
                        append("builder.set"+child.getColumnName()+"(("+subTable.getMyClassName()+")"+shortName+"Serializer.decode("+shortName+"List.get(0), 0));",12,buff);
                    }
                }
            }
