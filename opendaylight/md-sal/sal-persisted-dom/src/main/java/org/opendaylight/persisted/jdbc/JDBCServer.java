...
public class JDBCServer extends Thread {
    ...
    public JDBCServer(ObjectDataStore a) {
        super("JDBC Server");
        this.database = a;
        start();
    }
    ...
    public static void execute(JDBCResultSet rs, ObjectDataStore database) throws SQLException {
    ...
    public static int addNextLogicalField(String sql, int startIndex,
        ...
        if (node == null) {
            node = new TypeDescriptor(tblName, origTableName);
    ...
    public static void checkAndBreakSubQueries(JDBCResultSet rs,ObjectDataStore database) throws SQLException {
    ...
    public static void parseTables(JDBCResultSet rs) throws SQLException {
        ...
        while (tokens.hasMoreTokens()) {
            ...
            TypeDescriptor table = TypeDesciptorRepository.getInstance().getTypeDescriptorByShortClassName(tableName);
            if (table == null) {
    ...
    public static void addCriteria(AttributeDescriptor col, Criteria c,JDBCResultSet rs) {
    ...
    public static void parseFields(JDBCResultSet rs) throws SQLException {
        ...
        if(fields.equals("Objects")){
            rs.setCollectedDataType(JDBCResultSet.COLLECT_TYPE_OBJECTS);
            fields = "*";
        }
        ...
        while (tokens.hasMoreTokens()) {
            ...
            if (token.equals("*")) {
                for (TypeDescriptor table : rs.getTables()) {
                    ...
                    for(Class<?> augClass:knownAugmentations){
                        TypeDescriptor augTable = TypeDesciptorRepository.getInstance().getTypeDescriptorByClass(augClass);
            ...
            if (token.indexOf(".") != -1) {
                TypeDescriptor tbl = TypeDesciptorRepository.getInstance().getTypeDescriptorByShortClassName(token.substring(0, token.indexOf(".")).trim());
                ...
                for (TypeDescriptor table : rs.getTables()) {
                    ...
                    if(col==null){
                        for(Class<?> c:augClass){
                            TypeDescriptor augTable = TypeDesciptorRepository.getInstance().getTypeDescriptorByClass(c);
