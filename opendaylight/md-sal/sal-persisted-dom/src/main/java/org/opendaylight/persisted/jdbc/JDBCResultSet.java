...
public class JDBCResultSet implements ResultSet,ResultSetMetaData {
    ...
    public List<MDSALColumn> getFields() {
    ...
    public List<Record> addRecords(Object element, MDSALTable node,boolean root,MDSALDatabase db) {
        List<Record> result = new LinkedList<Record>();
        MDSALTable table = MDSALTableRepository.getInstance().getCTypeByObject(element);
        if(checkCriteria(element, table)){
            Record rec = new Record();
            Map augmentationMap = null;
            try{
                augmentationMap = (Map)table.getAugmentationField(element).get(element);
            }catch(Exception err){}
            for(MDSALColumn col:fieldsInQuery){
                rec.data.put(col.toString(), col.get(element,augmentationMap,table.getMyClass()));
            }
            result.add(rec);
            addRecord(rec.data);
        }
