...
public class JDBCResultSet implements ResultSet,ResultSetMetaData {
    ...
    public List<Record> addRecords(ObjectWithInfo info,boolean root) {
        ...
        if(checkCriteria(info.getObject(), info.getTable())){
            Record rec = new Record();
            addInfoToRecord(info, rec);
    ...
    private void addInfoToRecord(ObjectWithInfo info,Record rec){
    ...
    public boolean addParentData(ObjectWithInfo info, Record rec){
        if(this.tablesInQueryMap.size()>1){
            ...
            if(parent!=null && this.tablesInQueryMap.containsKey(parent.getMyClass().getSimpleName())){
                ...
                addInfoToRecord(parentInfo, rec);
            }
        }
        return true;
    }

    public boolean checkCriteria(Object element,MDSALTable table){
        if(!criteria.containsKey(table.getMyShortClassName())){
            return true;
        }else{
            return true;
        }
