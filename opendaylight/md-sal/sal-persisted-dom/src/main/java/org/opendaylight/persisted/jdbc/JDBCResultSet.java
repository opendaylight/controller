...
import org.opendaylight.persisted.MDSALDatabase.ObjectWithInfo;
...
public class JDBCResultSet implements ResultSet,ResultSetMetaData {
    ...
    private List<MDSALTable> tablesInQuery = new ArrayList<MDSALTable>();
    private Map<String, MDSALTable> tablesInQueryMap = new ConcurrentHashMap<String, MDSALTable>();
    private List<MDSALColumn> fieldsInQuery = new ArrayList<MDSALColumn>();
    private Map<String,List<MDSALColumn>> fieldsByTableInQuery = new ConcurrentHashMap<String,List<MDSALColumn>>();
    ...
    private Map<String, Map<MDSALColumn, List<Criteria>>> criteria = new ConcurrentHashMap<String, Map<MDSALColumn, List<Criteria>>>();
    ...
    public void addFieldToQuery(MDSALColumn field,String tableName){
        this.fieldsInQuery.add(field);
        List<MDSALColumn> fieldsInTable = fieldsByTableInQuery.get(tableName);
        if(fieldsInTable==null){
            fieldsInTable = new LinkedList<MDSALColumn>();
    ...
    public int isObjectFitCriteria(Map objValues, String tableName) {
        Map<MDSALColumn, List<Criteria>> tblCriteria = criteria
                .get(tableName);
        if (tblCriteria == null) {
            return 1;
        }
        for (Map.Entry<MDSALColumn, List<Criteria>> cc : tblCriteria
                .entrySet()) {
    ...
    public Map<String, Map<MDSALColumn, List<Criteria>>> getCriteria() {
    ...
    public List<MDSALTable> getTables() {
        return tablesInQuery;
    }

    public void addTableToQuery(MDSALTable node) {
        if (this.tablesInQueryMap.containsKey(node.getMyShortClassName())) {
            return;
        }
        this.tablesInQuery.add(node);
        this.tablesInQueryMap.put(node.getMyShortClassName(), node);
    }

    public List<MDSALColumn> getFieldsInQuery() {
        return this.fieldsInQuery;
    }

    public MDSALTable getMainTable() {
        if (tablesInQuery.size() == 1) {
            return tablesInQuery.get(0);
        }
        MDSALTable result = null;
        for (MDSALTable node : tablesInQuery) {
            if (result == null) {
                result = node;
    ...
    private void createRecord(Object data, MDSALTable node) {
        Map rec = new HashMap();
        for (MDSALColumn c : this.fieldsInQuery) {
            if (c.getTableName().equals(node.getMyShortClassName())) {
                try {
                    Method m = node.getMyClass().getMethod(c.getMethodName(), null);
    ...
    private boolean addInfoToRecord(ObjectWithInfo info,Record rec){
        ...
        if(checkCriteria(info, augmentationMap)){
            List<MDSALColumn> tableFields = fieldsByTableInQuery.get(info.getTable().getMyShortClassName());
            for(MDSALColumn col:tableFields){
                Object value = col.get(info.getObject(),augmentationMap,info.getTable().getMyClass());
                if(value!=null){
    ...
    public boolean addParentData(ObjectWithInfo info, Record rec){
        if(this.tablesInQueryMap.size()>1){
            MDSALTable parent = info.getTable().getParent();
            if(parent!=null && this.tablesInQueryMap.containsKey(parent.getMyClass().getSimpleName())){
    ...
    public boolean checkCriteria(ObjectWithInfo info,Map<?, ?> augmentationMap){
        if(criteria.containsKey(info.getTable().getMyShortClassName())){
            Map<MDSALColumn,List<Criteria>> map = criteria.get(info.getTable().getMyShortClassName());
            for(Map.Entry<MDSALColumn, List<Criteria>> entry:map.entrySet()){
                Object value = entry.getKey().get(info.getObject(), augmentationMap, info.getTable().getMyClass());
    ...
    private class FieldCompare implements Comparator<MDSALColumn>{
        @Override
        public int compare(MDSALColumn o1, MDSALColumn o2) {
