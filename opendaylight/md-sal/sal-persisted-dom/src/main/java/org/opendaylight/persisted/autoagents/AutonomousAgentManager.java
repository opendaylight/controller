...
public class AutonomousAgentManager extends Thread implements IFrameListener {
    ...
    public void processDestinationUnreachable(Packet frame) {
        AutonomousAgent h = networkIdToAgent.get(frame.getSource());
        if (h != null) {
            h.addFrame(frame);
            synchronized (agentsSyncObject) {
                agentsSyncObject.notifyAll();
            ...
            try {
                ...
                while (e != null) {
                    if (e.getName().indexOf("Handler") != -1) {
                        String className = getClassNameFromEntryName(e
                                .getName());
                        try {
                            URLClassLoader cl = new URLClassLoader(
                                    new URL[] { f.toURL() }, this.getClass()
                                            .getClassLoader());
                            /*
                             * @TODO ClassLoaders
                             */
                            ...
                            AutonomousAgent newHandler = (AutonomousAgent) handlerClass
                                    .getConstructor(
                                            new Class[] {
                                                    NetworkID.class,
                                                    AutonomousAgentManager.class })
                                    .newInstance(
                                            new Object[] {
                                                    this.networkNode
                                                            .getLocalHost(),
