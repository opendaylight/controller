...
public class MDSALBlockDataLocation {
    ...
    private void writeSubObjects(BytesArray ba, int parentRecordIndex) {
        Map<Integer, List<BytesArray>> subElementsData = ba.getSubElementsData();
        for (Map.Entry<Integer, List<BytesArray>> entry : subElementsData.entrySet()) {
            MDSALTable table = MDSALTableRepository.getInstance() .getCTypeByCode(entry.getKey());
            MDSALDataFile df = getDataFile(table.getMyClass());
            for (BytesArray subBA : entry.getValue()) {
                int recordIndex = df.writeData(parentRecordIndex,subBA.getMD5ID() ,subBA.getData());
                writeSubObjects(subBA, recordIndex);
            }
        }
    }

    public Object readObject(Class<?> clazz,MD5ID md5ID) {
    ...
    public Object readObjectNoChildren(MDSALTable table, int recordIndex) {
        MDSALDataFile dataFile = this.dataFiles.get(table.getMyClass());
        byte data[] = dataFile.read(recordIndex);
        if(data==null){
            data = new byte[2];
            MDSALEncoder.encodeNULL(data, 0);
    ...
    public MDSALDataFile getDataFile(Class<?> c) {
