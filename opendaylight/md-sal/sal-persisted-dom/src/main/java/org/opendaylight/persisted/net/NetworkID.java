...
public class NetworkID implements ISerializer{
	...
	static{
	    EncodeUtils.registerSerializer(NetworkID.class, new NetworkID());
	}
	...
	public static NetworkID valueOf(String str){
		...
		int addr = (Integer.parseInt(sdata[0]) * 16777216) + (Integer.parseInt(sdata[1]) * 65536) + (Integer.parseInt(sdata[2]) * 256) + (Integer.parseInt(sdata[3]));		
		return new NetworkID(addr, Integer.parseInt(port),Integer.parseInt(subSystemID));
	}
	...
	public String toString(){
		...
		if(address.length==3){
			...
			if(ipv4[0]<0) {ipString[0] = ""+(256+ipv4[0]);}else {ipString[0]=""+ipv4[0];}
			if(ipv4[1]<0) {ipString[1] = ""+(256+ipv4[1]);}else {ipString[1]=""+ipv4[1];}
			if(ipv4[2]<0) {ipString[2] = ""+(256+ipv4[2]);}else {ipString[2]=""+ipv4[2];}
			if(ipv4[3]<0) {ipString[3] = ""+(256+ipv4[3]);}else {ipString[3]=""+ipv4[3];}			
			buff.append(ipString[0]).append(".").append(ipString[1]).append(".").append(ipString[2]).append(".").append(ipString[3]).append(":").append(address[address.length-2]).append(":").append(address[address.length-1]);
    ...
    public void encode(Object value, byte[] byteArray, int start) {
        for(int i=0;i<address.length;i++){
            if(i<address.length-2)
                EncodeUtils.encodeInt32(address[i],byteArray, start);
            else
            if(i==address.length-2)
                EncodeUtils.encodeInt16(address[i],byteArray, start+i*4);                               
            else
                EncodeUtils.encodeInt16(address[i],byteArray, start+(i-1)*4+2);             
        }
    }

    @Override
    public void encode(Object value, EncodeDataContainer ba) {
        this.encode(value,ba.getBytes(),ba.getLocation());
    }
    ...
    public Object decode(EncodeDataContainer ba,int length) {
        Object nid = decode(ba.getBytes(),ba.getLocation(),length);
        ba.advance(8);
        return nid;
    }
