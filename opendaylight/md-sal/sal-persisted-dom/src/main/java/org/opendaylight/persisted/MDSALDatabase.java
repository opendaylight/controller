...
public class MDSALDatabase {
    ...
    public Object readNoChildren(Class<?> clazz, int index) {
        ...
        MDSALTable table = MDSALTableRepository.getInstance().getCtypeByClass(clazz);
        return gl.readObjectNoChildren(table, index);
    }

    public ObjectWithInfo readNoChildrenWithLocation(MDSALTable table, int index) {
        MDSALBlockDataLocation gl = getGLocation("Default");
    ...
    public static class ObjectWithInfo {
        private MDSALBlockDataLocation bLocation = null;
        private Object object = null;
        private int recordIndex = -1;
        private MDSALTable table = null;

        public ObjectWithInfo(MDSALBlockDataLocation loc,MDSALTable tbl, Object o, int recIndex){
            this.bLocation = loc;
            this.object = o;
            this.table = tbl;
        ...
        public MDSALBlockDataLocation getbLocation() {
            return bLocation;
        }
        ...
        public MDSALTable getTable() {
            return table;
        }

        public ObjectWithInfo getParenInfo(){
            MDSALDataFile df = this.bLocation.getDataFile(this.table.getMyClass());
            int index = df.getParentIndex(this.recordIndex);
            if(index!=-1){
                MDSALTable parentTable = this.table.getParent();
    ...
    public Object read(Object objectKey,Class<?> clazz){
        MDSALBlockDataLocation gl = getGLocation("Default");
        return gl.readObject(clazz, MD5ID.createX(objectKey.toString()));
    }
    ...
    public void executeSql(String sql, PrintStream out, boolean toCsv) {
        ...
        try {
            ...
            for (MDSALColumn c : rs.getFieldsInQuery()) {
                if (isFirst) {
                    isFirst = false;
                    if (toCsv) {
                        out.print("\"");
