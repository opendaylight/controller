...
public class MDSALEncoder {
    ...
    public static final void encodeBigDecimal(BigDecimal value, BytesArray ba) {
        if(value==null){
            encodeNULL(ba);
            return;
        }
    ...
    public static final BigDecimal decodeBigDecimal(BytesArray ba){
        if(isNULL(ba))
            return null;
    ...
    public static final void encodeArray(Object value[], BytesArray ba,Class<?> componentType) {
        if (value == null) {
            encodeNULL(ba);
        } else {
            encodeInt32(value.length, ba);
