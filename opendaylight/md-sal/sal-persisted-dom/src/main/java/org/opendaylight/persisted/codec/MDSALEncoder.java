...
public class MDSALEncoder {
    ...
    public static ISerializer getSerializer(Class<?> cls) {
        MDSALTable type = MDSALTableRepository.getInstance().getCtypeByClass(
                cls);
        if (type != null) {
    ...
    public static ISerializer getSerializer(int typeCode) {
        MDSALTable type = MDSALTableRepository.getInstance().getCTypeByCode(
                typeCode);
        if (type != null) {
            return type.getSerializer();
        }
    ...
    public static final void encodeInt16(int value, byte[] byteArray,
            int location) {
    ...
    public static final void encodeInt32(int value, BytesArray ba) {
        ba.adjustSize(4);
    ...
    public static final void encodeInt64(long value, byte[] byteArray,
            int location) {
    ...
    public static final void encodeByteArray(byte[] value, byte byteArray[],
            int location) {
    ...
    public static final void encodeBoolean(boolean value, byte[] byteArray,
            int location) {
        if (value)
    ...
    public static final void encodeAugmentations(Object value, BytesArray ba) {
        if (value == null) {
            encodeNULL(ba);
            return;
        }
        MDSALTable ctype = MDSALTableRepository.getInstance().getCtypeByClass(((DataObject) value).getImplementedInterface());
        if (!ctype.isAugmentationFieldInitialized()) {
            ctype.setAugmentationFieldInitialized(true);
            try {
                ctype.setAugmentationField(MDSALTable.findField(
    ...
    public static final void decodeAugmentations(Object builder, BytesArray ba,Class<?> augmentedClass) {
        if (isNULL(ba)) {
            return;
        } else {
            MDSALTable ctype = MDSALTableRepository.getInstance()
                    .getCtypeByClass(augmentedClass);
            if (!ctype.isAugmentationFieldBuilderInitialized()) {
                ctype.setAugmentationFieldBuilderInitialized(true);
                try {
                    ctype.setAugmentationFieldBuilder(MDSALTable.findField(
    ...
    public static final void encodeAndAddObject(Object value, BytesArray ba,Class<?> objectType) {
        if (value == null) {
            encodeNULL(ba);
        } else {
            int classCode = MDSALTableRepository.getInstance().getCtypeByClass(objectType).getClassCode();
            encodeInt16(classCode,ba);
            BytesArray subBA = new BytesArray(1024);
            ISerializer serializer = getSerializer(objectType);
            if (serializer != null) {
                encodeInt16(classCode, subBA);
                serializer.encode(value, subBA);
                ba.addSubElementData(classCode, subBA);
            } else {
    ...
    public static final void encodeObject(Object value, BytesArray ba,Class<?> objectType) {
        if (value == null) {
            encodeNULL(ba);
        } else {
            ISerializer serializer = getSerializer(objectType);
            if (serializer != null) {
                int classCode = MDSALTableRepository.getInstance()
                        .getCtypeByClass(objectType).getClassCode();
                encodeInt16(classCode, ba);
    ...
    public static final Object decodeObject(BytesArray ba) {
        ...
        if (serializer == null) {
            MDSALTableRepository.getInstance().getCTypeByCode(classCode).getMyClass();
        }
        if (serializer != null) {
    ...
    public static final Object decodeAndObject(BytesArray ba) {
        if (isNULL(ba)) {
            return null;
        }
        int classCode = decodeInt16(ba);
        ...
        if (serializer != null) {
            BytesArray subBA = ba.getSubElementsData().get(classCode).get(0);
            subBA.advance(2);
    ...
    public static final void encodeList(List<?> list, BytesArray ba,Class<?> componentType) {
        if (list == null) {
            encodeNULL(ba);
        } else {
    ...
    public static final void encodeAndAddList(List<?> list, BytesArray ba,Class<?> componentType) {
        if (list == null) {
            encodeNULL(ba);
        } else {
            encodeInt32(list.size(), ba);
            int componentCode = MDSALTableRepository.getInstance().getCtypeByClass(componentType).getClassCode();
            for (Object o: list) {
                BytesArray subBA = new BytesArray(1024);
                if (String.class.equals(componentType)) {
                ...
                ba.addSubElementData(componentCode, subBA);
            }
        }
    }

    public static final Object[] decodeArray(BytesArray ba,Class<?> componentType) {
        if (isNULL(ba)) {
            return null;
        }
        int size = decodeInt32(ba);
        ...
        for (int i = 0; i < result.length; i++) {
            if (int.class.equals(componentType)) {
                ...
                result[i] = decodeInt64(ba);
            } else if (boolean.class.equals(componentType)) {
                result[i] = decodeBoolean(ba);
    ...
    public static final List decodeList(BytesArray ba,Class<?> componentType) {
        if (isNULL(ba)) {
            return null;
        }
        int size = decodeInt32(ba);
    ...
    public static final List decodeAndList(BytesArray ba,Class<?> componentType) {
        if (isNULL(ba)) {
            return null;
        }
        int size = decodeInt32(ba);
        List result = new ArrayList(size);
        List<BytesArray> subElementData = ba.getSubElementsData().get(MDSALTableRepository.getInstance().getCtypeByClass(componentType).getClassCode());
        for (int i = 0; i < subElementData.size(); i++) {
            BytesArray subBA = subElementData.get(i);
            if (int.class.equals(componentType)) {
