...
public abstract class AutonomousAgent implements Runnable {
    ...
    public boolean pseudoSendEnabled = false;

    public AutonomousAgent(int subSystemID, NetworkID localHost,AutonomousAgentManager _manager) {
        this.agentID = new NetworkID(localHost.getIPv4Address(),localHost.getPort(), subSystemID);
    ...
    public void send(Object obj, NetworkID destination) {
        if(pseudoSendEnabled) return;
        EncodeDataContainer ba = new EncodeDataContainer(1024,this.manager.getTypeDescriptorsContainer());
        EncodeUtils.encodeObject(obj, ba,this.manager.getTypeDescriptorsContainer().getElementClass(obj));
