...
public abstract class AutonomousAgent implements Runnable{
	...
	public AutonomousAgent(int subSystemID,NetworkID localHost,AutonomousAgentManager _manager){
		this.handlerID = new NetworkID(localHost.getIPv4Address(), localHost.getPort(), subSystemID);
	...
	public void run(){
		...
		if(currentFrame.getDecodedObject() instanceof ISideTask){
			this.getHandlerManager().runSideTask((ISideTask)currentFrame.getDecodedObject());
		}else{
	...
	public void send(Object obj, NetworkID destination){
		...
		ser.encode(obj,ba);
		manager.getNetworkNode().send(ba.getData(), this.handlerID, destination);
	}
