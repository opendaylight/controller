...
public class NetworkNode extends Thread{
	...
	public NetworkNode(IFrameListener _frameListener){
		...
		for(int i=50000;i<60000;i++){
			try{
				...
				int localhost = NetworkID.valueOf(EncodeUtils.getLocalIPAddress()+":0:0").getIPv4Address();
	...
	public void shutdown(){
	    this.running = false;
        try{this.connection.shutdown();}catch(Exception err){}
	    try{this.socket.close();}catch(Exception err){}this.socket = null;
	    for(Map<Integer,NetworkNodeConnection> map:routingTable.values()){
	        for(NetworkNodeConnection con:map.values()){
	            try{con.shutdown();}catch(Exception err){}
	        }
	    }
	}
	...
	public void send(byte data[],NetworkID source,NetworkID dest){
		if(data.length<Packet.MAX_DATA_IN_ONE_PACKET){
			...
			for(int i=0;i<count;i++){
				...
				if(i<count-1){
					System.arraycopy(data, i*Packet.MAX_DATA_IN_ONE_PACKET, pData,0,pData.length);
				}else{
					System.arraycopy(data, i*Packet.MAX_DATA_IN_ONE_PACKET, pData,0,data.length-(i*Packet.MAX_DATA_IN_ONE_PACKET));
				}
	...
	private class RequestConnection extends Thread{
		...
		public RequestConnection(){
		    this.setName("Request Connection Thread R:"+getLocalHost().getPort()+"    ");
			this.start();
		}
			...
			for(Map.Entry<Integer, NetworkNodeConnection> portEntry:addrEntry.getValue().entrySet()){
				if(sourceAddress!=this.getLocalHost().getIPv4Address()){
					if(addrEntry.getKey()==this.getLocalHost().getIPv4Address()){
						try{portEntry.getValue().sendPacket(data);}catch(Exception err){err.printStackTrace();}
					}
				}else{
					try{portEntry.getValue().sendPacket(data);}catch(Exception err){err.printStackTrace();}					
				}
	...
	private class PacketProcessor extends Thread{
		...
		private class MultiPartContainer {
			...
			public byte[] toFrame(){
				...
				byte[] data = new byte[(firstPart.length-Packet.PACKET_DATA_LOCATION)*parts.size()+Packet.PACKET_DATA_LOCATION];
				System.arraycopy(firstPart,0, data,0, Packet.PACKET_DATA_LOCATION);
				int location = Packet.PACKET_DATA_LOCATION;
				for(byte[] p:parts){
					System.arraycopy(p, Packet.PACKET_DATA_LOCATION,data, location, p.length-Packet.PACKET_DATA_LOCATION);
		...
		public void addPacket(byte[] packet){
			...
			if(!multiPart){
				...
				Packet pID = (Packet)serializer.decode(packet, 0, Packet.PACKET_DATA_LOCATION);
				MultiPartContainer mpc = multiparts.get(pID);
				if(mpc==null){
					mpc = new MultiPartContainer();
					multiparts.put(pID,mpc);
					mpc.expectedCount = EncodeUtils.decodeInt32(packet,Packet.PACKET_DATA_LOCATION);
				}else{
			...
			while(running){
				...
				synchronized(incomingFrames){
					if(incomingFrames.size()==0){
						try{incomingFrames.wait(5000);}catch(Exception err){}
					}
					if(incomingFrames.size()>0){
				...
				if(frame!=null){
					...
					if(frameListener!=null){
						...
						if(f.getDestination().getIPv4Address()==0 && f.getDestination().getSubSystemID()==NetworkNodeConnection.DESTINATION_BROADCAST){
