...
public class NetworkNode extends Thread {
    ...
    public void send(Packet m) {
        if (this.connection != null) {
            try {
                byte pData[] = new byte[Packet.PACKET_DATA_LOCATION
                        + m.getData().length];
                m.encode(m, pData, 0);
            ...
            NetworkNodeConnection c = this.getNodeConnection(m.getDestination()
                    .getIPv4Address(), m.getDestination().getPort());
            if (c != null) {
                try {
                    byte pData[] = new byte[Packet.PACKET_DATA_LOCATION
                            + m.getData().length];
    ...
    public boolean registerNetworkNodeConnection(NetworkNodeConnection c,
            NetworkID source) {
        synchronized (this) {
            ...
            if (this.localHost.getPort() != 50000 && this.connection == null) {
                ...
                Map<Integer, NetworkNodeConnection> map = this.routingTable
                        .get(source.getIPv4Address());
                if (map == null) {
    ...
    public void broadcast(byte data[]) {
        int sourceAddress = EncodeUtils.decodeInt32(data,
                Packet.PACKET_SOURCE_LOCATION);
        for (Map.Entry<Integer, Map<Integer, NetworkNodeConnection>> addrEntry : this.routingTable
                .entrySet()) {
            for (Map.Entry<Integer, NetworkNodeConnection> portEntry : addrEntry
                    .getValue().entrySet()) {
                if (sourceAddress != this.getLocalHost().getIPv4Address()) {
                    if (addrEntry.getKey() == this.getLocalHost()
                            .getIPv4Address()) {
                        try {
                            portEntry.getValue().sendPacket(data);
                        } catch (Exception err) {
                            err.printStackTrace();
                        }
                    }
    ...
    private class PacketProcessor extends Thread {
        ...
        public void run() {
            while (running) {
                ...
                if (frame != null) {
                    Packet f = (Packet) serializer.decode(frame, 0,
                            frame.length);
