...
public class MDSALDBTest {
    ...
    private static boolean createTestResources = true;
    ...
    public void testSelect100Of10000(){
        setStartTime("Inserting 10");
        for(int i=0;i<10000;i++){
            SalPersistedDomTest before = buildTestElement(i,true,true,true,true);
            database.write(before, -1);
        }
        setEndTime();
        setStartTime("Selecting 1 of those 10...");
        executeQueryTest("select * from SalPersistedDomTest where RowIndex>=4 and RowIndex<=4;","testSelect100Of10000",database);
        setEndTime();
    }
