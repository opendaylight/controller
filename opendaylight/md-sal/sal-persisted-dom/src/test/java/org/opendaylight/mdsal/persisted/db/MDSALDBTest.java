...
public class MDSALDBTest {
    ...
    public void testThreads(){
        ThreadPool tp = new ThreadPool(5, "Test", 2000);
        for(int i=0;i<5;i++){
            tp.addTask(new Task(database));
        }
        try{Thread.sleep(10000);}catch(Exception err){err.printStackTrace();}
        for(int i=0;i<500;i++){
            SalPersistedDomTest after = (SalPersistedDomTest)database.read(SalPersistedDomTest.class, i);
            if(after==null){
    ...
    private class Task implements Runnable {
        ...
        public Task(ObjectDataStore _db){
            this.db = _db;
        }
        public void run(){
            for(int i=0;i<100;i++){
