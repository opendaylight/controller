 ...
 * which has dispatch methods for each defined notification. Methods are invoked based on
 * notification type (class).</li>
 * </ul>
 * <h3>Generic Listener</h3>
 * <p>
 * A generic listener implements the {@link NotificationListener} interface which has one callback
 * method <code>onNotification</code> that is invoked for any notification type the listener is
 * subscribed to.
 * <p>
 * A generic listener is subscribed using the
 * {@link #registerNotificationListener(Class, NotificationListener)} method by which you specify
 * the type of notification to receive. A generic listener may be registered for multiple
 * notification types via multiple subscriptions.
 * <p>
 * Generic listeners allow for a more flexible approach, allowing you to subscribe for just one type
 * of notification from a YANG model. You could also have a general subscription for all
 * notification in the system via
 *
 * <pre>
 ...
 *
 * @deprecated Please use {@link org.opendaylight.controller.md.sal.binding.api.NotificationService}
 *             instead.
 */
@Deprecated
