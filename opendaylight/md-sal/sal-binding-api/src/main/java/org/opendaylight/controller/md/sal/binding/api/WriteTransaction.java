...
public interface WriteTransaction extends AsyncWriteTransaction<InstanceIdentifier<?>, DataObject> {
     ...
     * <p>
     * This method is equivalent to invoking {@link #put(LogicalDatastoreType, InstanceIdentifier, DataObject, boolean)}
     ...
     * its pre-existing state by using put, consider using {@link #merge} instead.
     *
     * Note: Using <code>createMissingParents</code> with value true, may introduce garbage in data store,
     * or recreate nodes, which were deleted by previous transaction.
     *
     * @param store
     *            the logical data store which should be modified
     * @param path
     ...
     * @param data
     *            the data object to be written to the specified path
     * @param createMissingParents - If true, data broker will create missing parents, using merge
     *   merge operation for all parent nodes, which were missing at transaction start.
     * @throws IllegalStateException
     *             if the transaction has already been submitted
     */
     ...
     *
     * <p>
     * This method is equivalent to invoking {@link #merge(LogicalDatastoreType, InstanceIdentifier, DataObject, boolean)}
     * with <code>createMissingParents</code> set to false.
     *
     * @param store
     *            the logical data store which should be modified
