...
public class JDBCResultSet
    ...
    private void addToData(Record rec, XSQLBluePrintNode bpn, Map fullRecord) {
        if (bpn != null) {
            for (XSQLColumn c : fieldsInQuery) {
                if (((XSQLBluePrintNode) c.getBluePrintNode()).getODLNodeName()
                    .equals(XSQLODLUtils.getNodeIdentiofier(rec.element))) {
                ...
                for (Object c : _children) {
                    if (c.getClass().getName()
                        ...
                        if (tableName.indexOf(
                            XSQLODLUtils.getNodeIdentiofier(c).toString())
                            != -1) {
                            result.add(c);
                        }
                    }
        ...
        for (Record subRec : subRecords) {
            ...
            if (subO != null) {
                for (Object subData : subO) {
                    ...
                    if (bpn != null) {
                        Map allKeyValues =
                            collectColumnValues(rec.element, bpn);
