...
public abstract class AbstractNativeEosTest {
    ...
    protected static ClusterNode startup(final int port, final List<String> roles)
            throws ExecutionException, InterruptedException {
        return startup(port, roles, Collections.emptyList());
    }
    ...
    protected static void unregisterCandidates(final ClusterNode node,
    ...
    protected static void reachableMember(final ClusterNode node, final String role) {
        node.getOwnerSupervisor().tell(new MemberReachableEvent(
                new Address("akka", "ClusterSystem@127.0.0.1:2550"), Collections.singleton(role)));
    }

    protected static void unreachableMember(final ClusterNode node, final String role) {
        node.getOwnerSupervisor().tell(new MemberUnreachableEvent(
                new Address("akka", "ClusterSystem@127.0.0.1:2550"), Collections.singleton(role)));
    }
    ...
    protected static void verifyListenerState(final MockEntityOwnershipListener listener,
                                              ...
                                              final boolean wasOwner) {
        Awaitility.await().until(() -> listener.getChanges().size() > 0);
    ...
    protected static void verifyListenerState(final MockEntityOwnershipListener listener,
        ...
        Awaitility.await().until(() -> listener.getChanges().size() == changeIndex + 1);

        final DOMEntityOwnershipChange domEntityOwnershipChange = listener.getChanges().get(changeIndex);
        assertEquals(entity, domEntityOwnershipChange.getEntity());
    ...
    protected static void verifyNoNotifications(final MockEntityOwnershipListener listener) {
        Awaitility.await().pollDelay(2, TimeUnit.SECONDS).until(() -> listener.getChanges().size() == 0);
    }
    ...
    protected static final class MockEntityOwnershipListener implements DOMEntityOwnershipListener {
        ...
        private MockEntityOwnershipListener(final String member) {
