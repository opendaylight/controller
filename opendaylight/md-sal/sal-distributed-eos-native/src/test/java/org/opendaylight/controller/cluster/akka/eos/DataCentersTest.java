...
public class DataCentersTest extends AbstractNativeEosTest {
    ...
    public void setUp() throws Exception {
        node1 = startupWithDatacenter(2550, Collections.singletonList("member-1"), TWO_NODE_SEED_NODES, "dc-primary");
        node2 = startupWithDatacenter(2551, Collections.singletonList("member-2"), TWO_NODE_SEED_NODES, "dc-primary");
        node3 = startupWithDatacenter(2552, Collections.singletonList("member-3"), TWO_NODE_SEED_NODES, "dc-backup");
        node4 = startupWithDatacenter(2553, Collections.singletonList("member-4"), TWO_NODE_SEED_NODES, "dc-backup");
    ...
    public void testDatacenterActivation() throws Exception {
        ...
        // current index is size - 1
        final int listener1Index = listener1.getChanges().size() - 1;

        unregisterCandidates(node3, ENTITY_1, "member-3");

        // checking index after notif so current + 1
        verifyListenerState(listener1, ENTITY_1, listener1Index + 1, true, false, false);
