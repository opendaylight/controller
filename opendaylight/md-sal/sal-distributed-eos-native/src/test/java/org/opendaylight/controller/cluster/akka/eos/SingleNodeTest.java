...
public class SingleNodeTest extends AbstractNativeEosTest {
    ...
    public void tearDown() throws Exception {
        clusterNode.getActorSystem().terminate();
        clusterNode.getActorSystem().getWhenTerminated().toCompletableFuture().get();
    }
    ...
    public void testMultipleEntities() {
        ...
        unregisterCandidates(clusterNode, ENTITY_1, "member-1");
        verifyListenerState(listener1, ENTITY_1, 1, true, false, true);
        verifyNoNotifications(listener2);

        registerCandidates(clusterNode, ENTITY_2, "member-2");
        verifyListenerState(listener1, ENTITY_1, 1, true, false, true);
        verifyListenerState(listener2, ENTITY_2, 0, true, false, false);

        unregisterCandidates(clusterNode, ENTITY_2, "member-2");

        verifyListenerState(listener1, ENTITY_1, 1, true, false, true);
        verifyListenerState(listener2, ENTITY_2, 1, false, false, false);
    }
}
