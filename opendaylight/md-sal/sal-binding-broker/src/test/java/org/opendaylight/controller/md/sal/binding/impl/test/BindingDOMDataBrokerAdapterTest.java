...
public class BindingDOMDataBrokerAdapterTest {
    ...
    @Test
    public void testRemoteDataChangeListernerRegisteration() {
        ...
        ArgumentCaptor<RemoteDOMDataChangeListener> RemoteDOMListener = ArgumentCaptor.forClass(RemoteDOMDataChangeListener.class);
        ArgumentCaptor<LogicalDatastoreType> logicalDatastoreType = ArgumentCaptor.forClass(LogicalDatastoreType.class);
        ArgumentCaptor<AsyncDataBroker.DataChangeScope> dataChangeScope = ArgumentCaptor.forClass(AsyncDataBroker.DataChangeScope.class);
        ArgumentCaptor<YangInstanceIdentifier> yangInstanceIdentifier = ArgumentCaptor.forClass(YangInstanceIdentifier.class);
        ...
        Mockito.verify(dataBroker).registerDataChangeListener(logicalDatastoreType.capture(), yangInstanceIdentifier.capture(),
            RemoteDOMListener.capture(), dataChangeScope.capture());

    }

    private class TestListener implements RemoteDataChangeListener {
