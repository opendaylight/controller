 ...
 * which this class provides; and (b) to integrate it with the
 * XtendBuilderExtensions (for ">>" instead of "->" and no build() method calls).
 *
    ...
    public String getExpression(Object bean) {
        if (useBuilderExtensions(bean)) {
            return new StringBuilder("import static extension ").append(XtendBuilderExtensions.class.getName())
                    .append(".operator_doubleGreaterThan\n\n").append(super.getExpression(bean)).toString();
        } else {
            return super.getExpression(bean);
        }
    }
