...
public class RpcInvocationStrategyTest {
	...
	@Mock private BindingIndependentMappingService mockMappingService ;
	@Mock private RpcProvisionRegistry mockbiRpcRegistry ;
	...
	public void testInit() throws Exception {
	    URI urn = new URI( new String ( "urn:james:dean:lives:on") ) ;
	}

	private void setupInputForwardToDom( boolean hasOutput, int expectedErrorSize ) {

		if ( expectedErrorSize > 0 ) errors.add(rpcError);
        ...
        when(mockbiRpcRegistry.invokeRpc(eq(mockQName), any(CompositeNode.class))).thenReturn(futureCompNode) ;
		if ( hasOutput ) {
			when(mockMappingService.dataObjectFromDataDom(eq(rpcInvocationStrategy.getOutputClass().get()), any(CompositeNode.class))).thenReturn(outputForward) ;
		}

	}

	private void validateForwardToDomBroker(
			ListenableFuture<RpcResult<?>> forwardToDomBroker, boolean expectedSuccess, DataObject expectedResult, int expectedErrorSize )
	...
	private void setupTestMethod( String rpcName, String testMethodName, boolean hasInput ) throws NoSuchMethodException {
	    System.out.println( testMethodName + this );
        mockQName = new QName (urn, new Date(0L), new String ("prefix"), new String ( rpcName ) ) ;
        java.lang.reflect.Method rpcMethod = hasInput ? MockRpcService.class.getMethod(rpcName, DataObject.class ) : MockRpcService.class.getMethod( rpcName ) ;
        rpcInvocationStrategy = new RpcInvocationStrategy( mockQName, rpcMethod, mockMappingService, mockbiRpcRegistry );
	}
/*
 ...
 */
	...
	public void testForwardToDomBroker_WithInputNoOutput() throws Exception {
        ...
        setupInputForwardToDom( false, 0 );
        ListenableFuture<RpcResult<?>> forwardToDomBroker = rpcInvocationStrategy.forwardToDomBroker(inputForward);
	...
	public void testForwardToDomBroker_WithInputNoOutput_error() throws Exception {
        ...
        setupInputForwardToDom( false, 1);
        ListenableFuture<RpcResult<?>> forwardToDomBroker = rpcInvocationStrategy.forwardToDomBroker(inputForward);
	...
	private void validateReturnedImmediateFuture(
			ListenableFuture<RpcResult<CompositeNode>> immediateFuture, boolean expectedSuccess, CompositeNode expectedReturn, int expectedErrorSize)
			throws InterruptedException, ExecutionException {
	...
	public void testInvokeOn_NoInputNoOutput() throws Exception {
        ...
        ListenableFuture<RpcResult<CompositeNode>> immediateFuture = Futures.immediateFuture(rpcInvocationStrategy.invokeOn(mockRpcService, inputInvokeOn));
