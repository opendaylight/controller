...
final class GetValueRouteContextExtractor extends ContextReferenceExtractor {
    ...
    public GetValueRouteContextExtractor(final Method contextGetter, final Method getValueMethod) {
        try {
            contextHandle = MethodHandles.publicLookup().unreflect(contextGetter)
                    .asType(MethodType.methodType(Object.class,DataObject.class));
            valueHandle = MethodHandles.publicLookup().unreflect(getValueMethod)
