...
public class RpcProviderRegistryImpl implements RpcProviderRegistry, RouteChangePublisher<RpcContextIdentifier, InstanceIdentifier<?>> {
    ...
    private final LoadingCache<Class<? extends RpcService>, RpcRouter<?>> rpcRouters = CacheBuilder.newBuilder().weakKeys()
            .build(new CacheLoader<Class<? extends RpcService>, RpcRouter<?>>() {

                @Override
                public RpcRouter<?> load(final Class<? extends RpcService> type) throws Exception{
    ...
    public <T extends RpcService> RpcRouter<T> getRpcRouter(final Class<T> type) {
        try {
            return (RpcRouter<T>) rpcRouters.get(type);
