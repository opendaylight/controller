...
class RuntimeCodeGenerator extends AbstractRuntimeCodeGenerator {
    ...
    protected override <T extends RpcService> directProxySupplier(Class<T> iface) {
        return [|
    ...
    protected override <T extends RpcService> routerSupplier(Class<T> iface, RpcServiceMetadata metadata) {
        return [ |
            val supertype = iface.asCtClass
    ...
    protected override generateListenerInvoker(Class<? extends NotificationListener> iface) {
