...
class FlowCapableTopologyExporter implements FlowTopologyDiscoveryListener, OpendaylightInventoryListener {
    ...
    public void onNodeRemoved(final NodeRemoved notification) {
        processor.enqueueOperation(new TopologyOperation() {
            ...
            public void applyOperation(final ReadWriteTransaction transaction) {
                ...
                final CheckedFuture txState = transaction.submit();
                Futures.addCallback(txState, new FutureCallback() {
                    @Override
                    public void onSuccess(Object o) {
                        LOG.trace("Node {} removed.", nodeId);
                    }

                    @Override
                    public void onFailure(Throwable throwable) {
                        LOG.error("Error removing node {}.", nodeId, throwable);
                    }
                });
            }
        });
            ...
            processor.enqueueOperation(new TopologyOperation() {
                ...
                public void applyOperation(final ReadWriteTransaction transaction) {
                    ...
                    Futures.addCallback(txState, new FutureCallback() {
                        @Override
                        public void onSuccess(Object o) {
                            LOG.trace("Write operational data for node {} with path {} successful.", node, path);
                        }

                        @Override
                        public void onFailure(Throwable throwable) {
                            LOG.error("Error writing operational data for node {} with path {} successful.", node, path, throwable);
                        }
                    });
                }
            });
        ...
        processor.enqueueOperation(new TopologyOperation() {
            ...
            public void applyOperation(final ReadWriteTransaction transaction) {
                ...
                Futures.addCallback(future, new FutureCallback() {
                    @Override
                    public void onSuccess(Object o) {
                        LOG.trace("Deletion of operational data for node connector{} successful.", tpId);
                    }

                    @Override
                    public void onFailure(Throwable throwable) {
                        LOG.error("Error deleting operational data for node connector{}.", tpId, throwable);
                    }
                });
            }
        });
            ...
            processor.enqueueOperation(new TopologyOperation() {
                ...
                public void applyOperation(final ReadWriteTransaction transaction) {
                    ...
                    Futures.addCallback(txState, new FutureCallback() {
                        @Override
                        public void onSuccess(Object o) {
                            LOG.trace("Node connector {} with path {} updated.", nodeId, path);
                        }

                        @Override
                        public void onFailure(Throwable throwable) {
                            LOG.error("Error updating node connector {} with path.", nodeId, path, throwable);
                        }
                    });
        ...
        processor.enqueueOperation(new TopologyOperation() {
            ...
            public void applyOperation(final ReadWriteTransaction transaction) {
                ...
                Futures.addCallback(future, new FutureCallback() {
                    @Override
                    public void onSuccess(Object o) {
                        LOG.trace("Write data to operational datastore for link {} with path {} successful.", link, path);
                    }

                    @Override
                    public void onFailure(Throwable throwable) {
                        LOG.error("Error writin data to operational datastore for link {} with path {} successful.", link, path, throwable);
                    }
                });
            }
        });
    ...
    private void removeAffectedLinks(final ReadWriteTransaction transaction, final NodeId id) {
    ...
    private void removeAffectedLinks(final ReadWriteTransaction transaction, final TpId id) {
        CheckedFuture<Optional<Topology>, ReadFailedException> topologyDataFuture = transaction.read(LogicalDatastoreType.OPERATIONAL, topology);
        Futures.addCallback(topologyDataFuture, new FutureCallback<Optional<Topology>>() {
            @Override
            public void onSuccess(Optional<Topology> topologyOptional) {
