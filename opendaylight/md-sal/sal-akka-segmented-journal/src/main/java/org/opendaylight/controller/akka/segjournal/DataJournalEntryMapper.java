...
class DataJournalEntryMapper {
    ...
    EntryPosition getPositionOfSequenceNr(final long sequenceNr) {
        final EntryPosition positionFromMapping = sequenceNrToPositionMap.get(sequenceNr);
    ...
    private void rebuildMap() {
        ...
        try (SegmentedJournalReader<DataJournalEntry> reader = dataJournal.openReader(getLastDelete() + 1)) {
            ...
            //TODO: maybe propagate to the actor?
            LOG.error("{}: cannot read data journal - cannot rebuild map", persistenceId, e);
        }
    }
    ...
    long getHighestSequenceNr() {
        return !sequenceNrToPositionMap.isEmpty() ? sequenceNrToPositionMap.lastKey() : 0L;
    }
