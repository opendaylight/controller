...
final class SegmentedJournalActor extends AbstractActor {
    ...
    private void handleDeleteMessagesTo(final DeleteMessagesTo message) {
        ...
        long to = Long.min(dataJournal.writer().getLastIndex(), message.toSequenceNr);
        LOG.debug("{}: adjusted delete to {}", persistenceId, to);

        if (lastDelete < to) {

            to = adjustIndexAccordingToFragmentation(to);
    ...
    private long adjustIndexAccordingToFragmentation(final long wantedIndex) {
        try (SegmentedJournalReader<DataJournalEntry> reader = dataJournal.openReader(wantedIndex)) {
            if (reader.hasNext()) {
                Indexed<DataJournalEntry> next = reader.next();
                if (next.entry() instanceof FromFragmentedPersistence) {
                    LOG.info("{}: entry at index {} is a fragment. Adjusting index to the index of the last fragment.",
                        persistenceId, wantedIndex);
                    final FromFragmentedPersistence fragment = (FromFragmentedPersistence) next.entry();
                    return wantedIndex + (fragment.getFragmentCount() - (fragment.getFragmentIndex() + 1));
                }
            }
        } catch (Exception e) {
            LOG.warn("{}: can't read entry at index {} - can't check if fragment", persistenceId, wantedIndex, e);
        }
        return wantedIndex;
    }
