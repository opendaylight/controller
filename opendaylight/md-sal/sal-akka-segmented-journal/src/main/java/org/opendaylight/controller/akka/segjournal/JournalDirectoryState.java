...
abstract class JournalDirectoryState {
    ...
    static JournalDirectoryState forDirectory(final File directory) throws IOException {
        ...
        if (Files.deleteIfExists(tmpPath)) {
            // If there is a tmp file, delete it. We cannot trust its content, as the system might have crashed while
            // we were writing to it -- it can be empty or even have some garbage in it.
    ...
    final void checkpoint() throws IOException {
        ...
        // If we crash at this point we will discard the information in the tmpfile and start again from binfile.
