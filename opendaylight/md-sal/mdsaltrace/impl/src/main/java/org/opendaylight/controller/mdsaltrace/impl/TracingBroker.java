...
public class TracingBroker  implements DataBroker, DataTreeChangeService {
    ...
    static Logger LOG = LoggerFactory.getLogger(TracingBroker.class);
    ...
    private class Watch {
        public String iidString;
        public LogicalDatastoreType store = null;
        ...
        public boolean subtreesOverlap(InstanceIdentifier iid, LogicalDatastoreType store, DataChangeScope scope) {
            if (this.store != null && !Objects.equals(this.store, store)) {
                return false;
            }
    ...
    private List<Watch> registrationWatches = Lists.newArrayList();
    private List<Watch> writeWatches = Lists.newArrayList();
    ...
    String toPathString(InstanceIdentifier iid) {
        StringBuilder builder = new StringBuilder();
        Iterator<InstanceIdentifier.PathArgument> it = iid.getPathArguments().iterator();
        while(it.hasNext()) {
            builder.append('/').append(it.next().getType().getSimpleName());
