...
public abstract class AbstractClientHistoryTest<T extends AbstractClientHistory> {
    ...
    @Test
    abstract public void testOnTransactionComplete() throws Exception;

    @Test
    abstract public void testOnTransactionAbort() throws Exception;

    @Test
    abstract public void testOnTransactionReady() throws Exception;

    @Test
    abstract public void testOnTransactionReadyDuplicate() throws Exception;
    ...
    public void testStartReconnect() throws Exception {
        final ConnectedClientConnection newConn = Mockito.mock(ConnectedClientConnection.class);
        when(newConn.cookie()).thenReturn(0L);
        final ShardBackendInfo info = Mockito.mock(ShardBackendInfo.class);
        final DataTree tree = Mockito.mock(DataTree.class);
        when(info.getDataTree()).thenReturn(Optional.of(tree));
        when(newConn.getBackendInfo()).thenReturn(Optional.of(info));
    ...
    public void testStartReconnectMissingOldProxy() throws Exception {
        final ConnectedClientConnection newConn = Mockito.mock(ConnectedClientConnection.class);
        when(newConn.cookie()).thenReturn(1L);
