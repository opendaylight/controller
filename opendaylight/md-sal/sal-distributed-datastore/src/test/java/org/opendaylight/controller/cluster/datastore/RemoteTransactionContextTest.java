...
public class RemoteTransactionContextTest {
    ...
    @BeforeClass
    public static void staticBedore() {
        ACTOR_SYSTEM = ActorSystem.create();
    }

    @AfterClass
    public static void staticAfter() throws TimeoutException, InterruptedException {
        Await.ready(ACTOR_SYSTEM.terminate(), FiniteDuration.apply(5, TimeUnit.SECONDS));
    }

    @Before
    public void before() {
        kit = new TestKit(ACTOR_SYSTEM);
        actorContext = Mockito.spy(new ActorContext(ACTOR_SYSTEM, kit.getRef(), mock(ClusterWrapper.class),
    ...
    private static void assertFuture(Future<Object> future, OnComplete<Object> complete) throws TimeoutException,
