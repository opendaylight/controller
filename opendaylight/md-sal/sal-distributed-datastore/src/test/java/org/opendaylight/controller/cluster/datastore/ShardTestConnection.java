...
final class ShardTestConnection {
    ...
    <T extends Response<?, ?>> @NonNull T request(final Class<T> clazz,
            final Function<@NonNull ActorRef, Request<?, ?>> req) {
        return request(clazz, req, DEFAULT_MAX);
    }

    <T extends Response<?, ?>> @NonNull T request(final Class<T> clazz, final Function<@NonNull ActorRef,
            Request<?, ?>> req, final Duration max) {
        sendRequest(req.apply(connection.localActor()));
        return assertResponse(clazz, max);
    }

    void asyncRequest(final Function<@NonNull ActorRef, Request<?, ?>> req) {
        sendRequest(req.apply(connection.localActor()));
    }
    ...
    <T extends Response<?, ?>> @NonNull T assertResponse(final Class<T> clazz, final Duration max) {
        final var env = testKit.expectMsgClass(max, ResponseEnvelope.class);
        AccessClientUtil.completeRequest(connection, env);
        return assertInstanceOf(clazz, env.getMessage());
    }

    private void sendRequest(final Request<?, ?> reqest) {
        connection.sendRequest(reqest, ignored -> {
            // No-op, will be returned from assertResponse()
        });
    }
