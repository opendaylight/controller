...
import org.opendaylight.controller.cluster.access.client.AccessClientMockFactory;
...
public abstract class AbstractDataStoreClientBehaviorTest {
    ...
    public void setUp() throws Exception {
        ...
        clientContext = createClientActorContextMock(clientActorProbe.ref());
        behavior = createBehavior(clientContext, context);
    }

    protected abstract AbstractDataStoreClientBehavior createBehavior(ClientActorContext clientContext, ActorContext context);
    ...
    public void testClose() throws Exception {
        behavior.close();
        final ArgumentCaptor<InternalCommand> captor = ArgumentCaptor.forClass(InternalCommand.class);
        verify(clientContext, timeout(1000)).executeInActor(captor.capture());
        captor.getValue().execute(behavior);
        try {
    ...
    public void testGetConnection() throws Exception {
        ...
        final ArgumentCaptor<InternalCommand> captor = ArgumentCaptor.forClass(InternalCommand.class);
        verify(clientContext, timeout(1000)).executeInActor(captor.capture());
        final InternalCommand<ShardBackendInfo> command = captor.getValue();
    ...
    private static ClientActorContext createClientActorContextMock(final ActorRef actor) throws Exception {
        final ClientActorContext mock = AccessClientMockFactory.createClientActorContextMock(PERSISTENCE_ID);
        final ClientIdentifier id = ClientIdentifier.create(FRONTEND_ID, 0L);
        when(mock.getIdentifier()).thenReturn(id);
        when(mock.self()).thenReturn(actor);
        when(mock.ticker()).thenReturn(Ticker.systemTicker());
        return mock;
    }
