...
public class ThreePhaseCommitCohortProxyTest extends AbstractActorTest {
    ...
    public void testAllThreePhasesSuccessful() throws Exception {
        ...
        doReturn(mockTimer).when(actorContext).getOperationTimer("commit");
        doReturn(mockTimerContext).when(mockTimer).time();
        doReturn(mockSnapshot).when(mockTimer).getSnapshot();
        doReturn(TimeUnit.MILLISECONDS.toNanos(2000) * 1.0).when(mockSnapshot).get98thPercentile();
