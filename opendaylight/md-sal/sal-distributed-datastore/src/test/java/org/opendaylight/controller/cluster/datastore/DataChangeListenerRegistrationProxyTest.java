...
public class DataChangeListenerRegistrationProxyTest extends AbstractActorTest {
    ...
    public void testSuccessfulRegistration() {
        new JavaTestKit(getSystem()) {{
            ...
            Assert.assertEquals("isRemoteListener", false, registerMsg.isRemoteListener());
    ...
    @Test(timeout=10000)
    public void testSuccessfulRegistrationForRemoteListener() {
        new JavaTestKit(getSystem()) {{
            ActorContext actorContext = new ActorContext(getSystem(), getRef(),
                mock(ClusterWrapper.class), mock(Configuration.class));

            AsyncDataChangeListener<YangInstanceIdentifier, NormalizedNode<?, ?>> mockRemoteListener =
                Mockito.mock(RemoteDOMDataChangeListener.class);

            final DataChangeListenerRegistrationProxy proxy = new DataChangeListenerRegistrationProxy(
                "shard-1", actorContext, mockRemoteListener);
            ...
            Assert.assertEquals("getPath", path, registerMsg.getPath());
            Assert.assertEquals("getScope", scope, registerMsg.getScope());
            Assert.assertEquals("isRemoteListener", true, registerMsg.isRemoteListener());
