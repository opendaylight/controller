...
public class ConfigurationImplHybridTest extends ConfigurationImplBaseTest {
    ...
    public void setup() {
        ...
        configuration = new ConfigurationImpl(new HybridModuleShardConfigProvider(moduleShardsConf, "modules.conf"));
    }
    ...
    private Config generateModuleShards(List<String> shards) {
    ...
    private String generateShard(String name, String shardsName, List<String> replicas) {
        return "    {"
                + "        name = \"" + name + "\"\n"
                + "        shards = [\n"
                + "            {\n"
