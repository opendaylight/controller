...
public class ConfigurationImplHybridTest extends ConfigurationImplBaseTest {
    ...
    public ConfigurationImpl createConfiguration() {
        final var moduleShardsConf = generateModuleShards(List.of(
    ...
    public void testNullModuleShardsConf() {
        assertThrows(NullPointerException.class, () -> new HybridModuleShardConfigProvider(null, "modules.conf"));
    }

    private static Config generateModuleShards(final List<String> shards) {
        return ConfigFactory.parseString("module-shards = [%n%s]".formatted(String.join(",\n", shards)));
    }
