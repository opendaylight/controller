...
public class PrefixShardCreatorTest extends AbstractTest {
    ...
    @Test
    @Ignore("Needs other patches related to 5280")
    public void testWriteIntoMultipleShards() throws Exception {
        initEmptyDatastore("config");

        new JavaTestKit(leaderSystem){{
            final PrefixShardCreatorImpl creator = new PrefixShardCreatorImpl(shardedDOMDataTree,
                    Mockito.mock(DistributedDataStore.class),
                    leaderDistributedDataStore,
                    leaderSystem);

            creator.createPrefixShard(TEST_ID, Lists.newArrayList(MEMBER_NAME, MEMBER_2_NAME));
