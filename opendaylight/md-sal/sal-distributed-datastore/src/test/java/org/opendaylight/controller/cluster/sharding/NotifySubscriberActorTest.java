...
public class NotifySubscriberActorTest extends AbstractActorTest {

    @Test
    public void testOnDataTree() {
        new JavaTestKit(getSystem()) {
            {
                final LeaderLocationListener listener = mock(LeaderLocationListener.class);
                final Props props = NotifySubscriberActor.props(getRef(), listener);
        ...
        new JavaTestKit(getSystem()) {
            {
                ...
                final ActorRef subject = getSystem().actorOf(props, "testDataTreeChangedNotirsdfa");

                subject.tell(new LeaderStateChanged("member-1", null, (short) 0), getRef());
                verify(listener, timeout(5000)).onLeaderLocationChanged(eq(LeaderLocation.UNKNOWN));


                subject.tell(new LeaderStateChanged("member-1", "member-1", (short) 0), getRef());
                verify(listener, timeout(5000)).onLeaderLocationChanged(eq(LeaderLocation.LOCAL));

                subject.tell(new LeaderStateChanged("member-1", "member-2", (short) 0), getRef());
