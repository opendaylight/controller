...
public class ClientTransactionCommitCohortTest {
    ...
    public void setUp() {
        ...
        final List<AbstractProxyTransaction> proxies = transactions.stream()
    ...
    void replyAbortSuccess(final TransactionTester<?> tester) {
        final var success = new TransactionAbortSuccess(tester.getTransaction().getIdentifier(),
                tester.getLastReceivedMessage().getSequence());
        tester.replySuccess(success);
    }
    ...
    private <T> void testOpFail(final Function<ClientTransactionCommitCohort, ListenableFuture<T>> operation,
        ...
        final var cause = new RuntimeRequestException("fail", e);
        last.replyFailure(cause);
        //check future fail
        final var exception = assertOperationThrowsException(() -> getWithTimeout(canCommit), ExecutionException.class);
