...
//                expectMsgClass(Duration.ofSeconds(5), PrimaryNotFoundException.class);
    ...
    @Test
    public void testShardsInOperDatastoreWithVariousPersistence() {
        LOG.info("testShardsInOperDatastoreWithVariousPersistence starting");
        Persistence persistOperTrue = new PersistenceBuilder().setDatastore(DatastoreType.Operational)
                .setPersistent(true).build();
        Persistence persistOperFalse = new PersistenceBuilder().setDatastore(DatastoreType.Operational)
                .setPersistent(false).build();
        ...
        Persistence persistConfigTrue = new PersistenceBuilder().setDatastore(DatastoreType.Configuration)
                .setPersistent(true).build();
        Persistence persistConfigFalse = new PersistenceBuilder().setDatastore(DatastoreType.Configuration)
                .setPersistent(false).build();
        ...
        Persistence persistBothTrue = new PersistenceBuilder().setDatastore(DatastoreType.Both)
                .setPersistent(true).build();
        Persistence persistBothFalse = new PersistenceBuilder().setDatastore(DatastoreType.Both)
                .setPersistent(false).build();
    ...
    @Test
    public void testShardsInConfigDatastoreWithVariousPersistence() {
        LOG.info("testShardsInConfigDatastoreWithVariousPersistence starting");
        Persistence persistConfigTrue = new PersistenceBuilder().setDatastore(DatastoreType.Configuration)
                .setPersistent(true).build();
        Persistence persistConfigFalse = new PersistenceBuilder().setDatastore(DatastoreType.Configuration)
                .setPersistent(false).build();
    ...
    private void testShardPersistenceWhen(final LogicalDatastoreType datastore,
            final boolean datastoreContextPersistent, final Persistence perShardPersistence,
            final boolean expectedShardPersistence) {
        ...
        Shard.Builder shardBuilder = Shard.builder();
        ModuleShardConfiguration config = new ModuleShardConfiguration(URI.create("foo-ns"), "foo-module",
                "foo", null, members("member-1", "member-5", "member-6"), perShardPersistence);
        shardManager.tell(new CreateShard(config, shardBuilder, datastoreContext), kit.getRef());
