...
public class DataStoreUtilsTest {
    ...
    public void testDoPathsCoExist() {
        Assert.assertFalse(DataStoreUtils.doPathsCoExist(null, ""));
        Assert.assertFalse(DataStoreUtils.doPathsCoExist("", null));
        Assert.assertFalse(DataStoreUtils.doPathsCoExist("", ""));

        String path1 = "akka://test/user/$a";
        String path2 = "akka://test/user/$b";
        Assert.assertFalse(DataStoreUtils.doPathsCoExist(path1, path2));

        path1 = "akka.tcp://system@127.0.0.1:2550/";
        path2 = "akka.tcp://system@127.0.0.1:2550/";
        Assert.assertTrue(DataStoreUtils.doPathsCoExist(path1, path2));
