 ...
 */
@NonNullByDefault
abstract sealed class ActiveShard extends ShardBehavior permits StartingShard, StartedShard {
    ...
    public final boolean handleRaftActorMessage(final Object raftActorMessage, final @Nullable ActorRef sender) {
        switch (raftActorMessage) {
            ...
            default -> {
                if (raftActorMessage.equals(Shard.GET_SHARD_MBEAN_MESSAGE)) {
                    sender.tell(shardStatsBean, self());
                    return true;
                }
                return false;
            }
