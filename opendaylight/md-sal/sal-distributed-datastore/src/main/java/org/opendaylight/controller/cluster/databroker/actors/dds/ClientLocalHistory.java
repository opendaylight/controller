...
public final class ClientLocalHistory extends AbstractClientHistory implements AutoCloseable {
    ...
    public ClientTransaction createTransaction() {
        ...
        // FIXNME: take skippedTransactiuns into account
        return new ClientTransaction(getClient(), this,
            new TransactionIdentifier(getIdentifier(), NEXT_TX_UPDATER.getAndIncrement(this)), skippedTransactions);
    }
    ...
    void onTransactionSkipped(final ClientTransaction transaction) {
        skippedTransactions++;
        super.onTransactionSkipped(transaction);
    }

    @Override
    void transactionCommitted(final ClientTransaction transaction) {
        skippedTransactions = 0;
        super.transactionCommitted(transaction);
    }
}
