 ...
 */
class ShardedTransaction extends AbstractDOMStoreTransaction<TransactionIdentifier> implements DOMStoreReadWriteTransaction {
    private static enum TransactionState {
        OPEN,
        READY,
        CLOSED,
    }
    private static final Logger LOG = LoggerFactory.getLogger(ShardedTransaction.class);
    ...
    protected ShardedTransaction(final ComponentFactory componentFactory, final TransactionType type) {
    ...
    private CheckedFuture<Optional<NormalizedNode<?, ?>>, ReadFailedException> readAllData() {
        ...
        aggregateFuture = Futures.transform(listFuture, new Function<List<Optional<NormalizedNode<?, ?>>>, Optional<NormalizedNode<?, ?>>>() {
            @Override
            public Optional<NormalizedNode<?, ?>> apply(List<Optional<NormalizedNode<?, ?>>> input) {
                try {
    ...
    public final DOMStoreThreePhaseCommitCohort ready() {
        ...
        if (components.size() == 1) {
            ...
            return createSingleCommitCohort(e.getKey(), e.getValue());
        } else {
            return createMultiCommitCohort(components.entrySet());
        }
    }

    private DOMStoreThreePhaseCommitCohort createSingleCommitCohort(final String shardName, final AbstractTransactionComponent component) {
    ...
    private static String shardNameFromIdentifier(YangInstanceIdentifier path) {
        return ShardStrategyFactory.getStrategy(path).findShard(path);
    }

    private AbstractTransactionComponent getComponent(YangInstanceIdentifier path) {
        return getComponent(shardNameFromIdentifier(path));
    }

    private AbstractTransactionComponent getComponent(String shardName) {
