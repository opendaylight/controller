...
public final class OSGiDistributedDataStore {
    private final class DatastoreState implements FutureCallback<Object> {
        ...
        @Override
        public synchronized void onSuccess(final Object result) {
            LOG.debug("Distributed Datastore type {} reached initial settle");
        ...
        public synchronized void onFailure(final Throwable cause) {
            LOG.error("Distributed Datastore type {} failed to settle", cause);
        }
    ...
    // Visible for OSGiDOMStore
    static final Logger LOG = LoggerFactory.getLogger(OSGiDistributedDataStore.class);
    ...
    private DatastoreState createDatastore(final LogicalDatastoreType datastoreType, final String serviceType,
        ...
        Futures.addCallback(datastore.initialSettleFuture(), state, MoreExecutors.directExecutor());
        return state;
    }
}
