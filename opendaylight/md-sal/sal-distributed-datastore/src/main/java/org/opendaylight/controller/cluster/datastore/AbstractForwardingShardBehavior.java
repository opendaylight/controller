...
abstract class AbstractForwardingShardBehavior extends ShardBehavior {
    ...
    @Override
    void handleCanCommitTransaction(final ActorRef sender, final CanCommitTransaction message) {
        forwardOrStash(sender, message, "Could not canCommit transaction " + message.getTransactionID());
    }

    @Override
    final void handleCommitTransaction(final ActorRef sender, final CommitTransaction message) {
