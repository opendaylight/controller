...
public class TransactionProxy extends AbstractDOMStoreTransaction<TransactionIdentifier> implements DOMStoreReadWriteTransaction {
    ...
    private final Map<String, TransactionContextAdapter> txContextAdapters = new HashMap<>();
    ...
    public CheckedFuture<Boolean, ReadFailedException> exists(final YangInstanceIdentifier path) {
        ...
        TransactionContextAdapter contextAdapter = getContextAdapter(path);
        contextAdapter.enqueueTransactionOperation(new TransactionOperation() {
            @Override
    ...
    private CheckedFuture<Optional<NormalizedNode<?, ?>>, ReadFailedException> singleShardRead(
        ...
        TransactionContextAdapter contextAdapter = getContextAdapter(shardName);
        contextAdapter.enqueueTransactionOperation(new TransactionOperation() {
            @Override
            public void invoke(TransactionContext transactionContext) {
    ...
    public final void close() {
        ...
        for (TransactionContextAdapter contextAdapter : txContextAdapters.values()) {
            contextAdapter.enqueueTransactionOperation(new TransactionOperation() {
                @Override
                public void invoke(TransactionContext transactionContext) {
    ...
    public final AbstractThreePhaseCommitCohort<?> ready() {
        ...
        case 0:
            ...
            break;
        case 1:
            final Entry<String, TransactionContextAdapter> e = Iterables.getOnlyElement(txContextAdapters.entrySet());
    ...
    private AbstractThreePhaseCommitCohort<?> createSingleCommitCohort(final String shardName,
            final TransactionContextAdapter contextAdapter) {
        throttleOperation();
    ...
    private AbstractThreePhaseCommitCohort<ActorSelection> createMultiCommitCohort(
            final Set<Entry<String, TransactionContextAdapter>> txContextAdapterEntries) {

        throttleOperation();
        final List<Future<ActorSelection>> cohortFutures = new ArrayList<>(txContextAdapterEntries.size());
        for (Entry<String, TransactionContextAdapter> e : txContextAdapterEntries) {
            LOG.debug("Tx {} Readying transaction for shard {}", getIdentifier(), e.getKey());

            TransactionContextAdapter contextAdapter = e.getValue();
            final TransactionContext transactionContext = contextAdapter.getTransactionContext();
    ...
    private TransactionContextAdapter getContextAdapter(final YangInstanceIdentifier path) {
        return getContextAdapter(shardNameFromIdentifier(path));
    }

    private TransactionContextAdapter getContextAdapter(final String shardName) {
        final TransactionContextAdapter existing = txContextAdapters.get(shardName);
        if (existing != null) {
            return existing;
        }

        final TransactionContextAdapter fresh = txContextFactory.newTransactionAdapter(this, shardName);
