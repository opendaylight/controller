...
abstract class AbstractPayloadHandler<T extends SerializablePayload> implements PayloadHandler {
    ...
    protected abstract void doWriteSkipPayloadType(DataOutput out, T payload) throws IOException;

    protected abstract T doRead(DataInput in) throws IOException;

    @Override
    public final void writeTo(final DataOutput out, final SerializablePayload payload) throws IOException {
        out.writeByte(payload.getPayloadType().getOrdinalByte());
        doWriteSkipPayloadType(out, (T) payload);
    }
