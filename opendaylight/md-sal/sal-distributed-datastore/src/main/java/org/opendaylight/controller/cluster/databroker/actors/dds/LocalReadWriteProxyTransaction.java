...
final class LocalReadWriteProxyTransaction extends LocalProxyTransaction {
    ...
    @Override
    Response<?, ?> handleExistsRequest(final DataTreeSnapshot readOnlyView, final ExistsTransactionRequest request) {
        final var ex = recordedFailure;
        return ex == null ? super.handleExistsRequest(readOnlyView, request)
            : request.toRequestFailure(new RuntimeRequestException("Previous modification failed", ex));
    }

    @Override
    Response<?, ?> handleReadRequest(final DataTreeSnapshot readOnlyView, final ReadTransactionRequest request) {
        final var ex = recordedFailure;
        return ex == null ? super.handleReadRequest(readOnlyView, request)
            : request.toRequestFailure(new RuntimeRequestException("Previous modification failed", ex));
    }
