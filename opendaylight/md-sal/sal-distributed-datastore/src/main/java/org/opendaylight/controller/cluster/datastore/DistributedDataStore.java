...
public class DistributedDataStore implements DistributedDataStoreInterface, SchemaContextListener,
    ...
    public DistributedDataStore(ActorSystem actorSystem, ClusterWrapper cluster,
        ...
        try {
            client = DistributedDataStoreClientActor.getDistributedDataStoreClient(clientActor, 10, TimeUnit.SECONDS);
        } catch (Exception e) {
            LOG.error("Failed to get actor for {}", clientProps, e);
            clientActor.tell(PoisonPill.getInstance(), ActorRef.noSender());
            throw new IllegalStateException("Failed acquire client actor", e);
        }

        identifier = client.getIdentifier().getFrontendId();
        String shardManagerId = ShardManagerIdentifier.builder()
                .type(identifier.getClientType().getStoreName()).build().toString();
