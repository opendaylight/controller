...
final class FrontendLocalHistory implements Identifiable<LocalHistoryIdentifier> {
    ...
    FrontendTransaction createTransaction(final TransactionIdentifier id) throws RequestException {
    ...
    private FrontendTransaction ensureTransaction(final TransactionRequest<?> request, final long sequence)
        ...
        final FrontendLocalHistory history = localHistories.get(request.getTarget().getHistoryId());
        if (history == null) {
            LOG.debug("{}: unknown history for request {}", persistenceId, request);
            throw new UnknownLocalHistoryException(request.getTarget().getHistoryId());
        }

        final FrontendTransaction ret = history.createTransaction(id);
