 ...
 * arrived between the arrival of two instances of the same message and the amount of time it took to process each
 * of those messages.
 * <br/>
 ...
 *
 *     // Begin the tracking process. If this is not called then calling arrived and done on the resultant Context
 *     // will do nothing
 *     tracker.beginTracking();
 *
 *     .....
 *
 *     MessageTracker.Context context = tracker.arrived(message);
 *
...
public class MessageTracker {
    ...
    private final CurrentMessageContext currentMessageSnapshot = new CurrentMessageContext();
    ...
    public Context arrived(Object message){
        if(!enabled) return NO_OP_CONTEXT;
