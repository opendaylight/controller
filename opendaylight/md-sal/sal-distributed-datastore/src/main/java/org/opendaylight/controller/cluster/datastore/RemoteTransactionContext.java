...
/**
 * Class holding the remote context for a single {@link TransactionProxy} instance and
 * is shared by all {@link RemoteTransactionComponent}s attached to it.
 */
final class RemoteTransactionContext {
    ...
    String getTransactionChainId() {
        if (getIdentifier() instanceof ChainedTransactionIdentifier) {
            return ((ChainedTransactionIdentifier)getIdentifier()).getChainId();
        } else {
            return null;
        }
    }
