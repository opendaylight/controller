...
public abstract class ShardTransaction extends AbstractUntypedActor {
    ...
    public static Props props(final DOMStoreTransaction transaction,
            final ActorRef shardActor, final ShardContext shardContext) {
        return Props.create(new ShardTransactionCreator(transaction, shardActor, shardContext));
    }
        ...
        public ShardTransaction create() throws Exception {
            ...
            if(transaction instanceof DOMStoreReadWriteTransaction) {
                tx = new ShardReadWriteTransaction((DOMStoreReadWriteTransaction)transaction,
                        shardActor, shardContext.getSchemaContext());
            } else if(transaction instanceof DOMStoreReadTransaction) {
                tx = new ShardReadTransaction((DOMStoreReadTransaction)transaction, shardActor,
                        shardContext.getSchemaContext());
            } else {
