 ...
 */
@Beta
public final class ClientTransaction implements AutoCloseable, Identifiable<TransactionIdentifier> {
    ...
    private DOMStoreThreePhaseCommitCohort readySingle() {
        final AbstractProxyTransaction proxy = Iterables.getOnlyElement(proxies.values());
        proxies.clear();
        return new SingleDOMStoreThreePhaseCommitCohort(proxy);
    }
