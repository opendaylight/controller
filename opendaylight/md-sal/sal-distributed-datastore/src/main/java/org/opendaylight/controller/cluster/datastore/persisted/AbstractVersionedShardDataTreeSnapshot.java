...
/**
 * An internally-versioned {@link AbstractShardDataTreeSnapshot}. This class is an intermediate implementation-private
 * class.
 *
 * @author Robert Varga
 */
abstract class AbstractVersionedShardDataTreeSnapshot extends AbstractShardDataTreeSnapshot {
    private static final Logger LOG = LoggerFactory.getLogger(AbstractVersionedShardDataTreeSnapshot.class);

    static AbstractShardDataTreeSnapshot deserialize(final DataInputStream is) throws IOException {
        final PayloadVersion version = PayloadVersion.readFrom(is);
        switch (version) {
            case BORON:
                // Boron snapshots use Java Serialization
                try (final ObjectInputStream ois = new ObjectInputStream(is)) {
                    return (AbstractShardDataTreeSnapshot) ois.readObject();
    ...
    abstract PayloadVersion getVersion();
    ...
    public final byte[] serialize() throws IOException {
        try (final ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            try (final DataOutputStream dos = new DataOutputStream(bos)) {
                final PayloadVersion version = getVersion();
                version.writeTo(dos);
