...
public class ShardDataTree extends ShardDataTreeTransactionParent {
    ...
    private void finishCommit(final SimpleShardDataTreeCohort cohort) {
        ...
        LOG.debug("Resuming commit of transaction {}", txId);
    ...
    void startCommit(final SimpleShardDataTreeCohort cohort, final DataTreeCandidate candidate) {
        ...
        if (shard.canSkipPayload() || candidate.getRootNode().getModificationType() == ModificationType.UNMODIFIED) {
            LOG.debug("No replication required, proceeding to finish commit");
            finishCommit(cohort);
            return;
        }
        ...
        LOG.debug("Transaction {} submitted to persistence", txId);
    }
    ...
    private void payloadReplicationComplete(final TransactionIdentifier txId, final DataTreeCandidateSupplier payload) {
        ...
        if (!current.cohort.getIdentifier().equals(txId)) {
            LOG.warn("Head of queue is {}, ignoring consensus on transaction {}", current.cohort.getIdentifier(), txId);
            return;
        }
        ...
        if (currentTx != null && currentTx.lastAccess + timeout >= now) {
            switch (currentTx.cohort.getState()) {
                ...
                case PRE_COMMIT_COMPLETE:
                    ...
                    break;
                case COMMIT_PENDING:
                    LOG.warn("Transaction {} is still committing, cannot abort", currentTx.cohort.getIdentifier());
                ...
                case READY:
                default:
                    LOG.error("Transaction {} stuck in state {}", currentTx.cohort.getIdentifier(),
