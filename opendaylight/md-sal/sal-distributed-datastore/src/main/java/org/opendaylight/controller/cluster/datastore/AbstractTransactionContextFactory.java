...
abstract class AbstractTransactionContextFactory<F extends LocalTransactionFactory>
    ...
    private final ShardInfoListenerRegistration<AbstractTransactionContextFactory<F>> reg;
    ...
    private void onFindPrimaryShardSuccess(PrimaryShardInfo primaryShardInfo, TransactionProxy parent,
            String shardName, TransactionContextAdapter transactionContextAdapter) {
        if(LOG.isDebugEnabled()) {
        ...
        if(localContext != null) {
            transactionContextAdapter.executeTxOperationsOnComplete(localContext);
        } else {
    ...
    private void onFindPrimaryShardFailure(Throwable failure, TransactionProxy parent,
        ...
        transactionContextAdapter.executeTxOperationsOnComplete(new NoOpTransactionContext(failure,
                parent.getIdentifier(), parent.getLimiter()));
    }

    final TransactionContextAdapter newTransactionAdapter(final TransactionProxy parent, final String shardName) {
        final TransactionContextAdapter transactionContextAdapter = new TransactionContextAdapter(parent.getIdentifier());
