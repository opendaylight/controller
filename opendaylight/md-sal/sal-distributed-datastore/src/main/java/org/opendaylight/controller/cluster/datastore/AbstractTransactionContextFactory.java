...
abstract class AbstractTransactionContextFactory<F extends LocalTransactionFactory> implements AutoCloseable {
    ...
    private void onFindPrimaryShardSuccess(PrimaryShardInfo primaryShardInfo, TransactionProxy parent,
            String shardName, TransactionContextWrapper transactionContextWrapper, TransactionContext context) {
        ...
        TransactionContext localContext;
        if (context == null) {
            localContext = maybeCreateLocalTransactionContext(parent, shardName);
        } else {
    ...
    final TransactionContextWrapper newTransactionContextWrapper(final TransactionProxy parent,
        ...
        TransactionContextWrapper contextWrapper = new DelayedTransactionContextWrapper(parent.getIdentifier(), actorUtils, shardName);
        TransactionContext localContext = null;

        if (findPrimaryFuture.isCompleted() && findPrimaryFuture.value().get().isSuccess()) {
        ...
        if (findPrimaryFuture.isCompleted()) {
            ...
            TransactionContextWrapper finalContextWrapper = contextWrapper;
            TransactionContext finalLocalContext = localContext;
            findPrimaryFuture.onComplete(new OnComplete<PrimaryShardInfo>() {
                @Override
                public void onComplete(final Throwable failure, final PrimaryShardInfo primaryShardInfo) {
                    if (failure == null) {
                        onFindPrimaryShardSuccess(primaryShardInfo, parent, shardName, finalContextWrapper, finalLocalContext);
                    } else {
