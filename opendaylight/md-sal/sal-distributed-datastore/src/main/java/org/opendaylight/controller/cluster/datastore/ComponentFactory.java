...
abstract class ComponentFactory {
    ...
    final LocalThreePhaseCommitCohort createLocalCohort(TransactionIdentifier identifier, DataTreeModification modification) {
        return new LocalThreePhaseCommitCohort(actorContext, identifier, modification);
    }

    void updateShardInfo(final String shardName, final Optional<DataTree> maybeDataTree) {
        if (maybeDataTree.isPresent()) {
            knownLocal.putIfAbsent(shardName, factoryForShard(shardName, maybeDataTree.get()));
        }
    }
