...
public final class EntityOwnersModel {
    ...
    static NormalizedNode<?, ?> entityOwnersWithCandidate(final String entityType, final YangInstanceIdentifier entityId,
            final String candidateName) {
    ...
    public static String entityTypeFromEntityPath(final YangInstanceIdentifier entityPath){
        ...
        while(!parent.isEmpty()) {
            if (EntityType.QNAME.equals(parent.getLastPathArgument().getNodeType())) {
                final YangInstanceIdentifier.NodeIdentifierWithPredicates entityTypeLastPathArgument = (YangInstanceIdentifier.NodeIdentifierWithPredicates) parent.getLastPathArgument();
    ...
    static Entity createEntity(final YangInstanceIdentifier entityPath) {
        String entityType = null;
        YangInstanceIdentifier entityId = null;
        for(final PathArgument pathArg: entityPath.getPathArguments()) {
            if(pathArg instanceof NodeIdentifierWithPredicates) {
                final NodeIdentifierWithPredicates nodeKey = (NodeIdentifierWithPredicates) pathArg;
                final Entry<QName, Object> key = nodeKey.getKeyValues().entrySet().iterator().next();
    ...
    static DOMEntity createDOMEntity(final YangInstanceIdentifier entityPath) {
        String entityType = null;
        YangInstanceIdentifier entityId = null;
        for (final PathArgument pathArg : entityPath.getPathArguments()) {
            if (pathArg instanceof NodeIdentifierWithPredicates) {
