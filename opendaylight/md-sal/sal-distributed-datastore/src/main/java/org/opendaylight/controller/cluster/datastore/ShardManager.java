...
public class ShardManager extends AbstractUntypedPersistentActorWithMetering {
    ...
    public void handleCommand(Object message) throws Exception {
        if (message  instanceof FindPrimary) {
            ...
            createShardReplica((CreateShardReplica)message);
        } else if(message instanceof RemoveShardReplica){
            removeShardReplica((RemoveShardReplica)message);
        } else {
    ...
    private void createShardReplica (CreateShardReplica shardReplicaMsg) {
        ...
        if (localShards.containsKey(shardName)) {
            ...
            getSender().tell(new akka.actor.Status.Failure(new IllegalArgumentException(String.format("Local shard %s alreay exists", shardName))), getSelf());
            return;
        }
        // verifying if the shard is available in the cluster for replication on this node.
        if (!(this.configuration.checkModuleAvailability(shardName))) {
            LOG.debug ("Shard {} not available for replication", shardName);
            getSender().tell(new akka.actor.Status.Failure(new IllegalArgumentException(String.format("Shard %s not available in cluster", shardName))), getSelf());
            return;
        }
    ...
    private void removeShardReplica (RemoveShardReplica shardReplicaMsg) {
        String shardName = shardReplicaMsg.getShardName();
