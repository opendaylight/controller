...
public class ShardManager extends AbstractUntypedPersistentActorWithMetering {
            ...
            public void onComplete(Throwable failure, Object response) {
                if (failure != null) {
                    LOG.error ("Failed to receive response for FindPrimary of shard {}",
                                shardName);
                    sender.tell(new akka.actor.Status.Failure(new IllegalStateException(failure)),
                        getSelf());
                } else {
    ...
    private void addShard(final String shardName, RemotePrimaryShardFound respose,
        ...
        Future<Object> futureObj = ask(getContext().actorSelection(respose.getPrimaryPath()),
        ...
        futureObj.onComplete(new OnComplete<Object>() {
            ...
            public void onComplete(Throwable failure, Object response) {
                if (failure != null) {
                    LOG.error ("Failed to receive response for AddServer of shard {}",
                                shardName);
                    // Remove the shard
                    ...
                    AddServerReply reply = (AddServerReply)response;
                    onAddServerReply(shardName, shardInfo, reply, sender);
                }
            }
    ...
    private void onAddServerReply (String shardName, ShardInformation shardInfo,
                                   AddServerReply replyMsg, ActorRef sender) {
        if (replyMsg.getStatus() == ServerChangeStatus.OK) {
            ...
            sender.tell(new akka.actor.Status.Failure(new IllegalStateException(
                ...
                shardName, replyMsg.getStatus()))), getSelf());
        }
    }
