...
public class ShardManager extends AbstractUntypedPersistentActorWithMetering {
    ...
    private void createShardReplica (CreateShardReplica shardReplicaMsg) {
        ...
        // verifing the local shard replication is already available in the controller node
        if (localShards.containsKey(shardName)) {
            LOG.debug ("Local shard replica {} already available in the controller node", shardName);
            return;
        }
        // verifing if the shard is available in the cluster for replication on this node.
        if (this.configuration.checkModuleAvailability(shardName) != true) {
