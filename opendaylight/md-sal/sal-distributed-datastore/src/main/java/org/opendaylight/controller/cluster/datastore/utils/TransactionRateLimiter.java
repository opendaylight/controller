...
public class TransactionRateLimiter {
    ...
    private long acquisitionCount = 0;
    ...
    public void acquire(){
        ...
        if(newRateLimit < 1.0){
            double otherRateLimit = getRateLimitFromOtherDataStores();
    ...
    private double getRateLimitFromOtherDataStores(){
        ...
        for(String datastoreType: DatastoreContext.getGlobalDatastoreTypes()) {
            ...
            double newRateLimit = calculateNewRateLimit(actorContext.getOperationTimer(datastoreType, ActorContext.COMMIT), this.commitTimeoutInSeconds);
            if(newRateLimit > 0.0) {
    ...
    public static double calculateNewRateLimit(Timer commitTimer, long commitTimeoutInSeconds) {
        if(commitTimer == null) {
