...
abstract class AbstractShardBackendResolver extends BackendInfoResolver<ShardBackendInfo> {
    ...
    protected final ShardState resolveBackendInfo(final String shardName, final long cookie) {
        ...
        FutureConverters.toJava(actorContext.findPrimaryShardAsync(shardName)).whenComplete((info, failure) -> {
            ...
            if (failure instanceof NoShardLeaderException) {
                // There is no current leader, retry
                future.completeExceptionally(wrapInTimeoutException("No shard leader present", failure));
            ...
            .whenComplete((response, failure) -> {
                if (failure != null) {
                    if (failure instanceof AskTimeoutException) {
                        LOG.debug("Connect attempt to {} timed out, retrying", shardName, failure);
                        future.completeExceptionally(wrapInTimeoutException("Connect to leader timed out", failure));
                    }
    ...
    private static TimeoutException wrapInTimeoutException(final String message, final Throwable cause) {
        final TimeoutException ret = new TimeoutException("No leader found");
        ret.initCause(cause);
        return ret;
    }
}
