...
public class ShardedDataTreeActor extends AbstractUntypedPersistentActor {
    ...
    public void preStart() {
        final Subscribe<ORSet<PrefixShardConfiguration>> subscribe =
    ...
    private void onNotifyProducerCreated(final NotifyProducerCreated message) {
        ...
        try {
            final ActorProducerRegistration registration =
                    new ActorProducerRegistration(shardingService.createProducer(subtrees), subtrees);
    ...
    private void onCreatePrefixShard(final CreatePrefixShard message) {
        ...
        final Update<ORSet<PrefixShardConfiguration>> update =
    ...
    private void tempOnCreatePrefixShard(final CreatePrefixShard message) {
        ...
        final DOMDataTreeProducer producer =
                shardingService.createProducer(Collections.singleton(configuration.getPrefix()));
        ...
        final Props distributedDataStoreClientProps =
                DistributedDataStoreClientActor.props(clusterWrapper.getCurrentMemberName(),
                        "Shard-" + shardName, distributedDataStore.getActorContext());

        final ActorRef clientActor = actorSystem.actorOf(distributedDataStoreClientProps);
        final DistributedDataStoreClient client;
        try {
            client = DistributedDataStoreClientActor.getDistributedDataStoreClient(clientActor, 30L, TimeUnit.SECONDS);
    ...
    private void onRemovePrefixShard(final RemovePrefixShard message) {
        ...
        final Update<ORSet<PrefixShardConfiguration>> removal =
                new Update<>(ClusterUtils.CONFIGURATION_KEY, currentData, Replicator.writeLocal(),
