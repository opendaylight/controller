...
public class ShardedDataTreeActor extends AbstractUntypedPersistentActor {
    ...
    private void tempOnCreatePrefixShard(final CreatePrefixShard message) {
        ...
        final Props distributedDataStoreClientProps =
                SimpleDataStoreClientActor.props(MemberName.forName(cluster.getSelfRoles().iterator().next()),
                        "Shard-" + shardName, distributedDataStore.getActorContext(), shardName);

        final ActorRef clientActor = actorSystem.actorOf(distributedDataStoreClientProps);
        final DataStoreClient client;
        try {
            client = SimpleDataStoreClientActor.getDistributedDataStoreClient(clientActor, 30L, TimeUnit.SECONDS);
