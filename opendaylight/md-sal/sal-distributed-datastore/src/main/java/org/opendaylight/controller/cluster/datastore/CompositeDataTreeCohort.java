...
class CompositeDataTreeCohort {
    ...
    private boolean processResponses(Future<Iterable<Object>> resultsFuture, State preCommitSend,
            State preCommitSuccessful) throws TimeoutException {
        ...
        if (!Iterables.isEmpty(failed)) {
            changeStateFrom(State.CAN_COMMIT_SEND, State.FAILED);
            return false;
        }
        changeStateFrom(State.CAN_COMMIT_SEND, State.CAN_COMMIT_SUCCESSFUL);
        return true;
    }
