    ...
    private static final class Decoded implements Identifiable<TransactionIdentifier<?>> {
        ...
        static Decoded fromSerialized(final byte[] serialized) throws IOException {
            try (ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(serialized))) {
                ...
                try {
                    identifier = (TransactionIdentifier<?>) ois.readObject();
                } catch (ClassNotFoundException e) {
                    throw new IOException("Failed to load identifier class", e);
                }

                return new Decoded(identifier, DataTreeCandidateInputOutput.readDataTreeCandidate(ois));
            }
        }
    ...
    public static CommitTransactionPayload create(final TransactionIdentifier<?> transactionId,
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            try (ObjectOutputStream oos = new ObjectOutputStream(bos)) {
                ...
                DataTreeCandidateInputOutput.writeDataTreeCandidate(oos, candidate);
            }

            return new CommitTransactionPayload(bos.toByteArray(), transactionId, candidate);
        }
    }
