...
public abstract class CommitTransactionPayload extends Payload implements Serializable {
    ...
    public static CommitTransactionPayload create(final TransactionIdentifier transactionId,
        ...
        final ChunkedByteArray source = cos.toChunkedByteArray();
        LOG.debug("Initial buffer capacity {}, actual serialized size {}", initialSerializedBufferCapacity,
            source.size());

        // Yeah, not nice, but gets the job done
        final ImmutableList<byte[]> chunks = source.getChunks();
        return chunks.size() == 1 ? new Simple(chunks.get(0)) : new Chunked(source);
    }
