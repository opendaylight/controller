...
public abstract sealed class CommitTransactionPayload extends IdentifiablePayload<TransactionIdentifier>
    ...
    private static final class ProxySizeHolder {
        static final int PROXY_SIZE =
            SerializationUtils.serialize(new Simple(false, new byte[0]).writeReplace()).length;
        static final int LEGACY_PROXY_SIZE =
            SerializationUtils.serialize(new Simple(true, new byte[0]).writeReplace()).length;
