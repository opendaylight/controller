...
/**
 * A chain of {@link ShardedTransaction}s. Beside tracking transactions it also tracks
 * the 
 */
final class ShardedTransactionChain extends ComponentFactory implements DOMStoreTransactionChain {
    ...
    private static class Allocated extends State {
        private final ShardedTransaction transaction;

        Allocated(ShardedTransaction transaction) {
    ...
    ShardedTransactionChain(ActorContext actorContext) {
        super(actorContext);
        transactionChainId = actorContext.getCurrentMemberName() + "-txn-chain-" + CHAIN_COUNTER.incrementAndGet();
    }
...
//            transactionChainId, localState.getPreviousReadyFutures());
        ...
        return new ShardedTransaction(this, TransactionType.READ_ONLY);
    }
    ...
    private ShardedTransaction allocateWriteTransaction(TransactionType type) {
        State localState = currentState;
...
//                transactionChainId, localState.getPreviousReadyFutures());
        // FIXME: increment counter
        final ShardedTransaction ret = new ShardedTransaction(this, type);
