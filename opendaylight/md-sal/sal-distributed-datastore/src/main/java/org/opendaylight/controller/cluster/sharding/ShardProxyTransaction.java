...
/**
 * Proxy {@link DOMDataTreeShardWriteTransaction} that creates a proxy cursor that translates all calls into {@link ClientTransaction}
 * calls.
 */
class ShardProxyTransaction implements DOMDataTreeShardWriteTransaction {
    ...
    private List<DOMStoreThreePhaseCommitCohort> cohorts;
...
//        Preconditions.checkState(cohort == null, "Transaction was readied already");
        ...
        for (final ForeignShardModificationContext foreign : modification.getChildShards().values()) {
            foreign.
        }
