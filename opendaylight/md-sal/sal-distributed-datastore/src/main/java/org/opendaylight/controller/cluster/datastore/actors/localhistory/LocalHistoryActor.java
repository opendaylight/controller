...
/**
 * Backend actor handling all transaction requests tied to a particular local history.
 */
public final class LocalHistoryActor extends UntypedActor {
    ...
    private LocalHistoryActor(final LocalHistoryIdentifier historyId, final DataTree dataTree) {
        this.behavior = new IdleBehavior(new LocalHistoryContext(getContext().parent(), getSelf(), historyId, dataTree));
    }

    public static Props props(final LocalHistoryIdentifier historyId, final DataTree dataTree) {
        return Props.create(LocalHistoryActor.class,
            Preconditions.checkNotNull(dataTree),
            Preconditions.checkNotNull(historyId));
    }
