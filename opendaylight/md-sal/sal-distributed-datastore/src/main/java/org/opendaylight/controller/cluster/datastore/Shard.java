...
public class Shard extends RaftActor {
    ...
    private final ShardCommitCoordinator commitCoordinator = new ShardCommitCoordinator();

    private final long transactionCommitTimeout;

    private final Cancellable txCommitTimeoutCheckSchedule;
    ...
    protected Shard(ShardIdentifier name, Map<ShardIdentifier, String> peerAddresses,
        ...
        txCommitTimeoutCheckSchedule = getContext().system().scheduler().schedule(Duration.Zero(),
                Duration.create(transactionCommitTimeout / 3, TimeUnit.MILLISECONDS), getSelf(),
    ...
    private void handleTransactionCommitTimeoutCheck() {
        ...
        if(cohortEntry != null) {
            ...
            if(elapsed > transactionCommitTimeout) {
                ...
                handleAbortTransaction(cohortEntry.getTransactionID(), null);
            }
        }
    }

    private void handleCommitTransaction(CommitTransaction commit) {
        // Lookup the cohort entry in the commitCoordinator that was cached previously (or should
        // have been) via the ForwardedReadyTransaction message.
        ...
        ListenableFuture<Void> preCommitFuture = cohortEntry.getCohort().preCommit();

        cohortEntry.updateLastAccessTime();

        final ActorRef sender = getSender();
        Futures.addCallback(preCommitFuture, new FutureCallback<Void>() {
            @Override
            public void onSuccess(Void v) {
                if(persistent) {
                    Shard.this.persistData(sender, transactionID,
                            new CompositeModificationPayload(cohortEntry.getModification().toSerializable()));
                } else {
                    Shard.this.finishCommit(sender, transactionID);
                }
            }

            @Override
            public void onFailure(Throwable t) {
                LOG.error(t, "An exception occurred while preCommitting transaction {}",
    ...
    private void finishCommit(@Nonnull final ActorRef sender, final @Nonnull String transactionID) {
        // With persistence enabled, this method is called by the leader strategy after the commit
        // has been replicated to a majority of the followers.
        ...
        ListenableFuture<Void> future = cohortEntry.getCohort().commit();
        ...
        Futures.addCallback(future, new FutureCallback<Void>() {
            @Override
            public void onSuccess(Void v) {
                sender.tell(COMMIT_TRANSACTION_REPLY, self);
            ...
            @Override
            public void onFailure(Throwable t) {
                LOG.error(t, "An exception occurred while committing transaction {}", transactionID);
                shardMBean.incrementFailedTransactionsCount();
    ...
    private void handleCanCommitTransaction(CanCommitTransaction canCommit) {
        LOG.debug("Can commititing transaction {}", canCommit.getTransactionID());
    ...
    private void handleForwardedReadyTransaction(ForwardedReadyTransaction ready) {
        ...
        getSender().tell(new ReadyTransactionReply(self().path()).toSerializable(), getSelf());
    }

    private void handleAbortTransaction(AbortTransaction abort) {
        handleAbortTransaction(abort.getTransactionID(), getSender());
    }

    private void handleAbortTransaction(String transactionID, final ActorRef sender) {
        final CohortEntry cohortEntry = commitCoordinator.getCohortEntryIfCurrent(transactionID);
        if(cohortEntry != null) {
