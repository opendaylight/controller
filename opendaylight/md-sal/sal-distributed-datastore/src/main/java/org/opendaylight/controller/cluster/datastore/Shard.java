        ...
        if (datastoreContext.isSnapshotOnRootOverwrite() && data instanceof CommitTransactionPayload) {
            try {
                ...
                if (candidate.getRootNode().getModificationType().equals(ModificationType.WRITE)
                    ...
                    final RaftActorContext context = getRaftActorContext();
                    final ReplicatedLogEntry lastAppliedEntry = context.getReplicatedLog()
                        .get(context.getLastApplied());
                    if (lastAppliedEntry != null && lastAppliedEntry.getData().equals(data)) {
                        context.setRootOverwriteEntry(lastAppliedEntry);
                    }
                }
            } catch (IOException e) {
