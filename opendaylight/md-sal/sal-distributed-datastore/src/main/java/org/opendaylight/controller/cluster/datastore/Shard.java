    ...
    private void finishCommit(@Nonnull final ActorRef sender, @Nonnull final String transactionID, @Nonnull final CohortEntry cohortEntry) {
        ...
        try {
            try {
                ...
                // We may get a "store tree and candidate base differ" exception from commit under certain
                // edge case scenarios so we'll try to re-apply the candidate from scratch as a last resort.
                // Eg, we're a follower and a tx is replicated but the leader goes down before applying it to
                // the state. We then become the leader and a second tx is committed and replicated. This
                // causes the first tx to be applied as a foreign candidate via applyState while the second tx
                // has been pre committed. When the second tx gets here to commit it will get an exception.
        ...
        if (isLeader()) {
            ...
            if (leader == null) {
                messageRetrySupport.addMessageToRetry(canCommit, getSender(),
                        "Could not commit transaction " + canCommit.getTransactionID());
            } else {
