...
public class Shard extends RaftActor {
    ...
    protected Shard(final AbstractBuilder<?, ?> builder) {
        ...
        this.shardId = builder.getId().toString();
        this.context = builder.schemaContextProvider.getSchemaContext();
    ...
    protected void handleRecover(final Object message) {
        ...
        if (exportOnRecovery) {
            if (exportActor == null) {
                exportActor = getContext().actorOf(JsonExportActor.props(context));
            }

            if (message instanceof SnapshotOffer) {
                exportActor.tell(new JsonExportActor.ExportSnapshot(store.readCurrentData().get(), shardId),
                        ActorRef.noSender());
            } else if (message instanceof ReplicatedLogEntry) {
                exportActor.tell(new JsonExportActor.ExportJournal((ReplicatedLogEntry) message), ActorRef.noSender());
            } else if (message instanceof RecoveryCompleted) {
                exportActor.tell(new JsonExportActor.FinishExport(shardId), ActorRef.noSender());
            }
        }
