    ...
    private final class Recovering extends State {
        ...
        void onReceiveRecover(final Object recover) {
            if (recover instanceof FrontendIdentifier) {
                lastGeneration = ((FrontendIdentifier) recover).getGeneration();
            } else if (recover instanceof RecoveryCompleted) {
                final long myGeneration = lastGeneration + 1;
                // FIXME: switch to SAVING
                saveSnapshot(new FrontendGenerationSnapshot(myGeneration));
                state = new Saving(my)
            }
        }
    }
