...
import org.opendaylight.controller.cluster.datastore.shardmanager.RegisterForShardLeaderChanges;
...
final class ModuleShardBackendResolver extends AbstractShardBackendResolver {
    ...
    private volatile Registration shardLeaderChangesReg;
    ...
    ModuleShardBackendResolver(final ClientIdentifier clientId, final ActorContext actorContext) {
        ...
        final Future<Object> future = ask(actorContext.getShardManager(),
                new RegisterForShardLeaderChanges(this::onShardLeaderChange), Timeout.apply(5, TimeUnit.MINUTES));
        future.onComplete(new OnComplete<Object>() {
            @Override
            public void onComplete(Throwable failure, Object reply) {
                if (failure != null) {
                    LOG.error("RegisterForShardLeaderChanges failed", failure);
                    return;
                }

                shardLeaderChangesReg = (Registration) reply;
            }
        }, actorContext.getClientDispatcher());
    }

    private void onShardLeaderChange(String shardName) {
        LOG.debug("onShardLeaderChange for {}", shardName);
    ...
    @Override
    public void close() {
        if (shardLeaderChangesReg != null) {
            shardLeaderChangesReg.close();
        }
    }
}
