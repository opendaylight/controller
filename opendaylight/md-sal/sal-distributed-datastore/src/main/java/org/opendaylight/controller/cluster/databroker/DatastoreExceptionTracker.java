...
import org.opendaylight.controller.cluster.datastore.jmx.mbeans.DatastoreExceptionCountMXBean;
 ...
 */
public final class DatastoreExceptionTracker extends AbstractMXBean implements DatastoreExceptionCountMXBean {
    ...
    @Override
    public void setResetCounterTimerInterval(final long timerInterval) {
        LOG.debug("rescheduling the timer interval: {}" , resetTimerInterval);
        resetTimerInterval = timerInterval;

        // FIXME: OMFG: we really just want to reschedule the task!
        timer.cancel();
    ...
    private static String getExceptionTrackerCounterName(final Throwable throwable, final String transactionClassName) {
        ...
        // FIXME: Hard-coded translation -- note this has no relationship to config
        final String shard;
    ...
    private class ResetExceptionTracker extends TimerTask {
        ...
        private void createHistory() throws IOException {
            ...
            try (FileWriter fw = new FileWriter(HISTORY_FILE_NAME, StandardCharsets.UTF_8, true)) {
                Date date = new Date();
