...
public final class JsonExportActor extends AbstractUntypedActor {
    ...
    private String baseDirPath = "persistence-export";

    private String snapshotDirPath = baseDirPath + "/snapshots";

    private String journalDirPath = baseDirPath + "/journals";

    private File baseDir = new File(baseDirPath);

    private File snapshotDir = new File(snapshotDirPath);

    private File journalDir = new File(journalDirPath);
    ...
    private void onExportSnapshot(ExportSnapshot exportSnapshot) {
        try (JsonWriter writer = createSnapshotWriter(exportSnapshot.id)) {
    ...
    private void onFinishExport(FinishExport finishExport) {
        try (JsonWriter writer = createJournalWriter(finishExport.id)) {
            writer.beginObject();
            writer.name("Entries");
            writer.beginArray();
            for (ReplicatedLogEntry entry: entries) {
                if (entry.getData() instanceof CommitTransactionPayload) {
                    ...
                    writer.beginObject().name("Payload").value(entry.getData().toString()).endObject();
                }
            }
            writer.endArray();
            writer.endObject();
    ...
    private void writeNode(JsonWriter writer, DataTreeCandidate candidate) throws IOException {
        ...
        switch (node.getModificationType()) {
            ...
            case WRITE:
                ...
                writer.beginObject().name("Data").value(node.getDataAfter().get().getValue().toString()).endObject();
                writer.endArray();
                writer.endObject();
                break;
            default:
    ...
    private JsonWriter createJournalWriter(String value) throws IOException {
        createBaseDir();
        createJournalDir();

        final File filePath = Paths.get(journalDirPath + "/" + value + "-journal.json").toFile();
        LOG.debug("Creating JSON file : {}", filePath);
        FileOutputStream fileOutputStream = new FileOutputStream(filePath);
        Writer fileWriter = new OutputStreamWriter(fileOutputStream,"UTF-8");
        return new JsonWriter(fileWriter);
    }

    private JsonWriter createSnapshotWriter(String value) throws IOException {
        createBaseDir();
        createSnapshotDir();

        final File filePath = Paths.get(snapshotDirPath + "/" + value + "-snapshot.json").toFile();
        LOG.debug("Creating JSON file : {}", filePath);
        FileOutputStream fileOutputStream = new FileOutputStream(filePath);
        Writer fileWriter = new OutputStreamWriter(fileOutputStream,"UTF-8");
        return new JsonWriter(fileWriter);
    ...
    private void createBaseDir() {
        if (baseDir.mkdir()) {
            LOG.debug("Created {} directory.", baseDirPath);
        } else  if (baseDir.isDirectory()) {
            LOG.debug("Directory {} already exists.", baseDirPath);
        } else {
            LOG.error("Directory {} cannot be created.", baseDirPath);
        }
    }

    private void createJournalDir() {
        if (journalDir.mkdir()) {
            LOG.debug("Created {} directory.", journalDirPath);
        } else  if (journalDir.isDirectory()) {
            LOG.debug("Directory {} already exists.", journalDirPath);
        } else {
            LOG.error("Directory {} cannot be created.", journalDirPath);
        }
    }

    private void createSnapshotDir() {
        if (snapshotDir.mkdir()) {
            LOG.debug("Created {} directory.", snapshotDirPath);
        } else  if (snapshotDir.isDirectory()) {
            LOG.debug("Directory {} already exists.", snapshotDirPath);
        } else {
            LOG.error("Directory {} cannot be created.", snapshotDirPath);
        }
    }
            ...
            while (iterator.hasNext()) {
                ...
                switch (node.getModificationType()) {
                    ...
                    case WRITE:
                        ...
                        writer.beginObject().name("Path").value(path.toString()).endObject();
                        writer.beginObject().name("ModificationType").value("WRITE").endObject();
                        writer.beginObject().name("Data").value(node.getDataAfter().get().getValue().toString())
                                .endObject();
                        writer.endArray();
                        writer.endObject();
                        break;
