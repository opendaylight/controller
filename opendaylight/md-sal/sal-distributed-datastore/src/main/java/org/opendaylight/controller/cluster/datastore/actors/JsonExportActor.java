...
public final class JsonExportActor extends AbstractUntypedActor {
    ...
    private JsonWriter createWriter(String value) throws IOException {
        if (defaultDir.mkdir()) {
            LOG.debug("Created " + defaultDirPath + " directory.");
        } else  if (defaultDir.isDirectory()) {
            LOG.debug("Directory " + defaultDirPath + " already exists.");
        } else {
            LOG.error("Directory " + defaultDirPath + " cannot be created.");
        }
            ...
            while (iterator.hasNext()) {
                ...
                switch (node.getModificationType()) {
                    ...
                    case WRITE:
                        ...
                        writer.beginObject().name("Path").value(path.toString()).endObject();
                        writer.beginObject().name("Modification").value("WRITE").endObject();
                        writer.beginObject().name("Value").value(node.getDataAfter().get().getValue().toString())
                                .endObject();
                        writer.endArray();
                        writer.endObject();
                        break;
