...
class ShardTransactionMessageRetrySupport {
    ...
    void addMessageToRetry(Object message, ActorRef replyTo, String failureMessage) {
        ...
        FiniteDuration period = shard.getDatastoreContext().getShardLeaderElectionTimeout().duration();
        messageInfo.timer = shard.getContext().system().scheduler().schedule(period, period, shard.getSelf(),
