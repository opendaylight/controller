...
class TransactionContextWrapper {
    ...
    private void enqueueTransactionOperation(final TransactionOperation operation) {
        ...
        try {
            ...
            synchronized (queuedTxOperations) {
                ...
                finishHandoff = deferredTransactionContext != null;
            }
        } finally {
            if (cleanupEnqueue) {
                synchronized (queuedTxOperations) {
    ...
    private void deferredHandoff() {
        final TransactionContext localTransactionContext;
        synchronized (queuedTxOperations) {
            localTransactionContext = Verify.verifyNotNull(deferredTransactionContext);
            deferredTransactionContext = null;
        }

        executePriorTransactionOperations(localTransactionContext);
    }
