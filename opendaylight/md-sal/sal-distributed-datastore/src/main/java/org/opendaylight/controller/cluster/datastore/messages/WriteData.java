...
public class WriteData extends ModifyData {
    ...
    private static final Creator<WriteData> CREATOR = new Creator<WriteData>() {
        @Override
        public WriteData newInstance(YangInstanceIdentifier path, NormalizedNode<?, ?> node) {
            return new WriteData(path, node);
        }
    };
    ...
    public Object toSerializable() {
        return ShardTransactionMessages.WriteData.newBuilder().setSerializedNodeAndPath(
...
//                .setNormalizedNode(encoded.getEncodedNode().getNormalizedNode()).build();
    ...
    public static WriteData fromSerializable(Object serializable) {
        ...
        if(o.hasSerializedNodeAndPath()) {
            return SerializationUtils.pathAndNodeFromByteString(o.getSerializedNodeAndPath(), CREATOR);
        } else {
            // From older version
