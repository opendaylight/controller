...
public abstract class AbstractClientHistory extends LocalAbortable implements Identifiable<LocalHistoryIdentifier> {
    ...
    /*
     *  FIXME: CONTROLLER-1991: track shard -> last purged transaction. The idea here is that we need to soft-track
     ...
     *                          allow for some configurable skew (say. 100 transactions) to be indeterminate, before
     *                          deciding to tell the backends (on individual basis?) about this at the moment we
     *                          allocate a new transaction towards that shard.
     *
     *                          This means that while under 'normal' conditions we do not need to issue these explicit
     *                          messages (because we have 1:1 shard/history mapping), we place an upper bound on how
     *                          much backend transaction tracking can fragment (in terms of RangeSet).
     *
     ...
     *                          may take a a special-case path from createSnapshot()/createTransaction(). Those look
     *                          like they'll end up doing more than just returning a snapshot.
     *
