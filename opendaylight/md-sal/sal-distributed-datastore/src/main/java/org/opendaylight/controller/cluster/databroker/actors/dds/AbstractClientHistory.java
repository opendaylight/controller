...
abstract class AbstractClientHistory extends LocalAbortable implements Identifiable<LocalHistoryIdentifier> {
    ...
    @GuardedBy("this")
    private final Map<TransactionIdentifier, ClientSnapshot> openTransactions = new HashMap<>();
    @GuardedBy("this")
    ...
    final void localAbort(final Throwable cause) {
        ...
        if (oldState != State.CLOSED) {
            ...
            synchronized (this) {
                for (ClientSnapshot t : openTransactions.values()) {
                    t.localAbort(cause);
                }
     ...
     */
    synchronized void onTransactionAbort(final ClientSnapshot snapshot) {
