...
abstract class AbstractClientHistory extends LocalAbortable implements Identifiable<LocalHistoryIdentifier> {
    ...
    ReconnectCohort startReconnect(final Long shard, final ConnectedClientConnection newConnection) {
        final ProxyHistory h = histories.get(shard);
        return h == null ? null : h.startReconnect(newConnection);
    }
}
