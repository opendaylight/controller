...
final class FrontendTransaction {
    ...
    java.util.Optional<TransactionSuccess<?>> replaySequence(final long sequence) throws RequestException {
        ...
        if (lastPurgedSequence != null && Long.compareUnsigned(lastPurgedSequence.longValue(), sequence) >= 0) {
            LOG.warn("{}: ignoring purged sequence {}", id, Long.toHexString(sequence));
    ...
    private void handleTransactionPreCommit(final TransactionPreCommitRequest request,
        ...
        readyCohort.preCommit(new FutureCallback<DataTreeCandidate>() {
            ...
            public void onFailure(final Throwable failure) {
                failedPreCommit(envelope, failure);
            }
        });
    }

    private void failedPreCommit(final RequestEnvelope envelope, final Throwable cause) {
        // FIXME: implement this
        readyCohort = null;
    }
    ...
    private void handleTransactionDoCommit(final TransactionDoCommitRequest request, final RequestEnvelope envelope)
        ...
        readyCohort.commit(new FutureCallback<UnsignedLong>() {
            ...
            @Override
            public void onFailure(final Throwable failure) {
                failedCommit(envelope, failure);
            }
        });
    }

    private void failedCommit(final RequestEnvelope envelope, final Throwable cause) {
        recordAndSendFailure(envelope, new RuntimeRequestException("Commit failed", cause));
        readyCohort = null;
    }
    ...
    private void coordinatedCommit(final RequestEnvelope envelope) {
        readyCohort.canCommit(new FutureCallback<Void>() {
            ...
            @Override
            public void onFailure(final Throwable failure) {
                failedCanCommit(envelope, failure);
            }
        });
    }

    private void failedCanCommit(final RequestEnvelope envelope, final Throwable cause) {
    ...
    private void directCommit(final RequestEnvelope envelope) {
        readyCohort.canCommit(new FutureCallback<Void>() {
            ...
            @Override
            public void onFailure(final Throwable failure) {
                failedDirectCanCommit(envelope, failure);
            }
        });

    }

    private void failedDirectCanCommit(final RequestEnvelope envelope, final Throwable cause) {
        // FIXME: implement this
        readyCohort = null;
    }
    ...
    private void successfulDirectCanCommit(final RequestEnvelope envelope) {
        readyCohort.preCommit(new FutureCallback<DataTreeCandidate>() {
            ...
            @Override
            public void onFailure(final Throwable failure) {
                failedDirectPreCommit(envelope, failure);
            }
        });
    }

    private void failedDirectPreCommit(final RequestEnvelope envelope, final Throwable cause) {
        // FIXME: implement this
        readyCohort = null;
    }
    ...
    private void successfulDirectPreCommit(final RequestEnvelope envelope) {
        readyCohort.commit(new FutureCallback<UnsignedLong>() {
            ...
            @Override
            public void onFailure(final Throwable failure) {
                failedDirectCommit(envelope, failure);
            }
        });
    }

    private void failedDirectCommit(final RequestEnvelope envelope, final Throwable cause) {
        // FIXME: implement this
        readyCohort = null;
    }
