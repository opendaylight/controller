...
final class FrontendTransaction {
    ...
    TransactionSuccess<?> handleRequest(final TransactionRequest<?> request, final RequestEnvelope envelope)
        ...
        if (request instanceof ModifyTransactionRequest) {
            ...
            return handleExistsTransaction((ExistsTransactionRequest) request, envelope.getSequence());
        } else if (request instanceof ReadTransactionRequest) {
            return handleReadTransaction((ReadTransactionRequest) request, envelope.getSequence());
    ...
    private void recordAndSendSuccess(final RequestEnvelope envelope, final TransactionSuccess<?> success) {
        recordResponse(envelope.getSequence(), success);
    ...
    private void recordAndSendFailure(final RequestEnvelope envelope, final RuntimeRequestException failure) {
        recordResponse(envelope.getSequence(), failure);
    ...
    private TransactionSuccess<?> handleModifyTransaction(final ModifyTransactionRequest request, final RequestEnvelope envelope)
            throws RequestException {
        ...
        if (!maybeProto.isPresent()) {
            return replyModifySuccess(envelope.getSequence());
        }
