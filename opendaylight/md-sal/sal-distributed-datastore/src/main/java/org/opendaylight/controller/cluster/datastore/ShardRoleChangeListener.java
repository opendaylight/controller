...
public class ShardRoleChangeListener extends AbstractUntypedActor {
    ...
    private void onRoleChangeNotification(RoleChangeNotification roleChanged) {
        ...
        this.clusteringService.changeShardRole(getMemberForShard(roleChanged.getMemberId()), roleChanged.getMemberId(),
                getEnum(roleChanged.getOldRole()), getEnum(roleChanged.getNewRole()));
    }

    private ShardRole getEnum(String role) {
        if (role != null) {
            if (RaftState.Follower.toString().equals(role)) {
                return ShardRole.FOLLOWER;
            } else if (RaftState.Candidate.toString().equals(role)) {
                return ShardRole.CANDIDATE;
            } else if (RaftState.Leader.toString().equals(role)) {
                return ShardRole.LEADER;
            } else if (RaftState.IsolatedLeader.toString().equals(role)) {
                return ShardRole.ISOLATEDLEADER;
            }
        }
        return ShardRole.NONE;
    }
