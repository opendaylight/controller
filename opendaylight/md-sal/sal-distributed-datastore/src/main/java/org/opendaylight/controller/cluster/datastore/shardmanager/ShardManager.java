...
class ShardManager extends AbstractUntypedPersistentActorWithMetering {
    ...
    public void preStart() {
        ...
        final Subscribe<ORSet<PrefixShardConfiguration>> subscribe = new Subscribe<>(ClusterUtils.CONFIGURATION_KEY, self());
    ...
    private void onConfigChanged(final Changed<ORSet<PrefixShardConfiguration>> change) {
        LOG.debug("{}, ShardManager {} received config changed {}", cluster.getCurrentMemberName(), persistenceId, change.dataValue().getElements());

        final Set<PrefixShardConfiguration> changedConfig = change.dataValue().getElements();

        final Map<DOMDataTreeIdentifier, PrefixShardConfiguration> newConfig =
                changedConfig.stream().collect(Collectors.toMap(PrefixShardConfiguration::getPrefix, java.util.function.Function.identity()));
    ...
    private void resolveConfig(final Map<DOMDataTreeIdentifier, PrefixShardConfiguration> newConfig) {
        ...
        newConfig.forEach((p, c) ->
                LOG.debug("{} ShardManager : {}, received shard config " +
                        "for prefix {}, config {}", cluster.getCurrentMemberName(), persistenceId, p, c));
        ...
        // iter through to update existing shards, either start/stop replicas or update the shard to check for more peers
        resolveUpdates(Collections.emptySet());
    }

    private void resolveRemovals(final Set<DOMDataTreeIdentifier> removedConfigs) {
        LOG.debug("{} ShardManager : {}, resolving removed configs : {}", cluster.getCurrentMemberName(), persistenceId, removedConfigs);

        removedConfigs.forEach(id -> doRemovePrefixedShard(id));
    }

    private void resolveAdditions(final Set<DOMDataTreeIdentifier> addedConfigs, Map<DOMDataTreeIdentifier, PrefixShardConfiguration> configs) {
    ...
    private void doRemovePrefixedShard(final DOMDataTreeIdentifier prefix) {
        LOG.debug("{} ShardManager : {}, removing prefix shard: {}", cluster.getCurrentMemberName(), persistenceId, prefix);
    ...
    private void doCreatePrefixedShard(final PrefixShardConfiguration config) {
        ...
        final ShardIdentifier shardId = ClusterUtils.getShardIdentifier(cluster.getCurrentMemberName(), config.getPrefix());
        ...
        final Map<String, String> peerAddresses = getPeerAddresses(shardName, config.getShardMemberNames());
        final boolean isActiveMember = true;
        LOG.debug("{} doCreatePrefixedShard: persistenceId(): {}, memberNames: {}, peerAddresses: {}, isActiveMember: {}",
                shardId, persistenceId(), config.getShardMemberNames(), peerAddresses, isActiveMember);
