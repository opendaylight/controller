...
class ShardManager extends AbstractUntypedPersistentActorWithMetering {
    ...
    private void onInitConfigListener() {
        LOG.debug("{}: Initializing config listener.", cluster.getCurrentMemberName());
        ...
        configUpdateHandler = new ShardConfigUpdateHandler(self(), cluster.getCurrentMemberName());
    ...
    private void doCreatePrefixConfigShard(final CreatePrefixConfigShard message) {
        LOG.debug("{} - doCreatePrefixConfigShard.", cluster.getCurrentMemberName());
        ...
        ShardIdentifier shardId = ClusterUtils.getShardIdentifier(cluster.getCurrentMemberName(),
                ...
                ClusterUtils.PREFIX_CONFIG_SHARD_ID);
        ...
        final ArrayList<MemberName> members = new ArrayList<>();
        Cluster.get(context().system()).state().getMembers()
                .forEach(member -> members.add(MemberName.forName(member.getRoles().iterator().next())));

        final Set<String> allRoles = Cluster.get(context().system()).state().getAllRoles();

        LOG.debug("{}: all roles: {}", cluster.getCurrentMemberName(), allRoles);

        final Map<String, String> peerAddresses = getPeerAddresses(shardName, members);
//        final Map<String, String> peerAddresses = Collections.emptyMap();
    ...
    private void doCreatePrefixedShard(final PrefixShardConfiguration config) {
        LOG.debug("{}: doCreatePrefixShard : {}", cluster.getCurrentMemberName(), config.getPrefix());
    ...
    private void addPeerAddress(MemberName memberName, Address address) {
        ...
        for (ShardInformation info : localShards.values()) {
            ...
            if (shardName.equals(ClusterUtils.PREFIX_CONFIG_SHARD_ID)) {
                LOG.debug("{}: memberup triggered and we have the config shard", cluster.getCurrentMemberName());

                final ShardIdentifier shardId = ClusterUtils.getShardIdentifier(cluster.getCurrentMemberName(),
                        LogicalDatastoreType.valueOf(
                                datastoreContextFactory.getBaseDatastoreContext().getLogicalStoreType().name()),
                        ClusterUtils.PREFIX_CONFIG_SHARD_ID);
                final String shardAddress = peerAddressResolver.getShardActorAddress(shardName, memberName);

                final ActorRef actor = info.getActor();
                if (actor != null) {
                    actor.tell(new AddServer(shardId.getShardName(), shardAddress, true), ActorRef.noSender());
                } else {
                    LOG.warn("{}: Prefix config shard has no known actor, unable to add server",
    ...
    private void findLocalShard(FindLocalShard message) {
        ...
        if (shardInformation == null) {
            ...
            localShards.keySet().forEach(s -> {
                aa.append(s);
                aa.append(",");
            });

            LOG.debug("{} - findLocalShard : {}, shards present: {}",
                    cluster.getCurrentMemberName(), message.getShardName(), aa.toString());
            return;
        }
