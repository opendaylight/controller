...
class ShardManager extends AbstractUntypedPersistentActorWithMetering {
    ...
    private void doCreatePrefixConfigShard(final CreatePrefixConfigShard message) {
        LOG.debug("{}: doCreatePrefixConfigShard for {} data store", persistenceId(), message.getType());
        ...
        if (localShards.containsKey(shardName)) {
            LOG.warn("{}: Prefix configuration shard for data store {} already exists, aborting..",
                    persistenceId, message.getType());
        }
        ...
        // TODO reword this log to be more concise with rest
    ...
    private void doCreatePrefixedShard(final PrefixShardConfiguration config) {
        LOG.debug("{}: doCreatePrefixShard: {}", persistenceId(), config.getPrefix());
