...
/**
 * Handles commits and retries for the DOMEntityOwnershipShard.
 *
 */
public class DOMEntityOwnershipShardCommitCoordinator {

    private static final Object COMMIT_RETRY_MESSAGE = new Object() {
        @Override
        public String toString() {
            return "domEntityCommitRetry";
        }
    };
    private static final FrontendType FRONTEND_TYPE = FrontendType.forName("dom-entity-ownership-internal");
    ...
    DOMEntityOwnershipShardCommitCoordinator(final MemberName localMemberName, final Logger log) {
    ...
    boolean handleMessage(final Object message, final DOMEntityOwnershipShard shard) {
    ...
    private void retryInflightCommit(final DOMEntityOwnershipShard shard) {
    ...
    void inflightCommitFailure(final Throwable cause, final DOMEntityOwnershipShard shard) {
    ...
    private void scheduleInflightCommitRetry(final DOMEntityOwnershipShard shard) {
        final FiniteDuration duration = shard.getDatastoreContext().getShardRaftConfig().getElectionTimeOutInterval();

        log.debug("Scheduling retry for BatchedModifications commit {} in {}", inflightCommit.getTransactionID(),
                duration);
    ...
    void inflightCommitSucceeded(final DOMEntityOwnershipShard shard) {
    ...
    void commitNextBatch(final DOMEntityOwnershipShard shard) {
        if (inflightCommit != null || pendingModifications.isEmpty() || !shard.hasLeader()) {
            return;
        }

        inflightCommit = newBatchedModifications();
        final Iterator<Modification> iter = pendingModifications.iterator();
        while (iter.hasNext()) {
    ...
    void commitModification(final Modification modification, final DOMEntityOwnershipShard shard) {
    ...
    void commitModifications(final BatchedModifications modifications, final DOMEntityOwnershipShard shard) {
        if (modifications.getModifications().isEmpty()) {
            return;
        }

        final boolean hasLeader = shard.hasLeader();
    ...
    void onStateChanged(final DOMEntityOwnershipShard shard, final boolean isLeader) {
    ...
    private void newInflightCommitWithDifferentTransactionID() {
        final BatchedModifications newBatchedModifications = newBatchedModifications();
        newBatchedModifications.getModifications().addAll(inflightCommit.getModifications());
        inflightCommit = newBatchedModifications;
    }

    BatchedModifications newBatchedModifications() {
        final BatchedModifications modifications = new BatchedModifications(
