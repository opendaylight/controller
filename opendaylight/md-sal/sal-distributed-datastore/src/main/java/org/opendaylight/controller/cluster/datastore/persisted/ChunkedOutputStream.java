...
final class ChunkedOutputStream extends OutputStream {
    ...
    private ImmutableList<byte[]> result;
    ...
    ChunkedByteArray toChunkedByteArray() {
        ...
        return new ChunkedByteArray(size, result);
    }

    private ImmutableList<byte[]> computeResult() {
        if (prevChunks == null) {
            // Simple case: it's only the current buffer, return that
            return ImmutableList.of(trimChunk(currentChunk, currentOffset));
        }
        if (size <= MAX_ARRAY_SIZE) {
            ...
            if (currentOffset != 0 || prevChunks.size() != 1) {
                ...
                System.arraycopy(currentChunk, 0, result, offset, currentOffset);
            } else {
                // ... which we have readily available
                result = prevChunks.getFirst();
            }
        ...
        final ImmutableList.Builder<byte[]> builder = ImmutableList.builderWithExpectedSize(prevChunks.size() + 1);
    ...
    private void nextChunk(final int size) {
        if (prevChunks == null) {
