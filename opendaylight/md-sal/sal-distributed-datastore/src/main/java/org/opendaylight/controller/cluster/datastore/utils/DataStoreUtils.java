...
public class DataStoreUtils {
    ...
    private static DataStoreUtils dsUtils = new DataStoreUtils();
     ...
     * @return
     */
    public boolean doPathsCoExist(String path1, String path2) {
        if (path1 == null || path2 == null) return false;

        int atIndex1 = path1.indexOf("@");
        int atIndex2 = path2.indexOf("@");

        if (atIndex1 == -1 || atIndex2 == -1) return false;

        int slashIndex1 = path1.indexOf("/", atIndex1);
        int slashIndex2 = path2.indexOf("/", atIndex2);

        if (slashIndex1 == -1 || slashIndex2 == -1) return false;
