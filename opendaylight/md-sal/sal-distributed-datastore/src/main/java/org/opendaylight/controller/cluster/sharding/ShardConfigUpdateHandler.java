...
public class ShardConfigUpdateHandler {
    ...
    public ShardConfigUpdateHandler(final ActorRef handlingActor) {
    ...
    public void initListener(final AbstractDataStore dataStore, final LogicalDatastoreType type) {
        registrations.put(type, dataStore.registerShardConfigListener(
                ClusterUtils.SHARD_LIST_PATH, new ShardConfigHandler(dataStore, type, handlingActor)));
    }
    ...
    public static final class ShardConfigHandler implements DOMDataTreeChangeListener {

        private final AbstractDataStore datastore;
        private final LogicalDatastoreType type;
        private final ActorRef handlingActor;

        public ShardConfigHandler(final AbstractDataStore datastore,
        ...
        private void resolveWrite(final DataTreeCandidateNode rootNode) {
            ...
            LOG.warn("New config received {}", rootNode);
            LOG.warn("Data after: {}", rootNode.getDataAfter());
...
//                    shardNode.getChild(new NodeIdentifier(ShardConfigWriter.SHARD_PREFIX_QNAME)).get();
        ...
        private void resolveWrittenShard(final DataTreeCandidateNode childNode) {
            ...
            LOG.debug("Deserialized {} from datastore", identifier);
            ...
            LOG.debug("Replicas read from ds [}", retReplicas.toString());
            ...
            LOG.debug("Resulting config {}", newConfig);
