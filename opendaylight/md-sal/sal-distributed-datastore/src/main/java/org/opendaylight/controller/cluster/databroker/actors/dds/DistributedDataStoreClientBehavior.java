...
final class DistributedDataStoreClientBehavior extends ClientActorBehavior implements DistributedDataStoreClient {
    ...
    private final Map<TransactionIdentifier, ClientSingleTransaction> transactions = new ConcurrentHashMap<>();
    ...
    private void abortOperations(final Throwable cause) {
        ...
        for (ClientSingleTransaction t : transactions.values()) {
            t.close();
        }
        transactions.clear();
    }
    ...
    private <K, V extends AutoCloseable> V returnIfOperational(final Map<K , V> map, final K key, final V value) {
        Verify.verify(map.put(key, value) == null);

        final Throwable localAborted = aborted;
        if (localAborted != null) {
            try {
                value.close();
            } catch (Exception e) {
                LOG.debug("Close of {} failed", value, e);
            }
            histories.remove(key, value);
            throw Throwables.propagate(localAborted);
        }
    ...
    public ClientLocalHistory createLocalHistory() {
        ...
        return returnIfOperational(histories, historyId, history);
    }
    ...
    public ClientSingleTransaction createTransaction() {
        ...
        return returnIfOperational(transactions, txId, tx);
    }
