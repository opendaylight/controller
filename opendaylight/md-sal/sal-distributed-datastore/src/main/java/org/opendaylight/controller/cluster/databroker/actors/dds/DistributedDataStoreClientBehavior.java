...
final class DistributedDataStoreClientBehavior extends ClientActorBehavior implements DistributedDataStoreClient {
     ...
     */
    // TODO: it should be possible to move these twp into ClientActorBehavior (or ClientActorContext).
    ...
    private void backendConnectFinished(final Long shard, final ConnectingClientConnection<ShardBackendInfo> conn,
        ...
        try {
            ...
            if (backend.getDataTree().isPresent()) {
                newConn = localConnectionUp(shard, conn, backend);
            } else {
                newConn = remoteConnectionUp(shard, conn, backend);
            }
    ...
    private ConnectedClientConnection<ShardBackendInfo> localConnectionUp(final Long shard,
            final ConnectingClientConnection<ShardBackendInfo> conn, final ShardBackendInfo backend) {
     ...
     */
    @GuardedBy("connectionsLock")
    private ConnectedClientConnection<ShardBackendInfo> remoteConnectionUp(final Long shard,
            final ConnectingClientConnection<ShardBackendInfo> conn, final ShardBackendInfo backend) {
        LOG.debug("Completing connecting connection {} with {}", conn, backend);
        final ConnectedClientConnection<ShardBackendInfo> newConn = new ConnectedClientConnection<>(context(), shard,
                backend);
        ...
        conn.setForwarder(new SimpleReconnectForwarder(newConn));
        return newConn;
    }

    private static void updateConnection(final AbstractClientHistory history, final Long shard,
            final ShardBackendInfo backend, final ConnectedClientConnection<ShardBackendInfo> conn) {
    ...
    @Override
    protected void reconnectConnection(final AbstractClientConnection<?> conn) {
        LOG.debug("{}: initiating reconnect of {}", persistenceId(), conn);

        // TODO: this should not be needed
        Verify.verify(conn instanceof ConnectedClientConnection<?>);
        @SuppressWarnings("unchecked")
        final ConnectedClientConnection<ShardBackendInfo> previous =
        (ConnectedClientConnection<ShardBackendInfo>) conn;

        final ReconnectingClientConnection<ShardBackendInfo> newConn = previous.toReconnecting();
        connections.replace(conn.cookie(), previous, newConn);

        // FIXME: initiate reconnect attempt

        LOG.debug("{}: connection {} reconnecting as {}", persistenceId(), conn, newConn);
    }
