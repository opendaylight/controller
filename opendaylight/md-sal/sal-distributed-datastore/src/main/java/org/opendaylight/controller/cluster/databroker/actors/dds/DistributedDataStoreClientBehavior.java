...
final class DistributedDataStoreClientBehavior extends ClientActorBehavior implements DistributedDataStoreClient {
    ...
    protected void reconnectConnection(final ConnectedClientConnection<?> oldConn,
        ...
        connections.replace(oldConn.cookie(), (AbstractClientConnection<ShardBackendInfo>)oldConn,
            (AbstractClientConnection<ShardBackendInfo>)newConn);
        LOG.debug("{}: connection {} reconnecting as {}", persistenceId(), oldConn, newConn);

        // FIXME: initiate resolution of the new connection
    }
