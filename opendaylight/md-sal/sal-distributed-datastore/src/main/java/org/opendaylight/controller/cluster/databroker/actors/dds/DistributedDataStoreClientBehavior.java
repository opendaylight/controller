...
final class DistributedDataStoreClientBehavior extends ClientActorBehavior implements DistributedDataStoreClient {
    ...
    private void localConnectionUp(final Long shard, final ConnectingClientConnection conn,
        ...
        final Collection<ReconnectCohort> cohorts = new ArrayList<>();
        try {
            ...
            //         missing an entry
            for (ClientLocalHistory h : histories.values()) {
                final ReconnectCohort handle = h.freezeShard(shard);
                if (handle != null) {
                    cohorts.add(handle);
                }
            }
            ...
            for (ReconnectCohort c : cohorts) {
                // FIXME: do not use sendRequest as it may block
                c.replaySuccessfulRequests(newConn::sendRequest);
            }
            ...
            // Step 5:
            for (ReconnectCohort c : cohorts) {
                c.replaceConnection(newConn);
            }
        }
    }

    private static void updateConnection(final AbstractClientHistory history, final Long shard,
            final AbstractClientConnection conn) {
        final ReconnectCohort cohort = history.freezeShard(shard);
        if (cohort != null) {
            cohort.replaceConnection(conn);
        }
    }
    ...
    private void remoteConnectionUp(final Long shard, final ConnectingClientConnection conn,
        ...
        updateConnection(singleHistory, shard, newConn);
        for (ClientLocalHistory h : histories.values()) {
            updateConnection(h, shard, newConn);
        }
    }
