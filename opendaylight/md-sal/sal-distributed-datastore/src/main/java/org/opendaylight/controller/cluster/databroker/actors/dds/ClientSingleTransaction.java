...
public final class ClientSingleTransaction implements AutoCloseable {
    ...
    CompletionStage<Boolean> invokeExists(final TransactionStateTracker tracker, final YangInstanceIdentifier path) {
        final CompletableFuture<Boolean> future = new CompletableFuture<>();
        sendRequest(new ExistsTransactionRequest(tracker.getIdentifier(), tracker.nextSequence(),
            parent.getReplyTo(), path), t -> completeExists(future, t));
        return future;
    }
    ...
    CompletionStage<Optional<NormalizedNode<?, ?>>> invokeRead(final TransactionStateTracker tracker,
            final YangInstanceIdentifier path) {
        final CompletableFuture<Optional<NormalizedNode<?, ?>>> future = new CompletableFuture<>();
        sendRequest(new ExistsTransactionRequest(tracker.getIdentifier(), tracker.nextSequence(),
            parent.getReplyTo(), path), t -> completeRead(future, t));
        return future;
    }

    private void sendRequest(final TransactionRequest<?> request, final Consumer<Response<TransactionIdentifier, ?>> completer) {
        // FIXME: route to client frontend
    }
}
