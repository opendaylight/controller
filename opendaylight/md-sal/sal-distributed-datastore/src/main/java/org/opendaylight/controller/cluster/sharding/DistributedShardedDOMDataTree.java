...
public class DistributedShardedDOMDataTree implements DOMDataTreeService, DOMDataTreeShardingService,
    ...
    public DistributedShardedDOMDataTree(final ActorSystemProvider actorSystemProvider,
        ...
        shardedDataTreeActor = createShardedDataTreeActor(actorSystem,
                ...
                ACTOR_ID, distributedConfigDatastore.getActorContext().getDatastoreContext());
    ...
    private Entry<DataStoreClient, ActorRef> createDatastoreClient(
        ...
        final Props distributedDataStoreClientProps =
                SimpleDataStoreClientActor.props(memberName, "Shard-" + shardName, actorContext, shardName, true);
        final FiniteDuration minBackoff = Duration.create(
                actorContext.getDatastoreContext().getPersistentActorRestartMinBackoffInSeconds(), TimeUnit.SECONDS);
        final FiniteDuration maxBackoff = Duration.create(
                actorContext.getDatastoreContext().getPersistentActorRestartMaxBackoffInSeconds(), TimeUnit.SECONDS);
        final FiniteDuration resetBackoff = Duration.create(
                actorContext.getDatastoreContext().getPersistentActorRestartResetBackoffInSeconds(), TimeUnit.SECONDS);
