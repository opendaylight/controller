 ...
 * @author Robert Varga
 */
abstract class ClientBackedTransaction<T extends ClientSnapshot> extends
    ...
    private static final class Finalizer extends FinalizablePhantomReference<ClientBackedTransaction<?>> {
        ...
        private final ClientSnapshot transaction;

        private Finalizer(final ClientBackedTransaction<?> referent, final ClientSnapshot transaction) {
            super(referent, QUEUE);
            this.transaction = Preconditions.checkNotNull(transaction);
        }

        static @Nonnull <T extends ClientSnapshot> T recordTransaction(
