...
abstract class ClientBackedTransaction extends AbstractDOMStoreTransaction<TransactionIdentifier> {
    private static final class Finalizer extends FinalizablePhantomReference<ClientBackedTransaction> {
        ...
        private Finalizer(final ClientBackedTransaction referent, final ClientTransaction transaction) {
            super(referent, QUEUE);
            this.transaction = Preconditions.checkNotNull(transaction);
        }

        static @Nonnull ClientTransaction recordTransaction(@Nonnull final ClientBackedTransaction referent,
                @Nonnull final ClientTransaction transaction) {
