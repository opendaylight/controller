...
abstract class ProxyHistory implements Identifiable<LocalHistoryIdentifier> {
    ...
    private static abstract class AbstractRemote extends ProxyHistory {
        AbstractRemote(final AbstractClientConnection connection, final LocalHistoryIdentifier identifier) {
            super(connection, identifier);
        }

        @Override
        final AbstractProxyTransaction doCreateTransactionProxy(final AbstractClientConnection connection,
    ...
    private static final class Local extends AbstractLocal {
        ...
        @Override
        ProxyHistory createSuccessor(final AbstractClientConnection connection) {
    ...
    private static final class RemoteSingle extends AbstractRemote {
        RemoteSingle(final AbstractClientConnection connection, final LocalHistoryIdentifier identifier) {
            super(connection, identifier);
        }
    ...
    private ProxyHistory(final AbstractClientConnection connection, final LocalHistoryIdentifier identifier) {
    ...
    abstract ProxyHistory createSuccessor(AbstractClientConnection connection);

    ProxyReconnectCohort startReconnect(final ConnectedClientConnection newConnection) {
        lock.lock();
        if (successor != null) {
            lock.unlock();
