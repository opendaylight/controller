...
abstract class ProxyHistory implements Identifiable<LocalHistoryIdentifier> {
    ...
    ReconnectCohort startReconnect(final ConnectedClientConnection newConnection) {
        ...
        return new ReconnectCohort() {
            ...
            void replaySuccessfulRequests() {
                for (AbstractProxyTransaction t : proxies.values()) {
                    t.replaySuccessfulRequests(newConnection);
                }
            }
            ...
            void finishReconnect() {
                LOG.debug("Replacing connection {} in {} with {}", connection, ProxyHistory.this, newConnection);
            ...
            private void replayTransactionRequest(final TransactionRequest<?> request,
                    ...
                    final BiConsumer<Request<?, ?>, Consumer<Response<?, ?>>> replayTo) throws RequestException {
                // XXX: this should be safe for the most part. We can get called by delayed requests, but those should
                //      not proxies to be touched, as that would indicate multi-threaded -- which we do not allow.
                //      Nevertheless it would be nice if we could detect concurrent access...
