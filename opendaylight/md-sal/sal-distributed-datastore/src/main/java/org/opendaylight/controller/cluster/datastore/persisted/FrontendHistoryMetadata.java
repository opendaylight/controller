...
public final class FrontendHistoryMetadata implements WritableObject {
    ...
    public static FrontendHistoryMetadata readFrom(final DataInput in) throws IOException {
        ...
        long r = WritableObjects.readFirstLong(in, header);
        Verify.verify(r >= 0 && r <= Integer.MAX_VALUE);
        final int csize = (int) r;

        r = WritableObjects.readSecondLong(in, header);
        Verify.verify(r >= 0 && r <= Integer.MAX_VALUE);
        final int psize = (int) r;
