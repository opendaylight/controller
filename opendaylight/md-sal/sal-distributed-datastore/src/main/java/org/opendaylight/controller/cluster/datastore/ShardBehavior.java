...
abstract class ShardBehavior {
    ...
    static ShardBehavior forRaftBehavior(final Shard shard, final RaftActorBehavior raftBehavior) {
        ...
        switch (raftBehavior.state()) {
            case Candidate:
                return new ShardCandidateBehavior(shard, raftBehavior);
            case Follower:
    ...
    final RaftState state() {
        // FIXME: this is probably unneeded
    ...
    final ShardBehavior handleMessage(final ActorRef sender, final Object message) {
    ...
    final ShardBehavior changeState(final RaftActorBehavior raftBehavior) {
        switch (raftBehavior.state()) {
            case Candidate:
                return becomeCandidate(raftBehavior);
            case Follower:
    ...
    abstract ShardCandidateBehavior becomeCandidate(RaftActorBehavior raftBehavior);
