...
abstract class AbstractProxyTransaction implements Identifiable<TransactionIdentifier> {
    ...
    void abort(final VotingFuture<Void> ret) {
        ...
        client.sendRequest(new TransactionAbortRequest(getIdentifier(), client().self()), t -> {
    ...
    void preCommit(final VotingFuture<?> ret) {
        ...
        client.sendRequest(new TransactionPreCommitRequest(getIdentifier(), client().self()), t-> {
    ...
    void doCommit(final VotingFuture<?> ret) {
        ...
        client.sendRequest(new TransactionDoCommitRequest(getIdentifier(), client().self()), t-> {
            if (t instanceof TransactionCommitSuccess) {
                ret.voteYes();
            } else if (t instanceof RequestFailure) {
                ret.voteNo(((RequestFailure<?, ?>) t).getCause());
