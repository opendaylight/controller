...
abstract class AbstractProxyTransaction implements Identifiable<TransactionIdentifier> {
    ...
    final void replayMessages(final AbstractProxyTransaction successor,
        ...
        if (!successfulRequests.isEmpty()) {
            ...
            final long now = firstInQueue != null ? firstInQueue.getEnqueuedTicks() : 0;

            for (Object obj : successfulRequests) {
                if (obj instanceof TransactionRequest) {
                    LOG.debug("Forwarding successful request {} to successor {}", obj, successor);
                    successor.replaySuccessul((TransactionRequest<?>) obj, now);
                } else {
        ...
        while (it.hasNext()) {
            ...
            if (getIdentifier().equals(req.getTarget())) {
                ...
                successor.replayEnqueued((TransactionRequest<?>) req, e.getCallback(), e.getEnqueuedTicks());
                it.remove();
            }
        }
    ...
    private void replaySuccessul(final TransactionRequest<?> obj, final long now) {
     ...
     */
    private void replayEnqueued(final TransactionRequest<?> request, final Consumer<Response<?, ?>> callback,
            final long enqueuedTicks) {
        if (request instanceof AbstractLocalTransactionRequest) {
            handleForwardedLocalRequest((AbstractLocalTransactionRequest<?>) request, callback, enqueuedTicks);
        } else {
            handleForwardedRemoteRequest(request, callback, enqueuedTicks);
        }
    }
