...
abstract class AbstractProxyTransaction implements Identifiable<TransactionIdentifier> {
    ...
    void abort(final CollectingFuture<Void> ret) {
        ...
        client.sendRequest(new TransactionAbortRequest(getIdentifier(), 0, 0, client().self()), t -> {
    ...
    void preCommit(final CollectingFuture<?> ret) {
        ...
        client.sendRequest(new TransactionPreCommitRequest(getIdentifier(), 0, 0, client().self()), t-> {
    ...
    void doCommit(final CollectingFuture<?> ret) {
        ...
        client.sendRequest(new TransactionDoCommitRequest(getIdentifier(), 0, 0, client().self()), t-> {
            if (t instanceof TransactionCommitSuccess) {
                ret.voteYes();
            } else if (t instanceof RequestFailure) {
                ret.voteNo(((RequestFailure<?, ?>) t).getCause());
