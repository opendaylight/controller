...
abstract class AbstractProxyTransaction implements Identifiable<TransactionIdentifier> {
    ...
    final CompletionStage<TransactionSuccess<?>> commit(final boolean coordinated) {
        final CompletableFuture<TransactionSuccess<?>> ret = new CompletableFuture<>();
    ...
    abstract CheckedFuture<Boolean, ReadFailedException> exists(final YangInstanceIdentifier path);

    abstract CheckedFuture<Optional<NormalizedNode<?, ?>>, ReadFailedException> read(final YangInstanceIdentifier path);

    abstract void abort();

    abstract TransactionRequest<?> doCommit(boolean coordinated);

    private static void finishCommit(final CompletableFuture<TransactionSuccess<?>> future,
            final Response<?, ?> response) {
        if (response instanceof TransactionSuccess) {
            future.complete((TransactionSuccess<?>) response);
        } else if (response instanceof RequestFailure) {
            future.completeExceptionally(((RequestFailure<?, ?>) response).getCause());
        } else {
            future.completeExceptionally(new IllegalStateException("Unhandled response " + response.getClass()));
        }
    }
}
