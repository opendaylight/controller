...
final class ShardManagerInfo extends AbstractMXBean implements ShardManagerInfoMBean {
    ...
    private void requestSwitchShardState(final String shardName, final String newState, final long term) {
        ...
        switch (state) {
            ...
            case Leader:
                try {
                    Await.result(Patterns.ask(shardManager, new SwitchShardBehavior(shardName, state, term),
    ...
    public void switchShardState(String shardName, String newState, long term) {
        ...
        requestSwitchShardState(Preconditions.checkNotNull(shardName, "Shard name may not be null"), newState, term);
    }
}
