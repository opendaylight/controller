...
final class DataTreeChangeListener extends AbstractUntypedActor {
    ...
    private DataTreeChangeListener(DOMDataTreeChangeListener listener) {
    ...
    private void dataChanged(final DataTreeChanged message) {
        ...
        final DataTreeCandidate change = message.getChange();
        LOG.debug("Sending change notification {} to listener {}", change, listener);

        try {
            this.listener.onDataTreeChanged(Collections.singletonList(message.getChange()));
    ...
    private static class DataTreeChangeListenerCreator implements Creator<DataTreeChangeListener> {
        ...
        DataTreeChangeListenerCreator(DOMDataTreeChangeListener listener) {
            this.listener = Preconditions.checkNotNull(listener);
        }
