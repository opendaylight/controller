...
final class DataTreeCandidatePayload extends Payload implements Externalizable {
    ...
    static Entry<Payload, NormalizedNodeOutputDictionary> create(final DataTreeCandidate candidate,
            final NormalizedNodeOutputDictionary dictionary) {
        final ByteArrayDataOutput out = ByteStreams.newDataOutput();
        ...
        final NormalizedNodeOutputDictionary newDictionary;
        try {
            serializeCandidate(writer, candidate);
        } catch (IOException e) {
            throw new IllegalArgumentException(String.format("Failed to serialize candidate %s", candidate), e);
        } finally {
            newDictionary = writer.detachDictionary();
        }

        return new SimpleImmutableEntry<Payload, NormalizedNodeOutputDictionary>(
                new DataTreeCandidatePayload(out.toByteArray()), newDictionary);
    }
