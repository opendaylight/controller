...
final class DataTreeCandidatePayload extends Payload implements Externalizable {
    ...
    private static DataTreeCandidateNode readNode(final NormalizedNodeInputStreamReader reader, final DataInput in) throws IOException {
        ...
        switch (type) {
        case DELETE:
            return new PartialDataTreeCandidateNode(reader.readPathArgument());
        case SUBTREE_MODIFIED:
            return new PartialDataTreeCandidateNode(reader.readPathArgument(), readChildren(reader, in));
        case UNMODIFIED:
            return null;
        case WRITE:
            return new NormalizedNodeDataTreeCandidateNode(type, reader.readNormalizedNode());
        default:
    ...
    public int size() {
        if (serialized == null) {
            ...
            final NormalizedNodeOutputStreamWriter writer = new NormalizedNodeOutputStreamWriter(out);
            try {
                writer.writeYangInstanceIdentifier(candidate.getRootPath());
                writeNode(writer, out, candidate.getRootNode());
                writer.close();
            } catch (IOException e) {
    ...
    public void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        candidate = new DefaultDataTreeCandidate(reader.readYangInstanceIdentifier(), readNode(reader, input));
    }
