...
final class DataTreeCandidatePayload extends Payload implements Externalizable {
    ...
    static Payload create(final DataTreeCandidate candidate, final StreamWriterDictionary dictionary) {
        final ByteArrayDataOutput out = ByteStreams.newDataOutput();
        final NormalizedNodeOutputStreamWriter writer = NormalizedNodeInputOutput.newWriterForDictionary(out, dictionary);
    ...
    Entry<DataTreeCandidate, StreamReaderDictionary> getCandidate(@Nullable final StreamReaderDictionary dictionary)
        ...
        final NormalizedNodeInputStreamReader reader = dictionary == null ?
                NormalizedNodeInputOutput.newStreamReader(input) :
                NormalizedNodeInputOutput.newReaderForDictionary(input, dictionary);
