...
class EntityOwnershipListenerSupport {
    ...
    private ActorRef listenerActorFor(EntityOwnershipListener listener) {
        ListenerActorRefEntry listenerEntry = listenerActorMap.get(listener);
        if(listenerEntry.actorRef == null) {
            listenerEntry.actorRef = actorContext.actorOf(EntityOwnershipListenerActor.props(listener));

            LOG.debug("Created EntityOwnershipListenerActor {} for listener {}", listenerEntry.actorRef, listener);
        }

        return listenerEntry.actorRef;
    }

    private static class ListenerActorRefEntry {
        ActorRef actorRef;
        ...
        public String toString() {
            ...
            builder.append("ListenerActorRefEntry [actorRef=").append(actorRef).append(", referenceCount=")
                    .append(referenceCount).append("]");
