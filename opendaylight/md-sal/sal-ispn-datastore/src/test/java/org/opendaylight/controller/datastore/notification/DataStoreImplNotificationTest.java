...
public class DataStoreImplNotificationTest {
  ...
  public void testNotificationRegistrationInitialSnapshotEvent() throws Exception {
    ...
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("DataStoreImplNotificationTest-test1"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
  ...
  public void testMulitpleNotificationRegistration() throws Exception {
    ...
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("DataStoreImplNotificationTest-test2"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
    dataStore.onGlobalContextUpdated(schemaContext);
    final DOMStoreReadWriteTransaction domStoreReadWriteTransaction = dataStore.newReadWriteTransaction();
    ...
    t.preCommit();
    final ListenableFuture<Void> listenableFuture = t.commit();
    ...
    Map<String, Collection<?>> listenersMap = dataStore.listeners();
    for (Map.Entry<String, Collection<?>> registration : listenersMap.entrySet()) {
      Assert.assertEquals(registration.getKey(), instanceIdentifier.toString());
  ...
  @Test
  public void testNotificationRegistrationAtOuterListNode() throws Exception {
    InstanceIdentifier idOuterList = TestModel.OUTER_LIST_PATH;
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("DataStoreImplNotificationTest-test3"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
    dataStore.onGlobalContextUpdated(schemaContext);
    final DOMStoreReadWriteTransaction domStoreReadWriteTransaction = dataStore.newReadWriteTransaction();
    ...
    final ListenableFuture<Optional<NormalizedNode<?, ?>>> optionalListenableFuture = domStoreReadWriteTransaction.read(idOuterList);

    final Optional<NormalizedNode<?, ?>> normalizedNodeOptional = optionalListenableFuture.get();
    final NormalizedNode<?, ?> normalizedNode = normalizedNodeOptional.get();


    DOMStoreThreePhaseCommitCohort threePC = domStoreReadWriteTransaction.ready();
    threePC.preCommit();


    final ListenableFuture<Void> listenableFuture = threePC.commit();

    //here we want to register a single listener at the OUTER_LIST_PATH instance identifer level

    DataChangeListener dcl1 = new DataChangeListener(normalizedNode, DataChangeListener.ExpectedOperation.ADDED, idOuterList);

    dataStore.registerChangeListener(idOuterList, dcl1, AsyncDataBroker.DataChangeScope.BASE);
