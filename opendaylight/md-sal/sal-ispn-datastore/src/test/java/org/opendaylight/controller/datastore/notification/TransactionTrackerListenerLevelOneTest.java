...
public class TransactionTrackerListenerLevelOneTest {
  ...
  public void RegisterOneLevelListenerFamilyAndAddAChild() throws Exception {
    ...
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("RegisterOneLevelListenerFamilyAndAddAChild"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
  ...
  public void prepareFamilyTree1() throws Exception {
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("listenerlevelOne"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
    dataStore.onGlobalContextUpdated(schemaContext);
