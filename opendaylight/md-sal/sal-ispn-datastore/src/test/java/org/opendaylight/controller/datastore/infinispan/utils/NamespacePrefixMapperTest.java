...
public class NamespacePrefixMapperTest {
    ...
    public void testFromInstanceIdentifier() throws Exception {
        String output = new NamespacePrefixMapper().enableShortening(true).fromInstanceIdentifier("(abcdefgh)lmn/(fqrstkak)grm");
        Assert.assertEquals("(1)lmn/(2)grm", output);
    }

    @Test
    public void testFromFqn() throws Exception {
        final NamespacePrefixMapper mapper = new NamespacePrefixMapper().enableShortening(true);
