...
public class WriteDeleteTransactionTrackerTest {
  ...
  public void  RegisterBaseListenerOnChildAndUpdateChildName() throws Exception{
    ...
    NormalizedNode normalizedNode = prepareFamilyTree();
  ...
  public NormalizedNode<?, ?> prepareFamilyTree() throws Exception {
    ...
    final DataStoreImpl dataStore = new DataStoreImpl(TreeCacheManagerSingleton.get().getCache("WriteDeleteTransactionTrackerTest-test2"), "configuration", schemaContext, MoreExecutors.listeningDecorator(Executors.newSingleThreadExecutor()));
    dataStore.onGlobalContextUpdated(schemaContext);
    final DOMStoreReadWriteTransaction domStoreReadWriteTransaction = dataStore.newReadWriteTransaction();
