...
public class DataStoreImpl implements DOMStore, Identifiable<String>, SchemaContextListener {
  ...
  registerChangeListener(final InstanceIdentifier path, final L listener, final AsyncDataBroker.DataChangeScope scope) {
    ...
    synchronized (this) {
      //we want to register listener and check that if the path is in cache.
   ...
   public Map<InstanceIdentifier, Collection<?>> listeners(){
      Map<InstanceIdentifier,RegisterListenerNode> pathListeners= this.listenerManager.listeners();
      Map<InstanceIdentifier,Collection<?>>listenersMapping = new HashMap<InstanceIdentifier,Collection<?>>();
      for(Map.Entry<InstanceIdentifier,RegisterListenerNode>registration:pathListeners.entrySet()){
