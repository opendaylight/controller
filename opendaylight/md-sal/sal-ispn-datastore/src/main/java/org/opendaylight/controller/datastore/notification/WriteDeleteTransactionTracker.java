...
public class WriteDeleteTransactionTracker {
  ...
  public DOMImmutableDataChangeEvent prepareEvent(String path) {
    ...
    DOMImmutableDataChangeEvent.Builder builder = DOMImmutableDataChangeEvent.builder();
    ...
    if ((filteredSubTree == null) || filteredSubTree.isEmpty()) {
      ...
      snapshotTransactions.resolveLeaf(path, instanceIdentifier, transactionLog.get(path), builder);

    } else {
      //need to determine the ImmutableDataChangeEvent
      snapshotTransactions.resolve(path, filteredSubTree, builder);

      //ok here we need to get the snapshot at path level the new one.
      //TODO: ok is it fair to assume that the normalize node will have all the details of the children
  ...
  private void resolveLeaf(String path, InstanceIdentifier instanceIdentifier, TransactionLog newOne, DOMImmutableDataChangeEvent.Builder builder) {
    TransactionLog oldOne = transactionLog.get(path);

    if (oldOne == null) {
      LOG.debug("Node was added at path {}", path);
      builder.addCreated(instanceIdentifier, newOne.getNode());
    } else if (newOne.getOp() == Operation.REMOVED) {
      LOG.debug("Node was added at path {}", path);
      builder.addRemoved(instanceIdentifier, oldOne.getNode());
    } else if ((newOne.getOp() == Operation.UPDATED) && (oldOne.getOp() == Operation.VISITED)) {
      LOG.debug("Node was added at path {}", path);
      builder.addUpdated(instanceIdentifier, oldOne.getNode(), newOne.getNode());
  ...
  private void resolve(String path, SortedMap<String, TransactionLog> potentials, DOMImmutableDataChangeEvent.Builder builder) {
    ...
    if ((existing == null || existing.isEmpty())) {
      ...
      for (Map.Entry<String, TransactionLog> potential : potentials.entrySet()) {
        ...
        //spliting this and creating list
        List<InstanceIdentifier.PathArgument> pathArguments = new ArrayList<>();
        String[] ids = potential.getKey().split("/");
        for (String nodeId : ids) {
          if (!"".equals(nodeId)) {
            pathArguments.add(NodeIdentifierFactory.getArgument(nodeId));
          }
        ...
        //creating instance identifier
        final InstanceIdentifier instanceIdentifier = new InstanceIdentifier(pathArguments);
        builder.addCreated(instanceIdentifier, potential.getValue().getNode());
      }
      ...
      for (Map.Entry<String, TransactionLog> potential : potentials.entrySet()) {
        ...
        //creating instance identifier
        final InstanceIdentifier instanceIdentifier = new InstanceIdentifier(pathArguments);
        resolveLeaf(potential.getKey(), instanceIdentifier, potential.getValue(), builder);
      }
    }
  }
