...
public class MeterTest extends AbstractDataServiceTest{
	...
	private final static Logger log = Logger.getLogger(MeterTest.class.getName());
	
	private static final long METER_ID = (long) 14;
	...
	private static final InstanceIdentifier<Meter> METER_INSTANCE_BA = InstanceIdentifier.builder(Meters.class) //
	...
	public void testForMeter() throws InterruptedException, ExecutionException{
		...
		Meter meterUp = createMeterUp();
		
		DataObject createdMeter = CrudTestUtil.doCreateTest(meter, baDataService, METER_INSTANCE_BA);
        CrudTestUtil.doReadTest(createdMeter, baDataService, METER_INSTANCE_BA);
        CrudTestUtil.doUpdateTest(meterUp, createdMeter, baDataService, METER_INSTANCE_BA);
        CrudTestUtil.doRemoveTest(meterUp, baDataService, METER_INSTANCE_BA);
	...
	private Meter createMeter() {
		...
		List<MeterBandHeader> headertList = Collections.<MeterBandHeader>singletonList(header.build());
		headers.setMeterBandHeader(headertList);
		
		return new MeterBuilder()
				.setKey(METER_KEY)
				.setMeterName("foo")
				.setMeterBandHeaders(headers.build())
			.build();
	}
