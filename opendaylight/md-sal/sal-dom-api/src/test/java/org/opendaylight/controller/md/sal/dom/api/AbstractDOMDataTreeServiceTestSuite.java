...
public abstract class AbstractDOMDataTreeServiceTestSuite {
    ...
    public final void testBasicProducer() throws DOMDataTreeProducerException, TransactionCommitFailedException {
        ...
        try (final DOMDataTreeProducer prod = service().createProducer(Collections.singleton(UNORDERED_CONTAINER_TREE), true)) {
            assertNotNull(prod);

            final DOMDataWriteTransaction tx = prod.createTransaction();
            assertNotNull(tx);
