...
final class ResolveDataChangeEventsTask implements Callable<Iterable<ChangeListenerNotifyTask>> {
    ...
    private boolean resolveReplacedEvent(final ResolveDataChangeState state,
        ...
        DOMImmutableDataChangeEvent event = DOMImmutableDataChangeEvent.builder(DataChangeScope.BASE).setBefore(beforeData).setAfter(afterData)
                .addUpdated(state.getPath(), beforeData, afterData).build();
        state.addEvent(event);
        state.collectEvents(collectedEvents);
        return true;
    }
    ...
    private boolean resolveNodeContainerReplaced(final ResolveDataChangeState state,
        ...
        if (childChanged) {
            DOMImmutableDataChangeEvent event = DOMImmutableDataChangeEvent.builder(DataChangeScope.BASE)
                    .setBefore(beforeCont).setAfter(afterCont)
    ...
    private boolean resolveSubtreeChangeEvent(final ResolveDataChangeState state, final DataTreeCandidateNode modification) {
        ...
        if (scope != null) {
            DOMImmutableDataChangeEvent one = DOMImmutableDataChangeEvent.builder(scope).
                    ...
                    addUpdated(state.getPath(), modification.getDataBefore().get(), modification.getDataAfter().get()).build();
            state.addEvent(one);
        }
