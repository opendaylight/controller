...
/**
 * Implements the RPC defined by the root discovery profile yang. Responsible for starting / running
 * the discovery.
 *
 * <br><Br>DEMOSTRATES: Implementing a singleton RPC service from a defined yang model. This
 * example shows how you might implement a customized or specialized service based on a
 * singleton yang model.
 * @author Devin Avery
 * @author Greg Hall
 *
 */
public class IcmpDiscoveryServiceImpl implements IcmpDiscoveryService {
    ...
    public void setManagers(DeviceManager devMgr, IcmpProfileManager profileMgr, PingService pingService) {
        this.devMgr = devMgr;
    ...
    public Future<RpcResult<DiscoverOutput>> discover(DiscoverInput input) {
        ...
        if (profGrp != null) {
            ...
            for (IpAddress ipAddr : ipList) {
                String ipAddrStr = PingUtils.getIPAddressAsString(ipAddr);
