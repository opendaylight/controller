...
public class IcmpProfileManagerImpl implements IcmpProfileManager {
     ...
     */
    @Override
    public void finishDiscoveryProfile(ProfileId profId) {
        DataModificationTransaction txn = dataBrokerService.beginTransaction();

        ProfileBuilder profBldr = new ProfileBuilder().setId(profId)
    ...
    private Profiles overlayInputOnExisting(DiscoverInput profileInput, ProfileGrp existingProfGrp) {
        ...
        if ( profileInput.getIntervalSeconds() == null ) {
            if ( existingProfGrp != null ) {
                // copy the iplist from current profile state
                profBldr.setIntervalSeconds( existingProfGrp.getIntervalSeconds() ) ;
            } else {
                System.out.println("The discover rpc didn't specify an Interval, defaulting to 300");
                profBldr.setIntervalSeconds(300L) ;
            }
        }else {
            profBldr.setIntervalSeconds( profileInput.getIntervalSeconds() ) ;
        }
