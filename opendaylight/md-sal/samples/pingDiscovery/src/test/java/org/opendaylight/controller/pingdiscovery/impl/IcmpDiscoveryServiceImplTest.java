...
public class IcmpDiscoveryServiceImplTest {
    ...
    public void testInit() {
        ...
        idsi.setManagers( mockDevMgr,  mockProfMgr, mockPingService) ;
        setupDiscoveryObjects() ;
    }
    ...
    public void testDiscover_noPreExistingProfile_success() {
        ...
        when(mockPingService.ping(new String("1.1.1.1"), 1, 1)).thenReturn(105D);
        Future<RpcResult<DiscoverOutput>> result = idsi.discover(discoverInputFull) ;
        assertNotNull( result) ;
    }

    /**
     * Test the happy path through startDiscoveryProfile where a profile does
     * exists, and only the profile's id is passed through the rpc.
     * 
     */
    @Test
    public void testDiscover_WithPreExistingProfile_success() {

        when(mockProfMgr.startDiscoveryProfile(discoverInputIdOnly)).thenReturn(profileGrp_ipList_1_1_1_1) ;
        when(mockDevMgr.createDevice(any(String.class))).thenReturn(true) ;
        when(mockPingService.ping(new String("1.1.1.1"), 1, 1)).thenReturn(105D);
