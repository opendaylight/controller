...
public final class CarProvider {
    ...
    private ListenableFuture<RpcResult<StressTestOutput>> stressTest(final StressTestInput input) {
        ...
        if (input.getRate() == null || input.getRate().toJava() == 0) {
            BuyCarImpl.LOG.info("Exiting stress test as no rate is given.");
        ...
        BuyCarImpl.LOG.info("Stress test starting : rate: {} count: {}", inputRate, inputCount);
        ...
        try {
            ...
            BuyCarImpl.LOG.error("Put Cars failed",e);
        ...
        testThread = new Thread(() -> {
            ...
            while (!stopThread) {
                ...
                if (count.get() % 1000 == 0) {
                    BuyCarImpl.LOG.info("Cars created {}, time: {}", count.get(), sw.elapsed(TimeUnit.SECONDS));
                }
            ...
            BuyCarImpl.LOG.info("Stress test thread stopping after creating {} cars.", count.get());
        });
        testThread.start();
    ...
    private ListenableFuture<RpcResult<StopStressTestOutput>> stopStressTest(final StopStressTestInput input) {
        ...
        BuyCarImpl.LOG.info("Executed Stop Stress test; No. of cars created {}; No. of cars failed {}; ",
