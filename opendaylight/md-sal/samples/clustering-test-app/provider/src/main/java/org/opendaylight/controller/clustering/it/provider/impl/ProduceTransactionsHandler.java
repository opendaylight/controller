...
public class ProduceTransactionsHandler implements Runnable {
    ...
    private final ProduceTransactionsInput input;
    ...
    private DistributedShardFactory.DistributedShardRegistration shardRegistration;
    ...
    public void start(final SettableFuture<RpcResult<ProduceTransactionsOutput>> settableFuture) {
        ...
        final DOMDataTreeIdentifier identifier =
                new DOMDataTreeIdentifier(LogicalDatastoreType.CONFIGURATION, ID_INTS_YID);
        try {
            final CompletionStage<DistributedShardFactory.DistributedShardRegistration> completionStage =
                    distributedShardFactory.createDistributedShard(identifier, Collections.emptyList());

            shardRegistration = completionStage.toCompletableFuture().get();
        } catch (DOMDataTreeShardingConflictException | InterruptedException | ExecutionException e) {
            LOG.error("Unable to register shard for id-ints.", e);
        }
