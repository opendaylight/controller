...
public class MdsalLowLevelTestProvider implements OdlMdsalLowlevelControlService {
    ...
    public Future<RpcResult<UnsubscribeDtclOutput>> unsubscribeDtcl() {
        ...
        try {
            ...
            return failure(newError(ErrorType.RPC, "resource-denied-transport",
        ...
        if (!localCopyOpt.isPresent()) {
            return failure(newError(ErrorType.APPLICATION, "No notification received.",
        ...
        try {
            ...
            return failure(newError(ErrorType.APPLICATION, "Read failed.", "Final read from id-ints failed."));
        } finally {
            rTx.close();
        }

        if (!readResult.isPresent()) {
            return failure(newError(ErrorType.APPLICATION, "Final read empty.",
                    "No data read from id-ints list."));
        }

        return success(new UnsubscribeDtclOutputBuilder()
    ...
    public Future<RpcResult<UnsubscribeDdtlOutput>> unsubscribeDdtl() {
        ...
        try {
            ...
            return failure(newError(ErrorType.APPLICATION, "Unable to create ds client for read.",
        ...
        if (!optional.isPresent()) {
            ...
            return failure(newError(ErrorType.APPLICATION, "Read failed.", "Final read from id-ints is empty."));
        }

        return success(new UnsubscribeDdtlOutputBuilder().setCopyMatches(idIntsDdtl.checkEqual(optional.get()))
            .build());
    }

    private static <T> ListenableFuture<RpcResult<T>> failure(final RpcError error) {
        return Futures.immediateFuture(RpcResultBuilder.<T>failed().withRpcError(error).build());
    }
