...
//For some reason, the ToasterTest fails in jenkins builds due to failure to find the features-mdsal
// artifact via the maven URL. The test succeeds on local builds though. Therefore this test is configured
// not to run by default but still serves as an example.
