...
//  }
  ...
  public void setup() throws BundleException, InterruptedException {
    ...
    ServiceReference ref = ctx.getServiceReference(RemoteRpcServer.class);

    
    if (ref == null){
        int rpcPort = 5555;
        ServerImpl server = new ServerImpl(rpcPort);
      ServiceRegistration reg =
          ctx.registerService(RemoteRpcServer.class,server ,
              new Hashtable<String,String>()) ;

      _logger.debug("Re registering zeromq router [{}]", reg);
    }

    ref = ctx.getServiceReference(RemoteRpcServer.class);
    Assert.assertNotNull(ref);
    ServerImpl service = (ServerImpl) ctx.getService(ref);
    ...
    if (routerRef == null) {
      ...
      ServiceRegistration routerReg =
          ctx.registerService(Client.class, client, new Hashtable<String,String>()) ;
    }
