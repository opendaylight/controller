...
public class BucketStore extends UntypedActor {
    ...
    void receiveUpdateBucket(Bucket updatedBucket){
        ...
        log.info("Updated bucket [{}]-[{}]",updatedBucket.getVersion(), updatedBucket.getData());
     ...
     *
     * @param remoteBuckets buckets sent by remote {@link org.opendaylight.controller.remote.rpc.registry.gossip.Gossiper}
     */
    void receiveUpdateRemoteBuckets(Map<Address, Bucket> remoteBuckets){
        ...
        remoteBuckets.remove(selfAddress);
        ...
        for (Map.Entry<Address, Bucket> entry : remoteBuckets.entrySet()){
            ...
            Bucket remoteBucket = entry.getValue();

            if (remoteBucket == null)
                continue;

            Long remoteVersion = remoteBucket.getVersion();
            if (remoteVersion == null) remoteVersion = -1L;

            //update only if remote version is newer
            if ( remoteVersion > localVersion ) {
                remoteBuckets.put(entry.getKey(), remoteBucket);
