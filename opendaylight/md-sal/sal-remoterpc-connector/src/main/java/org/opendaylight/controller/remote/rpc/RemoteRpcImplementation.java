...
final class RemoteRpcImplementation implements DOMRpcImplementation {
    ...
    RemoteRpcImplementation(final ActorRef remoteRouter, final RemoteRpcProviderConfig config) {
        this.config = Preconditions.checkNotNull(config);
        this.remoteRouter = Preconditions.checkNotNull(remoteRouter);
    }
    ...
    public CheckedFuture<DOMRpcResult, DOMRpcException> invokeRpc(final DOMRpcIdentifier rpc,
        ...
        ret.completeWith(Patterns.ask(remoteRouter, ExecuteRpc.from(rpc, input), config.getAskDuration()));
        return ret;
    }
