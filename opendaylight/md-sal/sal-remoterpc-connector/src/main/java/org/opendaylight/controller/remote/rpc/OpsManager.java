...
public class OpsManager extends AbstractUntypedActor {
    ...
    public void preStart() throws Exception {
        ...
        rpcInvoker = getContext().actorOf(OpsInvoker.props(rpcServices, actionService)
                .withMailbox(config.getMailBoxName()), config.getRpcBrokerName());
        LOG.debug("Listening for RPC invocation requests with {}", rpcInvoker);

        rpcRegistrar = getContext().actorOf(OpsRegistrar.props(config, rpcProvisionRegistry, actionProvisionRegistry)
