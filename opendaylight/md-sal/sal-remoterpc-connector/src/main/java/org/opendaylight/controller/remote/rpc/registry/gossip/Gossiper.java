...
public class Gossiper extends AbstractUntypedActorWithMetering {
    ...
    public void preStart() {
        ...
        bucketStore = BucketStore.forContext(getContext(), config.getAskDuration());
     ...
     */
    private void receiveMemberUp(final Member member) {
    ...
    void receiveGossipStatus(final GossipStatus status) {
        ...
        if (peers.containsKey(status.from())) {
            ...
            bucketStore.getBucketVersions((t, versions) -> {
                if (t == null) {
                    processRemoteStatus(sender, status, versions);
                }
            });
        }
    }
    ...
    private void processRemoteStatus(final ActorRef remote, final GossipStatus status,
        ...
        if (!localIsNewer.isEmpty()) {
            ...
            bucketStore.getBucketsByMembers(localIsNewer, (t, buckets) -> {
                if (t == null) {
     ...
     */
    @VisibleForTesting
    <T extends BucketData<T>> void receiveGossip(final GossipEnvelope envelope) {
