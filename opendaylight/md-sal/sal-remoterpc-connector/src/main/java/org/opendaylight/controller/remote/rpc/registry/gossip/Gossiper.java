...
public class Gossiper extends UntypedActor {
    ...
    public void preStart(){
        ...
        if (autoStartGossipTicks) {
            gossipTask = getContext().system().scheduler().schedule(
                    new FiniteDuration(10, TimeUnit.SECONDS),        //initial delay
                    new FiniteDuration(100, TimeUnit.MILLISECONDS),  //interval
                    getSelf(),                                       //target
    ...
    public void onReceive(Object message) throws Exception {
        ...
        if (message instanceof GossipTick)
            ...
            receiveMemberUp((ClusterEvent.MemberUp) message);

        } else if (message instanceof ClusterEvent.MemberRemoved ||
                   message instanceof ClusterEvent.UnreachableMember) {
            receiveMemberDown((ClusterEvent.MemberEvent) message);

        } else
            unhandled(message);
    }

    void receiveMemberDown(ClusterEvent.MemberEvent message) {
        log.info("Member is down [{}]", message.member());
        if (message.member().address().equals(selfAddress)){
            getContext().stop(getSelf());
        }
        clusterMembers.remove(message.member().address());
        log.info("Member list [{}]", clusterMembers);
    }

    void receiveMemberUp(ClusterEvent.MemberUp message) {
        Member node = message.member();
        log.info("Member is up [{}]", node);
        if (selfAddress.equals(node.address()))
            return; //ignore up notification for self

        if (!clusterMembers.contains(node.address()))
            clusterMembers.add(node.address());
