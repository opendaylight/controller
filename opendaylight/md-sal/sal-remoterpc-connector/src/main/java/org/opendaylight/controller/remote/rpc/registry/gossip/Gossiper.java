...
public class Gossiper extends UntypedActor {
    ...
    void receiveMemberDown(Member member) {
        ...
        log.debug("Down member [{}], Active member list [{}]", member.address(), clusterMembers);
    }
    ...
    void receiveMemberUp(Member member) {
        ...
        log.debug("Up member [{}], Active member list [{}]", member.address(), clusterMembers);
    }
    ...
    /**
     * Returns remote gossiper for the gives address. This method caches all the found gossipers thus far.
     * Before returing the gossiper, it checks to make sure the reference is still active.
     *
     * @param remoteActorSystem
     * @return
     */
    ...
    /**
     * Helper that tries to resolve remote gossiper's {@link akka.actor.ActorRef}
     *
     * @param remoteActorSystem
     * @return
     */
    ActorRef findGossiperFor(Address remoteActorSystem) {
        ...
        Future<ActorRef> future = remote.resolveOne(new FiniteDuration(500, TimeUnit.MILLISECONDS));

        try {
            remoteGossiper = Await.result(future, new FiniteDuration(500, TimeUnit.MILLISECONDS));
        } catch (Exception e) {
            //TODO: write supervision to restart gossiper
