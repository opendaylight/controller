...
final class RemoteOpsImplementation implements DOMRpcImplementation, DOMActionImplementation {
    ...
    public ListenableFuture<DOMRpcResult> invokeRpc(final DOMRpcIdentifier rpc,
        ...
        DOMDataTreeIdentifier identifier = new DOMDataTreeIdentifier(LogicalDatastoreType.OPERATIONAL,
                YangInstanceIdentifier.create(
                        new YangInstanceIdentifier.NodeIdentifier(rpc.getType().getLastComponent())));
        ret.completeWith(Patterns.ask(remoteInvoker, ExecuteOps.from(rpc.getType().getLastComponent(), identifier,
