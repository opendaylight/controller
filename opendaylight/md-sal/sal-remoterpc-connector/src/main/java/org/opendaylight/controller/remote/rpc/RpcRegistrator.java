 ...
 * @author Robert Varga
 */
final class RpcRegistrator extends AbstractUntypedActor {
    ...
    public RpcRegistrator(final RemoteRpcProviderConfig config, final DOMRpcProviderService rpcProviderService) {
    ...
    public static Props props(final RemoteRpcProviderConfig config, final DOMRpcProviderService rpcProviderService) {
        ...
        return Props.create(RpcRegistrator.class, config, rpcProviderService);
    }
    ...
    private void updateRemoteEndpoints(final Map<Address, Optional<RemoteRpcEndpoint>> endpoints) {
        ...
        prevRegs.forEach(DOMRpcImplementationRegistration::close);
    }
}
