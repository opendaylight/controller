...
public class RpcRegistrarTest {
    private static final String TEST_REV = "2014-08-28";
    private static final String TEST_NS = "urn:test";
    private static final QName TEST_RPC_OUTPUT = QName.create(TEST_NS, TEST_REV, "output");

    private ActorSystem system;
    private RpcRegistrar rpcRegistrar;
    private TestActorRef<?> testActorRef;
    ...
    public void testHandleReceive() throws Exception {
        ...
        final DOMRpcImplementationRegistration<DOMRpcImplementation> bar =
                router.registerRpcImplementation((rpc, input) -> {
                    ...
                    return Futures.<DOMRpcResult, DOMRpcException>immediateCheckedFuture(
                            new DefaultDOMRpcResult(result));
        ...
        final RemoteRpcEndpoint remoteRpcEndpoint =
                new RemoteRpcEndpoint(testActorRef, identifiers);
        endpoints.put(Address.apply("foo", "bar"), Optional.of(remoteRpcEndpoint));

        when(service.registerRpcImplementation(bar.getInstance(), remoteRpcEndpoint.getRpcs()))
                .thenReturn(bar);
        ...
        verify(service, times(1)).registerRpcImplementation(bar.getInstance(), remoteRpcEndpoint.getRpcs());
    }
