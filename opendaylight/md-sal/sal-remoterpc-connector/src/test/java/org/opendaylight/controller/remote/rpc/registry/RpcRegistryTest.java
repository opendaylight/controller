...
public class RpcRegistryTest {
    ...
    public void testRpcAddedOnMultiNodes() throws Exception {
        final JavaTestKit testKit3 = new JavaTestKit(node3);
        ...
        UpdateRemoteEndpoints req1 = registrator3.expectMsgClass(Duration.create(3, TimeUnit.SECONDS),
        ...
        UpdateRemoteEndpoints req2 = registrator3.expectMsgClass(Duration.create(3, TimeUnit.SECONDS),
            UpdateRemoteEndpoints.class);

        Address node1Address = node1.provider().getDefaultAddress();
        Address node2Address = node2.provider().getDefaultAddress();

        Map<Address, Bucket<RoutingTable>> buckets = retrieveBuckets(registry3, testKit3, node1Address,
                node2Address);

        verifyBucket(buckets.get(node1Address), addedRouteIds1);
        verifyBucket(buckets.get(node2Address), addedRouteIds2);

        Map<Address, Long> versions = retrieveVersions(registry3, testKit3);
