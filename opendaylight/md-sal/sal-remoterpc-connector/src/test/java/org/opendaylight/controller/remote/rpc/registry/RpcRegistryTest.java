    ...
    private Boolean resolveReference(ActorSelection... gossipers) throws InterruptedException {
        ...
        for (int i=0; i< 5; i++) {
            ...
            for (ActorSelection gossiper : gossipers) {
                Future<ActorRef> future = gossiper.resolveOne(new FiniteDuration(500, TimeUnit.MILLISECONDS));

                ActorRef ref = null;
                try {
                    ref = Await.result(future, new FiniteDuration(500, TimeUnit.MILLISECONDS));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                ...
                if (ref == null)
                    ...
                    System.out.println("FOUND GOSSIPER: " + ref);
            }
