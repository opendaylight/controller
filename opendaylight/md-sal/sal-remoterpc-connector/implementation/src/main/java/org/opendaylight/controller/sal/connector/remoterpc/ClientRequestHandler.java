 ...
 *
 */
final class ClientRequestHandler implements AutoCloseable{
  ...
  protected ClientRequestHandler(ZMQ.Context context) {
    ...
    connectedServers = new ConcurrentSkipListSet<String>();
    start();
  }
    ...
    workerPool = new ThreadPoolExecutor(0, Integer.MAX_VALUE,
      ...
      protected void afterExecute(Runnable r, Throwable t) {
        ...
        if ( t != null ){
          ...
          connectedServers.add(remoteServerAddress);
          this.execute(r);
        }
  ...
  public Message handle(Message request) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    if ( connectedServers.contains(remoteServerAddress) )
