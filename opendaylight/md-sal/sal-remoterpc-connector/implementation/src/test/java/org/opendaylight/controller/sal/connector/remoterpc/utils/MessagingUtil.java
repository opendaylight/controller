...
public class MessagingUtil {
  ...
  public static Runnable startReplyServer(final ZMQ.Context context, final String serverAddress) {
    return new Runnable() {
      ...
      public void run() {
        ...
        try {
          while (!Thread.currentThread().isInterrupted()) {
            socket.bind("tcp://" + serverAddress);
            System.out.println(Thread.currentThread().getName() + " Starting server...");

            byte[] bytes = socket.recv();
            System.out.println(Thread.currentThread().getName() + " Got request ");
            socket.send(bytes);

            System.out.println(Thread.currentThread().getName() + " Sent response ");
          }
        } catch (Exception x) {
          x.printStackTrace();
        } finally {
          socket.close();
          System.out.println("Shutting down reply server");
        }
      }
    };
  }
