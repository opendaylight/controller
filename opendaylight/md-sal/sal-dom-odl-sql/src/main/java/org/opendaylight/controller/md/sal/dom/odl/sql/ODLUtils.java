...
public class ODLUtils {
    ...
    public static List<Object> collectModuleRoots(BluePrintNode table,LogicalDatastoreType type,DOMDataBroker domDataBroker) {
        ...
        try {
            for(Object odlSchemaNode:table.getODLSchemaNodes()){
                YangInstanceIdentifier instanceIdentifier = YangInstanceIdentifier.builder().node(ODLUtils.getPath(odlSchemaNode).get(0)).build();
            ...
            ODLSQLAdapter.log("Something went wrong trying to collect the model roots");
    ...
    public static boolean createOpenDaylightCache(BluePrint bluePrint,Object module) {
        BluePrintNode node = new BluePrintNode(module, 0,null);
        bluePrint.addToBluePrintCache(node,null);
        collectODL(bluePrint, node, ((Module) module).getChildNodes(), 1);
        return true;
    }

    private static void collectODL(BluePrint bluePrint,
        BluePrintNode parent, Collection<DataSchemaNode> nodes, int level) {
        if (nodes == null) {
            return;
        }
        for (DataSchemaNode n : nodes) {
            if (n instanceof DataNodeContainer) {
                BluePrintNode bn = new BluePrintNode(n, level,parent);
    ...
    public static Collection<?> getChildrenCollection(Object o) {
        ...
        if(value instanceof Collection){
            ...
            ODLSQLAdapter.log("Unknown Child Value Type="+value.getClass().getName());
    ...
    public static DataObjectsPath toBindingAwareObject(LinkedList<Object> objects,Object bindingToNormalizedNodeCodec){
        DataObjectsPath dataObjectPath = new DataObjectsPath();
        ...
        while(!pathArguments.isEmpty()){
            ...
            try{
                ...
                dataObjectPath.addDataObject((DataObject)entry.getValue());
