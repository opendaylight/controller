...
public class JDBCServer extends Thread {
    ...
    public JDBCServer(ODLSQLAdapter adapter) {
        super("JDBC Server");
    ...
    public static void execute(JDBCResultSet rs, ODLSQLAdapter adapter)throws SQLException {
    ...
    public static void parseLogicalFields(String sql, JDBCResultSet rs)
        ...
        if(sql.trim().toLowerCase().equals("select * from")){
            for (Map.Entry<String, JDBCResultSet> entry : rs.getSubQueries().entrySet()) {
                for(BluePrintNode node:entry.getValue().getTables()){
        ...
        Map<String, BluePrintNode> logicalNameToNode = new HashMap<String, BluePrintNode>();
        ...
        for (BluePrintNode node : logicalNameToNode.values()) {
            rs.addTableToQuery(node);
        }
    ...
    public static int addNextLogicalField(String sql, int startIndex,
            Map<String, BluePrintNode> logicalNameToNode,
            Map<String, String> origNameToName, List<Column> columnOrder) {
        ...
        BluePrintNode node = logicalNameToNode.get(tblName);
        if (node == null) {
            node = new BluePrintNode(tblName, origTableName, 0);
    ...
    public static void checkAndBreakSubQueries(JDBCResultSet rs,ODLSQLAdapter adapter) throws SQLException {
    ...
    public static void parseTables(JDBCResultSet rs, BluePrint bp)
            throws SQLException {
        ...
        while (tokens.hasMoreTokens()) {
            ...
            BluePrintNode table = bp.getBluePrintNodeByTableName(tableName);
            if (table == null) {
    ...
    public static void addCriteria(Column col, Criteria c,
            JDBCResultSet rs) {
        Map<Column, List<Criteria>> tblCriteria = rs.getCriteria().get(
                col.getTableName());
        if (tblCriteria == null) {
            tblCriteria = new ConcurrentHashMap<Column, List<Criteria>>();
    ...
    public static void parseFields(JDBCResultSet rs, BluePrint bp)
            throws SQLException {
        String lowSQL = rs.getSQL().toLowerCase();
        ...
        while (tokens.hasMoreTokens()) {
            ...
            if (token.equals("*")) {
                for (BluePrintNode table : rs.getTables()) {
                    rs.getFields().addAll(table.getColumns());
                }
                return;
            }
            if (token.indexOf(".") != -1) {
                BluePrintNode tbl = bp.getBluePrintNodeByTableName(token
                ...
                if (p.equals("*")) {
                    ...
                    Column col = tbl.findColumnByName(p);
    ...
    public static void parseCriteria(JDBCResultSet rs, BluePrint bp) {
        String lowSQL = rs.getSQL().toLowerCase();
        int where = lowSQL.indexOf("where");
        ...
        for (BluePrintNode tbl : rs.getTables()) {
