...
abstract class AbstractEnvelopeProxy<T extends Message<?, T>> implements Externalizable {
    ...
    public final void writeExternal(final ObjectOutput out) throws IOException {
        ...
        version.writeTo(out);
        out.writeObject(message.toVersion(version));
    }
    ...
    public final void readExternal(final ObjectInput in) throws IOException, ClassNotFoundException {
        ...
        version = ABIVersion.readFrom(in);
        message = (T) in.readObject();
    }

    abstract Envelope<T> createEnvelope(T message, ABIVersion version, long sequence, long retry);

    final Object readResolve() {
        return createEnvelope(message, version, sequence, retry);
    }
}