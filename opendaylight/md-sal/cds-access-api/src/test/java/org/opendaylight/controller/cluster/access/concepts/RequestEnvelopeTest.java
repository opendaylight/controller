...
public class RequestEnvelopeTest extends EnvelopeTest<RequestEnvelope, RequestEnvelopeProxy> {
    ...
    protected RequestEnvelope createEnvelope() {
        ...
        TransactionPurgeRequest message = new TransactionPurgeRequest(OBJECT, 2L, replyTo);
        return new RequestEnvelope(message, 1L, 2L);
    }

    @Override
    protected RequestEnvelopeProxy createProxy(RequestEnvelope envelope) {
        return envelope.createProxy();
    }

    @Override
    protected void doAdditionalAssertions(RequestEnvelope envelope, RequestEnvelopeProxy deserializedProxy,
                                          RequestEnvelope resolvedObject) {
        final Request<?, ?> actual = resolvedObject.getMessage();
        Assert.assertTrue(actual instanceof TransactionPurgeRequest);
        TransactionPurgeRequest purgeRequest = (TransactionPurgeRequest) actual;
