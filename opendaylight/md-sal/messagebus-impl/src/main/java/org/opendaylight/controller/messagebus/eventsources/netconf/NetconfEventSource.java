...
public class NetconfEventSource implements EventSource, DOMNotificationListener, DataChangeListener {
    ...
    public Future<RpcResult<JoinTopicOutput>> joinTopic(final JoinTopicInput input) {
        if(listenerRegistrationMap.containsKey(input.getTopicId())){
            throw new IllegalStateException("Can not join topic twice. Topic " + input.getTopicId().getValue() + " has been joined to node " + this.nodeId);
        }
        ...
        listenerRegistrationMap.put(input.getTopicId(), registerNotificationListener(matchingNotifications));
    ...
    private synchronized ListenerRegistration<NetconfEventSource> registerNotificationListener(final List<SchemaPath> notificationsToSubscribe) {
