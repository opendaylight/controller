...
/**
 * Source of event has to implement this interface. In this context, source of event is an object
 * that is able to produce notifications or another message about event. Event sources are managed
 * by their event source manager. If event source manager want to announce object as an event source then
 * it have to create an instance that implements EventSource interface and register
 * it via {@link org.opendaylight.controller.messagebus.api.EventSourceRegistry#registerEventSource(Node, EventSource)()}
 * Each event source should be represented as a node. Event Source Manager is may or may not add this node into inventory
 * or topology.
