...
public class StreamNotificationTopicRegistration extends NotificationTopicRegistration {
    ...
    public StreamNotificationTopicRegistration(final String streamName, final String notificationPrefix, final String nodeId, final DOMMountPoint netconfMount, NetconfEventSource notificationListener) {
        super(NotificationSourceType.NetconfDeviceStream, streamName, notificationPrefix);
    ...
    void activateNotificationSource() {
        if(isActive() == false){
            ...
            //TODO: get information if stream support replay notification and set this.replySupport;
            setActive(true);
        } else {
    ...
    void reActivateNotificationSource(){
        if(isActive()){
            LOG.info("Stream {} is reactivated active on node {}.", this.getStreamName(), this.nodeId);
            DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> inputBuilder =
                    ...
                    .withChild(ImmutableNodes.leafNode(STREAM_QNAME, this.getStreamName()));
            if(this.replySupport && this.getLastEventTime() != null){
                //TODO: implement time shift to get complete
