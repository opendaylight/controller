...
public abstract class AbstractClientActor extends AbstractActorWithStash {
    ...
    public void preStart() throws Exception {
        if (currentBehavior != null && currentBehavior.context() instanceof InitialClientActorContext initialContext) {
            // ensure snapshot store available
            if (initialContext.isSnapshotStoreAvailable()) {
                // trigger recovery flow by explicit call for snapshot data
                initialContext.startRecoveryFlow();
            } else {
                // no way to recover state from snapshot, jump to next stage
                switchBehavior(initialContext.finishRecovery());
            }
        }
        super.preStart();
    }
