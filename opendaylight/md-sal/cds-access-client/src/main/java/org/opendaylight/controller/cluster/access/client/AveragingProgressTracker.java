...
@Beta
@NotThreadSafe
public class AveragingProgressTracker extends ProgressTracker {
    ...
    public long estimateIsolatedDelay(final long now) {
        ...
        if (open >= tasksOpenLimit) {
            return (long) defaultTicksPerTask;
        }
        final double fullness = ((double) open) / tasksOpenLimit;
        final double delay = ticksWorkedPerClosedTask(now) * (fullness - 1.0 / 2) / (1.0 - fullness);
        return (long) Math.min(delay, defaultTicksPerTask);
    }
     ...
     */
    public AveragingProgressTracker(final long now, final long limit) {
        this(now, limit, 1_000_000_000.0);
    }
     ...
     */
    public AveragingProgressTracker(final long now, final long limit, final double ticksPerTask) {
        super(now, ticksPerTask);
     ...
     */
    public AveragingProgressTracker(final AveragingProgressTracker tracker) {
        super((ProgressTracker) tracker);
