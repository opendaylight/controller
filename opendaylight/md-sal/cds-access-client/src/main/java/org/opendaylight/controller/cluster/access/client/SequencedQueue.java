...
final class SequencedQueue {
    ...
    ClientActorBehavior complete(final ClientActorBehavior current, final ResponseEnvelope<?> envelope) {
        ...
        lastProgress = ticker.read();
        return maybeEntry.get().complete(envelope.getMessage());
    }

    private int retransmitEntries(final Queue<SequencedQueueEntry> queue, final int count) {
        int toSend = count;
    ...
    Optional<FiniteDuration> setBackendInfo(final CompletionStage<? extends BackendInfo> proof, final BackendInfo backend) {
        ...
        // We are un-blocking transmission, but we need to juggle the queues first
        final int txLimit = backend.getMaxMessages();

        // Move all previously-sent messages to lastInflight
        ...
        LOG.debug("Resending up to {} requests to backend {}", txLimit, backend);

        // Process lastInflight first, possibly clearing it
        int toSend = retransmitEntries(lastInflight, txLimit);
        if (lastInflight.isEmpty()) {
        ...
        toSend = retransmitEntries(pending, toSend);
