    ...
    final long enqueueEntry(final ConnectionEntry entry, final long now) {
        ...
        try {
            ...
            if (queue.isEmpty()) {
                ...
                // complicated.
                final long entryTimeoutTicks = entry.getEnqueuedTicks() + REQUEST_TIMEOUT_NANOS - now;
                final long timeoutTicks = Math.min(entryTimeoutTicks, BACKEND_ALIVE_TIMEOUT_NANOS);
                scheduleTimer(FiniteDuration.apply(timeoutTicks > 0 ? timeoutTicks : 0, TimeUnit.NANOSECONDS));
            }
