...
public abstract class AbstractClientConnection<T extends BackendInfo> {
     ...
     *                             time.
     */
    private ClientActorBehavior runTimer(final ClientActorBehavior current) {
        ...
        if (!isEmpty()) {
            ...
            if (ticksSinceProgress >= NO_PROGRESS_TIMEOUT_NANOS) {
                ...
                // poison(ex);
                // FIXME: call behavior to remove the connection
    ...
    abstract ClientActorBehavior reconnectConnection(ClientActorBehavior current);
    ...
    Optional<FiniteDuration> checkTimeout(final long now) {
        final ConnectionEntry head = pending.peek();
        if (head == null) {
            return Optional.empty();
        }

        final long delay = now - head.getEnqueuedTicks() + REQUEST_TIMEOUT_NANOS;
        if (delay <= 0) {
    ...
    public abstract void receiveResponse(final ResponseEnvelope<?> envelope);
