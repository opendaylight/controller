...
abstract class ProgressTracker {
     ...
     */
    private final long ticksStalling(final long now) {
     ...
     */
    private final long ticksWorked(final long now) {
     ...
     */
    private final long estimateDelay(final long now) {
     ...
     */
    private final long estimateAllowed(final long now) {
     ...
     * @return number of ticks (nanos) the caller thread should wait before opening another task
     */
    private final long reserveDelay(final long now) {
     ...
     */
    private final void unsafeCloseTask(final long now, final long enqueuedTicks, final long transmitTicks,
     ...
     * @param now tick number corresponding to caller's present
     */
    private final void openTaskWithoutThrottle(final long now) {
        if (isIdle()) {
            transitFromIdle(now);
        }
        tasksEncountered++;
