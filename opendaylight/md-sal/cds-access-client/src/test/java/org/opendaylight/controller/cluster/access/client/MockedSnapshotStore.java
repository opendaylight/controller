...
class MockedSnapshotStore extends SnapshotStore {
    ...
    private <T> Future<T> transform(final Future<Object> future) {
        final Mapper<Object, T> mapper = new Mapper<Object, T>() {
            @Override
            public T apply(final Object parameter) {
                if (parameter == Void.TYPE) {
                    //void
                    return null;
                }
                return (T) parameter;
        ...
        return future.map(mapper, context().dispatcher());
    }
