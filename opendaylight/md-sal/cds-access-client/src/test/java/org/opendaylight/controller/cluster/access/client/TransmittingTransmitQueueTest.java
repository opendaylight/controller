...
public class TransmittingTransmitQueueTest extends AbstractTransmitQueueTest<TransmitQueue.Transmitting> {
    ...
    public void testComplete() throws Exception {
        ...
        //enqueue 2 entries
        queue.enqueue(new ConnectionEntry(request1, callback1, now1), true, now1);
        queue.enqueue(new ConnectionEntry(request2, callback2, now2), true, now2);
    ...
    public void testEnqueueCanTransmit() throws Exception {
        ...
        queue.enqueue(new ConnectionEntry(request, callback, now), true, now);
    ...
    public void testSetForwarder() throws Exception {
        ...
        queue.enqueue(entry, true, secondEnqueueNow);
    ...
    public void testCompleteOrdering() {
        ...
        queue.enqueue(new ConnectionEntry(req0, callback, 0), true, 0);
        queue.enqueue(new ConnectionEntry(req1, callback, 0), true, 0);
        queue.enqueue(new ConnectionEntry(req2, callback, 0), true, 0);
        queue.enqueue(new ConnectionEntry(req3, callback, 0), true, 0);
        ...
        queue.enqueue(new ConnectionEntry(req4, callback, 0), true, 0);
        assertEqualRequests(queue.getInflight(), req2, req3, req4);
        assertEqualRequests(queue.getPending());

        // Enqueue req5, which should move to pending
        queue.enqueue(new ConnectionEntry(req5, callback, 0), true, 0);
        assertEqualRequests(queue.getInflight(), req2, req3, req4);
        ...
        queue.enqueue(new ConnectionEntry(req6, callback, 0), true, 0);
