...
public class TransmittingTransmitQueueTest extends AbstractTransmitQueueTest<TransmitQueue.Transmitting> {
    ...
    public void testComplete() throws Exception {
        ...
        final Consumer<Response<?, ?>> callback1 = mock(Consumer.class);
        final Consumer<Response<?, ?>> callback2 = mock(Consumer.class);
        ...
        final Optional<TransmittedConnectionEntry> completed1 =
                queue.complete(new SuccessEnvelope(success1, 0L, sequence1, 1L), now2);
        //check first entry
    ...
    public void testSetForwarder() throws Exception {
        ...
        queue.setForwarder(forwarder, 0L);
        verify(forwarder).forwardEntry(isA(TransmittedConnectionEntry.class), anyLong());
        queue.enqueue(entry, now);
        verify(forwarder).forwardEntry(eq(entry), anyLong());
    }
