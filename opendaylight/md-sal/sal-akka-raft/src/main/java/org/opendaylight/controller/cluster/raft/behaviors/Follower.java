...
public class Follower extends AbstractRaftActorBehavior {
    ...
    private boolean processNewEntries(final AppendEntries appendEntries, final ActorRef sender) {
        ...
        final var numLogEntries = entries.size();
        if (numLogEntries == 0) {
            return true;
        }
        ...
        final var lastEntryToAppend = entries.get(entries.size() - 1);
        final var appendAndPersistCallback = (Consumer<ReplicatedLogEntry>) logEntry -> {
