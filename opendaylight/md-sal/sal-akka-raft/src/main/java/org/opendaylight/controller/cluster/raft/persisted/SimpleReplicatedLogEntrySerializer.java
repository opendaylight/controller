...
public class SimpleReplicatedLogEntrySerializer extends JSerializer {
    ...
    public byte[] toBinary(Object obj) {
        ...
        LOG.info("Estimated serialized size {}, data size {} for payload: {}", estimatedSerializedSize,
                replicatedLogEntry.getData().size(), replicatedLogEntry.getData());

        final ByteArrayOutputStream bos = new ByteArrayOutputStream(estimatedSerializedSize);
        SerializationUtils.serialize((SimpleReplicatedLogEntry)obj, bos);
        final byte[] bytes = bos.toByteArray();

        LOG.info("Actual serialized size: {}", bytes.length);
