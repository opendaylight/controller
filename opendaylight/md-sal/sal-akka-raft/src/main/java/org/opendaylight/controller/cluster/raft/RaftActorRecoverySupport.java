...
class RaftActorRecoverySupport {
    ...
    private void onRecoveredSnapshot(SnapshotOffer offer) {
        ...
        if(!context.getPersistenceProvider().isRecoveryApplicable()) {
            ...
            // entries - we don't want to preserve these, only the server config and election term info. We'll
            // run the unapplied entries thur onRecoveredJournalLogEntry in case there's a
            // ServerConfigurationPayload or migrated payload entry.

            for(ReplicatedLogEntry entry: snapshot.getUnAppliedEntries()) {
                onRecoveredJournalLogEntry(entry);
            }
