 ...
 * <li>Call {@link RaftActor#leadershipTransferStarting} passing this RaftActorLeadershipTransferCohort
 *     instance. This allows derived classes to perform work prior to transferring leadership.</li>
 * <li>The {@link #proceedWithTransfer} method is called which in turn calls
...
public class RaftActorLeadershipTransferCohort {
    ...
    void init() {
        ...
        if(currentBehavior.state() != RaftState.Leader || !context.hasFollowers()) {
            LOG.debug("{}: Not the leader or no followers - skipping transfer", raftActor.persistenceId());
            finish(true);
            return;
        }

        raftActor.setLeadershipTransferInProgress(this);
        ...
        raftActor.leadershipTransferStarting(this);
    }
    ...
    private void finish(boolean success) {
        raftActor.setLeadershipTransferInProgress(null);
