...
public class SnapshotManager implements SnapshotState {
    ...
    public void captureToInstall(ReplicatedLogEntry lastLogEntry, long replicatedToAllIndex) {
        if(currentState instanceof Idle) {
            ...
            LOG.error("Illegal call to captureToInstall made in state = {}", this.currentState);
        }
    }
    ...
    public void capture(ReplicatedLogEntry lastLogEntry, long replicatedToAllIndex) {
        if(currentState instanceof Idle) {
            ...
            LOG.error("Illegal call to capture made in state = {}", this.currentState);
        }
    }

    @Override
    public void create(Procedure<Void> procedure) {
        if(currentState instanceof Capturing) {
            currentState.create(procedure);
        } else {
            LOG.error("Illegal call to create made in state = {}", this.currentState);
        }
    }
    ...
    public void persist(DataPersistenceProvider persistenceProvider, byte[] snapshotBytes, RaftActorBehavior currentBehavior) {
        if(currentState instanceof Creating) {
            ...
            LOG.error("Illegal call to persist made in state = {}", this.currentState);
        }
    }
    ...
    public void commit(DataPersistenceProvider persistenceProvider, long sequenceNumber) {
        if(currentState instanceof Persisting) {
            ...
            LOG.error("Illegal call to commit made in state = {}", this.currentState);
        }
    }
    ...
    public void rollback() {
        if(currentState instanceof Persisting) {
            ...
            LOG.error("Illegal call to rollback made in state = {}", this.currentState);
        }
    }

    @Override
    public long trimLog(long desiredTrimIndex) {
        if(currentState instanceof Idle || currentState instanceof Persisting) {
            return currentState.trimLog(desiredTrimIndex);
        } else {
            LOG.error("Illegal call to trimLog made in state = {}", this.currentState);
        }
        return -1;
    }
