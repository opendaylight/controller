...
public class SnapshotManager implements SnapshotState {
    ...
    public void persist(final Snapshot.State snapshotState, final Optional<OutputStream> installSnapshotStream,
        ...
        if (!(task instanceof Capture(final var lastSeq, final var captureSnapshot))) {
        ...
        if (dataSizeThresholdExceeded || logSizeExceededSnapshotBatchCount || captureSnapshot.isMandatoryTrim()) {
            ...
            replLog.snapshotPreCommit(captureSnapshot.getLastAppliedIndex(), captureSnapshot.getLastAppliedTerm());
            ...
            replLog.snapshotPreCommit(captureSnapshot.getReplicatedToAllIndex(),
    ...
    private long commit(final Persist persist) {
        return switch (persist) {
            case PersistApply(var lastSeq, ApplySnapshot(var snapshot, var callback)) -> {
                try {
    ...
    public @Nullable CaptureSnapshot getCaptureSnapshot() {
        return task instanceof Capture(var lastSeq, var captureSnapshot) ? captureSnapshot : null;
    }
