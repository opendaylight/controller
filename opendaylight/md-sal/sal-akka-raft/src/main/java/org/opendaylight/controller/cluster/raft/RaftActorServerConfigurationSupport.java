...
class RaftActorServerConfigurationSupport {
    ...
    private FiniteDuration FOLLOWER_CATCH_UP_TIMEOUT;
    // timeout handle
    ...
    private void processAddServer(RaftActor raftActor){
        ...
        if(initialState == FollowerState.VOTING_NOT_INITIALIZED){
            ...
            persistNewServerConfiguaration(raftActor, addSrv.getNewServerId());
        }
    }
    ...
    private void onUnInitializedFollowerSnapshotReply(UnInitializedFollowerSnapshotReply reply,
        ...
        LOG.debug("onUnInitializedFollowerSnapshotReply : {}",  reply);
        ...
        leader.calculateMinReplicaCountAndMinIsolatedLeaderPeerCount();
        persistNewServerConfiguaration(raftActor, followerId);
    }

    private void persistNewServerConfiguaration(RaftActor raftActor, String followerId){
   ...
   private void onFollowerCatchupTimeout(RaftActor raftActor, ActorRef sender, String serverId){
        ...
        LOG.info("onFollowerCatchupTimeout - Timeout occured for server - {} while installing snapshot", serverId);
   ...
   private void respondToClient(RaftActor raftActor, ServerChangeStatus result){
        ...
        if(result == ServerChangeStatus.OK){
            LOG.info("respondToClient returned SUCCESS for server : {} ",  fInfo.getAddServer().getNewServerId());
        } else{
            LOG.info("respondToClient returned TIMEOUT for server : {} ",  fInfo.getAddServer().getNewServerId());
        }
