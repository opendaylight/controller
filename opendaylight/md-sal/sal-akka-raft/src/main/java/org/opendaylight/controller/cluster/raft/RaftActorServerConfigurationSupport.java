...
class RaftActorServerConfigurationSupport {
    ...
    private void onRemoveServer(RemoveServer removeServer, RaftActor raftActor, ActorRef sender) {
        if(removeServer.getServerId().equals(raftActor.getLeaderId())){
        ...
        } else if(raftActor.getLeader() == null){
            LOG.debug("{}: No leader - returning NO_LEADER reply", raftContext.getId());
            sender.tell(new RemoveServerReply(ServerChangeStatus.NO_LEADER, raftActor.getLeaderId()), raftActor.getSelf());
            ...
            currentOperationState.onNewOperation(raftActor, new RemoveServerContext(removeServer, raftContext.getPeerAddress(removeServer.getServerId()), sender));
        } else {
            LOG.debug("{}: Not leader - forwarding to leader {}", raftContext.getId(), raftActor.getLeaderId());
            raftActor.getLeader().forward(removeServer, raftActor.getContext());
        }
    }
