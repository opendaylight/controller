...
class SnapshotSupportImpl implements SnapshotSupport {
    ...
    public void persist(ByteString stateInBytes, RaftActorBehavior behavior, boolean isLeader) {
        ...
        if (context.getReplicatedLog().dataSize() > dataThreshold) {
            ...
            if(localCaptureSnapshot.getReplicatedToAllIndex() >= 0) {
                behavior.setReplicatedToAllIndex(localCaptureSnapshot.getReplicatedToAllIndex());
            }

        } else if(localCaptureSnapshot.getReplicatedToAllIndex() != -1){
            // clear the log based on replicatedToAllIndex
            context.getReplicatedLog().snapshotPreCommit(localCaptureSnapshot.getReplicatedToAllIndex(),
    ...
    public long getLastSequenceNumber() {
        return currentCaptureSnapshot != null ? currentCaptureSnapshot.getLastSequenceNumber() : -1;
    }
}
