...
public abstract class RaftActor extends UntypedPersistentActor {
    ...
    public void onReceiveRecover(Object message) {
        if (message instanceof SnapshotOffer) {
            onSnapshotOfferDuringRecovery((SnapshotOffer)message);
        } else if (message instanceof ReplicatedLogEntry) {
            onReplicatedLogEntryRecovery((ReplicatedLogEntry)message);
    ...
    private void onSnapshotOfferDuringRecovery(SnapshotOffer offer) {
        ...
        timer.stop();
        LOG.info("Snapshot applied for {} in {}: snapshotIndex={}, snapshotTerm={}, journal-size=" +
                replicatedLog.size(), persistenceId(), timer.toString(),
                replicatedLog.snapshotIndex, replicatedLog.snapshotTerm);
    }

    private void onReplicatedLogEntryRecovery(ReplicatedLogEntry logEntry) {
        initRecoveryTimer();

        int batchSize = context.getConfigParams().getJournalRecoveryLogBatchSize();
        if(currentRecoveryBatchCount == 0) {
            replicatedLog.increaseJournalLogCapacity(batchSize);
            startRecoveryStateBatch();
        }

        replicatedLog.append(logEntry);
        appendRecoveryState(logEntry.getData());
        if(++currentRecoveryBatchCount >= batchSize) {
            applyCurrentRecoveryBatch();
        }
    }

    private void applyCurrentRecoveryBatch() {

        ReplicatedLogEntry lastEntry = replicatedLog.last();
        context.setLastApplied(lastEntry.getIndex());
        context.setCommitIndex(lastEntry.getIndex());

        applyCurrentRecoveryStateBatch();
     ...
     */
    protected abstract void startRecoveryStateBatch();
     ...
     */
    protected abstract void appendRecoveryState(Payload data);

    /**
     * This method is called during recovery at the end of a batch to apply the current batched
     * state. This method is called after {@link #appendRecoveryState}.
     */
    protected abstract void applyCurrentRecoveryStateBatch();
