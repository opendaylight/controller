...
public abstract class RaftActor extends AbstractUntypedPersistentActor {
    ...
    protected final void persistData(final ActorRef clientActor, final Identifier identifier, final Payload data,
        ...
        LOG.debug("{}: Append and persist data {}", persistenceId(), replicatedLogEntry);

        final ReplicatedLog replicatedLog = replicatedLog();
        if (!replicatedLog.append(replicatedLogEntry)) {
            return;
        }
        ...
        replicatedLog.persist(replicatedLogEntry,
            entry -> entryPersisted(raftContext, clientActor, identifier, entry), true);
    }

    void entryPersisted(final RaftActorContext raftContext, final ActorRef clientActor,
            final Identifier identifier, final ReplicatedLogEntry entry) {
        // Clear the persistence pending flag in the log entry.
        entry.setPersistencePending(false);

        if (hasFollowers()) {
            context.getReplicatedLog().captureSnapshotIfReady(entry);
        ...
        raftContext.setCommitIndex(entry.getIndex());
        raftContext.setLastApplied(entry.getIndex());

        // Apply the state immediately.
        handleApplyState(new ApplyState(clientActor, identifier, entry));

        // Send a ApplyJournalEntries message so that we write the fact that we applied
        // the state to durable storage
        self().tell(new ApplyJournalEntries(entry.getIndex()), self());
    }
