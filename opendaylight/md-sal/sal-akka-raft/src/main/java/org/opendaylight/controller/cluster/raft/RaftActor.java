...
public abstract class RaftActor extends AbstractUntypedPersistentActor {
    ...
    @NonNullByDefault
    static final class Initializing extends RaftState {
    ...
    @NonNullByDefault
    abstract static sealed class Initialized extends RaftState permits Started, Stopping {
    ...
    static final class Stopped extends RaftState {
    ...
    private final @NonNull RaftState stage;
    ...
    private void onLeaderTransitioning(final LeaderTransitioning leaderTransitioning) {
        ...
        if (roleChangeNotifier != null && getRaftState() == RaftRoles.Follower
    ...
    @NonNullByDefault
    private void notifyRoleChange(final ActorRef target, final RaftRoles newRole,
            final @Nullable RaftActorBehavior oldBehavior) {
        final RaftRoles oldRole;
        if (oldBehavior != null) {
    ...
    protected final boolean isLeaderActive() {
        return getRaftState() != RaftRoles.IsolatedLeader && getRaftState() != RaftRoles.PreLeader
    ...
    @VisibleForTesting
    protected final RaftRoles getRaftState() {
        ...
        if (isLeader()) {
            initiateLeadershipTransfer(new RaftActorLeadershipTransferCohort.OnComplete() {
                ...
                private void ensureFollowerState() {
                    ...
                    if (getRaftState() != RaftRoles.Follower) {
                        initializeBehavior();
                    }
                }
