    ...
    protected final void persistData(final ActorRef clientActor, final Identifier identifier, final Payload data) {
        ...
        boolean wasAppended = replicatedLog().appendAndPersist(replicatedLogEntry, persistedLogEntry -> {
            ...
            if (!hasFollowers()) {
                ...
                // Local persistence is complete so send the CheckUpdateCommitIndex message to the behavior (which
                // normally should still be the leader) to possibly update the commit index if replication of the entry
                // succeeded.
