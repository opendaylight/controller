...
public abstract class AbstractLeader extends AbstractRaftActorBehavior {
    ...
    private void purgeInMemoryLog() {
        ...
        for (FollowerLogInformation info : followerToLog.values()) {
            if (info.getMatchIndex() < minReplicatedToAllIndex) {
                minReplicatedToAllIndex = info.getMatchIndex();
            }
        }
        ...
        if (minReplicatedToAllIndex > -1 &&  minReplicatedToAllIndex > replicatedToAllIndex
                && context.getReplicatedLog().isPresent(minReplicatedToAllIndex))  {
            replicatedToAllIndex = minReplicatedToAllIndex;
            context.getReplicatedLog().snapshotPreCommit(replicatedToAllIndex, context.getTermInformation().getCurrentTerm());
