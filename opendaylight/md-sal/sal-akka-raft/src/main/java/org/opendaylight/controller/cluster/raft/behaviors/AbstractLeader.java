    ...
    protected RaftActorBehavior handleAppendEntriesReply(ActorRef sender,
        ...
        if (appendEntriesReply.isSuccess()) {
            ...
            if(followerLastLogIndex < 0 || (followersLastLogEntry != null &&
                ...
                // and the term's match, then the follower is just behind the leader's journal from
                // the last snapshot, if any, so start catching it up starting at the follower's last
                // log index.
                followerLogInformation.setMatchIndex(followerLastLogIndex);
                followerLogInformation.setNextIndex(followerLastLogIndex + 1);
            } else {
