...
class ReplicatedLogImpl extends AbstractReplicatedLogImpl {
    ...
    public void removeFromAndPersist(final long logEntryIndex) {
        final long adjustedIndex = adjustedIndex(logEntryIndex);
        if (adjustedIndex >= 0) {
            context.getLogger().debug("{}: Remove log entry and persist {} (adjusted {}) ", context.getId(),
                logEntryIndex, adjustedIndex);
