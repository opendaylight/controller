...
public class PreLeaderScenarioTest extends AbstractRaftActorIntegrationTest {
    ...
    public void testUnComittedEntryOnLeaderChange() {
        ...
        assertEquals("Role change 1", RaftRole.Candidate.name(), roleChange.get(0).newRole());
        assertEquals("Role change 2", RaftRole.PreLeader.name(), roleChange.get(1).newRole());
        assertEquals("Role change 3", RaftRole.Leader.name(), roleChange.get(2).newRole());
