...
public class RaftActorTestKit extends TestKit {
    ...
    public RaftActorTestKit(final ActorSystem actorSystem, final String actorName, final Path baseDir) {
        super(actorSystem);
        raftActor = getSystem().actorOf(MockRaftActor.builder().id(actorName).baseDir(baseDir).props(), actorName);
    }
