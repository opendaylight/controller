 ...
 * Created: Jun 13, 2014
 */
public class FlowNodeReconcilListener implements DataChangeListener, AutoCloseable {
    ...
    public FlowNodeReconcilListener (final ForwardingRulesManager manager, final DataBroker db) {
        ...
        this.listenerRegistration = db.registerDataChangeListener(LogicalDatastoreType.OPERATIONAL,
                flowNodeWildCardIdentifier, FlowNodeReconcilListener.this, DataChangeScope.BASE);
    }
    ...
    public void onDataChanged(final AsyncDataChangeEvent<InstanceIdentifier<?>, DataObject> changeEvent) {
        ...
        this.add(createdData);
    }

    private void remove(final Set<InstanceIdentifier<?>> removeData) {
        for (InstanceIdentifier<?> entryKey : removeData) {
            ...
            provider.unregisterNode(nodeIdent);
        }
    }

    private void add(final Set<InstanceIdentifier<?>> createdData) {
        for (InstanceIdentifier<?> entryKey : createdData) {
            final InstanceIdentifier<FlowCapableNode> nodeIdent = entryKey.firstIdentifierOf(FlowCapableNode.class);
            provider.registerNewNode(nodeIdent);
            reconciliation(nodeIdent);
        }
    }
