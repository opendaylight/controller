...
public class FlowNodeReconcilListener extends AbstractChangeListener {
    ...
    private Optional<FlowCapableNode> readFlowCapableNode(final InstanceIdentifier<FlowCapableNode> flowNodeIdent) {
        ...
        try {
            Optional<DataObject> confFlowNode =
                    readTrans.read(LogicalDatastoreType.CONFIGURATION, flowNodeIdent).get();
            if (confFlowNode.isPresent() && preconditionForChange(null, confFlowNode.get(), null)) {
                return Optional.<FlowCapableNode> of((FlowCapableNode) confFlowNode.get());
            } else {
                return Optional.absent();
            }
        }
        catch (InterruptedException | ExecutionException e) {
            String errMsg = "Unexpected exception by reading flow " + flowNodeIdent;
            LOG.error(errMsg, e);
            return Optional.absent();
        }
        finally {
