 ...
 * Created: Jun 13, 2014
 */
public class FlowNodeReconcilListener implements SalNodeReconciliationListener {
    ...
    public void onNodeReconcilAdd( final NodeReconcilAdd reconcilNotif ) {
        ...
        if (nodeData != null && nodeData instanceof Node) {
            ...
            final InstanceIdentifier<FlowCapableNode> flowNodeIdent = nodeIdent.builder()
            ...
            for (Group group : flowCapNode.getGroup()) {
                ...
                groupBuilder.setGroupRef( new GroupRef( flowNodeIdent.builder().child( Group.class, group.getKey() ).build() ) );
            ...
            for (Meter meter : flowCapNode.getMeter()) {
                ...
                meterBuilder.setMeterRef( new MeterRef( flowNodeIdent.builder().child( Meter.class, meter.getKey() ).build() ) );
            ...
            for (Table flowTable : flowCapNode.getTable()) {
                final InstanceIdentifier<Table> tableInstanceId =
                        flowNodeIdent.builder().child(Table.class, flowTable.getKey()).build();
                for (Flow flow : flowTable.getFlow()) {
                    ...
                    flowBuilder.setFlowRef( new FlowRef(tableInstanceId.builder().child( Flow.class, flow.getKey() ).build() ) );
    ...
    public void onNodeReconcilDel( NodeReconcilDel reconcilNotif ) {
        ...
        if (nodeData != null && nodeData instanceof Node) {
            ...
            final InstanceIdentifier<FlowCapableNode> flowNodeIdent =
                    nodeIdent.builder().augmentation( FlowCapableNode.class ).build();
