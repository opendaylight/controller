...
public class StatPermCollectorImpl implements StatPermCollector {
    ...
    private final static long STAT_COLLECT_TIME_OUT = 60000L;
    ...
    private void collectStatCrossNetwork() {
        for (final Entry<InstanceIdentifier<Node>, StatNodeInfoHolder> nodeEntity : statNodeHolder.entrySet()) {
            ...
            for (final StatCapabTypes statMarker : listNeededStat) {
                ...
                switch (statMarker) {
                case PORT_STATS:
                    LOG.info("STAT-MANAGER-collecting PORT-STATS for NodeRef {}", actualNodeRef);
                    manager.getRpcMsgManager().getAllPortsStat(actualNodeRef);
                    waitingForNotification();
                    break;
                case QUEUE_STATS:
                    LOG.info("STAT-MANAGER-collecting QUEUE-STATS for NodeRef {}", actualNodeRef);
                    manager.getRpcMsgManager().getAllQueueStat(actualNodeRef);
                    waitingForNotification();
                    break;
                case TABLE_STATS:
                    LOG.info("STAT-MANAGER-collecting TABLE-STATS for NodeRef {}", actualNodeRef);
                    manager.getRpcMsgManager().getAllTablesStat(actualNodeRef);
                    waitingForNotification();
                    break;
                case GROUP_STATS:
                    LOG.info("STAT-MANAGER-collecting GROUP-STATS for NodeRef {}", actualNodeRef);
                    ...
                    break;
                case METER_STATS:
                    LOG.info("STAT-MANAGER-collecting METER-STATS for NodeRef {}", actualNodeRef);
                    ...
                    break;
                case FLOW_STATS:
                    LOG.info("STAT-MANAGER-collecting FLOW-STATS-ALL_FLOWS for NodeRef {}", actualNodeRef);
                    manager.getRpcMsgManager().getAllFlowsStat(actualNodeRef);
                    waitingForNotification();
                    LOG.info("STAT-MANAGER-collecting FLOW-AGGREGATE-STATS for NodeRef {}", actualNodeRef);
