...
public class StatNotifyCommitPort extends StatAbstractNotifyCommit<OpendaylightPortStatisticsListener>
    ...
    private void statPortCommit(final List<NodeConnectorStatisticsAndPortNumberMap> portStats, final NodeId nodeId,
        ...
        for (final NodeConnectorStatisticsAndPortNumberMap nConnectPort : portStats) {
            ...
            final InstanceIdentifier<FlowCapableNodeConnectorStatisticsData> nodeConnectorRef = nodeIdent
                    .child(NodeConnector.class, key).augmentation(FlowCapableNodeConnectorStatisticsData.class);
            ...
            Futures.addCallback(hashIdUpd, new FutureCallback<Optional<Node>>() {
                ...
                public void onSuccess(final Optional<Node> result) {
                    if (result.isPresent()) {
                        trans.put(LogicalDatastoreType.OPERATIONAL, nodeConnectorRef, statisticsDataBuilder.build(), true);
                    }
                }
                @Override
