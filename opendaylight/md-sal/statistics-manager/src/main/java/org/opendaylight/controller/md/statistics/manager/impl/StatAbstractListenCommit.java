...
public abstract class StatAbstractListenCommit<T extends DataObject, N extends NotificationListener>
     ...
     * @param ReadWriteTransaction - Read needs to check actual DS before delete / Write to call delete
     ...
     */
    protected void cleaningOperationalDS(final boolean isResponseFromStatCollector, final ReadWriteTransaction trans,
        ...
        for (final InstanceIdentifier<T> deleteElem : elemForDelete) {
            final CheckedFuture<Optional<T>, ReadFailedException> future = trans
                    .read(LogicalDatastoreType.OPERATIONAL, deleteElem);
            Futures.addCallback(future, new FutureCallback<Optional<T>>() {
                @Override
                public void onSuccess(final Optional<T> result) {
                    if (result.isPresent()) {
                        trans.delete(LogicalDatastoreType.OPERATIONAL, deleteElem);
                    }
                }
                @Override
