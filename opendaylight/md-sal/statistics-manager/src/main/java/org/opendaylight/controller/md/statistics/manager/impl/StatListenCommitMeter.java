...
public class StatListenCommitMeter extends StatAbstractListenCommit<Meter, OpendaylightMeterStatisticsListener>
    ...
    private void statMeterCommit(final List<MeterStats> meterStats,
        ...
        for (final MeterStats mStat : meterStats) {
            ...
            final InstanceIdentifier<NodeMeterStatistics> msIdent = nodeIdent
            ...
            if (preConfigurationCheck(nodeIdent)) {
                if (meter.isPresent()) {
                    trans.put(LogicalDatastoreType.OPERATIONAL, msIdent, msBuilder.build(), true);
                } else {
                    trans.put(LogicalDatastoreType.OPERATIONAL, msIdent, msBuilder.build());
                }
            }
        }
        if ( ! meter.isPresent()) {
