...
class StatListenCommitFlow extends StatAbstractListenCommit<Flow, OpendaylightFlowStatisticsListener>
    ...
    private void repeaterForFlow(final InstanceIdentifier<FlowCapableNode> nodeIdent, final Optional<Flow> flow) {
        if ( ! flow.isPresent()) {
            LOG.error("Light-weight flow can not be null for Node {} stat update", nodeIdent);
            return;
        }
    ...
    private void statsFlowCommitForOne(final List<FlowAndStatisticsMapList> list,
            final InstanceIdentifier<FlowCapableNode> nodeIdent, final Optional<Flow> flow) {
    ...
    private void statsFlowCommitAll(final List<FlowAndStatisticsMapList> list,
        ...
        for (final Table table : flowNode.get().getTable()) {
            final TableKey tableKey = table.getKey();
            for (final Flow existFlow : table.getFlow()) {
                nodeExistedFlowPaths.add(nodeIdent.child(Table.class, tableKey).child(Flow.class, existFlow.getKey()));
            }
        }
        ...
        cleaningOperationalDS(trans, true, nodeExistedFlowPaths, nodeIdent);
    }
