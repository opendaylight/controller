...
public class StatListenCommitFlow extends StatAbstractListenCommit<Flow, OpendaylightFlowStatisticsListener>
    ...
    private void statsFlowCommitAll(final List<FlowAndStatisticsMapList> list,
        ...
        for (final FlowAndStatisticsMapList flowStat : list) {
            ...
            final TableFlowUpdateState tableState = nodeState.getTable(tableKey, trans);
            tableState.reportFlow(flowStat,trans);
        }

        for(final TableFlowUpdateState table : nodeState.getTables()) {
            table.removeUnreportedFlows(trans);
        }
    }
     ...
     */
    static String buildHashCode(final org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.Flow deviceFlow) {
    ...
    private class TableFlowUpdateState {
        ...
        private void ensureTable(final ReadWriteTransaction tx) {
            if(!tableEnsured) {
                final FlowHashIdMapping emptyMapping = new FlowHashIdMappingBuilder().setFlowHashIdMap(Collections.<FlowHashIdMap>emptyList()).build();
        ...
        void removeUnreportedFlows(final ReadWriteTransaction trans) {
            final InstanceIdentifier<Node> nodeIdent = tableRef.firstIdentifierOf(Node.class);
            final Map<InstanceIdentifier<Flow>, Integer> mapAllFlowsForDelete = mapNodesForDelete.get(nodeIdent);
            if (mapAllFlowsForDelete != null) {
                final Set<InstanceIdentifier<Flow>> deletePaths = mapAllFlowsForDelete.keySet();
                for (final InstanceIdentifier<Flow> deleteCandidate : deletePaths) {
                    final Integer deleteIndex = mapAllFlowsForDelete.remove(deleteCandidate);
                    if (deleteIndex.intValue() < 1) {
                        final Optional<Flow> flow = readLatestConfiguration(deleteCandidate);
                        if (flow.isPresent()) {
                            final String fHashIdMap = buildHashCode((org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.Flow) flow);
                            final FlowHashIdMapKey hashIdKey = new FlowHashIdMapKey(fHashIdMap);
                            final InstanceIdentifier<FlowHashIdMap> hashIdIdent = tableRef
                                    .augmentation(FlowHashIdMapping.class).child(FlowHashIdMap.class, hashIdKey);
                            trans.delete(LogicalDatastoreType.OPERATIONAL, hashIdIdent);
                        }
                        trans.delete(LogicalDatastoreType.OPERATIONAL, deleteCandidate);
                    } else {
                        mapAllFlowsForDelete.put(deleteCandidate, Integer.valueOf(deleteIndex.intValue() - 1));
                    }
                }
            }
        }
...
//                try {
//                    flowStatNodeCheck = trans.read(LogicalDatastoreType.OPERATIONAL, flowStatIdent).checkedGet();
//                }
//                catch (final ReadFailedException e) {
