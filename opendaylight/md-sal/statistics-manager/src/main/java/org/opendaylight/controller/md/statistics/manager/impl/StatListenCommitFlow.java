...
//                });
    ...
    private class TableFlowUpdateState {
        ...
        private void ensureTable(ReadWriteTransaction tx) {
            if(tableEnsured) {
                FlowHashIdMapping emptyMapping = new FlowHashIdMappingBuilder().setFlowHashIdMap(Collections.<FlowHashIdMap>emptyList()).build();
                tx.merge(LogicalDatastoreType.OPERATIONAL, tableRef.augmentation(FlowHashIdMapping.class), emptyMapping, true);
                tableEnsured = true;
            }
        }

        private FlowKey seachInConfiguration(FlowAndStatisticsMapList flowStat, ReadWriteTransaction trans) {
            initConfigFlows();
            Iterator<Flow> it = configFlows.iterator();
            while(it.hasNext()) {
                Flow cfgFlow = it.next();
        ...
        private FlowKey getFlowKeyAndRemoveHash(FlowHashIdMapKey key) {
            initHashCache();
            FlowId ret = flowIdByHash.get(key);
            if(ret != null) {
        ...
        void reportFlow(FlowAndStatisticsMapList flowStat, ReadWriteTransaction trans) {
            ...
            if (flowKey == null) {
                flowKey = seachInConfiguration(flowStat, trans);
                if ( flowKey == null) {
            ...
            trans.put(LogicalDatastoreType.OPERATIONAL, tableRef.child(Flow.class, flowKey), flowBuilder.build());
        }
        ...
        void removeUnreportedFlows(ReadWriteTransaction trans) {
