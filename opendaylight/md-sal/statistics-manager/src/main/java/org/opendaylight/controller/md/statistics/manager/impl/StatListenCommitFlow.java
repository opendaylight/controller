...
public class StatListenCommitFlow extends StatAbstractListenCommit<Flow, OpendaylightFlowStatisticsListener>
     ...
     *
     * FIXME: we expect same version for YANG models for all clusters and that has to be fix and create better key
     */
    static String buildFlowIdOperKey(final FlowAndStatisticsMapList deviceFlow) {
        return String.valueOf(new StringBuffer().append(deviceFlow.getMatch())
                .append(deviceFlow.getPriority()).append(deviceFlow.getCookie()).toString());
    }
