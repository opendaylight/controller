    ...
    public void removeStat(final InstanceIdentifier<Flow> keyIdent) {
        manager.enqueue(new StatDataStoreOperation() {
            ...
            public void applyOperation(final ReadWriteTransaction tx) {
                ...
                try {
                    flow = tx.read(LogicalDatastoreType.OPERATIONAL, keyIdent).get();
                }
                catch (InterruptedException | ExecutionException e) {
                    LOG.debug("Operational/DS for Flow fail! {}", keyIdent, e);
                }
                if ( ! flow.isPresent()) {
                    return;
                }
                ...
                final String hashKey = String.valueOf(builder.build().hashCode());
                ...
                final KeyedInstanceIdentifier<FlowHashIdMap, FlowHashIdMapKey> hashFlowId =
                        tableIdent.augmentation(FlowHashIdMapping.class)
                        .child(FlowHashIdMap.class, new FlowHashIdMapKey(hashKey));
                Optional<FlowHashIdMap> delHash = Optional.absent();
                try {
                    delHash = tx.read(LogicalDatastoreType.OPERATIONAL, hashFlowId).get();
                }
                catch (InterruptedException | ExecutionException e) {
                    LOG.debug("Operational/DS for FlowHashMap fail! {}", hashFlowId, e);
                }
                if (delHash.isPresent()) {
                    tx.delete(LogicalDatastoreType.OPERATIONAL, hashFlowId);
                }
            }
        });
    }
    ...
    public void onAggregateFlowStatisticsUpdate(final AggregateFlowStatisticsUpdate notification) {
        ...
        manager.enqueue(new StatDataStoreOperation() {
            ...
            public void applyOperation(final ReadWriteTransaction tx) {
                ...
                try {
                    fNode = tx.read(LogicalDatastoreType.OPERATIONAL, fNodeIdent).get();
                }
                catch (InterruptedException | ExecutionException e) {
            ...
            if (deviceFlow.getPriority().equals(flow.get().getPriority())) {
                ...
                manager.enqueue(new StatDataStoreOperation() {
                    ...
                    public void applyOperation(final ReadWriteTransaction trans) {
                        ...
                        try {
                            readHashId = trans.read(LogicalDatastoreType.OPERATIONAL, flHashIdent).get();
                        }
                        catch (InterruptedException | ExecutionException e) {
                        ...
                        if (readHashId.isPresent()) {
                            ...
                            try {
                                checkFlow = trans.read(LogicalDatastoreType.OPERATIONAL, flowIdentRemove).get();
                            }
                            catch (InterruptedException | ExecutionException e) {
                        ...
                        Optional<FlowCapableNode> fNode = Optional.absent();
                        try {
                            fNode = trans.read(LogicalDatastoreType.OPERATIONAL, fNodeIdent).get();
                        }
                        catch (InterruptedException | ExecutionException e) {
        ...
        for (final FlowAndStatisticsMapList flowStat : list) {
            ...
            manager.enqueue(new StatDataStoreOperation() {
                ...
                public void applyOperation(final ReadWriteTransaction trans) {
                    ...
                    try {
                        checkFlowNode = trans.read(LogicalDatastoreType.OPERATIONAL, fNodeIdent).get();
                    }
                    ...
                    if (flowHashMapping != null) {
                        ...
                        for (final FlowHashIdMap flowHashId : flowHashMap) {
                            if ( ! deviceHashMap.contains(flowHashId.getKey())) {
                                // delete flow if exist
                                final InstanceIdentifier<Flow> flowIdent = tableIdent
                                        .child(Flow.class, new FlowKey(flowHashId.getFlowId()));
                                Optional<Flow> delFlow = Optional.absent();
                                try {
                                    delFlow = trans.read(LogicalDatastoreType.OPERATIONAL, flowIdent).get();
                                }
                                catch (InterruptedException | ExecutionException e) {
                                    // NOOP - probably another transaction delete that node
                                }
                                if (delFlow.isPresent()) {
                                    trans.delete(LogicalDatastoreType.OPERATIONAL, flowIdent);
                                }
                                // delete FlowHashId if exist
                                final KeyedInstanceIdentifier<FlowHashIdMap, FlowHashIdMapKey> flHashIdent = tableIdent
                                        .augmentation(FlowHashIdMapping.class).child(FlowHashIdMap.class, flowHashId.getKey());
                                Optional<FlowHashIdMap> delHash = Optional.absent();
                                try {
                                    delHash = trans.read(LogicalDatastoreType.OPERATIONAL, flHashIdent).get();
                                }
                                catch (InterruptedException | ExecutionException e) {
                                    // NOOP - probably another transaction delete that node
                                }
