 ...
 * stack to listener when listner catchs the last Multipart msg.
 *
...
public interface StatRpcMsgManager extends Runnable, AutoCloseable {
     ...
     */
    Future<Boolean> isExpectedStatistics(TransactionId id);
     ...
     */
    <T extends TransactionAware, D extends DataObject> void registrationRpcFutureCallBack(Future<RpcResult<T>> future, D inputObj);

    /**
     * Method registers TransactionCacheContainer to the GuavaCache. Container has to contain TransactionId
     * and it could contain some input {@link DataObject} to identify Update Transaction
     * Input {@link DataObject} presented means Single Object statistics
     * Null Input {@link DataObject} presented means statistics for StatPermCollector statistics
     *
     * @param TransactionCacheContainer<? extends TransactionAware> container
     */
//    Future<Boolean> registerTransactionContainer(TransactionCacheContainer<? super TransactionAware> container);
