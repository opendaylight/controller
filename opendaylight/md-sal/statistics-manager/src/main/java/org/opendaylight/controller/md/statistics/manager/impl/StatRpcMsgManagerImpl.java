...
public class StatRpcMsgManagerImpl implements StatRpcMsgManager {
    ...
    /* sleep time to prevent overtake FRM */
//    private final static int WAIT_FOR_FRM = 10;



    private final Cache<String, TransactionCacheContainer<? super TransactionAware>> txCache;

    private final long maxLifeForRequest = 30; /* 50 second */
...
//            final boolean success = statsRpcJobQueue.offer(getStatJob, maxLifeForRequest, TimeUnit.MILLISECONDS);
            if ( ! success) {
                LOG.warn("Put RPC request for getStat fail! Queue is full.");
            }
//        }
//        catch (final InterruptedException e) {
//            LOG.warn("Put Statistic job to Queue interrupted!", e);
//        }
//        try {
...
//        }
            ...
            public void onSuccess(final RpcResult<? extends TransactionAware> result) {
                ...
                if (id == null) {
                    ...
                    final TransactionCacheContainer<? super TransactionAware> container =
                            new TransactionCacheContainerImpl<>(id, inputObj, nodeRef);
    ...
    public void addNotification(final TransactionAware notification) {
        ...
        final TransactionCacheContainer<? super TransactionAware> container = (txCache.getIfPresent(txId));
        if (container != null) {
...
//                Thread.sleep(WAIT_FOR_FRM);
                registrationRpcFutureCallBack(groupStatsService
                        .getGroupStatistics(inpBuil.build()), groupBl.build(), nodeRef);
                return null;
            }
        };
...
//                Thread.sleep(WAIT_FOR_FRM);
                registrationRpcFutureCallBack(meterStatsService
                        .getMeterStatistics(input.build()), mBuilder.build(), nodeRef);
                return null;
            }
        };
...
//                Thread.sleep(WAIT_FOR_FRM);
                registrationRpcFutureCallBack(queueStatsService.
                        getQueueStatisticsFromGivenPort(inputBuilder.build()), queBuld.build(), nodeRef);
                return null;
            }
        };
    ...
    public class TransactionCacheContainerImpl<T extends TransactionAware> implements TransactionCacheContainer<T> {
        ...
        public <D extends DataObject> TransactionCacheContainerImpl (final TransactionId id, final D input, final NodeRef ref) {
