...
public class StatRpcMsgManagerImpl implements StatRpcMsgManager {
    ...
    private final Object queueStackLock = new Object();
    ...
    private BlockingQueue<RpcJobsQueue> mainStatQueue;
    private BlockingQueue<RpcJobsQueue> secondaryGetAllStatQueue;
    ...
    public StatRpcMsgManagerImpl (final StatisticsManager manager,
        ...
        secondaryGetAllStatQueue = new LinkedBlockingQueue<>(queueCapacity);
        mainStatQueue = new LinkedBlockingQueue<>(queueCapacity);
        ...
        txCache = CacheBuilder.newBuilder().expireAfterWrite(maxLifeForRequest, TimeUnit.MILLISECONDS)
                .concurrencyLevel(1).maximumSize(10000).build();
    }
    ...
    public void close() {
        ...
        wakeUpRpcService();
        mainStatQueue = null;
        secondaryGetAllStatQueue = null;
    }

    private void wakeUpRpcService() {
        if (wakeMe) {
            synchronized (queueStackLock) {
                if (wakeMe) {
                    LOG.debug("pinging message harvester in starve status");
                    queueStackLock.notify();
                }
            }
        }
    }
    ...
    public void run() {
        ...
        while ( ! finishing) {
            while (( ! mainStatQueue.isEmpty()) || ( ! secondaryGetAllStatQueue.isEmpty())) {
                while ( ! mainStatQueue.isEmpty()) {
                    try {
                        mainStatQueue.take().call();
                    }
                    catch (final Exception e) {
                        LOG.warn("Stat Element RPC executor fail!", e);
                    }
                }
                if ( ! secondaryGetAllStatQueue.isEmpty()) {
                    try {
                        secondaryGetAllStatQueue.take().call();
                    }
                    catch (final Exception e) {
                        LOG.warn("Get All Stat Element RPC executor fail!", e);
                    }
                }
            }
            synchronized (queueStackLock) {
    ...
    private void addGetAllStatJob(final RpcJobsQueue getAllStatJob) {
        try {
            secondaryGetAllStatQueue.offer(getAllStatJob, maxLifeForRequest, TimeUnit.MILLISECONDS);
        }
        catch (final InterruptedException e) {
    ...
    public <T extends TransactionAware, D extends DataObject> void registrationRpcFutureCallBack(
        ...
        try {
            ...
            if (response.isSuccessful()) {
                final TransactionId id = response.getResult().getTransactionId();
                if (id == null) {
            ...
            LOG.warn("Statistics RPC call fail!", e);
        }
    }
    ...
    public void getAggregateFlowStat(final NodeRef nodeRef) {
        ...
        // TODO VD change it to get all aggregate FlowStats

        @SuppressWarnings("unchecked")
        final InstanceIdentifier<Node> nodeIdent =
                (InstanceIdentifier<Node>) nodeRef.getValue();
        ...
        try {
            ...
            LOG.warn("Read SwitchFeatures {} fail!", switchFeaturIdent);
        }
        ...
        for (short i = 0; i < cyles; i++) {
            ...
            final RpcJobsQueue getAggregateFlowStat = new RpcJobsQueue() {
                ...
                public Void call() throws Exception {
                    ...
                    builder.setNode(nodeRef);
                    builder.setTableId(new TableId(tableId));

                    final TableBuilder tbuilder = new TableBuilder();
                    tbuilder.setId(tableId);
                    registrationRpcFutureCallBack(flowStatsService
