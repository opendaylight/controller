...
public class StatListenCommitGroup extends StatAbstractListenCommit<Group, OpendaylightGroupStatisticsListener>
    ...
    public void onGroupFeaturesUpdated(final GroupFeaturesUpdated notification) {
        ...
        if ( ! isExpectedStatistics(transId)) {
            ...
            LOG.warn("STAT-MANAGER - GroupFeaturesUpdated: unimplement registred notification detect TransactionId {}", transId);
        }
    }
    ...
    private void statGroupCommit(final List<GroupStats> groupStats, final NodeId nodeId,
        ...
        for (final GroupStats groupStat : groupStats) {
            ...
            final InstanceIdentifier<NodeGroupStatistics> gsIdent = nodeIdent
            ...
            if (preConfigurationCheck(nodeIdent)) {
                if (group.isPresent()) {
                    trans.put(LogicalDatastoreType.OPERATIONAL, gsIdent, groupStatisticsBuilder.build(), true);
                } else {
                    trans.put(LogicalDatastoreType.OPERATIONAL, gsIdent, groupStatisticsBuilder.build());
                }
            }
        }
        if ( ! group.isPresent()) {
