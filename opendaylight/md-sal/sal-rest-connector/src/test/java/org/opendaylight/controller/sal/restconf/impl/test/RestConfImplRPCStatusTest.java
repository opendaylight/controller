...
public class RestConfImplRPCStatusTest extends JerseyTest {
    ...
    public static void init() throws FileNotFoundException {
        Set<Module> allModules = TestUtils.loadModules(RestconfImplTest.class.getResource("/invoke-rpc-status")
                .getPath());
    ...
    @Test
    public void testRPCStatusCodes() throws UnsupportedEncodingException, FileNotFoundException {

        String uri = createUri("/operations/", "invoke-rpc-status:testInput");

        InputStream xmlStream = RestconfImplTest.class.getResourceAsStream("/invoke-rpc-status/xml/data-rpc-input.xml");

        InputStream xmlStreamOutput = RestconfImplTest.class
                .getResourceAsStream("/invoke-rpc-status/xml/data-rpc-output.xml");
        CompositeNode outputCompositeNode = TestUtils.loadCompositeNodeWithXmlTreeBuilder(xmlStreamOutput);
        ...
        when(brokerFacade.invokeRpc(any(QName.class), any(CompositeNode.class))).thenReturn(
                new DummyRpcResult.Builder<CompositeNode>().isSuccessful(true).result(null).build());
        Response response = target(uri).request(MEDIA_TYPE).post(entity);
        assertEquals(204, response.getStatus());

        when(brokerFacade.invokeRpc(any(QName.class), any(CompositeNode.class))).thenReturn(
                new DummyRpcResult.Builder<CompositeNode>().isSuccessful(true).result(outputCompositeNode).build());
        response = target(uri).request(MEDIA_TYPE).post(entity);
        assertEquals(200, response.getStatus());

        when(brokerFacade.invokeRpc(any(QName.class), any(CompositeNode.class))).thenReturn(
                new DummyRpcResult.Builder<CompositeNode>().isSuccessful(false).result(null).build());
        response = target(uri).request(MEDIA_TYPE).post(entity);
