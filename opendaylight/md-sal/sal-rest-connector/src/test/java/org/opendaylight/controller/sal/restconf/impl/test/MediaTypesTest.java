...
public class MediaTypesTest extends JerseyTest {
    ...
    public void testPostConfigWithPathMediaTypes() throws UnsupportedEncodingException {
        ...
        final String uriPath = "ietf-interfaces:interfaces";
        final String uri = uriPrefix + uriPath;
        when(restconfService.createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class))).thenReturn(null);
        post(uri, null, Draft02.MediaTypes.DATA + JSON, jsonData);
        verify(restconfService, times(1)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
        post(uri, null, Draft02.MediaTypes.DATA + XML, xmlData);
        verify(restconfService, times(2)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
        post(uri, null, MediaType.APPLICATION_JSON, jsonData);
        verify(restconfService, times(3)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
        post(uri, null, MediaType.APPLICATION_XML, xmlData);
        verify(restconfService, times(4)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
        post(uri, null, MediaType.TEXT_XML, xmlData);
        verify(restconfService, times(5)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
        post(uri, "fooMediaType", MediaType.TEXT_XML, xmlData);
        verify(restconfService, times(6)).createConfigurationData(eq(uriPath), any(NormalizedNodeContext.class));
    }
