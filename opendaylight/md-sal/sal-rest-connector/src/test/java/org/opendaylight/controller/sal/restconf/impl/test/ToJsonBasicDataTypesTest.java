...
public class ToJsonBasicDataTypesTest {
    ...
    private void checkLfUnion(JsonReader jReader, String keyName, JsonToken peek) throws IOException {
        if (keyName.equals("lfunion1")) {
            ...
            assertEquals("Key " + keyName + " has incorrect type", JsonToken.BEGIN_ARRAY, peek);
            jReader.beginArray();
            assertEquals("Key " + keyName + " has incorrect type", JsonToken.NULL, jReader.peek());
            jReader.nextNull();
            assertEquals("Key " + keyName + " has incorrect type", JsonToken.END_ARRAY, jReader.peek());
            jReader.endArray();
