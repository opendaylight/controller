...
import org.opendaylight.controller.rest.common.IdentityValuesDTO.IdentityValue;
import org.opendaylight.controller.rest.common.IdentityValuesDTO.Predicate;
...
public class RestCodec {
    ...
    public static final Codec<Object, Object> from(final TypeDefinition<?> typeDefinition,
            final DOMMountPoint mountPoint, final RestSchemaController restSchemaCx) {
        return new ObjectCodec(typeDefinition, mountPoint, restSchemaCx);
    }
    ...
    public static final class ObjectCodec implements Codec<Object, Object> {
        ...
        private ObjectCodec(final TypeDefinition<?> typeDefinition, final DOMMountPoint mountPoint,
                final RestSchemaController restSchemaCx) {
            type = RestUtil.resolveBaseTypeFrom(typeDefinition);
            if (type instanceof IdentityrefTypeDefinition) {
                identityrefCodec = new IdentityrefCodecImpl(mountPoint, restSchemaCx);
            } else {
                identityrefCodec = null;
            }
            if (type instanceof InstanceIdentifierTypeDefinition) {
                instanceIdentifier = new InstanceIdentifierCodecImpl(mountPoint, restSchemaCx);
            } else {
    ...
    public static class IdentityrefCodecImpl implements IdentityrefCodec<IdentityValuesDTO> {
        ...
        public IdentityrefCodecImpl(final DOMMountPoint mountPoint, final RestSchemaController restSchemaCx) {
        ...
        public QName deserialize(final IdentityValuesDTO data) {
            ...
            final Module module = getModuleByNamespace(valueWithNamespace.getNamespace(), mountPoint, restSchemaCx);
            if (module == null) {
    ...
    public static class InstanceIdentifierCodecImpl implements InstanceIdentifierCodec<IdentityValuesDTO> {
        ...
        public InstanceIdentifierCodecImpl(final DOMMountPoint mountPoint, final RestSchemaController restSchemaCx) {
            this.mountPoint = mountPoint;
            this.restSchemaCx = restSchemaCx;
        }
        ...
        public YangInstanceIdentifier deserialize(final IdentityValuesDTO data) {
            ...
            for (int i = 0; i < identities.size(); i++) {
                ...
                URI validNamespace = resolveValidNamespace(identityValue.getNamespace(), mountPoint, restSchemaCx);
                final DataSchemaNode node = RestSchemaControllerImpl.findInstanceDataChildByNameAndNamespace(
                    ...
                    if (node instanceof LeafListSchemaNode) { // predicate is value of leaf-list entry
                        ...
                        for (final Predicate predicate : identityValue.getPredicates()) {
                            validNamespace = resolveValidNamespace(predicate.getName().getNamespace(), mountPoint,
                                    restSchemaCx);
                            final DataSchemaNode listKey = RestSchemaControllerImpl
    ...
    private static Module getModuleByNamespace(final String namespace, final DOMMountPoint mountPoint,
            final RestSchemaController restSchemaCx) {
        final URI validNamespace = resolveValidNamespace(namespace, mountPoint, restSchemaCx);

        Module module = null;
        if (mountPoint != null) {
            module = restSchemaCx.findModuleByNamespace(mountPoint, validNamespace);
        } else {
            module = restSchemaCx.findModuleByNamespace(validNamespace);
        }
        if (module == null) {
    ...
    private static URI resolveValidNamespace(final String namespace, final DOMMountPoint mountPoint,
        ...
        URI validNamespace;
        if (mountPoint != null) {
            validNamespace = restSchemaCx.findNamespaceByModuleName(mountPoint, namespace);
        } else {
            validNamespace = restSchemaCx.findNamespaceByModuleName(namespace);
        }
