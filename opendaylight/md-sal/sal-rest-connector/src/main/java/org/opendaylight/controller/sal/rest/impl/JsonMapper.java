...
class JsonMapper {
    ...
    private void writeChildrenOfParent(final JsonWriter writer, final CompositeNode parent,
        ...
        for (Node<?> child : parent.getValue()) {
            ...
            if (childSchema == null) {
                // Node may not conform to schema or allows "anyxml" - we'll
                // process it.
                ...
                if (!foundLists.contains(child.getNodeType())) {
                    ...
                    // Since we don't have a schema, we don't know which nodes
                    // are supposed to be
                    // lists so treat every one as a potential list to avoid
