...
public class RestBrokerFacadeImpl implements RestBrokerFacade {
    ...
    public RestBrokerFacadeImpl(final DOMDataBroker domDataBroker) {
    ...
    public CheckedFuture<Void, TransactionCommitFailedException> commitConfigurationDataPatch(
        ...
        final DOMDataReadWriteTransaction tx = domDataBroker.newReadWriteTransaction();
        checkIfDataExist(tx, LogicalDatastoreType.CONFIGURATION, path);
    ...
    /**
     * if data in datastore does not exist, this method throw 404 error (data
     * missing)
     *
     * @param tx
     * @param configuration
     * @param path
     */
    private void checkIfDataExist(final DOMDataReadWriteTransaction tx, final LogicalDatastoreType configuration,
            final YangInstanceIdentifier path) {
        final ListenableFuture<Boolean> futureDatastoreData = tx.exists(configuration, path);

        try {
            if (!futureDatastoreData.get()) {
                final String errMsg = "Patch Configuration via Restconf was not executed because data does not exist";
                LOG.trace(errMsg + ":{}", path);
                tx.cancel();
