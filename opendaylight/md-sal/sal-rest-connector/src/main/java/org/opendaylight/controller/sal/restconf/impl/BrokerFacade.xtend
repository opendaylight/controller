...
class BrokerFacade implements DataReader<InstanceIdentifier, CompositeNode> {
    ...
    def readConfigurationData(MountInstance mountPoint, InstanceIdentifier path) {
        checkPreconditions
        LOG.info("Read Configuration via Restconf: {}", path)
    ...
    def readOperationalData(MountInstance mountPoint, InstanceIdentifier path) {
        checkPreconditions
        LOG.info("Read Operational via Restconf: {}", path)
    ...
    def commitConfigurationDataPut(MountInstance mountPoint, InstanceIdentifier path, CompositeNode payload) {
        checkPreconditions
    ...
    def commitConfigurationDataPost(MountInstance mountPoint, InstanceIdentifier path, CompositeNode payload) {
        checkPreconditions
        val transaction = mountPoint.beginTransaction;
        transaction.putConfigurationData(path, payload);
        if (payload == transaction.createdConfigurationData.get(path)) {
    ...
    def commitConfigurationDataDelete(MountInstance mountPoint, InstanceIdentifier path) {
        checkPreconditions
        val transaction = mountPoint.beginTransaction;
        transaction.removeConfigurationData(path)
        return transaction.commit
