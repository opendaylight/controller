...
public class RestconfServiceBaseImpl extends AbstractRestconfServiceImpl implements RestconfServiceBase {
    ...
    public RestconfServiceBaseImpl(final RestBrokerFacade dataBroker, final RestSchemaContext schemaCx) {
    ...
    public NormalizedNodeContext getModules(final UriInfo uriInfo) {
        ...
        final SchemaContext schemaContext = schemaCx.getGlobalSchema();

        final Module restconfModule = getRestconfModule();
        final DataSchemaNode modulesSchemaNode = schemaCx.getRestconfModuleRestConfSchemaNode(restconfModule,
    ...
    public NormalizedNodeContext getModules(final String identifier, final UriInfo uriInfo) {
        ...
        if (!identifier.contains(RestSchemaContextImpl.MOUNT)) {
            final String errMsg = "URI has bad format. If modules behind mount point should be showed,"
                    + " URI has to end with " + RestSchemaContextImpl.MOUNT;
        ...
        final Set<Module> modules = schemaCx.getAllModules(mountPoint);
        ...
        return new NormalizedNodeContext(new InstanceIdentifierContext<>(null, modulesSchemaNode, mountPoint,
                schemaCx.getGlobalSchema()), moduleContainerBuilder.build(),
    ...
    public NormalizedNodeContext getModule(final String identifier, final UriInfo uriInfo) {
        ...
        if (identifier.contains(RestSchemaContextImpl.MOUNT)) {
            final InstanceIdentifierContext<?> mountPointIdentifier = schemaCx
                    .toMountPointIdentifier(identifier);
            mountPoint = mountPointIdentifier.getMountPoint();
            module = schemaCx.findModuleByNameAndRevision(mountPoint, moduleNameAndRevision);
            schemaContext = mountPoint.getSchemaContext();
        } else {
            module = schemaCx.findModuleByNameAndRevision(moduleNameAndRevision);
        ...
        final DataSchemaNode moduleSchemaNode = schemaCx.getRestconfModuleRestConfSchemaNode(restconfModule,
    ...
    public Response subscribeToStream(final String identifier, final UriInfo uriInfo) {
        ...
        dataBroker.registerToListenDataChanges(datastore, scope, listener);
    ...
    public NormalizedNodeContext getAvailableStreams(final UriInfo uriInfo) {
        ...
        final Module restconfModule = getRestconfModule();
        final DataSchemaNode streamSchemaNode = schemaCx.getRestconfModuleRestConfSchemaNode(restconfModule,
        ...
        final DataSchemaNode streamsContainerSchemaNode = schemaCx.getRestconfModuleRestConfSchemaNode(restconfModule,
    ...
    protected MapEntryNode toModuleEntryNode(final Module module, final DataSchemaNode moduleSchemaNode) {
        ...
        List<DataSchemaNode> instanceDataChildrenByName = RestSchemaContextImpl.findInstanceDataChildrenByName(
        ...
        instanceDataChildrenByName = RestSchemaContextImpl.findInstanceDataChildrenByName((listModuleSchemaNode),
                "revision");
    ...
    private QName getModuleNameAndRevision(final String identifier) {
        final int mountIndex = identifier.indexOf(RestSchemaContextImpl.MOUNT);
        String moduleNameAndRevision = "";
        if (mountIndex >= 0) {
            moduleNameAndRevision = identifier.substring(mountIndex + RestSchemaContextImpl.MOUNT.length());
        } else {
    ...
    protected MapEntryNode toStreamEntryNode(final String streamName, final DataSchemaNode streamSchemaNode) {
        ...
        instanceDataChildrenByName = RestSchemaContextImpl.findInstanceDataChildrenByName((listStreamSchemaNode),
                "description");
