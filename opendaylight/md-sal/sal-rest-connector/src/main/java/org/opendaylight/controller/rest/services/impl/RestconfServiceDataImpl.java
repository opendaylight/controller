...
public class RestconfServiceDataImpl extends AbstractRestconfServiceImpl implements RestconfServiceData {
     ...
     * @param schemaCx
     */
    public RestconfServiceDataImpl(final RestBrokerFacade dataBroker, final RestSchemaContext schemaCx) {
        super(dataBroker, schemaCx);
    }

    @Override
    public NormalizedNodeContext readConfigurationData(final String identifier, final UriInfo uriInfo) {
        final InstanceIdentifierContext<?> iiWithData = schemaCx.toInstanceIdentifier(identifier);
        final DOMMountPoint mountPoint = iiWithData.getMountPoint();
        NormalizedNode<?, ?> data = null;
        final YangInstanceIdentifier normalizedII = iiWithData.getInstanceIdentifier();
        if (mountPoint != null) {
            data = dataBroker.readConfigurationData(mountPoint, normalizedII);
        } else {
            data = dataBroker.readConfigurationData(normalizedII);
        }
        if (data == null) {
    ...
    public NormalizedNodeContext readOperationalData(final String identifier, final UriInfo uriInfo) {
        ...
        final YangInstanceIdentifier normalizedII = iiWithData.getInstanceIdentifier();
        if (mountPoint != null) {
            data = dataBroker.readOperationalData(mountPoint, normalizedII);
        } else {
            data = dataBroker.readOperationalData(normalizedII);
        }
        if (data == null) {
            final String errMsg = "Request could not be completed because the relevant data model content does not exist ";
            LOG.debug(errMsg + identifier);
    ...
    public Response updateConfigurationData(final String identifier, final NormalizedNodeContext payload) {
        ...
        while (true) {
            try {
                if (mountPoint != null) {
                    dataBroker.commitConfigurationDataPut(mountPoint, normalizedII, payload.getData()).checkedGet();
                } else {
                    dataBroker.commitConfigurationDataPut(schemaCx.getGlobalSchema(), normalizedII,
    ...
    public Response createConfigurationData(final NormalizedNodeContext payload, final UriInfo uriInfo) {
        ...
        try {
            if (mountPoint != null) {
                dataBroker.commitConfigurationDataPost(mountPoint, normalizedII, payload.getData()).checkedGet();
            } else {
                dataBroker.commitConfigurationDataPost(schemaCx.getGlobalSchema(), normalizedII,
                        payload.getData()).checkedGet();
            }
    ...
    public Response deleteConfigurationData(final String identifier) {
        ...
        try {
            if (mountPoint != null) {
                dataBroker.commitConfigurationDataDelete(mountPoint, normalizedII);
            } else {
                dataBroker.commitConfigurationDataDelete(normalizedII).get();
            }
        } catch (final Exception e) {
    ...
    private URI resolveLocation(final UriInfo uriInfo, final String uriBehindBase, final DOMMountPoint mountPoint,
        ...
        try {
            uriBuilder.path(schemaCx.toFullRestconfIdentifier(normalizedII, mountPoint));
        } catch (final Exception e) {
