...
public class BrokerFacade {
    ...
    private boolean ensureParentsByMerge(final LogicalDatastoreType store,
        ...
        try {
			...
			/*
			 * Catching the exception here, logging it and proceeding further for the following reasons.
			 *
			 * 1. For MD-SAL store if it fails we'll go with the next call anyway and let the failure happen there.
			 * 2. For NETCONF devices that can not handle these calls such as creation of empty lists etc, instead of
			 *   failing we'll go with the actual call. Devices should be able to handle the actual calls made without
			 *   the need to create parents. So instead of failing we will give a device a chance to configure the management
			 *   entity in question.
			 * 3. If this merge call is handled properly by MD-SAL data store or a Netconf device this is a no-op.
			 */
			mergeResult = false;
