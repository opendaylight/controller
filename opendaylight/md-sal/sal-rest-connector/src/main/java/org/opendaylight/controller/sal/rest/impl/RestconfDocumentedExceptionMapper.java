...
public class RestconfDocumentedExceptionMapper implements ExceptionMapper<RestconfDocumentedException> {
    ...
    private Object toJsonResponseBody(final NormalizedNodeContext errorsNode, final DataNodeContainer errorsSchemaNode) {
        ...
        // We create a delegating writer to special-case error-info as error-info is defined as a container
        // in the restconf yang schema but we treat it as a leaf node so we can output it. The delegate stream
        // writer validates the node type against the schema and thus will expect a LeafSchemaNode so, to
        // avoid an error, we override the leafNode behavior for error-info.
