...
/**
 * This class defines an ExceptionMapper that handles
 * RestconfDocumentedExceptions thrown by resource implementations and
 * translates appropriately to restconf error response as defined in the
 * RESTCONF RFC draft.
 *
...
public class RestconfDocumentedExceptionMapper implements ExceptionMapper<RestconfDocumentedException> {
    ...
    public Response toResponse(final RestconfDocumentedException exception) {
        ...
        if (errors.isEmpty()) {
            // We don't actually want to send any content but, if we don't set
            // any content here,
            // the tomcat front-end will send back an html error report. To
            // prevent that, set a
    ...
    private Node<?> parseErrorInfo(final String errorInfo) {
        ...
        // Wrap the error info content in a root <error-info> element so it can
        // be parsed
        // as XML. The error info content may or may not be XML. If not then it
        // will be
        ...
        try {
            ...
            // TODO: what if the content is text that happens to contain invalid
            // markup? Could
        ...
        if (errorInfoNode instanceof CompositeNode) {
            ...
            // At this point the QName for the "error-info" CompositeNode
            // doesn't contain the revision
            // as it isn't present in the XML. So we'll copy all the child nodes
            // and create a new
            // CompositeNode with the full QName. This is done so the XML/JSON
            // mapping code can
            // locate the schema.
