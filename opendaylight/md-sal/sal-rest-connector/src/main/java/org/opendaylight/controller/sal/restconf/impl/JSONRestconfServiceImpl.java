...
public class JSONRestconfServiceImpl implements JSONRestconfService {
    ...
    public void put(String uriPath, String payload) throws OperationFailedException {
        ...
        System.out.println("IID: "+context.getInstanceIdentifierContext().getInstanceIdentifier()+", \nNN: "+context.getData());

        try {
            Response response = RestconfImpl.getInstance().updateConfigurationData(uriPath, context);
        } catch (Exception e) {
    ...
    public Optional<String> get(String uriPath, LogicalDatastoreType datastoreType) throws OperationFailedException {
        try {
            ...
            return Optional.of(outputStream.toString(Charsets.UTF_8.name()));
        } catch (Exception e) {
            if(!isDataMissing(e)) {
                propagateExceptionAs(uriPath, e);
            }
