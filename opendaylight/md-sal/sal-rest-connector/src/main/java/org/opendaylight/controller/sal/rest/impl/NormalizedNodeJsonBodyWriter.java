...
public class NormalizedNodeJsonBodyWriter implements MessageBodyWriter<NormalizedNodeContext> {
    ...
    public void writeTo(final NormalizedNodeContext t, final Class<?> type, final Type genericType, final Annotation[] annotations,
        ...
        } else if (SchemaPath.ROOT.equals(path)) {
            // FIXME: Add proper handling of reading root.
        } else if (data instanceof MapEntryNode) {
            writeMapEntryStart(ouWriter,data.getNodeType(),context.getSchemaContext());
        ...
        NormalizedNodeStreamWriter jsonWriter = JSONNormalizedNodeStreamWriter.create(context.getSchemaContext(),path,ouWriter);
        NormalizedNodeWriter nnWriter = NormalizedNodeWriter.forStreamWriter(jsonWriter);
        nnWriter.write(data);
        nnWriter.flush();

        if(data instanceof MapEntryNode) {
            writeMapEntryEnd(ouWriter);
        }
        ouWriter.flush();
    }
    ...
    private void writeMapEntryEnd(OutputStreamWriter writer) throws IOException {
        writer.write('}');
    }
}
