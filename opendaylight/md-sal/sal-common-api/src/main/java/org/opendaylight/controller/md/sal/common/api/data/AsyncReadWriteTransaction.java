 ...
 * commiting transaction  by calling {@link #commit()}, which seals transaction
 ...
 *
 * tx1fterCommit = broker.newReadOnlyTransaction(); // read Transaction is snapshot of new state
 * tx1fterCommit.read(OPERATIONAL,PATH).get(); // returns Optional containing B
 * </pre>
 *
 ...
 *
 * tx1.read(OPERATIONAL,PATH).get(); // tx1 read-write transaction still sees Optional containing A
 ...
 *
 * tx2.read(OPERATIONAL,PATH).get() // tx2 read-write transaction still sees Optional containing A
 *                                  // since is isolated from B
 *
 * tx2.commit().get(); // data tree is updated, PATH contains B
 * tx1.read(OPERATIONAL,PATH).get(); // still returns Optional containing C since is isolated from B
 *
 * tx1fterCommit = broker.newReadOnlyTransaction(); // read Transaction is snapshot of new state
 * tx1fterCommit.read(OPERATIONAL,PATH).get(); // returns Optional containing B
 *
