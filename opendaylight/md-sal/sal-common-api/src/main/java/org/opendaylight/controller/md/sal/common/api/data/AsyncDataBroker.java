...
public interface AsyncDataBroker<P extends Path<P>, D, L extends AsyncDataChangeListener<P, D>> extends //
     ...
     * you would receive based on the scope you specify, when you
     * are registering for changes on container a.
     * <pre>
     ...
     * Following is an example model with comments describing what notifications
     * you would receive based on the scope you specify, when you
     * are registering for changes on list list (without specifying concrete item
     * in the list).
     *
     * <pre>
    ...
    /**
     * Registers a {@DataChangeListener} to receive
     * notifications when data changes under the given path in conceptual data
     * tree.
     * <p>
     * You are able to register for notifications for any node / subtree, which
     * is possible to reference via supplied path type.
     * <p>
     * If path type allows to specify path up to the leaf nodes, it is possible
     * to listen on leaf nodes.
     * <p>
     * You are able to register for data change notifications for subtree even
     * if it does not exists and you will receive notification once that node is
     * created.
     * <p>
     ...
     * contains all preexisting data marked as created.
     *
     * <p>
     * You are also able to specify scope of the changes you want to be
     * notified.
     * <p>
     * Supported scopes are:
     * <ul>
     * <li>{@link DataChangeScope#BASE} - notification events will be only
     * triggered when node referenced by path is created, removed or replaced.
     * <li>{@link DataChangeScope#ONE} - notifications events will be only
     * triggered when node referenced by path is created, removed or replaced,
     * or any of it's immediate children is created, removed or replaced.
     * <li>{@link DataChangeScope#SUBTREE} - notification events will be
     * triggered for any change of node path is referencing or any change of
     * children nodes.
     * </ul>
