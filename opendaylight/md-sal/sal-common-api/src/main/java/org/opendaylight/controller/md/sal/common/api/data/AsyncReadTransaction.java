 ...
 *
 * View of data tree is stable point-in-time snapshot of current
 * data tree state captured when transaction was created and
 * it's state and underlying data tree
 ...
 * Lest assume initial state of data tree for <code>PATH</code> is <code>A</code>.
 *
 * <pre>
 * <code>
 ...
 * that action happened after other asynchronous action. Use of blocking
 * call {@link ListenableFuture#get()} is discouraged for most uses
 * and you should use {@link ListenableFuture#addListener(Runnable, java.util.concurrent.Executor)}
...
public interface AsyncReadTransaction<P extends Path<P>, D> extends AsyncTransaction<P, D> {
     ...
     *
     * If the target is subtree, whole subtree is read (accessible from
     ...
     *         <ul>
     *         <li>If data at supplied path exists the {@link ListeblaFuture#get()}
     *         returns Optional object containing data once read is done.
