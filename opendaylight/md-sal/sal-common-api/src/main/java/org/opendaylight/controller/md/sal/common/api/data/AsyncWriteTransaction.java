 ...
 * <p>
 * Write transaction is isolated from other concurrent write transactions,
 * all writes are local to the transaction and represents only proposal
 * of state change for data tree and it is not visible to any
 * other concurrently running transactions.
 * <p>
 * Application (publishing) of changes proposed in this transaction
 * is done by commiting transaction via {@link #commit()}
 * message, which seals transaction (prevents any further writes
 * using this transaction) and submits it to be processed and applied
 * to global conceptual data tree.
 * <p>
 * Transaction commit may fail due to concurrent transaction modified
 * data in incompatible way and was commited earlier. See {@link #commit()}
...
public interface AsyncWriteTransaction<P extends Path<P>, D> extends AsyncTransaction<P, D> {
     ...
     *
     *
     * If you need to  make sure parent object exists,
     * but you do not want modify its preexisting state (if
     * node exists consider using
     ...
     * its child lists will be merged. Performing the following merge operations:
     *
     * <pre>
     * 1) container { list [ a ] }
     * 2) container { list [ b ] }
     ...
     *
     * <h4>Conflict of two transactions</h4> This example illustrates two
     * concurrent transactions, which introduces are derived from same initial
     * state of data tree and proposes conflicting modifications.
     *
     * <pre>
     * <code>
     ...
     *
     * txA.put(CONFIGURATION, PATH, A); // we writes to PATH value A
     * txB.put(CONFIGURATION, PATH, B) // we writes to PATH value B
     *
     ...
     * will not be applied. Future will fail with
     * {@link OptimisticLockFailedException} exception, which indicates
     * to client that concurrent transaction prevented submitted
     ...
     *         TODO: Usability: Consider change from ListenableFuture to
     *  {@link com.google.common.util.concurrent.CheckedFuture}
     * which will throw {@link TransactionCommitFailedException}.
     *
     * @throws IllegalStateException
