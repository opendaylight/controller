 ...
 * </ul>
 */
public final class RaftActor extends AbstractUntypedPersistentActor {

    private static final long APPLY_STATE_DELAY_THRESHOLD_IN_NANOS = TimeUnit.MILLISECONDS.toNanos(50L); // 50 millis

    private static final FiniteDuration RAFT_ASK_TIMEOUT = Duration.create(5000, TimeUnit.MILLISECONDS);

    private final Logger LOG = LoggerFactory.getLogger(RaftActor.class);
    ...
    public RaftActor(String id, Map<String, String> peerAddresses,
         Optional<ConfigParams> configParams, short payloadVersion, boolean persistence) {
    ...
    public static Props props(String id, Map<String, String> peers,
                              ...
                              short payloadVersion, boolean persistence) {
        return Props.create(RaftActor.class, id, peers, configParams, payloadVersion, persistence);
    }
    ...
    protected void handleNonRaftCommand(final Object message) {
        // Messages from Client land up here!
        if (message instanceof PersistDataRequest) {
            PersistDataRequest request = (PersistDataRequest) message;
            persistData(request.getClientActor(), request.getIdentifier(), request.getData());
        } else if (message instanceof SetPersistence) {
            SetPersistence request = (SetPersistence) message;
            setPersistence(request.persistence());
        }
        unhandled(message);
    }
    ...
    protected final void persistData(final ActorRef clientActor, final Identifier identifier, final Payload data) {
        ...
        // Do a check whether this instance can persistData
        /*
        If Raft can persist it will reply with canPersist:true and RaftClusterState as null.
        If Raft cannot persist it will reply with canPersist:false and RaftClusterState with required info.
         */
        if (!isLeader() && !isLeaderActive()) {
            RaftClusterState state = new RaftClusterState(isLeader(), isLeaderActive(),
                    getLeaderId(), getLeader());
            getSender().tell(new PersistDataReply(false, state), getSelf());
        } else {
            getSender().tell(new PersistDataReply(true, null), getSelf());
        }
     ...
     */
    private void applyState(ActorRef clientActor, Identifier identifier, Payload data) {
        org.opendaylight.controller.cluster.raft.client.messages.ApplyState applyState =
                new org.opendaylight.controller.cluster.raft.client.messages.ApplyState(
                        clientActor, identifier, data);
        // TODO: Do we need an ACK here from the client that it has applied it to its state?
        getContext().parent().tell(applyState, getSelf());
    }
     ...
     */
    @Nonnull
    private RaftActorRecoveryCohort getRaftActorRecoveryCohort() {
        return new RaftActorRecoveryCohortImpl(getContext().parent());
    }

    // TODO: Send a message to the client for recovery complete.
    /**
     * This method is called when recovery is complete.
     */
    private void onRecoveryComplete() {
        getContext().parent().tell(RecoveryComplete.INSTANCE, getSelf());
    }

    // TODO: Decide whether you even need this!
    /**
     * Returns the RaftActorSnapshotCohort to participate in persistence recovery.
     */
    @Nonnull
    private RaftActorSnapshotCohort getRaftActorSnapshotCohort() {
        return new RaftActorSnapshotCohortImpl(getContext().parent());
    }
    ...
    private void onStateChanged() {
        RaftClusterState state = new RaftClusterState(
                isLeader(), isLeaderActive(), getLeaderId(), getLeader());
        getContext().parent().tell(new RaftClusterStateChanged(state), getSelf());
    }

    // TODO: Ask the client for RoleChangeNotifier!
    /**
    ...
    // TODO: Ask the client for a Runnable to run when the leader is paused!
    /**
    ...
    private void onLeaderChanged(String oldLeader, String newLeader) {
        RaftClusterState state = new RaftClusterState(
                isLeader(), isLeaderActive(), newLeader, getLeader());
        getContext().parent().tell(new RaftClusterStateChanged(state), getSelf());
    }
    ...
    // TODO: Keep this method contained! Client doesn't need to know.
    ...
    private static class RaftActorRecoveryCohortImpl implements RaftActorRecoveryCohort {

        private static final Logger LOG = LoggerFactory.getLogger(RaftActorRecoveryCohortImpl.class);

        private final ActorRef clientActor;
        private List<Payload> payloads = null;
        private int payloadIdx = 0;

        public RaftActorRecoveryCohortImpl(ActorRef clientActor) {
            this.clientActor = clientActor;
        }

        @Override
        public void startLogRecoveryBatch(int maxBatchSize) {
            payloads = new ArrayList<>(maxBatchSize);
        }

        @Override
        public void appendRecoveredLogEntry(Payload data) {
            payloads.add(payloadIdx, data);
            ++payloadIdx;
        }

        @Override
        public void applyRecoverySnapshot(byte[] snapshotBytes) {
            // TODO: Evaluate Whether we require this or not.
        }

        @Override
        public void applyCurrentLogRecoveryBatch() {
            Future askFuture = Patterns.ask(clientActor,
                    new ApplyLogRecoveryBatchRequest(payloads),
                    RAFT_ASK_TIMEOUT.toMillis());
            try {
                Await.ready(askFuture, RAFT_ASK_TIMEOUT);
            } catch (InterruptedException e) {
                LOG.error("InterruptedException caught while applying log recovery batch: {}",
                        e.getMessage());
            } catch (TimeoutException e) {
                LOG.error("TimeoutException caught while applying log recovery batch: {}",
                        e.getMessage());
            }
            payloads.clear();
            payloads = null;
    ...
    private static class RaftActorSnapshotCohortImpl implements RaftActorSnapshotCohort {

        private final ActorRef clientActor;

        public RaftActorSnapshotCohortImpl(ActorRef clientActor) {
            this.clientActor = clientActor;
        }

        @Override
        public void createSnapshot(ActorRef actorRef) {
            clientActor.tell(new CreateSnapshotRequest(actorRef), null);
        }

        @Override
        public void applySnapshot(byte[] snapshotBytes) {
            clientActor.tell(new ApplySnapshot(snapshotBytes), null);
        }
    }
}
