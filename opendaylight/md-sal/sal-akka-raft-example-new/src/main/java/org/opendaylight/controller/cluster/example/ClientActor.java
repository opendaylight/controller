...
public class ClientActor extends UntypedActor {
    ...
    @Override public void onReceive(Object message) throws Exception {
        ...
        } else if (message instanceof BulkKeyValue) {
            BulkKeyValue bkv = (BulkKeyValue) message;
            for (int i = bkv.getRangeStart(); i <= bkv.getRangeEnd(); i++) {
                String idx = Integer.toString(i);
                target.tell(new KeyValue("key-" + idx, "val-" + idx), getSelf());
            }
        } else if(message instanceof KeyValueSaved) {
            System.out.println("KeyValue saved from: " + target);
        }
    }
}
