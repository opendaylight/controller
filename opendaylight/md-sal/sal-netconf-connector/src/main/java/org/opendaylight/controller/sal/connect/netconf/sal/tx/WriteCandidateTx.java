 ...
 * <ul>
 * <li/> If any issue occurs during edit, datastore is discarded using discard-changes rpc, unlocked and an exception is thrown
 * </ul>
 * <li/> Commit and Unlock candidate datastore on tx commit
 * </ol>
 */
public class WriteCandidateTx extends AbstractWriteTx {
    ...
    private void discardChanges() {
        try {
            invokeBlocking("Discarding candidate", new Function<NetconfBaseOps, ListenableFuture<RpcResult<CompositeNode>>>() {
                @Override
                public ListenableFuture<RpcResult<CompositeNode>> apply(final NetconfBaseOps input) {
                    return input.discardChanges(new NetconfRpcFutureCallback("Discarding candidate", id));
                }
            });
        } catch (final NetconfDocumentedException e) {
            LOG.warn("{}: Failed to discard changes. Failed to cleanup after successful transaction", id, e);
            throw new RuntimeException(id + ": Failed to discard changes. Failed to cleanup after successful transaction");
        }
    }
    ...
    protected void cleanupOnSuccess() {
        try {
            ...
            LOG.warn("{}: Failed to unlock candidate. Failed to cleanup after successful transaction", id);
            throw new RuntimeException(id + ": Failed to initialize netconf transaction (lock candidate)");
        }
    }
    ...
    private void unlock() throws NetconfDocumentedException {
        try {
            invokeBlocking("Unlocking candidate", new Function<NetconfBaseOps, ListenableFuture<RpcResult<CompositeNode>>>() {
                @Override
                public ListenableFuture<RpcResult<CompositeNode>> apply(final NetconfBaseOps input) {
                    return input.unlockCandidate(new NetconfRpcFutureCallback("Unlock candidate", id));
                }
            });
        } catch (final NetconfDocumentedException e) {
            LOG.warn("{}: Failed to unlock candidate", e);
            throw e;
        }
    }
