...
public final class NetconfDeviceSchemaContextProvider implements SchemaContextProvider {
    ...
    public static SchemaContextProvider createRemoteSchemaContext(final RemoteDeviceId deviceId, final Iterable<QName> capabilities,
                                     final SchemaSourceProvider<InputStream> sourceProvider) {
        ...
        if (sourceContext.getMissingSources().isEmpty() == false) {
            logger.warn("{}: Sources for following models are missing {}", deviceId,
                    sourceContext.getMissingSources());
        }

        logger.debug("{}: Trying to create schema context from {}", deviceId, sourceContext.getValidSources());
        final List<InputStream> modelsToParse = YangSourceContext.getValidInputStreams(sourceContext);

        Preconditions.checkState(sourceContext.getValidSources().isEmpty() == false, "%s: Unable to create schema context, no sources provided by device");
        try {
            final SchemaContext schemaContext = tryToParseContext(modelsToParse);
            logger.debug("{}: Schema context successfully created.", deviceId);
            return new NetconfDeviceSchemaContextProvider(schemaContext);
        } catch (final Exception e) {
            logger.error("{}: Unable to create schema context, unexpected parsing error", deviceId, e);
