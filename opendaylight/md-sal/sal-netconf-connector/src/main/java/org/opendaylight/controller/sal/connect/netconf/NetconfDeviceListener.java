...
class NetconfDeviceListener implements NetconfClientSessionListener {
    ...
    private void dispatchRequest() {
        while (!requests.isEmpty()) {
            ...
            if (!r.future.setUncancellable()) {
                requests.poll();
                continue;
            }

            final ChannelFuture f = session.sendMessage(r.request);
            f.addListener(new FutureListener<Void>() {
                @Override
                public void operationComplete(final Future<Void> future) throws Exception {
                    if (future.isSuccess()) {
                        LOG.trace("Finished sending request {}", r.request);
                    } else {
                        // We expect that a session down will occur at this point
                        LOG.debug("Failed to send request {}", r.request, future.cause());
                    }
                }
            });
            break;
    ...
    private synchronized void processMessage(final NetconfMessage message) {
        // FIXME: finish this: lookup the first executing message and pair it
    }

    synchronized ListenableFuture<RpcResult<CompositeNode>> sendRequest(final NetconfMessage message) {
        final Request req = new Request(new UncancellableFuture<RpcResult<CompositeNode>>(), message);
        requests.add(req);
        if (session != null && requests.peek() == req) {
            dispatchRequest();
        }
        return req.future;
    }
