        ...
        return Futures.transform(future, new Function<RpcResult<CompositeNode>, Optional<NormalizedNode<?, ?>>>() {
            ...
            public Optional<NormalizedNode<?, ?>> apply(final RpcResult<CompositeNode> result) {
                ...
                return data == null ?
                        ...
                        Optional.<NormalizedNode<?, ?>>of(transform(path, node));
            }
        });
    }

    private NormalizedNode<?, ?> transform(final InstanceIdentifier path, final CompositeNode node) {
        try {
            return normalizer.toNormalized(path, node).getValue();
