...
public final class NetconfStateSchemas {
    ...
    static {
        final ContainerNode filter = NetconfMessageTransformUtil.toFilterStructure(STATE_SCHEMAS_IDENTIFIER);
        GET_SCHEMAS_RPC
    ...
    private static NetconfStateSchemas create(final NetconfDeviceRpc deviceRpc, final NetconfSessionPreferences remoteSessionCapabilities, final RemoteDeviceId id) {
        ...
        final Optional<? extends NormalizedNode<?, ?>> schemasNode =
                NetconfMessageTransformUtil.findNode(schemasNodeResult.getResult(), DATA_STATE_SCHEMAS_IDENTIFIER);
    ...
    protected static NetconfStateSchemas create(final RemoteDeviceId id, final ContainerNode schemasNode) {
        ...
        Preconditions.checkState(child.isPresent(), "Unable to find container: %s in response: %s", Schema.QNAME.withoutRevision(), schemasNode);
