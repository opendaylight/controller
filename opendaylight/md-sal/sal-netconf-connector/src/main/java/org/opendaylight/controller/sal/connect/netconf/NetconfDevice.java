...
public final class NetconfDevice implements AutoCloseable, RemoteDevice<NetconfSessionCapabilities, NetconfMessage>,
    ...
    private final RemoteDeviceSalFacade salFacade;
    ...
    public static NetconfDevice createNetconfDevice(final RemoteDeviceId id,
            ...
            final ExecutorService executor, final RemoteDeviceSalFacade salFacade) {
        return new NetconfDevice(id, salFacade, schemaSourceProvider, executor, new NetconfMessageTransformer());
    }

    @VisibleForTesting
    protected NetconfDevice(final RemoteDeviceId id, final RemoteDeviceSalFacade salFacade,
            final AbstractCachingSchemaSourceProvider<String, InputStream> schemaSourceProvider,
            final ExecutorService processingExecutor, final MessageTransformer initialMessageTransformer) {
        this.id = id;
    ...
    public void onRemoteSessionInitialized(final NetconfSessionCapabilities remoteSessionCapabilities,
        ...
        final ListenableFuture<?> salInitializationFuture = processingExecutor.submit(new Runnable() {
            ...
            public void run() {
                ...
                updateMessageTransformer(schemaContextProvider, deviceRpc);
     ...
     * Also update Rpc implementation, to use schema aware transformer from now on
     */
    private void updateMessageTransformer(final SchemaContextProvider schemaContextProvider, final NetconfDeviceRpc deviceRpc) {
