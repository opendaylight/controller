...
public final class NetconfDevice implements RemoteDevice<NetconfSessionCapabilities, NetconfMessage> {
    ...
    private static class DeviceSourcesResolver implements Callable<DeviceSources> {
        ...
        public DeviceSources call() throws Exception {
            ...
            final Collection<SourceIdentifier> requiredSources = Collections2.transform(
                    remoteSessionCapabilities.getModuleBasedCaps(), QNAME_TO_SOURCE_ID_FUNCTION);

            // If monitoring is not supported, we will still attempt to create
            // schema, sources might be already provided
            ...
            final Collection<SourceIdentifier> providedSources = Collections2.transform(
                    availableSchemas.getAvailableYangSchemasQNames(), QNAME_TO_SOURCE_ID_FUNCTION);

            final Set<QName> requiredSourcesNotProvided = Sets.difference(
                    remoteSessionCapabilities.getModuleBasedCaps(), availableSchemas.getAvailableYangSchemasQNames());
    ...
    private static final class DeviceSources {
        ...
        private Collection<SourceIdentifier> remove(final Collection<SourceIdentifier> requiredSources, final SourceIdentifier sId) {
            final ArrayList<SourceIdentifier> sourceIdentifiers = Lists.newArrayList(requiredSources);
            sourceIdentifiers.remove(sId);
