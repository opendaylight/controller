...
package org.opendaylight.controller.karaf.it.base;
...
public abstract class AbstractKarafTestBase {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(AbstractKarafTestBase.class);
     ...
     */
    private static final String PROPERTIES_FILENAME = "abstractkaraftestbase.properties";
    ...
    protected Option getLoggingOption() {
        Option option = editConfigurationFilePut(ORG_OPS4J_PAX_LOGGING_CFG,
                        logConfiguration(AbstractKarafTestBase.class),
                        LogLevel.INFO.name());
        return option;
    }
    ...
    protected String getKarafDistro() {
        ...
        if (version == null) {
            ...
            try (InputStream testPropsInputStream = Thread.currentThread().getContextClassLoader()
                    .getResourceAsStream(PROPERTIES_FILENAME)) {
                testProps.load(testPropsInputStream);
            } catch (IOException e) {
                LOG.error("Unable to load {} to determine the Karaf version", PROPERTIES_FILENAME, e);
            }
            version = testProps.getProperty(KARAF_DISTRO_VERSION_PROP);
        }
    ...
    /**
     * Provide config options for PaxExam.
