 ...
 * @author Robert Varga
 */
final class ActionProviderBean {
    ...
    private void registerFallback(final Class<RpcService> interfaceClass) {
        final Collection<SchemaPath> paths = RpcUtil.decomposeRpcService(interfaceClass, null,
            RpcRoutingStrategy::isContextBasedRouted);
        if (paths.isEmpty()) {
