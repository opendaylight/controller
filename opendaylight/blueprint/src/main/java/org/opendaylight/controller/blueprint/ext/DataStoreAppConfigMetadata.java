...
public class DataStoreAppConfigMetadata implements DependentComponentFactoryMetadata {
    ...
    private void restartContainer() {
        try {
            ServiceReference<BlueprintContainerRestartService> serviceReference =
                    container.getBundleContext().getServiceReference(BlueprintContainerRestartService.class);
            if(serviceReference == null) {
                // This shouldn't happen as the BlueprintContainerRestartService is always registered by
                // the blueprint bundle and the bundle has a low start-level.
                LOG.warn("{}: BlueprintContainerRestartService reference not found", id);
                return;
            }

            BlueprintContainerRestartService restartService = (BlueprintContainerRestartService)
                    container.getService(serviceReference);
            if(restartService == null) {
                // This could happen on shutdown if the service was already unregistered so we log as debug.
                LOG.debug("{}: BlueprintContainerRestartService was not found", id);
                return;
            }
            ...
            // log as debug.
            LOG.debug("{}: Error obtaining BlueprintContainerRestartService", id, e);
        }
    }
