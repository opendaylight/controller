...
public class DataStoreAppConfigMetadata extends AbstractDependentComponentFactoryMetadata {
    ...
    private static final String DEFAULT_APP_CONFIG_FILE_PATH = "./etc/opendaylight/karaf";
    ...
    private NormalizedNode<?, ?> parsePossibleDefaultAppConfigXMLFile(SchemaContext schemaContext,
        ...
        if(appConfigFileName == null) {
            appConfigFileName = bindingContext.appConfigBindingClass.getPackage().getName() +
                "." + bindingContext.bindingQName.getLocalName() + ".xml";
        }
        ...
        try(FileInputStream fis = new FileInputStream(appConfigFile)) {
            ...
            setFailureMessage(String.format("%s: Cold not read/parse app config file %s", logName(), appConfigFile));
        }
