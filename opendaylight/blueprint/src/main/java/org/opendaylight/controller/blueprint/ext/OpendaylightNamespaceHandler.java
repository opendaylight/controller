...
//            serviceRef.getExtendedFilter().getStringValue();
        ...
        // FIXME - I'd rather set the extended filter b/c it can be substituted via cm-property-placeholder
        // but there's there's a bug in AbstractServiceReferenceRecipe wrt the extended filter
        // (https://issues.apache.org/jira/browse/ARIES-1507).
        //serviceRef.setExtendedFilter(createValue(context, filter));

        serviceRef.setFilter(filter);
        return component;
    }
