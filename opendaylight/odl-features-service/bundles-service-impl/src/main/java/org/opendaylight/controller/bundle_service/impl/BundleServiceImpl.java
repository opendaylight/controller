...
public class BundleServiceImpl implements BundleService, AutoCloseable, DataChangeListener{
    ...
    public BundleServiceImpl(DataBroker dataProvider, RpcProviderRegistry rpcRegistry){
        ...
        if (this.dataProvider != null){
            ReadTransaction ro = this.dataProvider.newReadOnlyTransaction();

            InstanceIdentifier<Bundle> TEST_CONFIG_ID =
                    InstanceIdentifier.builder(Bundle.class).build();

            System.out.println(ro.read(LogicalDatastoreType.OPERATIONAL,TEST_CONFIG_ID));

            WriteTransaction tx = this.dataProvider.newWriteOnlyTransaction();
            writeToOperational(tx);
...
//                    new NodesListener(), DataChangeScope.SUBTREE);
    ...
    private void writeToOperational(WriteTransaction tx){
        short count = 0;
        while (true){
            try {
                if (count == 4) {
                    ...
                    tx.put(LogicalDatastoreType.OPERATIONAL,
                            BUNDLE_CONFIG_ID,
                            new BundleBuilder().setBundleServiceStatus(
                                    BundleServiceStatus.Available).build());
                }
            }
            catch (com.google.common.util.concurrent.UncheckedExecutionException e){
                System.out.println("com.google.common.util.concurrent.UncheckedExecutionException");
                LOG.debug("com.google.common.util.concurrent.UncheckedExecutionException while executing write only transaction.");
                LOG.debug("retrying write only transaction");
                count++;
            }
        }
    }
    ...
    private class InstallBundlesTask implements Callable<Void> {
        ...
        public Void call() {
            try {
                ...
                LOG.error("Error install bundle: "
