...
public class FlowConsumerImpl {
	...
	private void allocateCaches() {
		...
		try {
			clusterContainerService.createCache("frm.originalSwView", EnumSet.of(IClusterServices.cacheMode.TRANSACTIONAL));
			clusterContainerService.createCache("frm.installedSwView", EnumSet.of(IClusterServices.cacheMode.TRANSACTIONAL));
			clusterContainerService.createCache("frm.staticFlows", EnumSet.of(IClusterServices.cacheMode.TRANSACTIONAL));
			clusterContainerService.createCache("frm.staticFlowsOrdinal", EnumSet.of(IClusterServices.cacheMode.TRANSACTIONAL));
			clusterContainerService.createCache("frm.inactiveFlows", EnumSet.of(IClusterServices.cacheMode.TRANSACTIONAL));
	...
	private final class internalTransaction implements DataCommitTransaction<InstanceIdentifier<?>, DataObject> {
		...
		public  Status validate(IContainer container, NodeFlow dataObject) {
			...
			if (dataObject.getFlowName() == null || dataObject.getFlowName().trim().isEmpty() || !dataObject.getFlowName().matches(NAMEREGEX)) {
		...
		private boolean flowEntryExists(NodeFlow config) {
			...
			for (ConcurrentMap.Entry<FlowKey, Flow> entry : originalSwView.entrySet()) {
				if (entry.getValue().getFlowName().equals(config.getFlowName()) && entry.getValue().getTableId().equals(config.getTableId())) {
					return true;
				}
			}
			return false;
