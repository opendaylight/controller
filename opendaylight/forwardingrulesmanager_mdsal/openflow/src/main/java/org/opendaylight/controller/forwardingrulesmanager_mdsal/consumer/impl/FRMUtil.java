...
public class FRMUtil {
	...
	public static enum operation {ADD, DELETE, UPDATE, GET};
	...
	public static boolean validateMatch(NodeFlow flow)
	{
		...
		if (match != null)
		{
			...
			{
				if ((ethernetmatch.getEthernetSource() != null) && !isL2AddressValid(ethernetmatch.getEthernetSource().toString())) {

					logger.error("Ethernet source address %s is not valid. Example: 00:05:b9:7c:81:5f", ethernetmatch.getEthernetSource());
					return false;
				}

				if ((ethernetmatch.getEthernetDestination() != null) && !isL2AddressValid(ethernetmatch.getEthernetDestination().toString())) {
					logger.error("Ethernet destination address %s is not valid. Example: 00:05:b9:7c:81:5f", ethernetmatch.getEthernetDestination());
					return false;
				}
	...
	public static boolean validateActions (List<org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Actions> actions)
	{
		...
		for (org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Actions curaction : actions)
		{
			org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.Action action = curaction.getAction();
	...
	{
		List<org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction> instructionsList = new ArrayList<>();
		Instructions instructions = flow.getInstructions();
		instructionsList = instructions.getInstruction();

		for (org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.instruction.list.Instruction instruction : instructionsList) {
			...
			if (curInstruction instanceof GoToTable) {
				...
				List<org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Actions> action = ((WriteActions) curInstruction).getActions();
				validateActions(action);

			}

			else if (curInstruction instanceof ApplyActions) {
				List<org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Actions> action = ((ApplyActions) curInstruction).getActions();
				validateActions(action);
			}

			else if (curInstruction instanceof ClearActions) {
				List<org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Actions> action = ((ClearActions) curInstruction).getActions();
				validateActions(action);
			}
