...
public class ContainerFlowConfig implements Serializable {
    ...
    private Status validateIPs() {
        if(nwSrc == null || nwSrc.trim().isEmpty()) {
            return new Status(StatusCode.BADREQUEST, "Empty network source address");
        }
        ...
        if(nwDst == null || nwDst.trim().isEmpty()) {
            return new Status(StatusCode.BADREQUEST, "Empty network destination address");
        }
