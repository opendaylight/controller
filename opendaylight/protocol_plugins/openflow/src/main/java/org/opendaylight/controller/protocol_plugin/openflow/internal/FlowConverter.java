...
public class FlowConverter {
    ...
    public FlowConverter(OFMappingContext context,OFMatch ofMatch, List<OFAction> actionsList) {
    ...
    public List<OFAction> createOFActions() {
   		List<Action> salActions = flow.getActions();
    		List<OFAction> openflowActions =new ArrayList<OFAction>(salActions.size());
    		
    		for (Action action : salActions) {
				SALActionMapper<? extends Action> mapper = mapping.getMapperForSalAction(action.getClass());
				if(mapper == null) {
					throw new IllegalStateException("Openflow mapping for " + action.getClass() + "is not defined.");
				}
        ...
        if (this.flow == null) {
            ...
            if (actionsList == null) {
                ...
                for (OFAction ofAction : actionsList) {
                	...
                	OFActionMapper<? extends OFAction> mapper = mapping.getMapperForOFAction(ofClass);
