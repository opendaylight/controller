...
public class FlowConverter {
    ...
    private OFTransformationContext mapping;

    public FlowConverter(OFTransformationContext context, OFMatch ofMatch,
    ...
    public FlowConverter(OFTransformationContext context, Flow flow) {
        this.mapping = context;
        this.ofMatch = null;
    ...
    private List<OFAction> createOFActions() {
        List<Action> salActions = flow.getActions();
        List<OFAction> openflowActions = new ArrayList<OFAction>(
                salActions.size());

        SALToOFActionTransformer<Action> mapper = mapping
                .getSalActionTransformer();
        for (Action action : salActions) {
            OFAction ofAction = mapper.transform(action);
        ...
        if (this.flow == null) {
            ...
            if (actionsList == null) {
                ...
                for (OFAction ofAction : actionsList) {
                    ...
                    OFActionToSalTransformer<? extends OFAction> mapper = mapping
                            .getOFActionTransformer(ofClass);
