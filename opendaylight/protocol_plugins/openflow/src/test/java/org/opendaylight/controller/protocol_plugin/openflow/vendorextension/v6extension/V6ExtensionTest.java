...
public class V6ExtensionTest {
	...
	public void testFromString() throws UnknownHostException{
		//This tests creating V6Match using fromString and OFMatch by comparing the results to each other
		...
		//V6Match is meant for IPv6, but if using OFMatch, it will be set to IPv4 values, as OF1.0 doesn't support IPv6.
		...
		Assert.assertTrue(Arrays.equals(match.getDataLayerSource(),match3.getDataLayerSource()));
		Assert.assertTrue(Arrays.equals(match.getDataLayerDestination(),match3.getDataLayerDestination()));
		Assert.assertTrue(match.getNetworkSrc().equals(match3.getNetworkSrc()));
		Assert.assertTrue(match.getNetworkDest().equals(match3.getNetworkDest()));
		Assert.assertTrue(match.getDataLayerVirtualLan()==match3.getDataLayerVirtualLan());
		Assert.assertTrue(match.getDataLayerVirtualLanPriorityCodePoint()==match3.getDataLayerVirtualLanPriorityCodePoint());
		Assert.assertTrue(match.getNetworkProtocol()==match3.getNetworkProtocol());
		Assert.assertTrue(match.getNetworkTypeOfService()==match3.getNetworkTypeOfService());
		Assert.assertTrue(match.getTransportSource()==match3.getTransportSource());
		Assert.assertTrue(match.getTransportDestination()==match3.getTransportDestination());
	...
	public void testReadWriteBuffer(){
		...
		Assert.assertTrue(Arrays.equals(match.getDataLayerSource(),match2.getDataLayerSource()));
		Assert.assertTrue(Arrays.equals(match.getDataLayerDestination(),match2.getDataLayerDestination()));
		
		Assert.assertTrue(match.getNetworkSrc().equals(match2.getNetworkSrc()));
		Assert.assertTrue(match.getNetworkDest().equals(match2.getNetworkDest()));
		
		Assert.assertTrue(match.getDataLayerVirtualLan()==match2.getDataLayerVirtualLan());
		//vlan pcp isn't part of write/read buffer
		Assert.assertTrue(match.getNetworkProtocol()==match2.getNetworkProtocol());
		Assert.assertTrue(match.getNetworkTypeOfService()==match2.getNetworkTypeOfService());
		Assert.assertTrue(match.getTransportSource()==match2.getTransportSource());
		Assert.assertTrue(match.getTransportDestination()==match2.getTransportDestination());
	...
	public void testClone(){
		...
		Assert.assertTrue(match.getNetworkDest().equals(match2.getNetworkDest()));
		Assert.assertTrue(match.getDataLayerVirtualLan()==match2.getDataLayerVirtualLan());
		Assert.assertTrue(match.getDataLayerVirtualLanPriorityCodePoint()==match2.getDataLayerVirtualLanPriorityCodePoint());
		Assert.assertTrue(match.getNetworkProtocol()==match2.getNetworkProtocol());
		Assert.assertTrue(match.getNetworkTypeOfService()==match2.getNetworkTypeOfService());
		Assert.assertTrue(match.getTransportSource()==match2.getTransportSource());
		Assert.assertTrue(match.getTransportDestination()==match2.getTransportDestination());
