...
IF "%~1" NEQ "" (
    ...
    IF "!CARG!"=="-help" (
        REM ECHO "Usage: %0 [-jmx] [-jmxport <num>] [-debug] [-debugsuspend] [-debugport <num>] [-start] [-consoleport <num>]] [-stop] [-status] [-console] [-help] [<other args will automatically be used for the JVM>]"
        REM ECHO Note: Enclose any JVM or System properties within double quotes.
        SHIFT
        SET CARG=%2
        IF "!CARG!" NEQ "" (
             CALL:!CARG!
...
ECHO. agentpath        ^[-agentpath:^<path to lib^>^]
ECHO.
ECHO.    Set path for start controller.
EXIT /B 1
