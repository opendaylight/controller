...
public class FileStorageAdapterTest {
    ...
    public void testWithFeatures() throws Exception {
        ...
        final FeaturesService mock = mock(FeaturesService.class);
        Feature f1 = mock(Feature.class);
        doReturn("f1-11").when(f1).getId();
        Feature f2 = mock(Feature.class);
        doReturn("f2-22").when(f2).getId();

        doReturn(new Feature[]{f1, f2}).when(mock).listInstalledFeatures();
