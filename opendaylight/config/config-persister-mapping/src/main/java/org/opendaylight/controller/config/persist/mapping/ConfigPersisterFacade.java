...
public class ConfigPersisterFacade {

    private static final Logger LOG = LoggerFactory.getLogger(ConfigPersisterFacade.class);
    private final YangStoreService yangStoreService;
    private final TransactionProvider transactionProvider;
    private final ConfigRegistryClient configRegistryClient;

    public ConfigPersisterFacade(ConfigRegistryClient configRegistryClient, YangStoreService yangStoreService, TransactionProvider transactionProvider) {
    ...
    public void executeConfigExecution(ConfigExecution configExecution) throws DocumentedException, ValidationException {
        //TODO do we have to synchronize here? or at all?
        if (configExecution.shouldTest()) {
            executeTests(configRegistryClient, configExecution);
        }

        if (configExecution.shouldSet()) {
            executeSet(configRegistryClient, configExecution);
        }
    }

    public void commitTransaction() throws DocumentedException, ValidationException, ConflictingVersionException {
    ...
    private void executeTests(ConfigRegistryClient configRegistryClient,
                              ConfigExecution configExecution) throws DocumentedException, ValidationException {
