...
/**
 * Provides ObjectName creation. Each created ObjectName consists of
 * domain that is defined as {@link #ON_DOMAIN} and at least one
 * key-value pair. The only mandatory property is {@link #TYPE_KEY}.
 * All transaction related mbeans have {@link #TRANSACTION_NAME_KEY}
 * property set.
 *
 */
@ThreadSafe
public class ObjectNameUtil {
	...
	private static Map<String, String> createModuleON(String moduleName, String instanceName) {
		Map<String, String> onParams = new HashMap<>();
	...
	public static ObjectName withoutTransactionName(ObjectName on) {
		if (getTransactionName(on) == null) {
			throw new IllegalArgumentException("Expected ObjectName with transaction:" + on);
		}
		if (ON_DOMAIN.equals(on.getDomain()) == false) {
			throw new IllegalArgumentException("Expected different domain: " + on);
		}
	...
	private static Set<String> blacklist = new HashSet<>(Arrays.asList(MODULE_FACTORY_NAME_KEY, INSTANCE_NAME_KEY, TYPE_KEY));
	...
	public static ObjectName createModulePattern(String moduleName, String instanceName) {
		...
		ObjectName namePattern = ObjectNameUtil
				.createON(ObjectNameUtil.ON_DOMAIN + ":"
						...
						+ "," + ObjectNameUtil.INSTANCE_NAME_KEY + "="
						+ instanceName);
		return namePattern;
	}
	...
	public static ObjectName createModulePattern(String ifcName, String instanceName, String transactionName) {
		return ObjectNameUtil.createON(ObjectNameUtil.ON_DOMAIN + ":type=Module,"
				+ ObjectNameUtil.MODULE_FACTORY_NAME_KEY + "=" + ifcName + "," + ObjectNameUtil.INSTANCE_NAME_KEY + "="
				+ instanceName + "," + ObjectNameUtil.TRANSACTION_NAME_KEY + "=" + transactionName);
	}
