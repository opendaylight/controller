...
/**
 * This exception is not intended to be used while implementing modules, itaggregates
 * validation exceptions and sends them back to the user.
 */
public class ValidationException extends RuntimeException {
	...
	public ValidationException(Map<String /* module name */, Map<String /* instance name */,
	...
	public static ValidationException createFromCollectedValidationExceptions(List<ValidationException> collectedExceptions){
		...
		for(ValidationException ve: collectedExceptions){
			for (Entry<String, Map<String, ExceptionMessageWithStackTrace>> outerEntry: ve.getFailedValidations().entrySet()){
				for(Entry<String, ExceptionMessageWithStackTrace> innerEntry: outerEntry.getValue().entrySet()){
					...
					Map<String, ExceptionMessageWithStackTrace> instanceToExMap = failedValidations.get(moduleName);
	...
	public static ValidationException createForSingleException(ModuleIdentifier moduleIdentifier,
			Exception e) {
		Map<String, Map<String, ExceptionMessageWithStackTrace>> failedValidations = new HashMap<>();
	...
	public static class ExceptionMessageWithStackTrace {
		...
		public String toString() {
			return "ExceptionMessageWithStackTrace [message=" + message + ", stackTrace=" + stackTrace + "]";
		}
