...
/**
 * This is a JMX bean representing current transaction. It contains {@link #transactionIdentifier}, unique version and parent
 * version for optimistic locking.
 */
class ConfigTransactionControllerImpl implements ConfigTransactionControllerInternal,
		ConfigTransactionControllerImplMXBean, NamedObject<TransactionIdentifier> {
	...
	/**
	 * Disables ability of {@link DynamicWritableWrapper} to change attributes during validation.
	 */
	@GuardedBy("this")
	private final AtomicBoolean configBeanModificationDisabled = new AtomicBoolean(false);
	private final ReadOnlyAtomicBoolean readOnlyAtomicBoolean = new ReadOnlyAtomicBooleanImpl(configBeanModificationDisabled);
    private final MBeanServer configMBeanServer;


	public ConfigTransactionControllerImpl(String transactionName, TransactionJMXRegistrator transactionRegistrator,
			long parentVersion, long currentVersion, List<? extends ModuleFactory> currentlyRegisteredFactories,
		...
		txModuleJMXRegistrator = transactionRegistrator.createTransactionModuleJMXRegistrator();
	...
	@Override
	public synchronized void copyExistingModule(ModuleInternalInfo oldConfigBeanInfo) throws InstanceAlreadyExistsException {
		...
		ModuleFactory moduleFactory = factoriesHolder.findByModuleName(moduleIdentifier.getFactoryName());
		...
		try {
			...
			throw new IllegalStateException(format("Error while copying old configuration from %s to %s",
	...
	@Override
	public synchronized ObjectName createModule(String factoryName, String instanceName) throws
		...
		return putConfigBeanToJMXAndInternalMaps(moduleIdentifier, module, moduleFactory, null);
	}

	private synchronized ObjectName putConfigBeanToJMXAndInternalMaps(ModuleIdentifier moduleIdentifier, Module module,
		...
		DynamicMBean writableDynamicWrapper = new DynamicWritableWrapper(module, moduleIdentifier,
				transactionIdentifier, readOnlyAtomicBoolean, transactionsMBeanServer, configMBeanServer);

		ObjectName writableON = ObjectNameUtil.createTransactionModuleON(transactionIdentifier.getName(), moduleIdentifier);
		// put wrapper to jmx
		TransactionModuleJMXRegistration transactionModuleJMXRegistration = txModuleJMXRegistrator.registerMBean
				(writableDynamicWrapper, writableON);
		ModuleInternalTransactionalInfo moduleInternalTransactionalInfo = new ModuleInternalTransactionalInfo(moduleIdentifier,
				module, moduleFactory, maybeOldConfigBeanInfo, transactionModuleJMXRegistration);
	...
	public void destroyModule(ObjectName objectName) throws InstanceNotFoundException {
		...
		ModuleInternalTransactionalInfo removedTInfo = dependencyResolverManager.destroyModule
				(moduleIdentifier);
		// remove from jmx
	...
	private void validate_noLocks() throws ValidationException {
		...
		for (Entry<ModuleIdentifier, Module> entry : dependencyResolverManager.getAllModules().entrySet()) {
			...
			try {
				...
				logger.warn("Validation exception in {}", getTransactionName(), e);
	...
	/**
	 * If this method passes validation, it will grab {@link TransactionStatus#secondPhaseCommitStarted} lock. This
	 * lock will prevent
	 * calling @{link #validateBeforeCommitAndLockTransaction}, effectively only allowing to call
	 * {@link #secondPhaseCommit} after successful return of this method.
	 */
	@Override
	public synchronized CommitInfo validateBeforeCommitAndLockTransaction() throws ValidationException {
		...
		// errors in this state are not recoverable. modules are not mutable anymore.
	...
	public synchronized List<ModuleIdentifier> secondPhaseCommit() {
		...
		if (configBeanModificationDisabled.get() == false) {
			throw new IllegalStateException("Internal error - validateBeforeCommitAndLockTransaction should be called " +
					"to obtain a lock");
		}
		...
		for (Entry<ModuleIdentifier, Module> entry : dependencyResolverManager.getAllModules().entrySet()) {
			...
			ModuleIdentifier name = entry.getKey();
			try {
				logger.debug("About to commit {} in transaction {}", transactionIdentifier, name);
				module.getInstance();
			} catch (Exception e) {
				logger.error("Commit failed on {} in transaction {}", name, transactionIdentifier, e);
				internalAbort();
				throw new RuntimeException(format("Error - getInstance() failed for %s in transaction %s", name,
	...
	public Set<ObjectName> lookupConfigBeans(String moduleName, String instanceName) {
		ObjectName namePattern = ObjectNameUtil.createModulePattern(moduleName, instanceName, transactionIdentifier.getName());
