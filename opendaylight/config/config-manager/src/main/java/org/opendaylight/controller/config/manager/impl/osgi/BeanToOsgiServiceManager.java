...
/**
 * Registers instantiated beans as OSGi services and unregisters these services if beans are destroyed.
 */
public class BeanToOsgiServiceManager {
	...
	/**
	 * To be called for every created, reconfigured and recreated config bean. It is expected that before using this
	 * method OSGi service registry will be cleaned from previous registrations.
	 */
	public OsgiRegistration registerToOsgi(Class<? extends Module> configBeanClass,
									AutoCloseable instance, ModuleIdentifier moduleIdentifier) {
		try {
			final Set<Class<?>> configuresInterfaces = InterfacesHelper.getOsgiRegistrationTypes(configBeanClass);
			...
			if (configuresInterfaces.isEmpty() == false) {
				...
				final ServiceRegistration<?> serviceRegistration = bundleContext.registerService(
						classesToNames(configuresInterfaces), instance, propertiesForOsgi);
				return new OsgiRegistration(serviceRegistration);
			} else {
				return new OsgiRegistration();
			}
		} catch (IllegalStateException e) {
			throw new IllegalStateException("Error while registering instance into OSGi Service Registry: " +
					moduleIdentifier, e);
		}
	}
	...
	private void checkInstanceImplementing(AutoCloseable instance, Set<Class<?>> configures) {
		...
		if (missing.isEmpty() == false) {
			throw new IllegalStateException(instance.getClass() +
					" does not implement following interfaces as announced by " + ServiceInterfaceAnnotation.class.getName() + " annotation :" +
					missing);
		}
	}
