...
/**
 * Holds information about modules being created and destroyed within this transaction. Observes usage of
 * DependencyResolver within modules to figure out dependency tree.
 */
public class DependencyResolverManager implements TransactionHolder {
	...
	public DependencyResolverManager(String transactionName, TransactionStatus transactionStatus) {
	...
	public synchronized DependencyResolverImpl getOrCreate(ModuleIdentifier name){
		...
		if (dependencyResolver == null){
			...
			dependencyResolver = new DependencyResolverImpl(name, transactionStatus, modulesHolder);
	...
	public ModuleInternalTransactionalInfo destroyModule(ModuleIdentifier moduleIdentifier) {
		transactionStatus.checkNotCommitted();
		ModuleInternalTransactionalInfo found = modulesHolder.destroyModule
				(moduleIdentifier);
