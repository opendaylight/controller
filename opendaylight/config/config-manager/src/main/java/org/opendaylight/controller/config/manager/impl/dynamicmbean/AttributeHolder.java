...
class AttributeHolder {
	...
	public AttributeHolder(String name, Object object, String returnType,
			boolean writable, @Nullable RequireInterface requireInterfaceAnnotation, String description) {
		if(name == null){
	...
	public MBeanAttributeInfo toMBeanAttributeInfo() {
		MBeanAttributeInfo info = new MBeanAttributeInfo(name, attributeType, description, true, true, false);
		return info;
	}

	/**
	 * @return annotation if setter sets ObjectName or ObjectName[], and is annotated. Return null otherwise.
	 */
	...
	/**
	 * Find @Description annotations in method class and all its exported interfaces.
	 * @param setter
	 * @param jmxInterfaces
	 * @return empty string if no annotation is found, or list of descriptions separated by newline
	 */
	...
	/**
	 * Find @RequireInterface annotation by searching method class and all exported interfaces.
	 * @param setter
	 ...
	 */
	static RequireInterface findRequireInterfaceAnnotation(final Method setter, Set<Class<?>> inspectedInterfaces) {

		// only allow setX(ObjectName y) or setX(ObjectName[] y) to continue
		if (setter.getParameterTypes().length != 1 ||
				(setter.getParameterTypes()[0].equals(ObjectName.class) == false &&
				setter.getParameterTypes()[0].equals(ObjectName[].class) == false)) {
			return null;
		}

		List<RequireInterface> foundRequireInterfaces = AnnotationsHelper.findMethodAnnotationInSuperClassesAndIfcs(
				setter, RequireInterface.class, inspectedInterfaces);
		// make sure the list if not empty contains always annotation with same value
		...
		if (foundValues.size() == 0) {
			...
			throw new IllegalStateException("Error finding @RequireInterface. " +
					"More than one value specified as required interface " + foundValues + " of "
					+ setter + " of " + setter.getDeclaringClass());
		} else {
