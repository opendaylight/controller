...
/**
 * Wraps {@link org.opendaylight.controller.config.spi.Module} instance in a {@link DynamicMBean} interface.
 * Inspects dependency attributes, identified by ObjectName getter/setter
 * and {@link RequireInterface} annotation.
 * Used to simplify client calls - to set a dependency, only instance
 * name is needed. This class creates new writable String attribute for each dependency
 * with 'Name' suffix backed by the actual ObjectName attribute.
 * <p>
 * Thread safety - setting attributes is synchronized on 'this'. Synchronization of {@link org.opendaylight.controller.config.spi.Module#validate()}
 * and {@link org.opendaylight.controller.config.spi.Module#getInstance()} is also guaranteed by
 * {@link org.opendaylight.controller.config.manager.impl.ConfigTransactionControllerInternal} so
 * the actual {@link org.opendaylight.controller.config.spi.Module} needs not to be thread safe.
 * </p>
 */
@ThreadSafe
public class DynamicWritableWrapper extends AbstractDynamicWrapper {
	...
	public DynamicWritableWrapper(Module module, ModuleIdentifier moduleIdentifier,
	                              TransactionIdentifier transactionIdentifier,
			ReadOnlyAtomicBoolean configBeanModificationDisabled, MBeanServer internalServer, MBeanServer configMBeanServer) {
		super(module, true, moduleIdentifier, ObjectNameUtil.createTransactionModuleON(transactionIdentifier.getName(),
				moduleIdentifier), getOperations(moduleIdentifier), internalServer, configMBeanServer);
	...
	public AttributeList setAttributes(AttributeList attributes) {
		...
		for (Object attributeObject : attributes) {
			...
			try {
				...
				throw new IllegalArgumentException("Setting attribute failed - " + attribute.getName() + " on " +
						moduleIdentifier, e);
			}
		}
		return result;
	...
	public Object invoke(String actionName, Object[] params, String[] signature) throws MBeanException,
		...
		if ("validate".equals(actionName) && (params == null || params.length == 0)
			...
			try {
				...
				throw ValidationException.createForSingleException(moduleIdentifier, e);
			}
			return Void.TYPE;
		}
