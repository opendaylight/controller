...
public class BundleContextBackedModuleFactoriesResolver implements ModuleFactoriesResolver {
	...
	public List<? extends ModuleFactory> getAllFactories() {
		...
		for(ServiceReference<ModuleFactory> serviceReference: serviceReferences){
			...
			// null if the service is not registered, the service object returned by a ServiceFactory does not
			// implement the classes under which it was registered or the ServiceFactory threw an exception.
			if (service != null){
				result.add(service);
			}
		}
