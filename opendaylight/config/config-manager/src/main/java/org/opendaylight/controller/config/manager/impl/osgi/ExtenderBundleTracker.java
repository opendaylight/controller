 ...
 * META-INF/services/org.opendaylight.controller.config.spi.ModuleFactory, each line should contain an implementation of
 * ModuleFactory interface. Creates new instance with default constructor and registers it into OSGi service registry.
 * There is no need for listening for implementing removedBundle as the services are unregistered automatically.
...
public class ExtenderBundleTracker extends BundleTracker<Object> {
	...
	public Object addingBundle(Bundle bundle, BundleEvent event) {
		...
		logger.trace("Got addingBundle event of bundle {}, resource {}, event {}", bundle, resource, event);
		if (resource != null) {
			try (InputStream inputStream = resource.openStream()) {
				...
				logger.error("Error while reading {}, stopping bundle {}",resource, bundle, e);
	...
	private static void stopBundleQuietly(Bundle bundle) {
		try {
			...
			logger.warn("Ignoring fact that bundle.stop failed on {}, reason {}", bundle, e2.toString());
		}
	}
	...
	private static ServiceRegistration<?> registerFactory(String factoryClassName, Bundle bundle) {
		...
		try {
			...
			if (ModuleFactory.class.isAssignableFrom(clazz)) {
				try {
					logger.debug("Registering {} in bundle {}", clazz.getName(), bundle);
					return bundle.getBundleContext().registerService(ModuleFactory.class.getName(),
			...
			errorMessage = logMessage("Could not find class {} in bunde {}, reason {}", factoryClassName, bundle, e);
		}
