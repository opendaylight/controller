...
/**
 * Singleton that is responsible for creating and committing Config Transactions.
 * It is registered in Platform MBean Server.
 */
@ThreadSafe
public class ConfigRegistryImpl implements AutoCloseable,
	...
	/**
	 * Contains current configuration in form of {moduleName:{instanceName,read only module}} for copying state to new transaction.
	 * Each running module is present just once, no matter how many interfaces it exposes.
	 */
	@GuardedBy("this")
	 ...
	 * In this case the server is unstable and its state is undefined.
	 */
	@GuardedBy("this")
	private boolean isHealthy = true;

	/**
	 * Holds Map<transactionName, transactionController> and purges it each time its content is requested.
	 */
	@GuardedBy("this")
	...
	// constructor
	public ConfigRegistryImpl(ModuleFactoriesResolver resolver, BundleContext bundleContext, MBeanServer configMBeanServer) {
        this(resolver, bundleContext, configMBeanServer, new BaseJMXRegistrator(configMBeanServer));
	}

	// constructor
	public ConfigRegistryImpl(ModuleFactoriesResolver resolver, BundleContext bundleContext, MBeanServer configMBeanServer, BaseJMXRegistrator baseJMXRegistrator) {
		this.resolver = resolver;
		this.beanToOsgiServiceManager = new BeanToOsgiServiceManager(bundleContext);
	...
	private synchronized ConfigTransactionControllerInternal beginConfigInternal() {
		...
		ConfigTransactionControllerInternal transactionController = new ConfigTransactionControllerImpl(
				transactionName, transactionRegistrator, version, versionCounter, resolver.getAllFactories(),
				transactionsMBeanServer, configMBeanServer);
		try {
			transactionRegistrator
					.registerMBean(transactionController, transactionController.getControllerObjectName());
	...
	public synchronized CommitStatus commitConfig(ObjectName transactionControllerON) throws ConflictingVersionException, ValidationException {
		...
		logger.info("About to commit {}. Current parentVersion: {}, versionCounter {}", transactionName,
		...
		if (version != configTransactionController.getParentVersion()) {
			throw new ConflictingVersionException(String.format(
					"Optimistic lock failed. Expected parent version %d, was %d", version,
		...
		try {
			final CommitStatus secondPhaseCommitStatus = secondPhaseCommit(configTransactionController, commitInfo);

			return secondPhaseCommitStatus;
		} catch (Throwable t) { // some libs throw Errors: e.g. javax.xml.ws.spi.FactoryFinder$ConfigurationError
			isHealthy = false;
	...
	private CommitStatus secondPhaseCommit(ConfigTransactionControllerInternal configTransactionController,
		...
		// close instances which were destroyed by the user, including (hopefully) runtime beans
		for (DestroyedModule toBeDestroyed : commitInfo.getDestroyedFromPreviousTransactions()) {
			toBeDestroyed.close(); // closes instance (which should close runtime jmx registrator),
			// also closes osgi registration and ModuleJMXRegistrator registration
			currentConfig.remove(toBeDestroyed.getName());
		}

		// set RuntimeBeanRegistrators on beans implementing RuntimeBeanRegistratorAwareModule, each module
		...
		for (ModuleInternalTransactionalInfo entry : commitInfo.getCommitted().values()) {
			...
			if (entry.hasOldModule() == false) {
				runtimeBeanRegistrator = baseJMXRegistrator.createRuntimeBeanRegistrator(entry.getName());
			} else {
		...
		for(ModuleIdentifier moduleIdentifier: orderedModuleIdentifiers){
			...
			// osgi registration - will be (re)created every time, so it needs to be closed here
			// module jmx registration - will be (re)created every time, needs to be closed here
			// runtime jmx registration - should be taken care of by module itself
			...
			if (entry.hasOldModule()) {
				...
				if (oldReadableConfigBean.getInstance().equals(module.getInstance())) {
					...
					// recreated instance:
					// it is responsibility of module to call the old instance - we just need to unregister configbean
			...
			DynamicReadableWrapper newReadableConfigBean = new DynamicReadableWrapper(module, module.getInstance(),
			...
			// register to OSGi
			OsgiRegistration osgiRegistration = beanToOsgiServiceManager.registerToOsgi(
					module.getClass(), newReadableConfigBean.getInstance(), entry.getName());

			RootRuntimeBeanRegistratorImpl runtimeBeanRegistrator = runtimeRegistrators.get(entry.getName());
			ModuleInternalInfo newInfo = new ModuleInternalInfo(entry.getName(), newReadableConfigBean,
	 ...
	 * Since this class is not responsible for registering itself under {@link ConfigRegistryMXBean#OBJECT_NAME},
	 * it will not unregister itself here.
	 */
	@Override
	public synchronized void close() {
		...
		for (ConfigTransactionControllerInternal configTransactionController : transactions.values()) {
			try {
				...
				logger.warn("Ignoring exception while aborting {}", configTransactionController, e);
			}
		}

		// destroy all live objects one after another in order of the dependency hierarchy
	...
	public Set<ObjectName> lookupRuntimeBeans(String moduleName, String instanceName) {
		...
		if (instanceName == null)
			instanceName = "*";
		ObjectName namePattern = ObjectNameUtil.createRuntimeBeanPattern(moduleName, instanceName);
		return baseJMXRegistrator.queryNames(namePattern, null);
	}
...
class ConfigHolder {
	...
	/**
	 * Add all modules to the internal map. Also add service instance to OSGi Service Registry.
	 */
	...
	private void add(ModuleInternalInfo configInfo) {
		ModuleInternalInfo oldValue = currentConfig.put(configInfo.getName(), configInfo);
		if (oldValue != null) {
			throw new IllegalStateException("Cannot overwrite module with same name:" + configInfo.getName()
					+ ":" + configInfo);
		}
	}
...
class TransactionsHolder {
	/**
	 * This map keeps transaction names and {@link ConfigTransactionControllerInternal} instances, because platform MBeanServer
	 * transforms mbeans into another representation. Map is cleaned every time current transactions are requested.
	 *
	 */
	...
	public void add(String transactionName, ConfigTransactionControllerInternal transactionController) {
		Object oldValue = transactions.put(transactionName, transactionController);
		if (oldValue != null) {
	...
	/**
	 * Purges closed transactions from transactions map.
	 * Can only be called from within synchronized method. Calling this method more than once within the method can modify the resulting map that
	 * was obtained in previous calls.
	...
	public Map<String, ConfigTransactionControllerInternal> getCurrentTransactions() {
		...
		for (Iterator<Entry<String, ConfigTransactionControllerInternal>> it = transactions.entrySet().iterator(); it.hasNext(); ) {
			Entry<String, ConfigTransactionControllerInternal> entry = it.next();
