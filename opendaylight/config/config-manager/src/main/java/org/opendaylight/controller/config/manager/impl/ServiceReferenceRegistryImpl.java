...
public class ServiceReferenceRegistryImpl implements CloseableServiceReferenceReadableRegistry, ServiceReferenceWritableRegistry {
    ...
    private final Map<ServiceReference, ModuleIdentifier> refNames;
    ...
    public static CloseableServiceReferenceReadableRegistry createInitialSRLookupRegistry() {
        ...
        return new ServiceReferenceRegistryImpl(Collections.<String, ModuleFactory>emptyMap(), lookupRegistry, serviceReferenceRegistratorFactory,
                Collections.<ServiceReference, ModuleIdentifier>emptyMap(), false);
    }
    ...
    public static ServiceReferenceWritableRegistry createSRWritableRegistry(ServiceReferenceReadableRegistry oldReadableRegistry,
        ...
        return new ServiceReferenceRegistryImpl(factories, txLookupRegistry, serviceReferenceRegistratorFactory,
                Collections.unmodifiableMap(old.refNames), true);
    }
    ...
    public static CloseableServiceReferenceReadableRegistry createSRReadableRegistry(ServiceReferenceWritableRegistry oldWritableRegistry,
        ...
        ServiceReferenceRegistryImpl newRegistry = new ServiceReferenceRegistryImpl(old.factories, lookupRegistry, serviceReferenceRegistratorFactory, Collections.unmodifiableMap(old.refNames), false);
        // copy dummy Service Reference MXBeans to somewhere
        ...
        for (Entry<ServiceReference, Entry<ServiceReferenceMXBeanImpl, ServiceReferenceJMXRegistration>> refNameEntry : old.mBeans.entrySet()) {
            ObjectName currentImplementationWithTx = refNameEntry.getValue().getKey().getCurrentImplementation();
            ObjectName currentImplementation = ObjectNameUtil.withoutTransactionName(currentImplementationWithTx);
            try {
                newRegistry.saveServiceReference(refNameEntry.getKey(), currentImplementation);
    ...
    private ServiceReferenceRegistryImpl(Map<String, ModuleFactory> factories, LookupRegistry lookupRegistry,
                                         ...
                                         Map<ServiceReference, ModuleIdentifier> refNamesToCopy,
                                         boolean writable) {
        ...
        // copy refNames
        this.refNames = new HashMap<>(refNamesToCopy);
    ...
    public synchronized boolean removeServiceReferences(ObjectName objectName) throws InstanceNotFoundException {
        ...
        ModuleIdentifier moduleIdentifier = new ModuleIdentifier(factoryName, instanceName);
        boolean found = false;
        for (Iterator<Entry<ServiceReference, ModuleIdentifier>> it = refNames.entrySet().iterator(); it.hasNext();) {
            Entry<ServiceReference, ModuleIdentifier> entry = it.next();
