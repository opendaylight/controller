...
/**
 * Hold sorted ConfigMBeanFactories by their module names. Check that module names are globally unique.
 */
public class HierarchicalConfigMBeanFactoriesHolder {
	 ...
	 * Create instance.
	 *
	 * @throws IllegalArgumentException if unique constraint on module names is violated
	 */
	public HierarchicalConfigMBeanFactoriesHolder(List<? extends ModuleFactory> list) {
		...
		for (ModuleFactory factory : list) {
			...
			String error = null;
			ModuleFactory conflicting = moduleNamesToConfigBeanFactories.get(moduleName);
			if (conflicting != null) {
				error = String.format(
		...
		this.moduleNamesToConfigBeanFactories = Collections.unmodifiableMap(moduleNamesToConfigBeanFactories);
	 ...
	 *
	 * @throws IllegalArgumentException if factory is not found
	 */
