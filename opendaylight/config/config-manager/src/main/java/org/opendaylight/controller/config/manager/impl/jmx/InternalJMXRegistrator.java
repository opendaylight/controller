...
public class InternalJMXRegistrator implements Closeable {
	...
	private synchronized void unregisterMBean(ObjectName on) {
		...
		if (!removed)
			throw new IllegalStateException(
					"Cannot unregister - ObjectName not found in 'registeredObjectNames': " + on);
		try {
	...
	public <T> T newMBeanProxy(ObjectName objectName, Class<T> interfaceClass, boolean notificationBroadcaster) {
		return JMX.newMBeanProxy(configMBeanServer, objectName, interfaceClass, notificationBroadcaster);
	}

	public <T> T newMXBeanProxy(ObjectName objectName, Class<T> interfaceClass) {
		return JMX.newMXBeanProxy(configMBeanServer, objectName, interfaceClass);
	}

	public <T> T newMXBeanProxy(ObjectName objectName, Class<T> interfaceClass,
	                            boolean notificationBroadcaster) {
		return JMX.newMXBeanProxy(configMBeanServer, objectName, interfaceClass, notificationBroadcaster);
	}
