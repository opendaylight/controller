...
/**
 * Contains common code for readable/rw dynamic mbean wrappers.
 * Routes all requests (getAttribute, setAttribute, invoke) into the actual instance,
 * but provides additional functionality - namely it disallows setting attribute on a read only wrapper.
 *
 */
abstract class AbstractDynamicWrapper implements DynamicMBeanModuleWrapper {
	...
	public AbstractDynamicWrapper(Module module, boolean writable, ModuleIdentifier moduleIdentifier,
			ObjectName thisWrapperObjectName, MBeanOperationInfo[] dOperations, MBeanServer internalServer,
		...
		registerActualModule(module, thisWrapperObjectName, objectNameInternal, internalServer, configMBeanServer);
		Set<Class<?>> jmxInterfaces = InterfacesHelper.getMXInterfaces(module.getClass());
		this.attributeHolderMap = buildMBeanInfo(module, writable, moduleIdentifier, jmxInterfaces, internalServer,
				objectNameInternal);
		this.mbeanInfo = generateMBeanInfo(module.getClass().getName(), module, attributeHolderMap, dOperations,
				jmxInterfaces);
	}


	/**
	 * Register module into an internal mbean server, attach listener to the platform mbean server. Wait until this
	 * wrapper gets unregistered, in that case unregister the module and remove listener.
	 */
	private final NotificationListener registerActualModule(Module module, final ObjectName thisWrapperObjectName,
			final ObjectName objectNameInternal, final MBeanServer internalServer, final MBeanServer configMBeanServer) {

	try {
			internalServer.registerMBean(module, objectNameInternal);
		} catch (InstanceAlreadyExistsException | MBeanRegistrationException | NotCompliantMBeanException |
				IllegalStateException e) {
			throw new IllegalStateException("Error occured during mbean registration ", e);
		}

		NotificationListener listener = new NotificationListener() {
			@Override
			public void handleNotification(Notification n, Object handback) {
				if (n instanceof MBeanServerNotification && n.getType().equals(
						MBeanServerNotification.UNREGISTRATION_NOTIFICATION)) {
					if (((MBeanServerNotification) n).getMBeanName().equals(thisWrapperObjectName)) {
						try {
						...
						} catch (MBeanRegistrationException | ListenerNotFoundException |
	 ...
	 private static MBeanInfo generateMBeanInfo(String className, Module module,
			Map<String, AttributeHolder> attributeHolderMap, MBeanOperationInfo[] dOperations,
		...
		return new MBeanInfo(className, dDescription, attributes.toArray(new MBeanAttributeInfo[0]),
	...
	private static Map<String, AttributeHolder> buildMBeanInfo(Module module,
			boolean writable,
			ModuleIdentifier moduleIdentifier, Set<Class<?>> jmxInterfaces,
			MBeanServer internalServer, ObjectName internalObjectName) {
		...
		for (Method method : methods) {
			...
			if (method.getParameterTypes().length == 1
				...
				AttributeHolder attributeHolder = new AttributeHolder(attribName,
						module, attributeMap.get(attribName).getType(),
						writable, ifc, description);
	...
	protected ObjectName fixObjectName(ObjectName on) {
		if(!ObjectNameUtil.ON_DOMAIN.equals(on.getDomain())) throw new IllegalArgumentException("Wrong domain, expected "
				+ ObjectNameUtil.ON_DOMAIN + " setter on "
				+ on);
		...
		if ("getAttribute".equals(actionName) && params.length == 1
			...
			throw new UnsupportedOperationException(format(
					"Operation not found on %s. Method invoke is only supported for getInstance and getAttribute(s) " +
							"method, got actionName %s, params %s, signature %s ", moduleIdentifier, actionName, params,
					signature));
		}
	}
