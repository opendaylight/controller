...
public class ModuleInternalTransactionalInfo implements NamedObject<ModuleIdentifier> {
	...
	ModuleInternalTransactionalInfo(ModuleIdentifier name, Module module, ModuleFactory moduleFactory,
	...
	public DestroyedModule toDestroyedModule() {
		...
		DynamicReadableWrapper oldModule = maybeOldInternalInfo.getReadableModule();
		return new DestroyedModule(name, oldModule.getInstance(), maybeOldInternalInfo.getModuleJMXRegistrator(),
				maybeOldInternalInfo.getOsgiRegistration(), maybeOldInternalInfo.getOrderingIdx());
	}
