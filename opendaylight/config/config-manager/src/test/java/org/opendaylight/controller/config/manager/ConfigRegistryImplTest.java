...
public class ConfigRegistryImplTest extends AbstractLockedPlatformMBeanServerTest {
	private static final Logger logger = LoggerFactory.getLogger(ConfigRegistryImplTest.class);

	@Test
	public void testFailOnTwoFactoriesExportingSameImpl() {
		ModuleFactory factory = new TestingFixedThreadPoolModuleFactory();
		ModuleFactoriesResolver resolver = new HardcodedModuleFactoriesResolver(factory, factory);

		BundleContext context = mock(BundleContext.class);

		ConfigRegistryImpl configRegistry = new ConfigRegistryImpl(resolver, context, ManagementFactory.getPlatformMBeanServer());
		try {
