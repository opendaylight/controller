 ...
 * </ol>
 * Only one bean is being configured - {@link TestingThreadPoolIfc} which has no dependencies.
 */
public class SimpleConfigurationTest extends AbstractConfigWithJolokiaTest {
	...
	private static final List<ObjectName> emptyONs = Collections.<ObjectName> emptyList();
	private static final ObjectName platformFixed1ON = ObjectNameUtil.createReadOnlyModuleON(TestingFixedThreadPoolModuleFactory.NAME, fixed1);
	...
	private ObjectName createFixedThreadPool(ConfigTransactionJMXClient transaction) throws InstanceAlreadyExistsException, InstanceNotFoundException {
	...
	public void testValidationUsingJolokiaClient() throws Exception {
		ConfigTransactionClient transaction = configRegistryJolokiaClient.createTransaction();
		testValidation(transaction);
	}

	private void testValidation(ConfigTransactionClient transaction) throws InstanceAlreadyExistsException,
	...
	public void test_createFixedThreadPool_destroyIt() throws Exception {
		...
		DynamicMBean dynamicMBean = configRegistryClient.newMBeanProxy(
				ObjectNameUtil.withoutTransactionName(fixed1name), DynamicMBean.class);
	...
	private void checkThreadPools(int expectedTotalNumberOfExecutors,
		...
		assertEquals(expectedTotalNumberOfExecutors, TestingFixedThreadPool.allExecutors.size());
	...
	private TestingFixedThreadPoolConfigMXBean startReconfiguringFixed1ThreadPool(
			ConfigTransactionJMXClient transaction) throws InstanceNotFoundException {
	...
	public void testAbort() {
		...
		try {
			transaction.createModule(TestingFixedThreadPoolModuleFactory.NAME, fixed1);
			fail();
		} catch (Exception e) {
			assertTrue(e.getCause() instanceof InstanceNotFoundException);
		}
	...
	@Test
	public void testOptimisticLock_ConfigTransactionJolokiaClient() throws Exception {
		ConfigTransactionJolokiaClient transaction1 = configRegistryJolokiaClient.createTransaction();
		ConfigTransactionJolokiaClient transaction2 = configRegistryJolokiaClient.createTransaction();
		transaction2.assertVersion(0, 2);
		transaction2.commit();
		try {
			transaction1.commit();
	...
	@Test
	public void testOptimisticLock_ConfigRegistryJolokiaClient() throws Exception {
		ConfigTransactionJolokiaClient transaction1 = configRegistryJolokiaClient.createTransaction();
		ConfigTransactionJolokiaClient transaction2 = configRegistryJolokiaClient.createTransaction();
		transaction2.assertVersion(0, 2);
		transaction2.commit();
		try {
			configRegistryJolokiaClient.commitConfig(transaction1.getObjectName());
			fail();
		} catch (ConflictingVersionException e) {
			assertEquals("Optimistic lock failed. Expected parent version 2, was 0", e.getMessage());
		}
	...
	public void testUsingJolokia() throws Exception {
		...
		ObjectName name = transactionClient.createModule(TestingFixedThreadPoolModuleFactory.NAME, fixed1);

		try {
			transactionClient.validateConfig();
			fail();
		} catch (ValidationException e) {
			assertThat(e.getMessage(), containsString("Parameter 'threadCount' must be greater than 0"));
		}

		transactionClient.setAttribute(name, "ThreadCount", numberOfThreads);
		// commit
		CommitStatus commitStatus = transactionClient.commit();
		CommitStatus expected = new CommitStatus(Arrays.asList(ObjectNameUtil.withoutTransactionName(name)),
				emptyONs, emptyONs);
		assertEquals(expected, commitStatus);
	}
