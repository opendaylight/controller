...
public class TwoInterfacesExportTest extends AbstractScheduledTest {
	...
	private void assertExists(@Nullable ConfigTransactionJMXClient transaction, String moduleName,
	                          String instanceName) throws InstanceNotFoundException {
		if (transaction != null) {
			transaction.lookupConfigBean(moduleName, instanceName);
			// make a dummy call
			configRegistryClient.newMBeanProxy(
					ObjectNameUtil.createTransactionModuleON(transaction.getTransactionName(), moduleName, instanceName), DynamicMBean.class).getMBeanInfo();
		} else {
			configRegistryClient.lookupConfigBean(moduleName, instanceName);
			// make a dummy call
			configRegistryClient.newMBeanProxy(
					ObjectNameUtil.createReadOnlyModuleON(moduleName, instanceName), DynamicMBean.class)
					.getMBeanInfo();
		}
	}
	...
	private void assertNotExists(@Nullable ConfigTransactionJMXClient transaction, String moduleName,
	                             String instanceName) {
	...
	public void twoInterfaceNamesAfterCreatingConfigBean() throws Exception {
		...
		transaction.destroyModule(ObjectNameUtil.createTransactionModuleON(transaction.getTransactionName(), moduleName,
				scheduled1));
		transaction.commit();
		assertEquals(1, TestingScheduledThreadPoolImpl.allExecutors.size());
	...
	public void testWithAPSP_useScheduledNames() throws InstanceAlreadyExistsException, ValidationException {
		...
		TestingParallelAPSPConfigMXBean apspProxy = transaction.newMBeanProxy(apspName,
	...
	public void testWithAPSP_useIfcNameMismatch() throws Exception {
		...
		TestingParallelAPSPConfigMXBean apspProxy = transaction.newMBeanProxy(apspName,
				TestingParallelAPSPConfigMXBean.class);
		apspProxy.setThreadPool(ObjectNameUtil.createReadOnlyModuleON(TestingScheduledThreadPoolModuleFactory.NAME,
				scheduled1));
		apspProxy.setSomeParam("someParam");
		transaction.validateConfig();
		transaction.commit();
