 ...
 * {@link org.opendaylight.controller.config.manager.testingservices.scheduledthreadpool.TestingScheduledThreadPoolModuleFactory#NAME},<br>
 * {@link org.opendaylight.controller.config.manager.testingservices.threadpool.TestingFixedThreadPoolModuleFactory#NAME}<br>
 * <br>
 * It also exports 2 runtime beans, one default and one with
 * additional properties of {'a':'b'}.
 */
public class RuntimeBeanTest extends AbstractScheduledTest {

	ObjectName ifc1runtimeON1 = ObjectNameUtil.createRuntimeBeanName(TestingScheduledThreadPoolModuleFactory.NAME, scheduled1, Maps.<String, String>newHashMap());
	...
	private ObjectName createScheduled() throws InstanceAlreadyExistsException, ConflictingVersionException,
			ValidationException {
	...
	public void testRecreate() throws Exception {
		...
		TestingScheduledThreadPoolConfigBeanMXBean scheduledWritableProxy = configTransaction.newMXBeanProxy(
				scheduledWritableON, TestingScheduledThreadPoolConfigBeanMXBean.class);
	...
	public void testDestroy_shouldUnregisterRuntimeBeans() throws Exception {
		ObjectName createdConfigBean = createScheduled();
		ConfigTransactionJMXClient configTransaction = configRegistryClient.createTransaction();
		configTransaction.destroyModule(ObjectNameUtil.createTransactionModuleON(configTransaction.getTransactionName(),
				createdConfigBean));
