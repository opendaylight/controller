 ...
 * {@link org.opendaylight.controller.config.manager.impl.ConfigRegistryImpl} is registered to platform MBean Server using
 * {@link #initConfigTransactionManagerImpl(org.opendaylight.controller.config.manager.impl.factoriesresolver.ModuleFactoriesResolver)} typically
 * during setting up the each test.
 */
public abstract class AbstractConfigTest extends AbstractLockedPlatformMBeanServerTest {
	...
	protected void initConfigTransactionManagerImpl(ModuleFactoriesResolver resolver) {
		...
		doReturn(mockedServiceRegistration).when(context).registerService(Matchers.any(String[].class), any(Closeable.class), any(Dictionary.class));
		...
		configRegistry = new ConfigRegistryImpl(resolver, context, platformMBeanServer, baseJmxRegistrator);
		try{
	...
	protected void assertSame(ObjectName oN1, ObjectName oN2) {
		assertEquals(oN1.getKeyProperty("instanceName"), oN2.getKeyProperty("instanceName"));
		assertEquals(oN1.getKeyProperty("interfaceName"), oN2.getKeyProperty("interfaceName"));
	}
	...
	protected void assertStatus(CommitStatus status, int expectedNewInstances, int expectedRecreatedInstances, int expectedReusedInstances) {
		...
		assertEquals(expectedRecreatedInstances, status.getRecreatedInstances().size());
		assertEquals(expectedReusedInstances, status.getReusedInstances().size());
	}

	@Deprecated
	protected ObjectName createTestConfigBean(ConfigTransactionJMXClient transaction, String implementationName, String name, Class<?> clz)
			throws InstanceAlreadyExistsException {
		ObjectName nameCreated = transaction.createModule(implementationName, name);
		transaction.newMXBeanProxy(nameCreated, clz);
		return nameCreated;
	}

	protected ObjectName createTestConfigBean(ConfigTransactionJMXClient transaction, String implementationName, String name)
			throws InstanceAlreadyExistsException {
		ObjectName nameCreated = transaction.createModule(implementationName, name);
		return nameCreated;
	}
	...
	protected void assertBeanExists(int count, String moduleName, String instanceName) {
		assertEquals(1, configRegistry.lookupConfigBeans(moduleName, instanceName).size());
	}

	/**
	 *
	 * @param configBeanClass Empty constructor class of config bean to be instantiated whenever create
