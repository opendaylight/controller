...
public class HardcodedModuleFactoriesResolver implements ModuleFactoriesResolver {
    ...
    public HardcodedModuleFactoriesResolver(ModuleFactory... list) {
        ...
        factories = new HashMap<>(factoryList.size());
        for (ModuleFactory moduleFactory : list) {
            ...
            if (error == null) {
                factories.put(moduleName, new AbstractMap.SimpleImmutableEntry<>(moduleFactory,
                        bundleContext));
            } else {
                errors.append(error);
            }
            ...
            factories.put(moduleFactory.getImplementationName(), new AbstractMap.SimpleImmutableEntry<>(moduleFactory, bundleContext));
        }
    }
