...
public class DependentWiringTest extends AbstractParallelAPSPTest {
	...
	public void testDependencies() throws Exception {
		ObjectName apspON;
		{
			...
			parallelAPSPConfigProxy.setSomeParam("");// trigger validation failure
			try {
			...
			parallelAPSPConfigProxy.setSomeParam("abc");// fix validation failure
			transaction.commit();
			apspON =  ObjectNameUtil.withoutTransactionName(apspNameTransactionON);
		}

		// test reported apsp number of threads
		TestingParallelAPSPConfigMXBean parallelAPSPRuntimeProxy = configRegistryClient.newMBeanProxy(
				apspON, TestingParallelAPSPConfigMXBean.class);
		...
		int newNumberOfThreads = TestingParallelAPSPImpl.MINIMAL_NUMBER_OF_THREADS * 2;
		{
			...
			TestingFixedThreadPoolConfigMXBean fixedConfigTransactionProxy = transaction.newMXBeanProxy(threadPoolNames_newTx,
	...
	public void testUsingJolokia() throws Exception {
		...
		// fixed1
		ObjectName fixed1ON = transactionClient.createModule(getThreadPoolImplementationName(), fixed1);
		...
		ObjectName apsp1ON = transactionClient.createModule(TestingParallelAPSPModuleFactory.NAME, apsp1);
		...
		ObjectName apspReadOnlyON = ObjectNameUtil.withoutTransactionName(apsp1ON);
		JSONObject threadPoolONJson = (JSONObject) configRegistryJolokiaClient.getAttribute(apspReadOnlyON, threadPoolString);
		ObjectName fixed1ReadOnlyON = ObjectNameUtil.withoutTransactionName(fixed1ON);
		assertEquals(fixed1ReadOnlyON, ObjectNameUtil.createON(threadPoolONJson.get("objectName").toString()));
		assertEquals(fixed1ReadOnlyON, configRegistryJolokiaClient.getAttributeON(apspReadOnlyON, threadPoolString));
