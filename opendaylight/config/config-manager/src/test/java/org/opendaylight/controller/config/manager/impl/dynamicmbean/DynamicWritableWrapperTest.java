...
public class DynamicWritableWrapperTest extends AbstractDynamicWrapperTest {
	...
	protected AbstractDynamicWrapper getDynamicWrapper(Module module, ModuleIdentifier moduleIdentifier) {
        return new DynamicWritableWrapper(module, moduleIdentifier, new TransactionIdentifier("transaction-1"),
				readOnlyAtomicBoolean, MBeanServerFactory.createMBeanServer(), platformMBeanServer);
	}
	...
	/*
	 * Try to call setter with ObjectName containing transaction name. Verify that ObjectName
	 * without transaction name was actually passed on the config bean.
	 */
	@Test
