...
abstract class AbstractParallelAPSPTest extends AbstractConfigWithJolokiaTest {
	...
	protected ObjectName createParallelAPSP(ConfigTransactionJMXClient transaction, ObjectName threadPoolON)
			throws InstanceAlreadyExistsException {
		ObjectName apspName = transaction.createModule(TestingParallelAPSPModuleFactory.NAME, apsp1);
		TestingParallelAPSPConfigMXBean parallelAPSPConfigProxy = transaction.newMXBeanProxy(apspName,
	...
	protected ObjectName createFixed1(ConfigTransactionJMXClient transaction, int numberOfThreads)
			throws InstanceAlreadyExistsException {
		ObjectName name = transaction.createModule(getThreadPoolImplementationName(), fixed1);

		TestingFixedThreadPoolConfigMXBean fixedConfigProxy = transaction.newMXBeanProxy(name,
