...
public class RuntimeRegistratorTest {
	...
	protected RuntimeBeanEntry prepareRootRB(List<RuntimeBeanEntry> children) {
		...
		return new RuntimeBeanEntry("pa.cka.ge", dataSchemaNodeForReporting, "module-name", "ModuleName", true,
				Optional.<String> absent(), Collections.<AttributeIfc> emptyList(), children, Collections.<Rpc>emptySet());
	}

	protected RuntimeBeanEntry prepareChildRB(List<RuntimeBeanEntry> children, String prefix) {
		DataSchemaNode dataSchemaNodeForReporting = mock(DataSchemaNode.class);
		doReturn("DataSchemaNode").when(dataSchemaNodeForReporting).toString();
		return new RuntimeBeanEntry("pa.cka.ge", dataSchemaNodeForReporting, prefix + "child-name", capitalize(prefix)
				+ "ChildName", false, Optional.<String> absent(), Collections.<AttributeIfc> emptyList(), children, Collections.<Rpc>emptySet());
	}
	...
	public void testHierarchy() {
		...
		doReturn(new QName(URI.create("urn:x"), "leaf-local-name")).when(leaf).getQName();
