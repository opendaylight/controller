...
public class ModuleMXBeanEntryNameConflictTest extends AbstractYangTest {
	...
	private static final Logger logger = LoggerFactory.getLogger(ModuleMXBeanEntryNameConflictTest.class);
	...
	public void testNameConflicts() throws Exception {
		...
		for (Map.Entry<File, String> currentTestEntry : testedFilesToYangModules.entrySet()) {
			...
			try {
				...
				ModuleMXBeanEntry.create(testedModule, new HashMap<QName, ServiceInterfaceEntry>(), context,
                        new TypeProviderWrapper(new TypeProviderImpl(context)), PACKAGE_NAME);
				fail(yangFile.toString() + " did not cause a name conflict and should");
			} catch (NameConflictException e) {
				assertEquals(testedYangModulesToExpectedConflictingName.get(moduleName), e.getConflictingName());
			}
		}
	}

	private void prepareSamples() {
		File first = new File(getClass().getResource("/duplicates/config-test-duplicate-attribute-in-list.yang")
				.getFile());
	...
	private void prepareExceptionAssertions() {
		...
		testedYangModulesToExpectedConflictingName.put("config-test-runtime-bean-list-name-conflict", "StateARuntimeMXBean");
		testedYangModulesToExpectedConflictingName.put("config-test-runtime-bean-list-name-conflict2", "StateARuntimeMXBean");
		testedYangModulesToExpectedConflictingName.put("config-test-runtime-bean-name-conflict", "StateARuntimeMXBean");
		testedYangModulesToExpectedConflictingName.put("config-test-runtime-bean-name-conflict2", "StateARuntimeMXBean");
	}
	...
	private Module loadYangs(File testedModule, String moduleName) throws Exception {
		...
		Preconditions.checkNotNull(module, "Cannot get module %s from %s", moduleName, namesToModules.keySet());
		return module;
	}
