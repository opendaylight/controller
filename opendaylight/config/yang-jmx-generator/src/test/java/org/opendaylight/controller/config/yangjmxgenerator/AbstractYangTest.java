...
public abstract class AbstractYangTest {
	...
	protected Map<String, Module> namesToModules; // are module names globally unique?
	protected Module configModule, rpcContextModule, threadsModule, threadsJavaModule, bgpListenerJavaModule,
			ietfInetTypesModule, jmxModule, jmxImplModule, testFilesModule, testFiles1Module;
	...
	public void loadYangFiles() throws Exception {
		...
		yangISs.addAll(getStreams("/test-config-threads.yang",
				"/test-config-threads-java.yang", "/config-bgp-listener-impl.yang",
				"/ietf-inet-types.yang", "/config-jmx-it.yang",
				"/config-jmx-it-impl.yang", "/test-config-files.yang", "/test-config-files1.yang"));
		...
		ietfInetTypesModule = namesToModules.get(ConfigConstants.IETF_INET_TYPES);
	...
	public Map<QName, IdentitySchemaNode> mapIdentitiesByQNames(Module module) {
		...
		for (IdentitySchemaNode identitySchemaNode : module.getIdentities()) {
			...
			Preconditions.checkArgument(result.containsKey(qName) == false, format("Two identities of %s contain same " +
					"qname %s", module, qName));
