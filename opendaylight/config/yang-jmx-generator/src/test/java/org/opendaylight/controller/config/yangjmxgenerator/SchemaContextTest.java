...
public class SchemaContextTest extends AbstractYangTest {
	...
	public void testReadingIdentities_threadsModule() {
		...
		Map<String /*identity name*/, Optional<QName>> expectedIdentitiesToBases = ImmutableMap.of(
				"eventbus", Optional.<QName>absent(), "threadfactory", Optional.<QName>absent(),
				"threadpool",Optional.<QName>absent(),
				"scheduled-threadpool", Optional.<QName>absent());
		...
		for (IdentitySchemaNode id : threadsModule.getIdentities()) {
			...
			// all except scheduled-threadpool should have base set to serviceType
	...
	private void assertAllIdentitiesAreExpected(Module module, Map<String /*identity name*/,
		...
		for (IdentitySchemaNode id : module.getIdentities()) {
			...
			if (maybeExpectedBaseQName.isPresent()){
				assertEquals("Unexpected base identity of " + localName, maybeExpectedBaseQName.get(),
						id.getBaseIdentity().getQName());
			}
		}
		assertEquals("Expected identities not found " + copyOfExpectedNames, Collections.EMPTY_MAP,
				copyOfExpectedNames);
	}

	@Test
	public void testReadingIdentities_threadsJavaModule() {
		Map<String /*identity name*/, Optional<QName>> expectedIdentitiesToBases = ImmutableMap.of("eventbus",
				Optional.of(MODULE_TYPE_Q_NAME), "async-eventbus", Optional.of(MODULE_TYPE_Q_NAME),
