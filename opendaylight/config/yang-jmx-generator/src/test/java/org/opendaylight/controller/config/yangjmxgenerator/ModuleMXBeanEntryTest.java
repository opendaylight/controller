...
public class ModuleMXBeanEntryTest extends AbstractYangTest {
	...
	protected Map<String /* identity local name */, ModuleMXBeanEntry> loadThreadsJava() {
		Map<String /* identity local name */, ModuleMXBeanEntry> namesToMBEs = ModuleMXBeanEntry.create(
				threadsJavaModule, modulesToSIEs, context, new TypeProviderWrapper(new TypeProviderImpl(context)),
				PACKAGE_NAME);
		assertNotNull(namesToMBEs);
		Set<String> expectedMXBeanNames = Sets.newHashSet(EVENTBUS_MXB_NAME, ASYNC_EVENTBUS_MXB_NAME,
				THREADFACTORY_NAMING_MXB_NAME, THREADPOOL_DYNAMIC_MXB_NAME);
	...
	@Before
	public void setUp(){
		modulesToSIEs = ServiceInterfaceEntry.create(threadsModule, "packages.sis");
	}

	@Test
	public void test_jmxImplModule() {
		Map<QName, ServiceInterfaceEntry> modulesToSIEs = ServiceInterfaceEntry.create(threadsModule, PACKAGE_NAME);
		modulesToSIEs.putAll(ServiceInterfaceEntry.create(jmxModule, PACKAGE_NAME));
		Map<String /* identity local name */, ModuleMXBeanEntry> namesToMBEs = ModuleMXBeanEntry.create
				(jmxImplModule, modulesToSIEs, context,  new TypeProviderWrapper(new TypeProviderImpl(context)), PACKAGE_NAME);
		Map<String, AttributeIfc> attributes = namesToMBEs.get("impl-netconf").getAttributes();
		//
	...
	private void assertMatches(String prefix, String input) {
		...
		Matcher output = ModuleMXBeanEntry.getWhenConditionMatcher(prefix, whenConstraint);
	...
	public void testThreadsJava() {
		...
		{ // check threadpool-dynamic
			...
			Map<String, AttributeIfc> attributes = dynamicThreadPool.getAttributes();
			// core-size, keepalive, maximum-size
			// threadfactory
			Set<String> longAttribs = Sets.newHashSet("core-size", "maximum-size");
			for (String longAttrib : longAttribs) {

				TypedAttribute attribute = (TypedAttribute) attributes.get(longAttrib);
				assertThat("Failed to check " + longAttrib, attribute.getType(),
						is((Type) Types.typeForClass(Long.class)));
			}
			// check dependency on thread factory
			QName threadfactoryQName = new QName(THREADS_NAMESPACE, THREADS_REVISION_DATE, "threadfactory");
			ServiceInterfaceEntry threadFactorySIEntry = modulesToSIEs.get(threadfactoryQName);
			...
			AttributeIfc expectedDependencyAttribute = new DependencyAttribute(mockedDataSchemaNode,
					threadFactorySIEntry, expectedMandatory, "threadfactory description");
			assertThat(actualThreadFactory, is(expectedDependencyAttribute));
			assertThat(dynamicThreadPool.getFullyQualifiedName("DynamicThreadPoolModuleMXBean"), is(PACKAGE_NAME
					+ ".DynamicThreadPoolModuleMXBean"));
			...
			Collection<RuntimeBeanEntry> runtimeBeans = dynamicThreadPool.getRuntimeBeans();
			assertThat(runtimeBeans.size(), is(1));
			RuntimeBeanEntry rootRB = findFirstByYangName(runtimeBeans, THREADPOOL_DYNAMIC_MXB_NAME);
		...
		{// check threadfactory-naming
			...
			{
				RuntimeBeanEntry threadRB = findFirstByYangName(runtimeBeans, "thread");
				...
				AttributeIfc threadNameAttr = threadRB.getAttributes().iterator().next();
				assertThat(threadNameAttr.getAttributeYangName(), is("name"));
				assertTrue(threadNameAttr instanceof JavaAttribute);
				assertThat(((JavaAttribute) threadNameAttr).getType().getFullyQualifiedName(),
						is(String.class.getName()));
				assertThat(threadRB.getRpcs().size(), is(2));
			}
			{
				RuntimeBeanEntry streamRB = findFirstByYangName(runtimeBeans, "stream");
