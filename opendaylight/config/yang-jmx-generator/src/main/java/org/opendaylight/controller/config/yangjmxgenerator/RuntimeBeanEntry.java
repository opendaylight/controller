...
/**
 * Holds information about runtime bean to be generated. There are two kinds of RuntimeBeanEntry instances: if isRoot
 * flag
 * is set to true, this bean represents state that must be present at time of configuration module instantiation.
 * Root RB must have depthLevel set to 0 and cannot have children. There might be other RBs defined in yang,
 * but no other RB can have isRoot set to true. At least one RB must be root and all other RBs must be lined via
 * children so that a tree with all beans can be created.
 */
public class RuntimeBeanEntry {
	...
	@VisibleForTesting
	public RuntimeBeanEntry(String packageName, DataSchemaNode nodeForReporting, String yangName,
			String javaNamePrefix, boolean isRoot, Optional<String> keyYangName, List<AttributeIfc> attributes,
			List<RuntimeBeanEntry> children, Set<Rpc> rpcs) {

		checkArgument(isRoot == false || keyYangName.isPresent() == false, "Root RuntimeBeanEntry must not have key "
				+ "set");
		...
		for (AttributeIfc a : attributes) {
			checkState(map.containsKey(a.getAttributeYangName()) == false,
					"Attribute already defined: " + a.getAttributeYangName() + " in " + nodeForReporting);
			map.put(a.getAttributeYangName(), a);
		}

		if (keyYangName.isPresent()) {
			AttributeIfc keyJavaName = map.get(keyYangName.get());
			checkArgument(keyJavaName != null, "Key " + keyYangName.get() + " not found in attribute " + "list "
					+ attributes + " in " + nodeForReporting);
			this.keyJavaName = Optional.of(keyJavaName.getUpperCaseCammelCase());
		} else {
	...
	/**
	 * @return map containing all class names as key, extracted RuntimeBeans as values. If more than zero values is
	 * returned, exactly one RuntimeBeanEntry will have isRoot set to true, even if yang does not contain special
	 * configuration for it.
	 */
	public static Map<String, RuntimeBeanEntry> extractClassNameToRuntimeBeanMap(String packageName,
            ChoiceCaseNode container, String moduleYangName, TypeProviderWrapper typeProviderWrapper,
			String javaNamePrefix, Module currentModule) {

		Map<QName, Set<RpcDefinition>> identitiesToRpcs = getIdentitiesToRpcs(currentModule);

		AttributesRpcsAndRuntimeBeans attributesRpcsAndRuntimeBeans = extractSubtree(packageName, container,
				typeProviderWrapper, currentModule, identitiesToRpcs);
		...
		RuntimeBeanEntry rootRuntimeBeanEntry = createRoot(packageName, container, moduleYangName, attributes,
				javaNamePrefix, attributesRpcsAndRuntimeBeans.getRuntimeBeanEntries(), rpcs);
	...
	private static Map<QName/*of identity*/, Set<RpcDefinition>> getIdentitiesToRpcs(Module currentModule) {
		...
		for (RpcDefinition rpc : currentModule.getRpcs()) {
			...
			for (UsesNode uses : input.getUses()) {
				...
				if (false == qname.equals(ConfigConstants.RPC_CONTEXT_REF_GROUPING_QNAME))
					continue;
				...
				for (SchemaNode refinedNode : uses.getRefines().values()) {
					...
					for (UnknownSchemaNode unknownSchemaNode : refinedNode.getUnknownSchemaNodes()) {
						if (ConfigConstants.RPC_CONTEXT_INSTANCE_EXTENSION_QNAME.equals(unknownSchemaNode
								.getNodeType())) {
							String localIdentityName = unknownSchemaNode.getNodeParameter();
							QName identityQName = new QName(currentModule.getNamespace(), currentModule.getRevision(),
									localIdentityName);
							Set<RpcDefinition> rpcDefinitions = result.get(identityQName);
							if (rpcDefinitions == null) {
								throw new IllegalArgumentException("Identity referenced by rpc not found. Identity:"
										+ localIdentityName + " , rpc " + rpc);
							}
	...
	/**
	 * Get direct descendants of this subtree, together with attributes defined in subtree.
	 */
	private static AttributesRpcsAndRuntimeBeans extractSubtree(String packageName, DataNodeContainer subtree,
...
//		List<TOAttribute> toAttributes = new ArrayList<>();
		...
		for (DataSchemaNode child : subtree.getChildNodes()) {
			// child leaves can be java attributes, TO attributes, or child runtime beans
			if (child instanceof LeafSchemaNode) {
				...
				if(isInnerStateBean(child)) {
					...
					RuntimeBeanEntry hierarchicalChild = createHierarchical(packageName, listSchemaNode,
							typeProviderWrapper, currentModule, identitiesToRpcs);
					runtimeBeanEntries.add(hierarchicalChild);
				} else /* ordinary list attribute*/ {
					ListAttribute listAttribute = ListAttribute.create((ListSchemaNode) child, typeProviderWrapper);
					attributes.add(listAttribute);
				}

			} else {
				throw new IllegalStateException("Unknown running-data node " + child + " , " + ""
						+ "expected leaf or container");
			}
		}
		...
		for (UnknownSchemaNode unknownSchemaNode : subtreeSchemaNode.getUnknownSchemaNodes()) {
			if (ConfigConstants.RPC_CONTEXT_INSTANCE_EXTENSION_QNAME.equals(unknownSchemaNode.getNodeType())) {
				...
				if (rpcDefinitions == null) {
					throw new IllegalArgumentException("Cannot find identity " + localIdentityName + " to be used as "
							+ "context reference when resolving " + unknownSchemaNode);
				}
				// convert RpcDefinition to Rpc
				for (RpcDefinition rpcDefinition : rpcDefinitions) {
					String name = ModuleMXBeanEntry.findJavaParameter(rpcDefinition);
					String returnType;
					if (rpcDefinition.getOutput() == null || rpcDefinition.getOutput().getChildNodes().size() == 0) {
						...
						checkArgument(returnDSN instanceof LeafSchemaNode, "Unexpected type of rpc return type. "
								+ "Currently only leafs and empty output nodes are supported, got " + returnDSN);
						LeafSchemaNode returnLeaf = (LeafSchemaNode) returnDSN;
						// TODO CISCO-1364
						String localName = returnLeaf.getQName().getLocalName();
						checkArgument(localName.equals("result"),
								"Unexpected name of leaf in output element, expected leaf named result, was %s at %s",
								localName, currentModule.getName());

						returnType = typeProviderWrapper.getType(returnLeaf).getFullyQualifiedName();
					} else {
						throw new IllegalArgumentException("More than one child node in rpc output is not supported. "
								+ "Error occured in " + rpcDefinition);
					}
					List<JavaAttribute> parameters = new ArrayList<>();
					for (DataSchemaNode childNode : rpcDefinition.getInput().getChildNodes()) {
						if (childNode.isAddedByUses() == false) { // skip refined context-instance
							...
							JavaAttribute javaAttribute = new JavaAttribute((LeafSchemaNode) childNode,
									typeProviderWrapper);
	...
	private static RuntimeBeanEntry createHierarchical(String packageName, ListSchemaNode listSchemaNode,
			TypeProviderWrapper typeProviderWrapper, Module currentModule,
			Map<QName, Set<RpcDefinition>> identitiesToRpcs) {

		// supported are numeric types, strings, enums
		// get all attributes
		AttributesRpcsAndRuntimeBeans attributesRpcsAndRuntimeBeans = extractSubtree(packageName, listSchemaNode,
				typeProviderWrapper, currentModule, identitiesToRpcs);
		...
		if (listSchemaNode.getKeyDefinition().size() == 0) {
			...
			QName keyQName = listSchemaNode.getKeyDefinition().iterator().next();
		...
		String javaNamePrefix = ModuleMXBeanEntry.findJavaNamePrefix(listSchemaNode);

		RuntimeBeanEntry rbFromAttributes = new RuntimeBeanEntry(packageName, listSchemaNode, listSchemaNode.getQName()
				.getLocalName(), javaNamePrefix, false, keyYangName, attributesRpcsAndRuntimeBeans.getAttributes(),
				attributesRpcsAndRuntimeBeans.getRuntimeBeanEntries(), attributesRpcsAndRuntimeBeans.getRpcs());

		return rbFromAttributes;
	}

	private static RuntimeBeanEntry createRoot(String packageName, DataSchemaNode nodeForReporting,
			String attributeYangName, List<AttributeIfc> attributes, String javaNamePrefix,
			List<RuntimeBeanEntry> children, Set<Rpc> rpcs) {
		return new RuntimeBeanEntry(packageName, nodeForReporting, attributeYangName, javaNamePrefix, true,
	...
	private static class AttributesRpcsAndRuntimeBeans {
		...
		public AttributesRpcsAndRuntimeBeans(List<RuntimeBeanEntry> runtimeBeanEntries,
	...
	public static class Rpc {
		...
		Rpc(String returnType, String name, String yangName, List<JavaAttribute> parameters) {
	...
	public String toString() {
		return "RuntimeBeanEntry{" + "isRoot=" + isRoot + ", yangName='" + yangName + '\'' + ", packageName='"
				+ packageName + '\'' + ", keyYangName=" + keyYangName + '}';
	}
}
