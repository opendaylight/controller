...
public class JavaAttribute extends AbstractAttribute implements TypedAttribute {
     ...
     */
    private Type getBaseType(TypeProviderWrapper typeProviderWrapper, TypeDefinition<?> baseType) {
        while(baseType.getBaseType()!=null) {
            baseType = baseType.getBaseType();
        }
        return typeProviderWrapper.getType(baseType, baseType);
    }
    ...
    public OpenType<?> getOpenType() {
        Type baseType = getBaseType(typeProviderWrapper, typeDefinition);

        if (isArray()) {
            return getArrayType();
        } else if (isDerivedType(baseType)) {
            return getCompositeType(baseType);
        }
    ...
    private OpenType<?> getSimpleType() {
     ...
     private OpenType<?> getCompositeType(Type baseType) {
