...
public class TOAttribute extends AbstractAttribute {
	...
	public static <T extends DataNodeContainer & AugmentationTarget & DataSchemaNode> TOAttribute create(
		...
		for (DataSchemaNode dataSchemaNode : containerSchemaNode.getChildNodes()) {
			try {
				...
				map.put(yangName, createInnerAttribute(dataSchemaNode, typeProviderWrapper));
			} catch (IllegalArgumentException e) {
				throw new IllegalStateException("Unable to create TO");
			}
		}
		return new TOAttribute(containerSchemaNode, map, attributeNameMap, containerSchemaNode.getDescription());
	}

	private static AttributeIfc createInnerAttribute(DataSchemaNode dataSchemaNode,
		...
		if (type.equals(LeafSchemaNode.class))
			...
			return ListAttribute.create((ListSchemaNode) dataSchemaNode, typeProviderWrapper);
			...
			return TOAttribute.create((ContainerSchemaNode) dataSchemaNode, typeProviderWrapper);
	...
	private static Class<? extends DataSchemaNode> isAllowedType(DataSchemaNode dataSchemaNode) {
		...
		throw new IllegalArgumentException("Illegal child node for TO: " + dataSchemaNode.getClass()
				+ " allowed node types: " + ALLOWED_CHILDREN);
	}

	private TOAttribute(DataSchemaNode attrNode, Map<String, AttributeIfc> transferObject,
	...
	public Map<String, AttributeIfc> getCapitalizedPropertiesToTypesMap() {
		Map<String, AttributeIfc> capitalizedPropertiesToTypesMap = Maps.newHashMap();
		for (Entry<String, AttributeIfc> entry : yangNameToAttributeMap.entrySet()) {

			capitalizedPropertiesToTypesMap.put(ModuleMXBeanEntry.convertToJavaName(entry.getKey(), true),
					entry.getValue());
		}
	...
	public boolean equals(Object o) {
		...
		if (nullableDefault != null ? !nullableDefault.equals(that.nullableDefault) : that.nullableDefault != null)
			return false;
		if (nullableDescription != null ? !nullableDescription.equals(that.nullableDescription)
				: that.nullableDescription != null)
			return false;
		if (yangNameToAttributeMap != null ? !yangNameToAttributeMap.equals(that.yangNameToAttributeMap)
	...
	public int hashCode() {
		...
		result = 31 * result + (nullableDescription != null ? nullableDescription.hashCode() : 0);
		result = 31 * result + (nullableDefault != null ? nullableDefault.hashCode() : 0);
		result = 31 * result + (yangNameToAttributeMap != null ? yangNameToAttributeMap.hashCode() : 0);
		return result;
	}

	@Override
	public String toString() {
		return "TOAttribute{" + getAttributeYangName() + "," + "to=" + yangNameToAttributeMap + '}';
	}

	@Override
	public OpenType<?> getOpenType() {
		String description = getNullableDescription() == null ? getAttributeYangName() : getNullableDescription();
		final String[] itemNames = new String[yangNameToAttributeMap.keySet().size()];
		...
		OpenType<?>[] itemTypes = Collections2.transform(jmxPropertiesToTypesMap.entrySet(), functionImpl).toArray(
				new OpenType<?>[] {});
		try {
			// TODO add package name to create fully qualified name for this type
			CompositeType compositeType = new CompositeType(getUpperCaseCammelCase(), description, itemNames,
	...
	private static final class FunctionImpl implements Function<Entry<String, AttributeIfc>, OpenType<?>> {
