...
public class ModuleMXBeanEntry extends AbstractEntry {
	...
	private static final String MODULE_CONDITION_XPATH_TEMPLATE =
			"^/MAGIC_STRING:modules/MAGIC_STRING:module/MAGIC_STRING:type\\s*=\\s*['\"](.+)['\"]$";
	private static final SchemaPath expectedConfigurationAugmentationSchemaPath = new SchemaPath(
		Arrays.asList(createConfigQName("modules"),
				createConfigQName("module"), createConfigQName("configuration")),true);
	private static final SchemaPath expectedStateAugmentationSchemaPath = new SchemaPath(
			Arrays.asList(createConfigQName("modules"),
					createConfigQName("module"), createConfigQName("state")),true);
	...
	/*
	threadpool-dynamic from the example above, taken from when condition, not  the case name
	 */
	private final String globallyUniqueName;

	private Map<String, AttributeIfc> yangToAttributes;

	private final String nullableDescription, packageName, javaNamePrefix, namespace;
	...
	public ModuleMXBeanEntry(IdentitySchemaNode id, Map<String, AttributeIfc> yangToAttributes, String packageName,
			Map<String, String> providedServices2, String javaNamePrefix, String namespace,
			Collection<RuntimeBeanEntry> runtimeBeans) {
	...
	public static Map<String/* identity local name */, ModuleMXBeanEntry> create(
		...
		logger.debug("Generating ModuleMXBeans of {} to package {}", currentModule.getNamespace(), packageName);
			...
			if (id.getBaseIdentity() != null
				...
				for (UnknownSchemaNode unknownNode : id.getUnknownSchemaNodes()) {
					...
					if (ConfigConstants.PROVIDED_SERVICE_EXTENSION_QNAME.equals(unknownNode.getNodeType())) {
						...
						// 0..1 allowed
						checkState(providedServiceWasSet == false,
		...
		for (AugmentationSchema augmentation : currentModule.getAugmentations()) {
			...
			if (childNodes.size() == 1) {
				...
				if (when instanceof ChoiceCaseNode) {
					...
					if (choiceCaseNode.getConstraints() == null || choiceCaseNode.getConstraints().getWhenCondition()
							== null) {
						continue;
					}
					...
					Map<String, String> providedServices = findProvidedServices(
							moduleIdentity, currentModule, qNamesToSIEs,schemaContext);
					...
					if (expectedConfigurationAugmentationSchemaPath.equals(augmentation.getTargetPath())) {
						...
						yangToAttributes  = fillConfiguration(choiceCaseNode,
								currentModule, typeProviderWrapper, qNamesToSIEs, schemaContext);
						...
						try {
							runtimeBeans = fillRuntimeBeans(choiceCaseNode, currentModule, typeProviderWrapper,
									packageName, moduleLocalNameFromXPath, javaNamePrefix);
					...
					if (result.containsKey(moduleLocalNameFromXPath)) {
						...
						ModuleMXBeanEntry moduleMXBeanEntry = new ModuleMXBeanEntry(moduleIdentity, yangToAttributes,
								packageName, providedServices, javaNamePrefix, currentModule.getNamespace().toString(),
	                            runtimeBeans);
		...
		for (Entry<String, ModuleMXBeanEntry> entry: result.entrySet()){
			...
			if (module.getAttributes() == null){
				module.setYangToAttributes(Collections.<String, AttributeIfc>emptyMap());
			} else if (module.getRuntimeBeans() == null){
				module.setRuntimeBeans(Collections.<RuntimeBeanEntry>emptyList());
			}
		}
	...
	private static void checkUniqueAttributesWithGeneratedClass(Map<String, QName> uniqueGeneratedClassNames, QName parentQName, Map<String, AttributeIfc> yangToAttributes) {
		for (Entry<String, AttributeIfc> attr : yangToAttributes.entrySet()) {
			...
			} else if (attr.getValue() instanceof ListAttribute && ((ListAttribute) attr.getValue()).getInnerAttribute() instanceof TOAttribute) {
				checkUniqueTOAttr(uniqueGeneratedClassNames, parentQName, (TOAttribute) ((ListAttribute) attr.getValue()).getInnerAttribute());
			}
		}
	}
	...
	private static Collection<RuntimeBeanEntry> fillRuntimeBeans(ChoiceCaseNode choiceCaseNode,
	                                                             Module currentModule,
	                                                             TypeProviderWrapper typeProviderWrapper,
	                                                             String packageName,
	                                                             String moduleLocalNameFromXPath,
	                                                             String javaNamePrefix) {

		return RuntimeBeanEntry.extractClassNameToRuntimeBeanMap(packageName,
				choiceCaseNode,
						moduleLocalNameFromXPath,
						typeProviderWrapper, javaNamePrefix,
						currentModule).values();

	}

	private static Map<String, AttributeIfc>  fillConfiguration(ChoiceCaseNode choiceCaseNode,
	                                                            Module currentModule,
	                                                            TypeProviderWrapper typeProviderWrapper,
	                                                            Map<QName, ServiceInterfaceEntry> qNamesToSIEs,
	                                                            SchemaContext schemaContext) {
		...
		for (DataSchemaNode attrNode : choiceCaseNode.getChildNodes()) {
			AttributeIfc attributeValue = getAttributeValue(attrNode, currentModule, qNamesToSIEs,
					typeProviderWrapper, schemaContext);
	...
	private static Map<String, String> findProvidedServices(
		...
		for (UnknownSchemaNode unknownNode : moduleIdentity
			...
			if (ConfigConstants.PROVIDED_SERVICE_EXTENSION_QNAME
				...
				result.put(sie.getFullyQualifiedName(), sie.getQName().getLocalName());
			}
		}
		return result;
	}
	...
	public static String convertToJavaName(SchemaNode schemaNode,
		...
		return convertToJavaName(schemaNode.getQName().getLocalName(), capitalizeFirstLetter);
	}
	...
	private static AttributeIfc getAttributeValue(DataSchemaNode attrNode, Module currentModule,
		...
		if (attrNode instanceof LeafSchemaNode) {
			...
			if (containerSchemaNode.getUses().size() == 1 && getChildNodeSizeWithoutUses(containerSchemaNode) == 0) {
				// reference
				UsesNode usesNode = containerSchemaNode.getUses().iterator().next();
				checkState(usesNode.getRefines().size() == 1,
						"Unexpected 'refine' child node size of " + containerSchemaNode);
				...
				return new DependencyAttribute(attrNode, serviceInterfaceEntry, mandatory, attrNode.getDescription());
			} else {
				return TOAttribute.create(containerSchemaNode, typeProviderWrapper);
			}
			...
			return ListAttribute.create((ListSchemaNode) attrNode, typeProviderWrapper);
		} else {
