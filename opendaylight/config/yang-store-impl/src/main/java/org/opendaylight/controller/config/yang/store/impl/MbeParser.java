...
public class MbeParser {
	...
	public YangStoreSnapshot parseYangFiles(Collection<? extends InputStream> allInput) throws YangStoreException {
        ...
        for (Entry<InputStream, Module> moduleEntry : allYangModules.entrySet()) {
            ...
            modulesMap.put(moduleEntry.getValue().getName(), Maps.immutableEntry(moduleEntry.getValue(), yangAsString));
            Map<String /* MB identity local name */, ModuleMXBeanEntry> namesToMBEs = ModuleMXBeanEntry.create(moduleEntry.getValue(),
					qNamesToSIEs, resolveSchemaContext, typeProviderWrapper, packageName);
	...
	public Map<Module, String> parseYangFilesToString(Collection<? extends InputStream> allYangs) {
        ...
        Map<InputStream, Module> allYangModules = parser.parseYangModelsFromStreamsMapped(Lists.newArrayList(allYangs));
