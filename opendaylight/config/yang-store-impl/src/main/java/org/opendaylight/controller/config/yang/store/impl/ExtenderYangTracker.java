...
public class ExtenderYangTracker extends BundleTracker<Object> implements YangStoreService {

	private static final Logger logger = LoggerFactory.getLogger(ExtenderYangTracker.class);

	private final Multimap<Bundle, URL> bundlesToYangURLs = HashMultimap.create();
    ...
    public synchronized YangStoreSnapshot getYangStoreSnapshot() throws YangStoreException {
        Optional<YangStoreSnapshot> yangStoreOpt = cache.getCachedYangStore(bundlesToYangURLs);
        ...
        try {
            YangStoreSnapshot yangStoreSnapshot = mbeParser.parseYangFiles(fromUrlsToInputStreams());
            logger.debug("{} module entries parsed successfully from {} yang files",
                    yangStoreSnapshot.countModuleMXBeanEntries(), bundlesToYangURLs.values().size());
            cache.cacheYangStore(bundlesToYangURLs, yangStoreSnapshot);

            return yangStoreSnapshot;
        } catch (RuntimeException e) {
            logger.warn("Unable to parse yang files, yang files that were picked up so far: {}", bundlesToYangURLs,
                    e);
    ...
    private Collection<InputStream> fromUrlsToInputStreams() {
		return Collections2.transform(bundlesToYangURLs.values(), new Function<URL, InputStream>() {
	...
	private static final class YangStoreCache {
		...
		Optional<YangStoreSnapshot> getCachedYangStore(Multimap<Bundle, URL> bundlesToYangURLs) {
		...
		void cacheYangStore(Multimap<Bundle, URL> urls, YangStoreSnapshot yangStoreSnapshot) {
