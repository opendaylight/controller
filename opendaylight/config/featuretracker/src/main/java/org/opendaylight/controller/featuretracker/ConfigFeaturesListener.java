...
public class ConfigFeaturesListener implements FeaturesListener {
    ...
    public ConfigFeaturesListener(FeaturesService f, ConfigPusher p) {
        ...
        final Thread pushingThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                List<String> pushedConfigs = new ArrayList<String>();
                while(true) {
                    FeatureEvent event;
                    try {
                            ...
                            if(event != null && event.getFeature() !=null) {
                                processFeatureEvent(event,toInstall,pushedConfigs);
                            }
    ...
    private void processFeatureEvent(FeatureEvent event, List<Feature> toInstall, List<String> pushedConfigs) {
        ...
        if(event.getType() == EventType.FeatureInstalled) {
            ...
            attempt(toInstall,pushedConfigs);
        } else if(event.getType() == EventType.FeatureUninstalled) {
            toInstall.remove(event.getFeature());
        }
    }

    private void attempt(List<Feature> toInstall, List<String> pushedConfigs) {
        List<Feature> features = Arrays.asList(featureService.listInstalledFeatures());
        for(Feature feature: ImmutableList.copyOf(toInstall)) {
            if(features.contains(feature)) {
                //todo Look into what isReplay means
                try {
                    toInstall.remove(feature);
                    List<ConfigSnapshotHolder> configs = featureToConfigSnapsShots(feature);
                    configs = removeAlreadyPushedConfigs(configs,pushedConfigs);
                    if(configs != null && !configs.isEmpty()) {
                        pusher.addJob(configs);
                    }
    ...
    private List<ConfigSnapshotHolder> removeAlreadyPushedConfigs(
            List<ConfigSnapshotHolder> configs, List<String> pushedConfigs) {

        for(ConfigSnapshotHolder c : ImmutableList.copyOf(configs)) {
            if(pushedConfigs.contains(c.getConfigSnapshot())) {
                configs.remove(c);
                logger.info("Ignoring duplicate config: ",c.getConfigSnapshot());
            } else {
                pushedConfigs.add(c.getConfigSnapshot());
            }
        }
        return configs;
    }
    ...
    protected List<ConfigSnapshotHolder> featureToConfigSnapsShots(Feature feature) {
        List<ConfigSnapshotHolder> configs = new ArrayList<ConfigSnapshotHolder>();
        List<Feature> dependentFeatures = getDepedentFeatures(feature);
    ...
    private List<Feature> getDepedentFeatures(Feature feature) {
        ...
        for(Dependency dependency: dependencies) {
            ...
            if(fi == null) {
                logger.warn("Skipping Feature: " + feature.getName() +
                                    " Version: " + feature.getVersion() +
