...
/**
 * This class interfaces with yang-maven-plugin. Gets parsed yang modules in {@link SchemaContext}, and parameters form
 * the plugin configuration, and writes service interfaces and/or modules.
 */
public class JMXGenerator implements CodeGenerator {
	...
	@Override
	public Collection<File> generateSources(SchemaContext context, File outputBaseDir,
											Set<Module> yangModulesInCurrentMavenModule) {
		...
		Preconditions.checkArgument(namespaceToPackageMapping != null && !namespaceToPackageMapping.isEmpty(),
		...
		for (Module module : context.getModules()) {
			...
			if (yangModulesInCurrentMavenModule.contains(module)) {
				...
				for (ServiceInterfaceEntry sie : namesToSIEntries.values()) {
					try {
						generatedFiles.addFile(codeWriter.writeSie(sie, outputBaseDir));
					} catch (Exception e) {
		...
		for (Module module : yangModulesInCurrentMavenModule) {
			String packageName = packageTranslator.getPackageName(module);
			Map<String /* MB identity local name */, ModuleMXBeanEntry> namesToMBEs = ModuleMXBeanEntry.create(module,
					qNamesToSIEs, context, new TypeProviderWrapper(new TypeProviderImpl(context)), packageName);

			for (Entry<String, ModuleMXBeanEntry> mbeEntry : namesToMBEs.entrySet()) {
				ModuleMXBeanEntry mbe = mbeEntry.getValue();
				try {
					List<File> files1 = codeWriter.writeMbe(mbe, outputBaseDir, mainBaseDir, resourceBaseDir);
		...
		if (fullyQualifiedNamesOfFactories.length() > 0 && generateModuleFactoryFile) {
			File serviceLoaderFile = JMXGenerator.concatFolders(resourceBaseDir, "META-INF", "services",
			...
			try {
				...
				FileUtils.write(serviceLoaderFile, fullyQualifiedNamesOfFactories.toString());
			} catch (IOException e) {
    ...
    public void setAdditionalConfig(Map<String, String> additionalCfg) {
        if (logger != null)
            logger.debug(getClass().getCanonicalName(), ": Additional configuration received: ",
    ...
    static Pattern namespaceMappingPattern = Pattern.compile("(.+)" + NAMESPACE_TO_PACKAGE_DIVIDER + "(.+)");
    ...
    private static NamespaceMapping extractNamespaceMapping(String mapping) {
        ...
        Preconditions.checkArgument(matcher.matches(), String.format(
                "Namespace to package mapping:%s is in invalid format, requested format is: %s", mapping,
                namespaceMappingPattern));
    ...
    public void setMavenProject(MavenProject project) {
        ...
        if (logger != null)
            logger.debug(getClass().getCanonicalName(), " project base dir: ", projectBaseDir);
    }
		...
		void addFile(File file) {
			if (files.contains(file)) {
				...
				if(undeletedFiles.isEmpty() == false) {
					logger.error("Illegal state occurred: Unable to delete already generated files, undeleted files: {}", undeletedFiles);
				}
