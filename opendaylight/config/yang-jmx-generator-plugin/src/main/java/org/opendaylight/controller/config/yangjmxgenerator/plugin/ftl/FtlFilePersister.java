...
public class FtlFilePersister {
	...
	public Map<FtlTemplate, String> serializeFtls(Collection<? extends FtlTemplate> ftlFiles) {
		...
		for (FtlTemplate ftlFile : ftlFiles) {
			...
			try (Writer writer = new StringWriter()) {
				Template template = getCfg().getTemplate(ftlFile.getFtlTempleteLocation());
				try {
					template.process(ftlFile, writer);
				} catch (TemplateException e) {
					throw new IllegalStateException("Template error while generating " + ftlFile, e);
				}
				result.put(ftlFile, writer.toString());
			} catch (IOException e) {
	...
	public List<File> persist(Collection<? extends FtlTemplate> ftlFiles, File dstFolder,
		...
		for (Entry<FtlTemplate, String> entry : ftlFileStringMap.entrySet()) {
			...
			if (targetFile.exists() && overwrite == false) {
				...
				try (Writer fileWriter = new FileWriter(targetFile)) {
					Template template = getCfg().getTemplate(ftlFile.getFtlTempleteLocation());
					try {
						template.process(ftlFile, fileWriter);
					} catch (TemplateException e) {
						throw new IllegalStateException("Template error while generating " + targetFile, e);
					}
				}
	...
	private Configuration getCfg() {
		...
		cfg.setSharedVariable("typeDeclarationD", new TypeDeclarationDirective());
		...
		cfg.setSharedVariable("unimplementedExceptionD", new UnimplementedExceptionDirective());
		return cfg;
	}
