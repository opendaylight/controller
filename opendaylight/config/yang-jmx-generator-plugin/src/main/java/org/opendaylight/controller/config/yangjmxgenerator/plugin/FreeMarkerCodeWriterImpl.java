...
final class FreeMarkerCodeWriterImpl implements CodeWriter {
	...
	public List<File> writeMbe(ModuleMXBeanEntry mbe, File targetBaseDir, File mainBaseDir, File resourceBaseDir) {
		try {
			...
			generatedFiles.addAll(ftlFilePersister.persist(TemplateFactory.getFtlTemplates(mbe).values(),
					targetBaseDir, true));
			generatedFiles.addAll(ftlFilePersister.persist(TemplateFactory.getFtlStubTemplates(mbe).values(),
					mainBaseDir, false));
			...
			if (runtimeBeans.size() > 0) {
				...
				{ // registrators
						Map<String, FtlTemplate> registratorNamesToFtls = RuntimeRegistratorFtlTemplate.create
							(RuntimeRegistratorFtlTemplate.findRoot(runtimeBeans));
				...
				{ // TOs, MXBean interfaces
					for(RuntimeBeanEntry runtimeBeanEntry : runtimeBeans) {
						Collection<FtlTemplate> ftlFiles = TemplateFactory
								.getTOAndMXInterfaceFtlFiles(runtimeBeanEntry).values();
