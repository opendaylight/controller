...
public class AbstractFactoryTemplate extends GeneralClassTemplate {
	...
	private static final List<String> implementedIfcs = Lists.newArrayList(ModuleFactory.class
			.getCanonicalName());

	private final String globallyUniqueName, moduleInstanceType;
	private final List<String> providedServices;

	public AbstractFactoryTemplate(Header header, String packageName, String abstractFactoryName,
			String globallyUniqueName, String moduleInstanceType, List<Field> fields, List<String> providedServices) {
		super(header, packageName, abstractFactoryName, Collections.<String> emptyList(), implementedIfcs, fields,
				Collections.<MethodDefinition> emptyList(), true, false, Collections.<Constructor> emptyList());
