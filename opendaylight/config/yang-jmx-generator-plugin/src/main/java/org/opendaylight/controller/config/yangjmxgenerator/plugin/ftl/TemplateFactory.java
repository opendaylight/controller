...
public class TemplateFactory {
	...
	public static Map<String, FtlTemplate> getFtlTemplates(ModuleMXBeanEntry entry) {
		Map<String, FtlTemplate> result = new HashMap<>();

		result.putAll(TemplateFactory.tOsFromMbe(entry));

		// IFC
		result.put(entry.getMXBeanInterfaceName() + ".java", TemplateFactory.mXBeanInterfaceTemplateFromMbe(entry));

		// ABS fact
		result.put(entry.getAbstractFactoryName() + ".java", TemplateFactory.abstractFactoryTemplateFromMbe(entry));

		// ABS module
		result.put(entry.getAbstractModuleName() + ".java", TemplateFactory.abstractModuleTemplateFromMbe(entry));

		return result;
	}

	public static Map<String, FtlTemplate> getFtlStubTemplates(ModuleMXBeanEntry entry) {
		Map<String, FtlTemplate> result = new HashMap<>();
		// STUB fact
		result.put(entry.getStubFactoryName() + ".java", TemplateFactory.stubFactoryTemplateFromMbe(entry));

		result.put(entry.getStubModuleName() + ".java", TemplateFactory.stubModuleTemplateFromMbe(entry));
		return result;
	}

	public static Map<String, FtlTemplate> getFtlTemplates(ServiceInterfaceEntry entry) {

		Map<String, FtlTemplate> result = new HashMap<>();
		result.put(entry.getTypeName() + ".java", TemplateFactory.serviceInterfaceFromSie(entry));

		return result;
	}

	/**
	 * Get map of file name as key, FtlFile instance representing runtime mx bean as value that should be
	 * persisted from this instance.
	 */
	public static Map<String, FtlTemplate> getTOAndMXInterfaceFtlFiles(RuntimeBeanEntry entry) {
		Map<String, FtlTemplate> result = new HashMap<>();
		{ // create GeneralInterfaceFtlFile for runtime MXBean. Attributes will be transformed to getter methods
			...
			for (AttributeIfc attributeIfc : entry.getAttributes()) {
				...
				if (attributeIfc instanceof TypedAttribute) {
					returnType = ((TypedAttribute) attributeIfc).getType().getFullyQualifiedName();
				} else if (attributeIfc instanceof TOAttribute) {
					String fullyQualifiedName = FullyQualifiedNameHelper.getFullyQualifiedName(entry.getPackageName(),
							attributeIfc.getUpperCaseCammelCase());

					returnType = fullyQualifiedName;
				} else if (attributeIfc instanceof ListAttribute) {
					AttributeIfc innerAttr = ((ListAttribute) attributeIfc).getInnerAttribute();

					String innerTpe = innerAttr instanceof TypedAttribute ? ((TypedAttribute) innerAttr).getType()
							.getFullyQualifiedName() : FullyQualifiedNameHelper.getFullyQualifiedName(entry.getPackageName(),
							attributeIfc.getUpperCaseCammelCase());
				...
				MethodDeclaration getter = new MethodDeclaration(returnType, getterName,
			...
			for (Rpc rpc : entry.getRpcs()) {
				...
				for (JavaAttribute ja : rpc.getParameters()) {
					Field field = new Field(Collections.<String> emptyList(), ja.getType().getFullyQualifiedName(),
							ja.getLowerCaseCammelCase());
					fields.add(field);
				}
				MethodDeclaration operation = new MethodDeclaration(rpc.getReturnType(), rpc.getName(), fields);
				methods.add(operation);
			}

			//FIXME header
			GeneralInterfaceTemplate runtimeMxBeanIfc = new GeneralInterfaceTemplate(null, entry.getPackageName(),
					mxBeanTypeName, extendedInterfaces, methods);
	...
	public static GeneralInterfaceTemplate serviceInterfaceFromSie(ServiceInterfaceEntry sie) {
		...
		List<String> extendedInterfaces = Lists.newArrayList(AbstractServiceInterface.class.getCanonicalName());
		if (sie.getBase().isPresent()) {
			extendedInterfaces.add(sie.getBase().get().getFullyQualifiedName());
		}

		// FIXME header
		GeneralInterfaceTemplate sieTemplate = new GeneralInterfaceTemplate(getHeaderFromEntry(sie),
				sie.getPackageName(), sie.getTypeName(), extendedInterfaces, Lists.<MethodDeclaration> newArrayList());
		...
		sieTemplate.getAnnotations()
				.add(Annotation.createSieAnnotation(sie.getQName(), sie.getExportedOsgiClassName()));
	...
	public static AbstractFactoryTemplate abstractFactoryTemplateFromMbe(ModuleMXBeanEntry mbe) {
		...
		Collection<String> transformed = Collections2.transform(mbe.getProvidedServices().keySet(),
		...
		return new AbstractFactoryTemplate(getHeaderFromEntry(mbe), mbe.getPackageName(), mbe.getAbstractFactoryName(),
				mbe.getGloballyUniqueName(), mbe.getFullyQualifiedName(mbe.getStubModuleName()),
				attrProcessor.getFields(), Lists.newArrayList(transformed));
	}
	...
	public static AbstractModuleTemplate abstractModuleTemplateFromMbe(ModuleMXBeanEntry mbe) {
		...
		List<String> implementedIfcs = Lists.newArrayList(Module.class.getCanonicalName(),
        ...
        if(mbe.getRuntimeBeans()!= null && mbe.getRuntimeBeans().isEmpty() == false){
            generateRuntime = true;
            RuntimeBeanEntry rootEntry = RuntimeRegistratorFtlTemplate.findRoot(mbe.getRuntimeBeans());
            registratorFullyQualifiedName = rootEntry.getPackageName().concat(".").
		...
		AbstractModuleTemplate abstractModuleTemplate = new AbstractModuleTemplate(getHeaderFromEntry(mbe),
				mbe.getPackageName(), mbe.getAbstractModuleName(), implementedIfcs, moduleFields,
                attrProcessor.getMethods(), generateRuntime, registratorFullyQualifiedName);

		if (mbe.getNullableDescription() != null)
			abstractModuleTemplate.getAnnotations().add(
					Annotation.createDescriptionAnnotation(mbe.getNullableDescription()));
		return abstractModuleTemplate;
	}

	public static StubFactoryTemplate stubFactoryTemplateFromMbe(ModuleMXBeanEntry mbe) {
		return new StubFactoryTemplate(getHeaderFromEntry(mbe), mbe.getPackageName(), mbe.getStubFactoryName(),
				mbe.getFullyQualifiedName(mbe.getAbstractFactoryName()), mbe.getStubModuleName());
	}

	public static StubModuleTemplate stubModuleTemplateFromMbe(ModuleMXBeanEntry mbe) {
		return new StubModuleTemplate(getHeaderFromEntry(mbe), mbe.getPackageName(), mbe.getStubModuleName(),
	...
	public static GeneralInterfaceTemplate mXBeanInterfaceTemplateFromMbe(ModuleMXBeanEntry mbe) {
		...
		attrProcessor.processAttributes(mbe.getAttributes(), mbe.getPackageName());
		GeneralInterfaceTemplate ifcTemplate = new GeneralInterfaceTemplate(getHeaderFromEntry(mbe),
				mbe.getPackageName(), mbe.getMXBeanInterfaceName(), Lists.<String> newArrayList(),
	...
	public static Map<String, GeneralClassTemplate> tOsFromMbe(ModuleMXBeanEntry mbe) {
		...
		for (org.opendaylight.controller.config.yangjmxgenerator.plugin.ftl.TemplateFactory.TOAttributesProcessor.TOInternal to : processor
			...
			retVal.put(
					to.getType(),
					new GeneralClassTemplate(header, mbe.getPackageName(), to.getName(), Collections
							.<String> emptyList(), Collections.<String> emptyList(), to.getFields(), to.getMethods(),
							false, false, constructors));
		}
		return retVal;
	}
	...
	public static Map<String, GeneralClassTemplate> tOsFromRbe(RuntimeBeanEntry rbe) {
		Map<String, GeneralClassTemplate> retVal = Maps.newHashMap();
		TOAttributesProcessor processor = new TOAttributesProcessor();
		processor.processAttributes(rbe.getYangPropertiesToTypesMap(), rbe.getPackageName());
		for (org.opendaylight.controller.config.yangjmxgenerator.plugin.ftl.TemplateFactory.TOAttributesProcessor.TOInternal to : processor
				.getTOs()) {
			List<Constructor> constructors = Lists.newArrayList();
			constructors.add(new Constructor(to.getName(), "super();"));
			...
			retVal.put(
					to.getType(),
					new GeneralClassTemplate(null, rbe.getPackageName(), to.getName(),
							Collections.<String> emptyList(), Collections.<String> emptyList(), to.getFields(), to
									.getMethods(), false, false, constructors));
		}
		return retVal;
	}
	...
	private static class TOAttributesProcessor {
		...
		private void createTOInternal(String packageName, AttributeIfc attributeIfc) {
			String fullyQualifiedName = FullyQualifiedNameHelper.getFullyQualifiedName(packageName,
					attributeIfc.getUpperCaseCammelCase());

			String type = fullyQualifiedName;
			String name = attributeIfc.getUpperCaseCammelCase();
			Map<String, AttributeIfc> attrs = ((TOAttribute) attributeIfc).getCapitalizedPropertiesToTypesMap();
		...
		private static class TOInternal {
			...
			public TOInternal(String type, String name, Map<String, AttributeIfc> attrs, String packageName) {
				super();
				this.type = type;
				this.name = name;
			...
			private void processAttrs(Map<String, AttributeIfc> attrs, String packageName) {
				...
				for (Entry<String, AttributeIfc> attrEntry : attrs.entrySet()) {
					...
					String varName = BindingGeneratorUtil.parseToValidParamName(attrEntry.getKey());

					String fullyQualifiedName = null;
					if (attrEntry.getValue() instanceof TypedAttribute) {
						Type innerType = ((TypedAttribute) attrEntry.getValue()).getType();
						fullyQualifiedName = innerType.getFullyQualifiedName();
					} else if (attrEntry.getValue() instanceof ListAttribute) {
						AttributeIfc innerAttr = ((ListAttribute) attrEntry.getValue()).getInnerAttribute();

						String innerTpe = innerAttr instanceof TypedAttribute ? ((TypedAttribute) innerAttr).getType()
								.getFullyQualifiedName() : FullyQualifiedNameHelper.getFullyQualifiedName(packageName,
								attrEntry.getValue().getUpperCaseCammelCase());

						fullyQualifiedName = "java.util.List<" + innerTpe + ">";
					} else
						fullyQualifiedName = FullyQualifiedNameHelper.getFullyQualifiedName(packageName, attrEntry
								.getValue().getUpperCaseCammelCase());

					fields.add(new Field(fullyQualifiedName, varName));

					String getterName = "get" + innerName;
					MethodDefinition getter = new MethodDefinition(fullyQualifiedName, getterName,
							Collections.<Field> emptyList(), "return " + varName + ";");

					String setterName = "set" + innerName;
					MethodDefinition setter = new MethodDefinition("void", setterName, Lists.newArrayList(new Field(
							fullyQualifiedName, varName)), "this." + varName + " = " + varName + ";");
					methods.add(getter);
					methods.add(setter);
				}
			...
			for (Entry<String, AttributeIfc> attrEntry : attributes.entrySet()) {
				...
				if (attributeIfc instanceof TypedAttribute) {
					...
					if (innerAttr instanceof JavaAttribute) {
						fullyQualifiedName = ((JavaAttribute) innerAttr).getType().getFullyQualifiedName();
					} else if (innerAttr instanceof TOAttribute) {
						fullyQualifiedName = FullyQualifiedNameHelper.getFullyQualifiedName(packageName,
								innerAttr.getUpperCaseCammelCase());
					}

					returnType = STRING_FULLY_QUALIFIED_NAME.concat("<").concat(fullyQualifiedName).concat(">");
				} else {
					throw new UnsupportedOperationException("Attribute not supported: " + attributeIfc.getClass());
				}
				...
				MethodDeclaration setter = new MethodDeclaration("void", setterName, Lists.newArrayList(new Field(
						returnType, varName)));
				methods.add(getter);
				methods.add(setter);
	...
	private static class AbstractFactoryAttributesProcessor {
		...
		void processAttributes(Map<String, AttributeIfc> attributes, String packageName) {
			for (Entry<String, AttributeIfc> attrEntry : attributes.entrySet()) {
				...
				if (attributeIfc instanceof TypedAttribute) {
					type = ((TypedAttribute) attributeIfc).getType().getFullyQualifiedName();
				} else if (attributeIfc instanceof TOAttribute) {
					String fullyQualifiedName = FullyQualifiedNameHelper.getFullyQualifiedName(packageName,
							attributeIfc.getUpperCaseCammelCase());
					...
					type = STRING_FULLY_QUALIFIED_NAME.concat("<").concat(fullyQualifiedName).concat(">");

				} else {
					throw new UnsupportedOperationException("Attribute not supported: " + attributeIfc.getClass());
				}

				fields.add(new Field(type, attributeIfc.getUpperCaseCammelCase()));
			}
		}
	...
	private static class AbstractModuleAttributesProcessor {
        ...
        void processAttributes(Map<String, AttributeIfc> attributes, String packageName) {
            for (Entry<String, AttributeIfc> attrEntry : attributes.entrySet()) {
                ...
                List<Annotation> annotations = Lists.newArrayList(overrideAnnotation);

                if (attributeIfc instanceof DependencyAttribute) {
                    isDependency = true;
                    dependency = ((DependencyAttribute) attributeIfc).getDependency();
                    annotations.add(Annotation.createRequireIfcAnnotation(dependency.getSie()));
                }


                String varName = BindingGeneratorUtil.parseToValidParamName(attrEntry.getKey());
                moduleFields.add(new ModuleField(type, varName, attributeIfc.getUpperCaseCammelCase(), attributeIfc
                        .getNullableDefault(), isDependency, dependency));

                String getterName = "get" + attributeIfc.getUpperCaseCammelCase();
                MethodDefinition getter = new MethodDefinition(type, getterName, Collections.<Field>emptyList(), 
                		Lists.newArrayList(overrideAnnotation), "return " + varName + ";");
                ...
                MethodDefinition setter = new MethodDefinition("void", setterName, Lists.newArrayList(new Field(type,
                        varName)), annotations, "this." + varName + " = " + varName + ";");
