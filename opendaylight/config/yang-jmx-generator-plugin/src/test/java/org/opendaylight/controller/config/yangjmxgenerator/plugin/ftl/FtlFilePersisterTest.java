...
public class FtlFilePersisterTest {
	...
	public void testGeneralInterface() {
		...
		methods.add(new MethodDeclaration("String", "executeOperation", Collections.<Field> emptyList()));

		List<String> mods = Lists.newArrayList();
		List<String> mods2 = Lists.newArrayList("final");
		methods.add(new MethodDeclaration("String", "executeOperation", Arrays.asList(new Field(mods, "int", "param1"),
				new Field(mods2, "long", "param2"))));

		GeneralInterfaceTemplate generalInterface = new GeneralInterfaceTemplate(null, packageName, name,
				extendedInterfaces, methods);

		Map<FtlTemplate, String> abstractFtlFileStringMap = tested.serializeFtls(Arrays.asList(generalInterface));
		String content = FormattingUtil.cleanUpEmptyLinesAndIndent(abstractFtlFileStringMap.get(generalInterface));
