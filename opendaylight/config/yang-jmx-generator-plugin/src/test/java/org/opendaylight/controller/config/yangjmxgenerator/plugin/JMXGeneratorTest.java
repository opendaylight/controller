...
public class JMXGeneratorTest extends AbstractGeneratorTest {
	...
	private static final List<String> expectedModuleFileNames = ServiceInterfaceEntryTest.toFileNames
	...
	private static final List<String> expectedBGPNames = ServiceInterfaceEntryTest.toFileNames("[AbstractBgpListenerImplModule.java, " +
			"AbstractBgpListenerImplModuleFactory.java, BgpListenerImplModule.java, " +
			"BgpListenerImplModuleFactory.java, BgpListenerImplModuleMXBean.java, Peers.java]");

	private static final List<String> expectedNetconfNames = ServiceInterfaceEntryTest.toFileNames("[AbstractNetconfTestImplModule.java, " +
			"AbstractNetconfTestImplModuleFactory.java, AbstractTestImplModule.java, " +
			"AbstractTestImplModuleFactory.java, AutoCloseableServiceInterface.java, " +
			"ComplexDtoBInner.java, ComplexList.java, Deep.java, DtoA.java, DtoA1.java, DtoAInner.java, DtoAInnerInner.java, DtoB.java, DtoC.java," +
            "NetconfTestImplModule.java, NetconfTestImplModuleFactory.java," +
			"NetconfTestImplModuleMXBean.java, Peer.java, SimpleList.java, TestImplModule.java, " +
			"TestImplModuleFactory.java," +
			" TestImplModuleMXBean.java" +
			"]");
	...
	@Before
	public void setUp() {
		map.put(JMXGenerator.NAMESPACE_TO_PACKAGE_PREFIX + "1", ConfigConstants.CONFIG_NAMESPACE
				+ JMXGenerator.NAMESPACE_TO_PACKAGE_DIVIDER + PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX);
		...
		doNothing().when(mockedLog).error(any(CharSequence.class), any(Throwable.class));
	...
	public void generateSIsMBsTest() {
		...
		//TODO: separate expectedAllFileNames into expectedBGPNames, expectedNetconfNames
	...
	private void verifyModuleFactoryFile(boolean shouldBePresent) {
		File factoryFile = new File(generatedResourcesDir, "META-INF" + File.separator + "services" + File.separator
				+ ModuleFactory.class.getName());
		if (!shouldBePresent)
			assertFalse("Factory file should not be generated", factoryFile.exists());
		else
	...
	public void generateSIEsTest() throws IOException {
		...
		for (File file : files) {
			...
			assertThat(verifier.extnds, containsString("org.opendaylight.controller.config.api.annotations.AbstractServiceInterface"));
			...
			if ("ThreadPoolServiceInterface.java".equals(fileName)) {
				...
				assertContains(verifier.sieAnnotValue,
						"threadpool");
				assertContains(verifier.sieAnnotOsgiRegistrationType, PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX + ".threadpool.ThreadPool.class");
			} else if ("ScheduledThreadPoolServiceInterface.java".equals(fileName)) {
				assertContains(verifier.extnds, PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +".threads.ThreadPoolServiceInterface");
				...
				assertContains(verifier.sieAnnotOsgiRegistrationType,
						PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +".threadpool.ScheduledThreadPool.class");
				...
				assertContains(verifier.sieAnnotValue,
						"threadfactory");
				assertContains(verifier.sieAnnotOsgiRegistrationType, "java.util.concurrent.ThreadFactory.class");

			} else if ("ScheduledExecutorServiceServiceInterface.java".equals(fileName)) {
				assertContains(verifier.sieAnnotOsgiRegistrationType, "java.util.concurrent.ScheduledExecutorService.class");
			} else {
	...
	public void generateMBEsListTest() throws Exception {
		...
		Collection<File> files = jmxGenerator.generateSources(context, outputBaseDir,
				Sets.newHashSet(bgpListenerJavaModule));
	...
	public void generateMBEsTest() throws Exception {
		...
		Collection<File> files = jmxGenerator.generateSources(context, outputBaseDir, Sets.newHashSet
				(threadsJavaModule));
		...
		Collection<File> xmlFiles = Collections2.filter(files, new Predicate<File>() {
		...
		Collection<File> javaFiles = Collections2.filter(files, new Predicate<File>() {
		...
		Set<String> lines = Sets.newHashSet(FileUtils.readLines(moduleFactoryFile));
	...
	private void assertEventBusModuleFactory(MbeASTVisitor visitor) {
		...
		assertContains(visitor.extnds, PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +
		...
		assertEquals("Incorrenct number of generated constructors", 0, visitor.constructors.size());
		assertEquals("Incorrenct number of generated method descriptions", 0, visitor.methodDescriptions.size());
		assertEquals("Incorrenct number of generated method javadoc", 0, visitor.methodJavadoc.size());
	}
	...
	private void assertAsyncEventBusModule(MbeASTVisitor visitor) {
		...
		assertEquals("Incorrenct number of generated constructors", 2, visitor.constructors.size());
		assertEquals("Incorrenct number of generated method descriptions", 0, visitor.methodDescriptions.size());
		assertEquals("Incorrenct number of generated method javadoc", 0, visitor.methodJavadoc.size());
	}
	...
	private void assertAbstractNamingThreadFactoryModuleFactory(MbeASTVisitor visitor) {
		...
		assertDeclaredField(fieldDeclarations, "public static final java.lang.String NAME=\"threadfactory-naming\"");
		assertDeclaredField(
				fieldDeclarations,
		...
		assertEquals("Incorrenct number of generated methods", 5, visitor.methods.size());
		assertEquals("Incorrenct number of generated method descriptions", 0, visitor.methodDescriptions.size());
		assertEquals("Incorrenct number of generated method javadoc", 0, visitor.methodJavadoc.size());
	...
	private void assertAbstractDynamicThreadPoolModule(MbeASTVisitor visitor) {
		...
		assertContains(visitor.implmts, PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +
				".threads.java.DynamicThreadPoolModuleMXBean",
				Module.class.getCanonicalName(), PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +
				".threads.ScheduledThreadPoolServiceInterface",
				PackageTranslatorTest.EXPECTED_PACKAGE_PREFIX +
						".threads.ThreadPoolServiceInterface");
		assertEquals(2, visitor.constructors.size());
		Set<String> fieldDeclarations = visitor.fieldDeclarations;
		assertDeclaredField(fieldDeclarations, "private java.lang.Long maximumSize");
		assertDeclaredField(fieldDeclarations, "private javax.management.ObjectName threadfactory");
		assertDeclaredField(fieldDeclarations, "private java.util.concurrent.ThreadFactory threadfactoryDependency");
		assertDeclaredField(fieldDeclarations, "private java.lang.Long keepAlive=10");
		assertDeclaredField(fieldDeclarations, "private java.lang.Long coreSize");
		...
		assertEquals("Incorrenct number of generated methods", 24, visitor.methods.size());
		assertEquals("Incorrenct number of generated method descriptions", 3, visitor.methodDescriptions.size());
		assertEquals("Incorrenct number of generated method javadoc", 3, visitor.methodJavadoc.size());
		assertNotNull("Missing javadoc for setMaximumSize method", visitor.methodJavadoc.get("setMaximumSize"));
	}
	...
	private static class SieASTVisitor extends ASTVisitor {
		protected String packageName, descriptionAnotValue, sieAnnotValue, sieAnnotOsgiRegistrationType, type, extnds,
				javadoc;
		...
		public boolean visit(NormalAnnotation node) {
			if (node.getTypeName().toString().equals(Description.class.getCanonicalName())) {
				if (node.getParent() instanceof TypeDeclaration) {
					...
					methodDescriptions.put(((MethodDeclaration) node.getParent()).getName().toString(), descr);
				}
		...
		public boolean visit(TypeDeclaration node) {
			javadoc = node.getJavadoc() == null ? null : node.getJavadoc().toString();
			type = node.getName().toString();
			List<?> superIfcs = node.superInterfaceTypes();
			extnds = superIfcs != null && !superIfcs.isEmpty() ? superIfcs.toString() : null;
			return super.visit(node);
		}
	}
	...
	private static class MbeASTVisitor extends SieASTVisitor {
		...
		public boolean visit(TypeDeclaration node) {
			...
			extnds = node.getSuperclassType() == null ? null : node.getSuperclassType().toString();
			return visit;
		}
	...
	private void processGeneratedCode(Collection<File> files, Map<String, ASTVisitor> verifiers) throws IOException {
		...
		for (File file : files) {
			...
			for (IProblem c : cu.getProblems()) {
				// 1610613332 = Syntax error, annotations are only available if source level is 5.0
				if (c.getID() == 1610613332)
					continue;
				// 1610613332 = Syntax error, parameterized types are only available if source level is 5.0
				if (c.getID() == 1610613329)
					continue;
				fail("Error in generated source code " + file + ":" + c.getSourceLineNumber() + " " + c.toString());
			}
