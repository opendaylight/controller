...
public class RuntimeRegistratorFtlFileTest extends RuntimeRegistratorTest {
	...
	public void testRootWithoutAnything() {
		...
		String rootRegistratorName = RuntimeRegistratorFtlTemplate.getJavaNameOfRuntimeRegistration(rootRB
				.getJavaNamePrefix());
		FtlTemplate rootFtlFile = createdFtls.get(rootRegistratorName);
		assertNotNull(rootFtlFile);

		Map<FtlTemplate, String> serializedFtls = ftlFilePersister.serializeFtls(createdFtls.values());
	...
	public void testHierarchy2() {
		...
		assertThat(findRegistrationOutput(createdFtls, grandChildRB, serializedFtls), not(containsString(" register(")));

		FtlTemplate registrator = createdFtls.get(RuntimeRegistratorFtlTemplate
				.getJavaNameOfRuntimeRegistrator(rootRB));
		FormattingUtil.cleanUpEmptyLinesAndIndent(serializedFtls.get(registrator));

	}


	private String findRegistrationOutput(Map<String, FtlTemplate> createdFtls, RuntimeBeanEntry rb,
			Map<FtlTemplate, String> serializedFtls) {
		RuntimeRegistratorFtlTemplate rbFtlFile = (RuntimeRegistratorFtlTemplate)createdFtls.get(RuntimeRegistratorFtlTemplate
				.getJavaNameOfRuntimeRegistration(rb.getJavaNamePrefix()));
