...
abstract class ListableJolokiaClient {
	...
	public ListableJolokiaClient(String url, ObjectName objectName) {
		...
		if(!url.endsWith("/")){
			throw new IllegalArgumentException("Parameter 'url' must end with '/'");
		}
	...
	protected <R extends J4pResponse<T>, T extends J4pRequest> R execute(T pRequest) {
		try {
			Map<J4pQueryParameter, String> pProcessingOptions = new HashMap<J4pQueryParameter, String>();
			pProcessingOptions.put(J4pQueryParameter.INCLUDE_STACKTRACE, "true");
	...
	protected void tryToConvertException(String remoteStackTrace, JSONObject errorValue) {
		String conflictPrefix = ConflictingVersionException.class.getName() + ": ";
		if (remoteStackTrace.startsWith(conflictPrefix)) {
			remoteStackTrace = remoteStackTrace.substring(conflictPrefix.length());
			Pattern p = Pattern.compile("\r?\n");
			remoteStackTrace = Arrays.asList(p.split(remoteStackTrace)).iterator().next();
	...
	static ValidationException createValidationExceptionFromJSONObject(JSONObject errorValue) {
		String fValsKey = "failedValidations";
		JSONObject failedVals = (JSONObject) errorValue.get(fValsKey);

		checkArgument(!failedVals.isEmpty(), fValsKey + " was not present in received JSON: " + errorValue.toJSONString());
		...
		for (Object key : failedVals.keySet()) {
			checkArgument(key instanceof String, "Unexpected key " + key + ", expected instance of String");
			...
			for (Object innerKey : ((JSONObject)failedVals.get(key)).keySet()) {
				checkArgument(innerKey instanceof String, "Unexpected key " + innerKey + ", expected instance of String");
				JSONObject exWithStackTraceVal = (JSONObject) (((JSONObject)failedVals.get(key)).get(innerKey));
				Object mess = exWithStackTraceVal.get("message");
				Object stack = exWithStackTraceVal.get("trace");
				checkArgument(mess!=null && stack!=null, "\"Message\" and \"trace\" elements expected in received json: " + errorValue.toJSONString());
				innerMap.put(innerKey.toString(), new ExceptionMessageWithStackTrace((String) mess, (String) stack));
			}
	...
	private static void checkArgument(boolean b, String string) {
		if(b==false) throw new IllegalArgumentException(string);
	}
	...
	public Map<String, AttributeEntry> getAttributes(ObjectName on) {
		...
		J4pReadRequest j4pReadRequest = new J4pReadRequest(on, listMap.keySet().toArray(new String[0]));
		J4pResponse<J4pReadRequest> readResponse = execute(j4pReadRequest);
		Object readResponseValue = readResponse.getValue();
		// readResponseValue can be String if there is just one attribute or JSONObject
		...
		for (Entry<String, JSONObject> entry : listMap.entrySet()) {
			String key = entry.getKey();
			Object value = attribsToValues.size() > 0 ? attribsToValues.get(key) : readResponseValue;
			...
			AttributeEntry attributeEntry = new AttributeEntry(key, description, value, type, rw);
	...
	protected List<ObjectName> jsonArrayToObjectNames(JSONArray jsonArray) {
		...
		for (Object entry : jsonArray) {
			...
			try {
				...
				throw new IllegalStateException("Cannot convert " + objectNameString + " to ObjectName", e);
			}
		}
		return result;
	}
	...
	protected Set<ObjectName> lookupSomething(String signature, Object[] parameters) {
		J4pExecRequest req = new J4pExecRequest(objectName, signature, parameters);
	...
	public Set<ObjectName> lookupConfigBeans(String ifcName) {
		return lookupSomething("lookupConfigBeans(java.lang.String)", new Object[]{ifcName});
	}

	public Set<ObjectName> lookupConfigBeans(String ifcName, String instanceName) {
		return lookupSomething("lookupConfigBeans(java.lang.String,java.lang.String)", new Object[]{ifcName, instanceName});
	}

	public ObjectName lookupConfigBean(String ifcName, String instanceName) throws InstanceNotFoundException {
		J4pExecRequest req = new J4pExecRequest(objectName, "lookupConfigBean(java.lang.String,java.lang.String)", new Object[] {
				ifcName, instanceName });
		try {
			J4pResponse<J4pExecRequest> resp = execute(req);
			return extractObjectName(resp);
		} catch (RuntimeException e) {
			if (e.getMessage() != null
					&& e.getMessage().startsWith(InstanceNotFoundException.class.getName()))
	...
	public Set<String> getAvailableModuleNames() {
		J4pReadRequest req = new J4pReadRequest(objectName, "AvailableModuleNames");
