...
public class ConfigTransactionJolokiaClient extends ListableJolokiaClient implements ConfigTransactionClient {

	private final ConfigRegistryJolokiaClient configRegistryJolokiaClient;

	public ConfigTransactionJolokiaClient(String url, ObjectName transactionControllerON,
	...
	public ObjectName createModule(String moduleName, String instanceName)
		...
		J4pExecRequest execReq = new J4pExecRequest(objectName, "createModule", moduleName,
				instanceName);
		try {
			return extractObjectName(execute(execReq));
		} catch (RuntimeException e) {
			if (e.getMessage() != null
					&& e.getMessage().startsWith(InstanceAlreadyExistsException.class.getName()))
	...
	public void destroyConfigBean(String moduleName, String instanceName) throws InstanceNotFoundException {
		destroyModule(ObjectNameUtil.createTransactionModuleON(getTransactionName(), moduleName, instanceName));
	}
	...
	public void validateConfig() throws ValidationException {
		J4pExecRequest execReq = new J4pExecRequest(objectName, "validateConfig");
		execute(execReq);
	}
	...
	public void setAttribute(ObjectName configBeanTransactionON, String key, Object value) {
		J4pWriteRequest req = new J4pWriteRequest(configBeanTransactionON, key, value);
		try {
			execute(req);
		} catch (RuntimeException e) {
			if (e.getMessage() != null
					&& e.getMessage().startsWith(AttributeNotFoundException.class.getName())) {
				...
				for (AttributeEntry attrib : allAttributes.values()) {
					if (attrib.getKey().equalsIgnoreCase(key)) {
						req = new J4pWriteRequest(configBeanTransactionON, attrib.getKey(), value);
						execute(req);
						return;
					}
				}
