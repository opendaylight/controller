...
public class ConfigTransactionJMXClient implements ConfigTransactionClient {
	...
	public ConfigTransactionJMXClient(ConfigRegistryMXBean configTransactionManagerProxy,
			ObjectName configTransactionControllerON, MBeanServer configMBeanServer) {
		...
		this.configControllerProxy = JMX.newMXBeanProxy(configMBeanServer, configTransactionControllerON,
	...
	public CommitStatus commit() throws ConflictingVersionException, ValidationException {
		return configTransactionManagerProxy.commitConfig(configTransactionControllerON);
	}
	...
	public long getParentVersion() {
		try {
			return (Long) configMBeanServer.getAttribute(configTransactionControllerON, "ParentVersion");
		} catch (JMException e) {
	...
	public long getVersion() {
		try {
			return (Long) configMBeanServer.getAttribute(configTransactionControllerON, "Version");
		} catch (JMException e) {
			throw new RuntimeException(e);
		}
	}
	...
	public Set<ObjectName> lookupConfigBeans(String moduleName, String instanceName) {
		return configControllerProxy.lookupConfigBeans(moduleName, instanceName);
	}
