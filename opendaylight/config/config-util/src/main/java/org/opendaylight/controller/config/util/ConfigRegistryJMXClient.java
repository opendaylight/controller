...
public class ConfigRegistryJMXClient implements ConfigRegistryClient {
	...
	public ConfigRegistryJMXClient(MBeanServer configMBeanServer) {
		...
		configRegistryProxy = JMX
				.newMXBeanProxy(configMBeanServer, configRegistryON, ConfigRegistryMXBean.class, false);
	}
	...
	public ConfigTransactionJMXClient getConfigTransactionClient(ObjectName objectName) {
		return new ConfigTransactionJMXClient(configRegistryProxy, objectName, configMBeanServer);
	}
	...
	@Override
	public CommitStatus commitConfig(ObjectName transactionControllerON) throws ConflictingVersionException,
			ValidationException {
	...
	public long getVersion() {
		try {
			return (Long) configMBeanServer.getAttribute(configRegistryON, "Version");
		} catch (JMException e) {
	...
	public Object invokeMethod(ObjectName on, String name, Object[] params, String[] signature) {
		try {
			...
			throw new RuntimeException("Unable to invoke operation " + name + " on " + on + " with attributes "
					+ Arrays.toString(params) + " and signature " + Arrays.toString(signature), e);
		}
	}
	...
	public Object getAttributeCurrentValue(ObjectName on, String attributeName) {
		try {
			...
			throw new RuntimeException("Unable to get attribute " + attributeName + " for " + on, e);
		}
	}
