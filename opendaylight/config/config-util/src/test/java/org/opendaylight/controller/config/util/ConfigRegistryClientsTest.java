...
public class ConfigRegistryClientsTest {
	...
	private Set<ObjectName> clientLookupRuntimeBeansWithModuleAndInstance(
		...
		Set<ObjectName> beans = client
				.lookupRuntimeBeans(moduleName, instanceName);
		if (beans.size() > 0) {
	...
	public void testValidationExceptionDeserialization() {
		try {
			...
			assertThat(e.getFailedValidations().containsKey(moduleName), is(true));
			assertThat(e.getFailedValidations().size(), is(1));
			assertThat(e.getFailedValidations().get(moduleName).size(), is(1));
			assertThat(e.getFailedValidations().get(moduleName).containsKey(instanceName), is(true));
			assertThat(e.getFailedValidations().get(moduleName).get(instanceName).getMessage(), is("message"));
			assertThat(e.getFailedValidations().get(moduleName).get(instanceName).getTrace(), containsString("org.opendaylight.controller.config.util.TestingConfigRegistry.commitConfig"));
		}
	}
