...
public class ConfigurationIntegrationTest {
    private Logger logger = LoggerFactory.getLogger(ConfigurationIntegrationTest.class);
    // get the OSGI bundle context
    @Inject
    private BundleContext bc;
    private IClusterServices clusterService = null;
    private IConfigurationService configService = null;
    private IConfigurationContainerAware iConfigurationContainerAwareService = null;
    private Set<IConfigurationContainerAware> configurationAwareList = (Set<IConfigurationContainerAware>) Collections.synchronizedSet(new HashSet<IConfigurationContainerAware>());
    ...
    public Option[] config() {
        return options(
                ...
                mavenBundle("org.opendaylight.controller","sal","0.5.0-SNAPSHOT"), // SAL connects the protocols plug-ins to other stuff
                ...
                mavenBundle("org.opendaylight.controller","clustering.services", "0.4.0-SNAPSHOT"),  // what are the clustering services for
                mavenBundle("org.opendaylight.controller","clustering.stub","0.4.0-SNAPSHOT"),
    ...
    */
    ...
    public void areWeReadyForClustering() {
        ...
        for (int i = 0; i < b.length; i++) {
            ...
            if (state != Bundle.ACTIVE && state != Bundle.RESOLVED) {
            	logger.debug("Bundle:" + b[i].getSymbolicName() + " state:"+ stateToString(state));
                debugit = true;
            }
        }
        if (debugit) {
        ...
        assertFalse(debugit);
        ServiceReference r = bc.getServiceReference(IClusterServices.class.getName());
        if (r != null) {
    ...
    public void areWeReadyForConfiguration() {
        ...
        // Assert if true, if false we are good to go!
        assertFalse(debugit);
        ServiceReference r = bc.getServiceReference(IConfigurationService.class.getName());
        if (r != null) {
    ...
    public void saveConfiguration() {
    	...
    	if (r != null) {
    		this.iConfigurationContainerAwareService = (IConfigurationContainerAware) bc.getService(r);
    		configurationAwareList.add(this.iConfigurationContainerAwareService); 
    	}
        ...
        IConfigurationService configService = (IConfigurationService) ServiceHelper.getGlobalInstance(IConfigurationService.class, this);
        if (configService != null) {
        	status = configService.saveConfigurations();
        }
        if (status.getCode().equals(StatusCode.SUCCESS)){
        	Assert.assertTrue("Successfully saved config for " + configService.getClass().getSimpleName(),status.getCode().equals(StatusCode.SUCCESS));
        }else{
        	Assert.assertFalse("Failed to save config for " + configService.getClass().getSimpleName(),status.getCode().equals(StatusCode.INTERNALERROR));
        }
    }
