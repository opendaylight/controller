...
public class SetNwDst extends Action {
    ...
    public static final String NAME = "SET_NW_DST";
    ...
    @Override
    public SetNwDst fromString(String actionString, Node node) {
        Matcher matcher = PATTERN.matcher(removeSpaces(actionString));
        if (matcher.matches()) {
            return new SetNwDst(NetUtils.parseInetAddress(matcher.group(1)));
        }
        return null;
    }
