...
public class Match2 implements Cloneable, Serializable {
        private static final long serialVersionUID = 1L;
    private Map<String, MatchField2> fields;

    public Match2() {
        fields = new HashMap<String, MatchField2>();
    }

    public Match2(Match2 match) {
        fields = new HashMap<String, MatchField2>(match.fields);
    }
     ...
     */
    public void setField(MatchField2 field) {
        if (field.isValid()) {
     ...
     * @return
     */
    public MatchField2 getField(String type) {
    ...
    public List<MatchField2> getMatchFields() {
        return new ArrayList<MatchField2>(fields.values());
    }
    ...
    public boolean isIPv6() {
        return (isPresent(DlType.TYPE)
                && ((Short) getField(DlType.TYPE).getValue()).equals(EtherTypes.IPv6.shortValue())
                || isPresent(NwProtocol.TYPE)
                && ((Byte) getField(NwProtocol.TYPE).getValue()).equals(IPProtocols.IPV6ICMP.byteValue())
                || isPresent(NwSrc.TYPE) && getField(NwSrc.TYPE).getValue() instanceof Inet6Address || isPresent(NwDst.TYPE)
                && getField(NwDst.TYPE).getValue() instanceof Inet6Address);
    }
    ...
    public Match2 clone() {
        Match2 cloned = null;
        try {
            cloned = (Match2) super.clone();
            cloned.fields = new HashMap<String, MatchField2>();
            for (Entry<String, MatchField2> entry : this.fields.entrySet()) {
     ...
     * @return
     */
    public Match2 reverse() {
        Match2 reverse = new Match2();
        for (MatchField2 field : fields.values()) {
     ...
     * TP_DST = 80
     *
     * Then the function would return false as the two Match2 are not
     * conflicting.
     *
     ...
     *
     * @param match
     *            the Match2 describing the filter
     ...
     *         otherwise
     */
    public boolean conflictWithFilter(Match2 filter) {
        return !this.intersetcs(filter);
    }

    /**
     * Merge the current Match2 fields with the fields of the filter Match2. A
     * check is first run to see if this Match2 is compatible with the filter
     * Match2. If it is not, the merge is not attempted.
     *
     ...
     * @return a new Match2 object describing the set of packets represented by
     *         the intersection of this and the filter matches. null if the
     *         intersection is empty.
     */
    public Match2 mergeWithFilter(Match2 filter) {
     ...
     * @return a new Match2 object representing the intersection of the set of
     ...
     */
    public Match2 getIntersection(Match2 other) {
        ...
        Match2 intersection = new Match2();
        for (String type : allTypes) {
            ...
            case NwSrc.TYPE:
            case NwDst.TYPE:
                MatchField2 thisField = this.getField(type);
                MatchField2 otherField = other.getField(type);
                ...
                MatchField2 field = (type.equals(NwSrc.TYPE)) ? new NwSrc(subnetPrefix, subnetMask) : new NwDst(
     ...
     *         is non empty
     */
    public boolean intersetcs(Match2 other) {
