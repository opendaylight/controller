...
public class ModifyFlow extends OsgiCommandSupport{
    ...
    private IFlowProgrammerServiceShell sal;
    ...
    protected Object doExecute() throws Exception {
        ...
        Flow flowA = sal.getFlow(node);
        Flow flowB = sal.getFlow(node);
        Match matchB = flowB.getMatch();
        matchB.setField(MatchType.NW_DST,
                InetAddress.getByName("190.190.190.190"));
        flowB.setMatch(matchB);
        System.out.println(sal.modifyFlow(node, flowA, flowB));
        return null;
    }

    public void setSal(IFlowProgrammerServiceShell sal){
        this.sal = sal;
    }
}