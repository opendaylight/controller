    ...
    private boolean resolveLeafSchemaNodeAsMethod(final GeneratedTypeBuilder typeBuilder, final LeafSchemaNode leaf) {
        if ((leaf != null) && (typeBuilder != null)) {
            ...
            if (leafName != null) {
                ...
                } else if (typeDef instanceof UnionType || typeDef instanceof BitsTypeDefinition) {
                    String className = BindingGeneratorUtil.parseToClassName(leaf.getQName().getLocalName());
                    GeneratedTOBuilder genTOBuilder = typeBuilder.addEnclosingTransferObject(className);
                    ((TypeProviderImpl) typeProvider).setTOBuilderAccordingToTypeDef(genTOBuilder, typeDef);
                    ReferencedTypeImpl refTypeImpl = new ReferencedTypeImpl(genTOBuilder.getPackageName(),
                            genTOBuilder.getName());
                    constructGetter(typeBuilder, leafName, leafDesc, refTypeImpl);
                } else {
                    returnType = typeProvider.javaTypeForSchemaDefinitionType(typeDef);
                }
                if (returnType != null) {
