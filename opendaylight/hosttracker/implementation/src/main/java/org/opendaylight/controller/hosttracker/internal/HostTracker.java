...
/***
 *
 * HostTracker db key scheme implementation support.
 * Support has been added for IP only or IP + MAC scheme as of now.
 * User can use either of the schemes based on the configuration done in config.ini file.
 * By default IP only key scheme is choosen.
 * The attribute to be set in config.ini is hosttracker.keyscheme.
 * It could have a value of 0 or 1 as of now.
 * 0 is for IP only scheme. 1 is for IP + MAC scheme.
 *
 *
 */
...
public class HostTracker implements IfIptoHost, IfHostListener, ISwitchManagerAware, IInventoryListener,
    ...
    private boolean hostExists(HostNodeConnector host) {
        IHostId id = HostIdFactory.create(keyScheme, host.getNetworkAddress(), host.getDataLayerAddress());
    ...
    private Entry<NodeConnector, HostNodeConnector> getHostFromInactiveDB(IHostId id) {
        for (Entry<NodeConnector, HostNodeConnector> entry : inactiveStaticHosts.entrySet()) {
            ...
            IHostId cmpId = HostIdFactory.create(keyScheme, hnc.getNetworkAddress(), hnc.getDataLayerAddress());
            if (cmpId.equals(id)) {
    ...
    private class NotifyHostThread extends Thread {
        ...
        public void run() {
            ...
            IHostId id = HostIdFactory.create(keyScheme, networkAddr, host.getDataLayerAddress());
    ...
    protected Status addStaticHostReq(InetAddress networkAddr, byte[] dataLayerAddress, NodeConnector nc, short vlan) {
        ...
        try {
            ...
            IHostId id = HostIdFactory.create(keyScheme, networkAddr, new EthernetAddress(dataLayerAddress));
            if (hostExists(host)) {
    ...
    public Status removeStaticHostReq(InetAddress networkAddress, DataLinkAddress mac) {
        ...
        IHostId id = HostIdFactory.create(keyScheme, networkAddress, mac);
    ...
    public Status removeStaticHost(String networkAddress) {
        try {
            if (keyScheme != DEFAULT_IP_KEY_SCHEME) {
