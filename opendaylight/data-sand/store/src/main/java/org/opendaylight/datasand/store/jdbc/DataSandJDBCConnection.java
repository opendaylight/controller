...
public class DataSandJDBCConnection extends AutonomousAgent implements Connection {
    ...
    private Map<Integer,QueryContainer> queries = new HashMap<Integer,QueryContainer>();
    private Map<Integer,QueryUpdater> updaters = new HashMap<Integer,QueryUpdater>();
        ...
        if(message instanceof DataSandJDBCMessage){
            ...
            case DataSandJDBCMessage.TYPE_DELEGATE_QUERY:
                try{
                    ...
                    updaters.put(msg.getRS().getID(), u);
                    new Thread(u).start();
                }catch (Exception err) {
    ...
    private class QueryUpdater implements Runnable {
        ...
        public void run() {
            int count = 0;
            while (rs.next()) {
                DataSandJDBCMessage rec = new DataSandJDBCMessage(rs.getCurrent(), rs.getID(),0);
                send(rec,source);
                count++;
                if(count>=DataSandJDBCResultSet.RECORD_Threshold_Release){
                    synchronized(waitingObject){
                        DataSandJDBCMessage m = new DataSandJDBCMessage(getAgentID(),rs.getID());
                        send(m,source);
            ...
            DataSandJDBCMessage end = new DataSandJDBCMessage(rs.getID(),0,0);
            send(end,source);
        }
    }
