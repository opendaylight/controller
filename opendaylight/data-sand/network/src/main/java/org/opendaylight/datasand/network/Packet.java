...
public class Packet implements ISerializer {
    ...
    static {
        EncodeUtils.registerSerializer(Packet.class, new Packet(),NetworkClassCodes.CODE_Packet);
    }
    ...
    public Object decode(byte[] byteArray, int start, int length) {
        Packet m = new Packet();
        m.source = (NetworkID) EncodeUtils.getSerializer(NetworkID.class,null)
                .decode(byteArray, start + PACKET_SOURCE_LOCATION, 8);
        m.destination = (NetworkID) EncodeUtils.getSerializer(NetworkID.class,null)
    ...
    public void decode(TypeDescriptorsContainer container) {
        if (this.decodedObject == null) {
            if(this.getSource().equals(NetworkNodeConnection.PROTOCOL_ID_UNREACHABLE)){
                ...
                EncodeDataContainer ba = new EncodeDataContainer(origData,container);
                try{
                ...
                EncodeDataContainer ba = new EncodeDataContainer(this.getData(),container);
                try{
                    decodedObject = EncodeUtils.decodeObject(ba);}
                catch(Exception err){
                    System.err.println("Unable to decode...");
    ...
    public NetworkID getUnreachableOrigAddress(){
        int destAddress = EncodeUtils.decodeInt32(this.getData(),PACKET_DEST_LOCATION);
        int destPort = EncodeUtils.decodeInt16(this.getData(), PACKET_DEST_LOCATION+4);
        if(destAddress==0){
            destAddress = EncodeUtils.decodeInt32(this.getData(), this.getData().length-6);
            destPort = EncodeUtils.decodeInt16(this.getData(), this.getData().length-2);
        }
