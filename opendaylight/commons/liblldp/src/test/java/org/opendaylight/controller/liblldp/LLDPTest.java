...
public class LLDPTest {
     ...
     */
    @Test
    public void serializeLLDP() throws PacketException {
        ...
        // lldpBuilder.setChassisId(dummyCustomTlv(LLDPTLV.TLVType.Custom.getValue(), OUI, OUI_SUBTYPE_A,
        // CUSTOM_SUBTYPE_A_LENGTH, CUSTOM_SUBTYPE_A_VALUE));
        // lldpBuilder.setChassisId(dummyCustomTlv(LLDPTLV.TLVType.Custom.getValue(), OUI, OUI_SUBTYPE_B,
    ...
    private int checkTLV(byte[] serializedData, int offset, byte typeTLVBits, String typeTLVName, short lengthTLV,
    ...
    private byte[] awaitedBytes(byte typeTLV, short length, byte[] value, byte[] bytesBeforeValue) {
    ...
    private LLDPTLV dummyCustomTlv(final byte tlvType, byte[] oui, byte[] ouiSubtype, short customLength,
            byte[] subtypeValue) {
        byte[] fullCustomValue = new byte[0];
        ArrayUtils.addAll(fullCustomValue, oui);
        ArrayUtils.addAll(fullCustomValue, ouiSubtype);
        ArrayUtils.addAll(fullCustomValue, subtypeValue);
        return dummyTlv(tlvType, customLength, fullCustomValue);
    }

    private LLDPTLV dummyTlv(final byte concreteTlv, final short concreteLength, final byte[] concreteValue) {
