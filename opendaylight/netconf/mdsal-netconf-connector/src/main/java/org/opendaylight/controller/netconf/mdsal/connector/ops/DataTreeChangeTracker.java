...
public class DataTreeChangeTracker {
    ...
    private static final QName OPERATION_ATTRIBUTE;

    static {
        QName OPERATION_ATTRIBUTE1;
        try {
            OPERATION_ATTRIBUTE1 = QName.create(new URI("urn:ietf:params:xml:ns:netconf:base:1.0"), null, "operation");
        } catch (URISyntaxException e) {
            OPERATION_ATTRIBUTE1 = null;
        }
        OPERATION_ATTRIBUTE = OPERATION_ATTRIBUTE1;
    }
    ...
    public ModifyAction popAction() {
        ModifyAction popResult = actions.pop();
    ...
    public void pushPath(PathArgument pathArgument) {
    ...
    public static final class NetconfOperationLeafStrategy implements LeafStrategy {
        ...
        @Override
        public NormalizedNode applyStrategy(NormalizedNodeAttrBuilder builder) {
    ...
    public static final class NetconfOperationContainerStrategy implements ParsingStrategy {
        ...
        @Override
        public DataContainerNode<?> applyStrategy(DataContainerNodeBuilder containerBuilder) {
            DataContainerNode node = (DataContainerNode) containerBuilder.build();
            ModifyAction currentAction = dataTreeChangeTracker.popAction();
        ...
        @Override
        public void prepareAttribues(Map attributes, DataContainerNodeBuilder containerBuilder) {
            dataTreeChangeTracker.pushPath(containerBuilder.build().getIdentifier());
            String operation = ((Map<QName, String>) attributes).get(OPERATION_ATTRIBUTE);
            if (operation != null) {
    ...
    public static final class DataTreeChange {
        ...
        private final ArrayList<PathArgument> path;

        public DataTreeChange(NormalizedNode changeRoot, ModifyAction action, ArrayList<PathArgument> path) {
        ...
        public ArrayList<PathArgument> getPath() {
            return path;
        }
    }
}
