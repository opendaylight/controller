...
public class Close extends AbstractCommand {
    ...
    public Close(final QName qName, final InputDefinition args, final OutputDefinition output) {
        super(qName, args, output);
    }

    @Override
    public Output invoke(final RpcImplementation rpc, final Input inputArgs) throws CommandInvocationException {
    ...
    public static Command create(final RpcDefinition rpcDefinition) {
        return new Close(rpcDefinition.getQName(), getInputDefinition(rpcDefinition), getOutputDefinition(rpcDefinition));
    }
