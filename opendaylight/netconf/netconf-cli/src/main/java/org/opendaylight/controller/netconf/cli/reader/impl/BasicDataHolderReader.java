...
public abstract class BasicDataHolderReader<T extends DataSchemaNode> extends AbstractReader<T> {
    ...
    public List<Node<?>> readInner(final T schemaNode) throws IOException, ReadingException {
        ...
        while (baseTypeFor(type) instanceof UnionTypeDefinition) {
            Optional<TypeDefinition<?>> optionalTypeDef = new UnionTypeReader(console).read(type);
        ...
        if (type instanceof IdentityrefTypeDefinition) {
            ...
            try {
                ...
                throw new ReadingException("It wasn't possible deserialize value " + rawValue + ".", e);
            }
    ...
    private List<Node<?>> postSkipOperations(DataSchemaNode schemaNode) throws IOException {
