...
public class CommandArgHandlerRegistry {
    ...
    public synchronized Reader<? extends DataSchemaNode> getCustomReader(final Class<? extends Reader<DataSchemaNode>> readerType) {
        return customReaders.get(readerType).provide(consoleIO, getRemoteSchema(readerType), this);
    }

    private SchemaContext getRemoteSchema(final Class<?> handlerType) {
    ...
    public synchronized Writer<DataSchemaNode> getCustomWriter(final Class<? extends Writer<DataSchemaNode>> writerType) {
        return customWriters.get(writerType).provide(consoleIO, getRemoteSchema(writerType), this);
    }
    ...
    private static interface ReaderProvider {
        Reader<? extends DataSchemaNode> provide(ConsoleIO consoleIO, SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry);
    }

    private static final class FilterReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
            return new FilterReader(consoleIO, schema);
        }
    }
    ...
    private static final class ConfigReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
            return new ConfigReader(consoleIO, schema, commandArgHandlerRegistry);
        }
    }
    ...
    private static final class EditContentReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
            return new EditContentReader(consoleIO, commandArgHandlerRegistry, schema);
        }
    }
    ...
    private static final class PasswordReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
            return new PasswordReader(consoleIO, schema);
        }
    }
