...
/**
 * Keeps custom and generic input/output arguments handlers. Custom handlers are constructed lazily, due to remote schema context acquisition.
 */
public class CommandArgHandlerRegistry {
    ...
    public synchronized Reader<? extends DataSchemaNode> getCustomReader(final Class<? extends Reader<DataSchemaNode>> readerType) {
        return customReaders.get(readerType).provide(consoleIO, this, schemaContextRegistry);
    }

    private static SchemaContext getRemoteSchema(final Class<?> handlerType, final SchemaContextRegistry schemaContextRegistry) {
        final Optional<SchemaContext> remoteSchemaContext = schemaContextRegistry.getRemoteSchemaContext();
        Preconditions.checkState(remoteSchemaContext.isPresent(), "Remote schema context not acquired yet, cannot get handler %s", handlerType);
    ...
    public synchronized Writer<DataSchemaNode> getCustomWriter(final Class<? extends Writer<DataSchemaNode>> writerType) {
        return customWriters.get(writerType).provide(consoleIO, getRemoteSchema(writerType, schemaContextRegistry), this);
    }
    ...
    private static interface ReaderProvider {
        Reader<? extends DataSchemaNode> provide(ConsoleIO consoleIO, final CommandArgHandlerRegistry commandArgHandlerRegistry, final SchemaContextRegistry schemaContextRegistry);
    }

    private static final class FilterReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final CommandArgHandlerRegistry commandArgHandlerRegistry, final SchemaContextRegistry schemaContextRegistry) {
    ...
    private static final class ConfigReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final CommandArgHandlerRegistry commandArgHandlerRegistry, final SchemaContextRegistry schemaContextRegistry) {
            return new ConfigReader(consoleIO, getRemoteSchema(ConfigReader.class, schemaContextRegistry), commandArgHandlerRegistry);
        }
    }
    ...
    private static final class EditContentReaderProvider implements ReaderProvider {
        @Override
        public Reader<? extends DataSchemaNode> provide(final ConsoleIO consoleIO, final CommandArgHandlerRegistry commandArgHandlerRegistry, final SchemaContextRegistry schemaContextRegistry) {
            return new EditContentReader(consoleIO, commandArgHandlerRegistry, getRemoteSchema(EditContentReader.class, schemaContextRegistry));
        }
    }
    ...
    private static interface WriterProvider {
        Writer<DataSchemaNode> provide(ConsoleIO consoleIO, SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry);
    }

    private class DataWriterProvider implements WriterProvider {
        @Override
        public Writer<DataSchemaNode> provide(final ConsoleIO consoleIO, final SchemaContext schema, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
