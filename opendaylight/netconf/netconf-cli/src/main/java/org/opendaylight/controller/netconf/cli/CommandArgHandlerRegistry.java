...
import org.opendaylight.yangtools.yang.data.impl.schema.transform.dom.serializer.GenericWriter;
...
public class CommandArgHandlerRegistry {
    ...
    public CommandArgHandlerRegistry(final ConsoleIO consoleIO) {
        ...
        this.outHandler = new GenericWriter(consoleIO, "");
        this.inHandler = new GenericReader(consoleIO);
    }

    public synchronized void addCustomReaders(final SchemaContext remoteSchemaContext) {
    ...
    public synchronized void removeCustomHandlers() {
        customReaders.clear();
        customWriters.clear();
    }
