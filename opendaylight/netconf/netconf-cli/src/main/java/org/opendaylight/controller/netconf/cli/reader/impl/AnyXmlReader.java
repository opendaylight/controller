...
public class AnyXmlReader extends AbstractReader<AnyXmlSchemaNode> {
    ...
    public AnyXmlReader(final ConsoleIO console) {
        super(console);
    }

    @Override
    protected List<Node<?>> readInner(final AnyXmlSchemaNode schemaNode) throws IOException, ReadingException {
        ...
        if (!rawValue.equals(SKIP)) {
            ...
            if(value.isPresent()) {
                newNode = NodeFactory.createImmutableCompositeNode(schemaNode.getQName(), null, Collections.<Node<?>>singletonList(value.get()));
            } else {
    ...
    private Optional<Node<?>> tryParse(final String rawValue) {
        try {
            Document dom = XmlUtil.readXmlToDocument(rawValue);
