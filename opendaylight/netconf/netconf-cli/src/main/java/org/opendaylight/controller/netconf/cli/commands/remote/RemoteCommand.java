...
public class RemoteCommand extends AbstractCommand {
    ...
    public Output invoke(final RpcImplementation rpc, final Input inputArgs) throws CommandInvocationException {
        try {
            // TODO if times out, we could still be receiving response
            return new Output(rpc.invokeRpc(getCommandId(), inputArgs.wrap(getCommandId()))
                    .get(DEFAULT_TIMEOUT, DEFAULT_TIMEOUT_UNIT).getResult());
