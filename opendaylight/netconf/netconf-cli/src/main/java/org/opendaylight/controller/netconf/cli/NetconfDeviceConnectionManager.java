...
public class NetconfDeviceConnectionManager implements Closeable {
    ...
    public NetconfDeviceConnectionManager(final CommandDispatcher commandDispatcher, final CommandArgHandlerRegistry argumentHandlerRegistry, final SchemaContextRegistry schemaContextRegistry) {
        ...
        netconfClientDispatcher = new NetconfClientDispatcherImpl(nettyThreadGroup, nettyThreadGroup, new HashedWheelTimer());
    }

    // TODO we receive configBuilder in order to add SessionListener, Session Listener should not be part of config
    public synchronized void connect(final String name, final NetconfReconnectingClientConfigurationBuilder configBuilder) {
        ...
        handler = new NetconfDeviceConnectionHandler(commandDispatcher, argumentHandlerRegistry, schemaContextRegistry);
     ...
     */
    public synchronized void connectBlocking(final String name, final NetconfReconnectingClientConfigurationBuilder configBuilder) {
