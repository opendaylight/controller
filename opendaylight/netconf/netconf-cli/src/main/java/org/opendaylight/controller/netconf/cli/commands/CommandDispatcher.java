...
public class CommandDispatcher {
    ...
    public static final Collection<String> BASE_NETCONF_SCHEMA_PATHS = Lists.newArrayList("/schema/remote/ietf-netconf.yang",
            "/schema/remote/ietf-netconf-cli.yang", "/schema/common/ietf-inet-types.yang");
    ...
    public static final Collection<String> LOCAL_SCHEMA_PATHS = Lists.newArrayList("/schema/local/netconf-cli.yang", "/schema/common/ietf-inet-types.yang");

    public synchronized void addLocalCommands(final NetconfDeviceConnectionManager connectionManager) {
        this.addLocalCommands(connectionManager, parseSchema(LOCAL_SCHEMA_PATHS));
    }

    public synchronized void addLocalCommands(final NetconfDeviceConnectionManager connectionManager, final SchemaContext localSchema) {
        for (final Module module : localSchema.getModules()) {
            for (final RpcDefinition rpcDefinition : module.getRpcs()) {
                ...
                if (rpcDefinition.getQName().equals(CommandConstants.HELP_QNAME)) {
                    localCommand = Help.create(rpcDefinition);
                    ...
                    localCommand = Connect.create(rpcDefinition, connectionManager);
