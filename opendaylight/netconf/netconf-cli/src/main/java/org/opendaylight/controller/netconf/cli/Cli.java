...
public class Cli implements Runnable {
    ...
    public Cli(final ConsoleIO consoleIO, final CommandDispatcher commandRegistry, final CommandArgHandlerRegistry argumentHandlerRegistry) {
        ...
        for (final DataSchemaNode schemaNode : unwrap.keySet()) {
            ...
            try {
                ...
                if (customReaderClassOpt.isPresent()) {
                    final Writer<DataSchemaNode> customReaderInstance = argumentHandlerRegistry.getCustomWriter(customReaderClassOpt.get());
                    Preconditions.checkNotNull(customReaderInstance, "Unknown custom writer: %s",customReaderClassOpt.get());
    ...
    private void handleEmptyOutput(final Writer<DataSchemaNode> outHandler, final CompositeNode output) throws WriteException {
        try {
            consoleIO.writeLn("ok");
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        // FIXME removing null nodes in generic writer removes ok as well
    ...
    private Input handleInput(final InputDefinition inputDefinition) {
        final Map<QName, List<Node<?>>> allArgs = Maps.newHashMap();
        ...
        for (final DataSchemaNode schemaNode : inputDefinition) {
            try {
                ...
                if (customReaderClassOpt.isPresent()) {
                    final Reader<DataSchemaNode> customReaderInstance = argumentHandlerRegistry.getCustomReader(customReaderClassOpt.get());
                    ...
                    read = argumentHandlerRegistry.getGenericReader().read(schemaNode);
                }

                allArgs.put(schemaNode.getQName(), read);
    ...
    private static final class RootConsoleContext implements ConsoleContext {
        ...
        private class CommandCompleter extends StringsCompleter {
            ...
            public int complete(final String buffer, final int cursor, final List<CharSequence> candidates) {
                getStrings().clear();
                for (final String candidate : commandRegistry.getCommandIds()) {
                    getStrings().add(candidate);
                }
