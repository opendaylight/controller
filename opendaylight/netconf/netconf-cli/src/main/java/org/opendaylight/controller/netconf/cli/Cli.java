...
public class Cli implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            try {
                String commandName = consoleIO.read();
                Map<QName, Command> commands = commandRegistry.getCommand(commandName);
    ...
    private void handleOutput(final Command command, final Output response) {
        Map<DataSchemaNode, List<Node<?>>> unwrap = response.unwrap(command.getOutputDefinition());

        Writer<DataSchemaNode> outHandler = new GenericWriter(consoleIO, "  ");

        for (DataSchemaNode schema : unwrap.keySet()) {
            try {
                // TODO remove indent,
    ...
    private Input handleInput(final InputDefinition inputDefinition) {
        List<Node<?>> allArgs = Lists.newArrayList();

        Reader<DataSchemaNode> argHandler = new GenericReader(consoleIO);
        for (DataSchemaNode dataSchemaNode : inputDefinition) {
            try {
