...
public class Cli implements Runnable {
    ...
    public Cli(final CommandDispatcher commandRegistry, final RpcImplementation rpcImplementation, final SchemaContext remoteSchemaContext) {
        this.commandRegistry = commandRegistry;
        this.rpcImplementation = rpcImplementation;
        try {
            // TODO instantiate IO sooner and inject here
            // TODO add status output during initializationl
        ...
        customReaders.put(FilterReader.class, new FilterReader(consoleIO, remoteSchemaContext));
    }
    ...
    public void run() {
        ...
        while (true) {
            try {
                ...
                try {
                    final Output response = command.invoke(rpcImplementation, handleInput(command.getInputDefinition()));
    ...
    private Input handleInput(final InputDefinition inputDefinition) {
        ...
        for (final DataSchemaNode dataSchemaNode : inputDefinition) {
            try {
                ...
                final Optional<Class<? extends Reader<DataSchemaNode>>> customReaderClassOpt = tryGetCustomReader(dataSchemaNode);
                ...
                if(customReaderClassOpt.isPresent()) {
                    ...
                    read = customReaderInstance.read(dataSchemaNode);
                } else {
                    read = argHandler.read(dataSchemaNode);
                }
