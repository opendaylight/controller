...
public class Disconnect extends AbstractCommand {
    ...
    public Disconnect(final QName qName, final InputDefinition inputDefinition, final OutputDefinition outputDefinition, final NetconfDeviceConnectionManager connectionManager) {
        super(qName, inputDefinition, outputDefinition);
    ...
    public Output invoke(final Input inputArgs) {
        ...
        return new Output(new CompositeNodeTOImpl(getCommandId(), null,
                Lists.<Node<?>> newArrayList(new SimpleNodeTOImpl<>(new QName(getCommandId(), "status"), null, "Connection disconnected"))));
    }

    public static Command create(final RpcDefinition rpcDefinition, final NetconfDeviceConnectionManager commandDispatcher) {
        return new Disconnect(rpcDefinition.getQName(), getInputDefinition(rpcDefinition), getOutputDefinition(rpcDefinition), commandDispatcher);
    }
}
