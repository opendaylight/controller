 ...
 */
public class DataReader extends AbstractReader<DataSchemaNode> {
    ...
    public DataReader(final ConsoleIO console, final SchemaContext remoteSchemaContext, final CommandArgHandlerRegistry commandArgHandlerRegistry) {
        super(console);
    ...
    private List<Node<?>> readInnerNode(final String pathString) throws ReadingException {
        final Optional<DataNodeContainer> schema = getCurrentNode(remoteSchemaContext, pathString);
        Preconditions.checkState(schema.isPresent(), "Unable to find schema for %s", pathString);
        return commandArgHandlerRegistry.getGenericReader().read((DataSchemaNode) schema.get());
    }
