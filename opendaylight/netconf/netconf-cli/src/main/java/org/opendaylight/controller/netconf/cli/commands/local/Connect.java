...
public class Connect extends AbstractCommand {
    ...
    private Connect(final QName qName, final InputDefinition args, final OutputDefinition output,
        ...
        super(qName, args, output);
        this.connectManager = connectManager;
    }

    @Override
    public Output invoke(final Input inputArgs) {
        final NetconfReconnectingClientConfigurationBuilder config = getConfig(inputArgs);
        return invoke(config);
    }

    private Output invoke(final NetconfReconnectingClientConfigurationBuilder config) {
        connectManager.connectBlocking("console", config);

        return new Output(new CompositeNodeTOImpl(getCommandId(), null,
                Lists.<Node<?>> newArrayList(new SimpleNodeTOImpl<>(new QName(getCommandId(), "status"), null, "Connection initiated"))));
    }

    private NetconfReconnectingClientConfigurationBuilder getConfig(final Input inputArgs) {
        ...
        return NetconfReconnectingClientConfigurationBuilder.create().withAddress(inetAddress)
                .withConnectionTimeoutMillis(DEFAULT_CONNECTION_TIMEOUT_MS).withReconnectStrategy(strategy)
    ...
    private <T> Optional<T> getArgumentOpt(final Input inputArgs, final String argName, final Class<T> type) {
        final QName argQName = new QName(getCommandId(), argName);
        final List<Node<?>> argumentNodes = inputArgs.getArgs().get(argQName);
        if(argumentNodes == null || argumentNodes.isEmpty()) {
            return Optional.absent();
        }
        Preconditions.checkArgument(argumentNodes.size() == 1, "Only single type argument supported, %s", argQName);
        final Node<?> next = argumentNodes.iterator().next();
        Preconditions.checkArgument(next instanceof SimpleNode, "Only simple type argument supported, %s", argQName);

        final Object value = next.getValue();
        Preconditions.checkArgument(type.isInstance(value), "Unexpected instance type: %s for argument: %s", value.getClass(), argQName);
    ...
    public static ReconnectStrategy getReconnectStrategy() {
        ...
        final Long maxAttempts = null;

        return new TimedReconnectStrategy(GlobalEventExecutor.INSTANCE, 10000, minSleep, sleepFactor, maxSleep, maxAttempts, deadline);
    }
    ...
    public static Command create(final RpcDefinition rpcDefinition, final NetconfDeviceConnectionManager connectManager) {
        return new Connect(rpcDefinition.getQName(), getInputDefinition(rpcDefinition),
                getOutputDefinition(rpcDefinition), connectManager);
    }
}
