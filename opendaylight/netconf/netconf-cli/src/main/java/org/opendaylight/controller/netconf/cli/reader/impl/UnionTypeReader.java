...
public class UnionTypeReader {
    ...
    public TypeDefinition<?> read(final TypeDefinition<?> unionTypeDefinition) throws IOException {
        console.enterContext(getContext(unionTypeDefinition));
        try {
            console.writeLn("The element is of type union. Choose type:");

            while (true) {
                String rawValue = console.read();
                Map<String, TypeDefinition<?>> mapping = ((UnionConsoleContext) console.getContext())
                        .getMenuItemMapping();
    ...
    private ConsoleContext getContext(final TypeDefinition<?> typeDefinition) {
