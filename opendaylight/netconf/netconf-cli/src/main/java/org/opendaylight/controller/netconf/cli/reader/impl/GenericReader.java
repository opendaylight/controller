...
public class GenericReader extends AbstractReader<DataSchemaNode> {
    ...
    public GenericReader(final ConsoleIO console, final CommandArgHandlerRegistry argumentHandlerRegistry, final SchemaContext schemaContext) {
        ...
        do {
            try {
                if (schemaNode instanceof LeafSchemaNode) {
                    return new LeafReader(console, getSchemaContext()).read((LeafSchemaNode) schemaNode);
                } else if (schemaNode instanceof ContainerSchemaNode) {
                    return new ContainerReader(console, argumentHandlerRegistry, getSchemaContext()).read((ContainerSchemaNode) schemaNode);
                } else if (schemaNode instanceof ListSchemaNode) {
                    return new GenericListReader<>(console, new ListEntryReader(console, argumentHandlerRegistry, getSchemaContext()), getSchemaContext())
                            .read((ListSchemaNode) schemaNode);
                } else if (schemaNode instanceof LeafListSchemaNode) {
                    return new GenericListReader<>(console, new LeafListEntryReader(console, getSchemaContext()), getSchemaContext())
                            .read((LeafListSchemaNode) schemaNode);
                } else if (schemaNode instanceof ChoiceNode) {
                    return new ChoiceReader(console, argumentHandlerRegistry, getSchemaContext()).read((ChoiceNode) schemaNode);
                } else if (schemaNode instanceof AnyXmlSchemaNode) {
                    return new AnyXmlReader(console, getSchemaContext()).read((AnyXmlSchemaNode) schemaNode);
                }
                isRedCorrectly = true;
