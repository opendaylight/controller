...
public class GenericReader extends AbstractReader<DataSchemaNode> {
    ...
    private List<Node<?>> readGeneric(final DataSchemaNode schemaNode) throws ReadingException {
        final List<Node<?>> newNodes = new ArrayList<>();
        if (schemaNode instanceof LeafSchemaNode) {
            return new LeafReader(console).read((LeafSchemaNode) schemaNode);
        } else if (schemaNode instanceof ContainerSchemaNode) {
            return new ContainerReader(console, argumentHandlerRegistry).read((ContainerSchemaNode) schemaNode);
        } else if (schemaNode instanceof ListSchemaNode) {
            return new GenericListReader<>(console, new ListEntryReader(console, argumentHandlerRegistry)).read((ListSchemaNode) schemaNode);
        } else if (schemaNode instanceof LeafListSchemaNode) {
            return new GenericListReader<>(console, new LeafListEntryReader(console))
                    .read((LeafListSchemaNode) schemaNode);
        } else if (schemaNode instanceof ChoiceNode) {
            return new ChoiceReader(console, argumentHandlerRegistry).read((ChoiceNode) schemaNode);
