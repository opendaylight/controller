...
public class FilterReader extends AbstractReader<DataSchemaNode> {
    ...
    public static final QName FILTER_TYPE_QNAME = QName.create(
            "urn:ietf:params:xml:ns:netconf:base:1.0", "2011-06-01", "type");
    ...
    protected List<Node<?>> readWithContext(final DataSchemaNode schemaNode) throws IOException, ReadingException {
        ...
        final Map<QName, String> attributes = Collections.singletonMap(FILTER_TYPE_QNAME, FILTER_TYPE_VALUE_DEFAULT);
        final Node<?> newNode = previous == null ? null : ImmutableCompositeNode.create(schemaNode.getQName(),
                attributes, Collections.<Node<?>>singletonList(previous));
