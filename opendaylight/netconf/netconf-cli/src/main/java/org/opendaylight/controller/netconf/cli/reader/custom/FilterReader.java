    ...
    protected List<Node<?>> readInner(final DataSchemaNode schemaNode) throws IOException, ReadingException {
        ...
        if (!rawValue.equals(SKIP)) {
            ...
            for (QName qName : Lists.reverse(filterPartsQNames)) {
                previous = new CompositeNodeTOImpl(qName, null, previous == null ? Collections.<Node<?>>emptyList() : Collections.<Node<?>>singletonList(previous));
            }

            newNode = previous == null ? null : new CompositeNodeTOImpl(schemaNode.getQName(), null, Collections.<Node<?>>singletonList(previous));
        }
