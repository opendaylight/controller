...
public class DeviceCliHandler implements RemoteDeviceHandler<NetconfSessionCapabilities> {
    ...
    public void onDeviceConnected(final SchemaContextProvider contextProvider,
        ...
        final SchemaContext schema = parseSchema("/schema/remote/ietf-netconf.yang",
        ...
        final CommandDispatcher commandRegistry = new NetconfCliInitializer().initCommands(schemaL,
                contextProvider.getSchemaContext(), schema);
        try {
            new Cli(commandRegistry, rpcImplementation, contextProvider.getSchemaContext()).run();
