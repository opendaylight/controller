...
public class ContainerReader extends AbstractReader<ContainerSchemaNode> {
    ...
    public List<Node<?>> readWithContext(final ContainerSchemaNode containerNode) throws IOException, ReadingException {
        ...
        final SeparatedNodes separatedNodes = SeparatedNodes.separateNodes(containerNode);
        for (final DataSchemaNode childNode : sortChilds(separatedNodes.getMandatoryNotKey())) {
            final List<Node<?>> redNodes = argumentHandlerRegistry.getGenericReader(getSchemaContext()).read(childNode);
            if (redNodes.isEmpty()) {
        ...
        for (final DataSchemaNode childNode : sortChilds(separatedNodes.getOthers())) {
            compositeNodeBuilder.addAll(argumentHandlerRegistry.getGenericReader(getSchemaContext()).read(childNode));
        }
        return Collections.<Node<?>> singletonList(compositeNodeBuilder.toInstance());
    }

    private List<DataSchemaNode> sortChilds(final Set<DataSchemaNode> unsortedNodes) {
