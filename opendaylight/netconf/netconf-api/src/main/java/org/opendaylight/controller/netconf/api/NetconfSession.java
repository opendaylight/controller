...
public abstract class NetconfSession extends AbstractProtocolSession<NetconfMessage> {
    ...
    private ChannelHandler originEncoder;
    private ChannelHandler originDecoder;
    private ChannelHandler newEncoder;
    ...
    public void sendMessage(NetconfMessage netconfMessage) {
        ...
        if (switchNew) {
            channel.pipeline().replace(originEncoder, "",newEncoder);
            switchNew = false;
        }

        if (switchOriginal) {
            channel.pipeline().replace(newEncoder, "",originEncoder);
    ...
    public void replaceEncoder(ChannelHandler handler, String handlerName) {
        this.originEncoder = channel.pipeline().get(ProtocolMessageEncoder.class);
        this.newEncoder = handler;
        this.switchNew = true;
    }
    ...
    public void replaceDecoder(ChannelHandler handler, String handlerName) {
        this.originDecoder = channel.pipeline().get(ProtocolMessageDecoder.class);
        channel.pipeline().replace(originDecoder, handlerName, handler);
    }

    public void switchToOriginDecoder(String handlerName)
    {
        channel.pipeline().replace(channel.pipeline().get(handlerName), "",originDecoder);
    }
