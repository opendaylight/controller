...
public class Activator implements BundleActivator {
    ...
    public void start(final BundleContext context) throws Exception {
        ...
        YangStoreServiceTracker yangStoreServiceTracker = new YangStoreServiceTracker(context);
    ...
    private final class YangStoreServiceTracker implements ServiceTrackerCustomizer<YangStoreService, YangStoreService>, AutoCloseable {

        private final BundleContext context;
        private final ServiceTracker<YangStoreService, YangStoreService> serviceTracker;
        private AutoCloseable listenerRegistration;

        public YangStoreServiceTracker(BundleContext context) {
            this.context = context;
            this.serviceTracker = new ServiceTracker<>(context, YangStoreService.class, this);
            serviceTracker.open();
        }
        ...
        @Override
        public YangStoreService addingService(ServiceReference<YangStoreService> reference) {
            final YangStoreService yangStoreService = context.getService(reference);
            listenerRegistration = yangStoreService.registerModuleListener(netconfNotificationManager);
            return null;
        }

        @Override
        public void modifiedService(ServiceReference<YangStoreService> reference, YangStoreService service) {
            try {
                close();
            } catch (Exception e) {
                LOG.warn("Unable to close listener registration", e);
            }
            service.registerModuleListener(netconfNotificationManager);
        }

        @Override
        public void removedService(ServiceReference<YangStoreService> reference, YangStoreService service) {
            try {
                close();
            } catch (Exception e) {
                LOG.warn("Unable to close listener registration", e);
