...
public class ConfigPusherImpl implements ConfigPusher {
    ...
    public void process(List<AutoCloseable> autoCloseables, MBeanServerConnection platformMBeanServer, Persister persisterAggregator) throws InterruptedException {
        ...
        while(true) {
            ...
            try {
                ...
                logger.debug("ConfigPusher has pushed configs {}", configs);
            }
    ...
    public synchronized void pushConfigs(List<? extends ConfigSnapshotHolder> configs) throws InterruptedException {
        logger.debug("Requested to push configs {}", configs);
        this.queue.put(configs);
    }

    private synchronized LinkedHashMap<? extends ConfigSnapshotHolder, EditAndCommitResponse> internalPushConfigs(List<? extends ConfigSnapshotHolder> configs) throws NetconfDocumentedException {
