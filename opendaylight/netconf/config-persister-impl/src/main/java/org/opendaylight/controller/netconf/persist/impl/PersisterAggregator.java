...
public final class PersisterAggregator implements Persister {
    ...
    private static class PersisterWithConfiguration {
        private final Persister storage;
        private final boolean readOnly;

        private PersisterWithConfiguration(Persister storage, boolean readOnly) {
    ...
    private static PersisterWithConfiguration loadConfiguration(final String index, final PropertiesProvider propertiesProvider) {
        String storageAdapterClass = propertiesProvider.getFullKeyForReporting(index + ConfigPersisterActivator.STORAGE_ADAPTER_CLASS_PROP_SUFFIX);
        ...
        try {
            ...
            PropertiesProvider innerProvider = new PropertiesProvider() {
                @Override
                public String getProperty(String key) {
                    return propertiesProvider.getProperty(getFullKeyForReporting(key));
                }

                @Override
                public String getFullKeyForReporting(String key) {
                    return index + propertiesProvider.getFullKeyForReporting("properties") + "." + key;
                }
            };

            boolean readOnly = false;
            String readOnlyProperty = propertiesProvider.getFullKeyForReporting(index + "readOnly");
            if (readOnlyProperty != null && readOnlyProperty.equals("true")) {
                readOnly = new Boolean(readOnlyProperty);
            }
    ...
    public static PersisterAggregator createFromProperties(PropertiesProvider propertiesProvider) {
        ...
        String prefixes = propertiesProvider.getProperty(ConfigPersisterActivator.NETCONF_CONFIG_PERSISTER);
        if (prefixes == null || prefixes.isEmpty()) {
            persisterWithConfigurations.add(PersisterAggregator.loadConfiguration("", propertiesProvider));
        } else {
...
//        for (ListIterator<PersisterWithConfiguration> iterator = persisterWithConfigurations.listIterator();iterator.hasPrevious();){
//            PersisterWithConfiguration persisterWithConfiguration = iterator.previous();
//            Optional<ConfigSnapshotHolder> configSnapshotHolderOptional = persisterWithConfiguration.storage.loadLastConfig();
//            if (configSnapshotHolderOptional.isPresent()) {
//                return configSnapshotHolderOptional;
//            }
