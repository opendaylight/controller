...
public class BaseNodeManager implements NodeManager {
    ...
    @Override
    public ListenableFuture<Node> nodeCreated( final NodeId nodeId,  final Node configNode) {
        LOG.debug("Creating Node {}, with configuration: {}", nodeId.getValue(), configNode);
        return delegate.nodeCreated(nodeId, configNode);
    }

    @Override
    public ListenableFuture<Node> nodeUpdated( final NodeId nodeId,  final Node configNode) {
