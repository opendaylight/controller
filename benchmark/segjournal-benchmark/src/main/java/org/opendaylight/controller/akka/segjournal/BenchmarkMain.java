...
import static org.opendaylight.controller.akka.segjournal.BenchmarkUtils.buildMetricId;
...
public class BenchmarkMain {
    ...
    void execute() {
        ...
        final var progressReporter = new ProgressReporter(executor, results, config.messagesNum(), 10, TimeUnit.SECONDS);
        ...
        metricsRegistry.getMeters().forEach((key, meter) -> {
            LOG.info("Meter '{}':", buildMetricId(key));
        ...
        metricsRegistry.getTimers().forEach((key, timer) -> {
            LOG.info("Timer '{}':", buildMetricId(key));
        ...
        metricsRegistry.getHistograms().forEach((key, histogram) -> {
            LOG.info("Histogram '{}':", buildMetricId(key));
    ...
    private static final class ProgressReporter implements Runnable {
        ...
        public void run() {
            ...
            // count those in the middle
            final var completed = released + queue.stream().filter(Future::isCompleted).count();

            LOG.info("{} of {} = {}% messages written", completed, total, completed * 100 / total);
            if (total == completed) {
