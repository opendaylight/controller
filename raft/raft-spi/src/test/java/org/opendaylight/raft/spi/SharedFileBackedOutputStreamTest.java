...
class SharedFileBackedOutputStreamTest {
    ...
    void testSingleUsage() throws Exception {
        ...
        try (var fbos = new SharedFileBackedOutputStream(5, tempDir.toString())) {
            ...
            assertNotNull("Expected temp file created", FileBackedOutputStreamTest.findTempFileName(tempDir));
            fbos.cleanup();
            assertNull("Found unexpected temp file", FileBackedOutputStreamTest.findTempFileName(tempDir));
        }
    ...
    void testSharing() throws Exception {
        ...
        try (var fbos = new SharedFileBackedOutputStream(5, tempDir.toString())) {
            ...
            fbos.cleanup();
            assertNotNull("Expected temp file exists", FileBackedOutputStreamTest.findTempFileName(tempDir));
