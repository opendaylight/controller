...
record ArenaFileMapper(
    ...
    private ArenaMappedFile mapImpl(final FileChannel channel, final MapMode mode, final long offset,
        ...
        try {
            segment = fileChannelMap.invokeExact(channel, mode, offset, size, arena);
        } catch (Throwable e) {
            arenaClose.invokeExact(arena);
            throw e;
        }

        return new ArenaMappedFile((ByteBuffer) segmentAsByteBuffer.invokeExact(segment), this, arena, segment);
    }
