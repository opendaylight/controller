...
class LeaderTest extends AbstractLeaderTest<Leader> {
    ...
    @Test
    void testHandleMessageForUnknownMessage() {
    ...
    @Test
    void testThatLeaderSendsAHeartbeatMessageToAllFollowers() {
    ...
    private RaftActorBehavior sendReplicate(final MockRaftActorContext actorContext, final long term, final long index,
        ...
        actorContext.getReplicatedLog().append(new DefaultLogEntry(index, term, payload));
        return leader.handleMessage(leaderActor, new Replicate(index, true, null));
    }

    @Test
    void testHandleReplicateMessageSendAppendEntriesToFollower() {
    ...
    @Test
    void testHandleReplicateMessageWithHigherTermThanPreviousEntry() {
    ...
    @Test
    void testHandleReplicateMessageCommitIndexIncrementedBeforeConsensus() {
    ...
    @Test
    void testMultipleReplicateShouldNotCauseDuplicateAppendEntriesToBeSent() {
        logStart("testHandleReplicateMessageSendAppendEntriesToFollower");
    ...
    @Test
    void testMultipleReplicateWithReplyShouldResultInAppendEntries() {
    ...
    @Test
    void testDuplicateAppendEntriesWillBeSentOnHeartBeat() {
    ...
    @Test
    void testHeartbeatsAreAlwaysSentIfTheHeartbeatIntervalHasElapsed() {
    ...
    @Test
    void testSendingReplicateImmediatelyAfterHeartbeatDoesReplicate() {
    ...
    @Test
    void testHandleReplicateMessageWhenThereAreNoFollowers() {
        ...
        log.append(new DefaultLogEntry(newLogIndex, term, data));

        final var identifier = new MockIdentifier("state-id");
        // State should not change
        assertSame(leader, leader.handleMessage(leaderActor, new Replicate(newLogIndex, true, identifier)));
        ...
        for (int i = 0; i <= newLogIndex - 1; i++) {
            ...
            assertEquals("getIndex", i + 1, applyState.entry().index());
            assertEquals("getTerm", term, applyState.entry().term());
        }

        ApplyState last = applyStateList.get((int) newLogIndex - 1);
        assertEquals("getData", data, last.entry().command());
        assertEquals("getIdentifier", identifier, last.identifier());
    }

    @Test
    void testSendAppendEntriesOnAnInProgressInstallSnapshot() throws Exception {
    ...
    @Test
    void testSendAppendEntriesSnapshotScenario() {
        ...
        // new entry
        actorContext.getReplicatedLog().append(new DefaultLogEntry(newEntryIndex, currentTerm, new MockCommand("D")));
        ...
        RaftActorBehavior raftBehavior = leader.handleMessage(leaderActor,
            new Replicate(newEntryIndex, true, new MockIdentifier("state-id")));
    ...
    @Test
    void testInitiateInstallSnapshot() {
        ...
        // new entry
        log.append(new DefaultLogEntry(newEntryIndex, currentTerm, new MockCommand("D")));

        //update follower timestamp
        leader.markFollowerActive(FOLLOWER_ID);

        leader.handleMessage(leaderActor, new Replicate(newEntryIndex, true, new MockIdentifier("state-id")));
    ...
    @Test
    void testInitiateForceInstallSnapshot() throws Exception {
        ...
        for (int i = 0; i < 4; i++) {
            log.append(new DefaultLogEntry(i, 1, new MockCommand("X" + i)));
        }
        ...
        persistence.setActor(Runnable::run);
    ...
    @Test
    void testHandleInstallSnapshotReplyLastChunk() throws Exception {
    ...
    @Test
    void testSendSnapshotfromInstallSnapshotReply() {
    ...
    @Test
    void testHandleInstallSnapshotReplyWithInvalidChunkIndex() {
    ...
    @Test
    void testHandleSnapshotSendsPreviousChunksHashCodeWhenSendingNextChunk() {
    ...
    @Test
    void testLeaderInstallSnapshotState() throws IOException {
    ...
    private @NonNull MockRaftActorContext createActorContext(final String id, final ActorRef actorRef,
        ...
        final var context = new MockRaftActorContext(id, stateDir, getSystem(), actorRef, payloadVersion);
    ...
    void testLeaderCreatedWithCommitIndexLessThanLastIndex() {
        ...
        leaderActorContext.getReplicatedLog().clear();

        //create 3 entries
        final var leaderLog = new MockRaftActorContext.Builder().createEntries(0, 3, 1).build();
        ...
        followerActorContext.getReplicatedLog().clear();
        final var followerLog = new MockRaftActorContext.Builder().createEntries(0, 3, 1).build();
    ...
    @Test
    void testLeaderCreatedWithCommitIndexLessThanFollowersCommitIndex() {
    ...
    void testHandleAppendEntriesReplyFailureWithFollowersLogBehindTheLeader() {
        ...
        var followerLog = new MockRaftActorContext.Builder().createEntries(0, 1, 1).build();
        ...
        assertEquals("Follower's first ApplyState index", 1, applyState.entry().index());
        assertEquals("Follower's first ApplyState term", 1, applyState.entry().term());
        assertEquals("Follower's first ApplyState data", leadersSecondLogEntry.command(), applyState.entry().command());

        applyState = applyStateList.get(1);
        assertEquals("Follower's second ApplyState index", 2, applyState.entry().index());
        assertEquals("Follower's second ApplyState term", 1, applyState.entry().term());
        assertEquals("Follower's second ApplyState data", leadersThirdLogEntry.command(), applyState.entry().command());
    ...
    @Test
    void testHandleAppendEntriesReplyFailureWithFollowersLogEmpty() {
        ...
        followerActorContext.resetReplicatedLog(new MockRaftActorContext.Builder().build());
        ...
        ApplyState applyState = applyStateList.get(0);
        assertEquals("Follower's first ApplyState index", 0, applyState.entry().index());
        assertEquals("Follower's first ApplyState term", 1, applyState.entry().term());
        assertEquals("Follower's first ApplyState data", leadersFirstLogEntry.command(), applyState.entry().command());

        applyState = applyStateList.get(1);
        assertEquals("Follower's second ApplyState index", 1, applyState.entry().index());
        assertEquals("Follower's second ApplyState term", 1, applyState.entry().term());
    ...
    @Test
    void testHandleAppendEntriesReplyFailureWithFollowersLogTermDifferent() {
        ...
        final long leaderCommitIndex = 1;
        final var leaderLog = new MockRaftActorContext.Builder().createEntries(0, 2, 2).build();
        leaderLog.setCommitIndex(leaderCommitIndex);
        leaderLog.setLastApplied(leaderCommitIndex);
        leaderActorContext.resetReplicatedLog(leaderLog);
        ...
        ApplyState applyState = applyStateList.get(0);
        assertEquals("Follower's first ApplyState index", 0, applyState.entry().index());
        assertEquals("Follower's first ApplyState term", 2, applyState.entry().term());
        assertEquals("Follower's first ApplyState data", leadersFirstLogEntry.command(), applyState.entry().command());

        applyState = applyStateList.get(1);
        assertEquals("Follower's second ApplyState index", 1, applyState.entry().index());
        assertEquals("Follower's second ApplyState term", 2, applyState.entry().term());
        assertEquals("Follower's second ApplyState data", leadersSecondLogEntry.command(),
            applyState.entry().command());
    ...
    @Test
    void testHandleAppendEntriesReplyWithNewerTerm() {
    ...
    @Test
    void testHandleAppendEntriesReplyWithNewerTermWhenElectionsAreDisabled() {
    ...
    @Test
    void testHandleAppendEntriesReplySuccess() {
        logStart("testHandleAppendEntriesReplySuccess");

        MockRaftActorContext leaderActorContext = createActorContextWithFollower();

        final var log = new Builder().createEntries(0, 3, 1).build();
        log.setCommitIndex(1);
        log.setLastApplied(1);
    ...
    @Test
    void testHandleAppendEntriesReplyUnknownFollower() {
    ...
    @Test
    void testFollowerCatchUpWithAppendEntriesMaxDataSizeExceeded() {
        ...
        final var leaderLog = new MockRaftActorContext.Builder().createEntries(0, 4, 1).build();
        ...
        var followerLog = new MockRaftActorContext.Builder().build();
        followerLog.setCommitIndex(-1);
    ...
    @Test
    void testHandleRequestVoteReply() {
    ...
    @Test
    void testIsolatedLeaderCheckNoFollowers() {
    ...
    @Test
    void testIsolatedLeaderCheckNoVotingFollowers() {
        ...
        assertInstanceOf(Leader.class, leader.handleMessage(leaderActor, Leader.ISOLATED_LEADER_CHECK));
    }
    ...
    @Test
    void testLaggingFollowerStarvation() {
        ...
        MockRaftActorContext leaderActorContext = new MockRaftActorContext(leaderActorId, stateDir, getSystem(),
            leaderActor);
    ...
    @Test
    void testReplicationConsensusWithNonVotingFollower() {
        logStart("testReplicationConsensusWithNonVotingFollower");

        MockRaftActorContext leaderActorContext = createActorContextWithFollower();
        ((DefaultConfigParamsImpl) leaderActorContext.getConfigParams()).setHeartBeatInterval(Duration.ofSeconds(1000));

        final var leaderLog = new MockRaftActorContext.Builder().build();
        leaderLog.setCommitIndex(-1);
    ...
    @Test
    void testTransferLeadershipWithFollowerInSync() {
        logStart("testTransferLeadershipWithFollowerInSync");

        MockRaftActorContext leaderActorContext = createActorContextWithFollower();
        ((DefaultConfigParamsImpl) leaderActorContext.getConfigParams()).setHeartBeatInterval(Duration.ofSeconds(1000));
        leaderActorContext.resetReplicatedLog(new MockRaftActorContext.Builder().build());
    ...
    @Test
    void testTransferLeadershipWithEmptyLog() {
    ...
    @Test
    void testTransferLeadershipWithFollowerInitiallyOutOfSync() {
    ...
    @Test
    void testTransferLeadershipWithFollowerSyncTimeout() {
    ...
    @Test
    void testReplicationWithPayloadSizeThatExceedsThreshold() {
        ...
        final int serializedSize = SerializationUtils.serialize(new AppendEntries(1, LEADER_ID, -1, -1,
                List.of(new DefaultLogEntry(0, 1, new MockCommand("large"))), 0, -1, (short)0)).length;
    ...
    @Test
    void testLargePayloadSlicingExpiration() {
    ...
    @Test
    void testLeaderAddressInAppendEntries() {
