...
final class ReplicatedLogImpl extends AbstractReplicatedLog<JournaledLogEntry> {
     ...
     */
    @NonNullByDefault
    private abstract static class UncheckedPersistCallback extends PersistCallback {
        @Override
    ...
    @NonNullByDefault
    private abstract static class AppendCallback extends UncheckedPersistCallback {
        final Consumer<? super JournaledLogEntry> callback;
        final JournaledLogEntry entry;

        AppendCallback(final JournaledLogEntry entry, final Consumer<? super JournaledLogEntry> callback) {
    ...
    @NonNullByDefault
    private final class AppendReceivedCallback extends AppendCallback {
        ...
        void invoke(final long journalIndex) {
            invokeSync(entry, () -> callback.accept(entry));
        }
    }

    @NonNullByDefault
    private final class AppendSubmittedCallback extends AppendCallback {
    ...
    public boolean appendReceived(final LogEntry entry, final Consumer<LogEntry> callback) {
        ...
        if (appendImpl(adopted)) {
            context.entryStore().persistEntry(adopted, new AppendReceivedCallback(adopted, callback));
        }
    ...
    public void markLastApplied() {
        context.entryStore().markLastApplied(lastAppliedJournalIndex());
    }
