...
public interface EntryStore {
     ...
     * before {@link RaftActor} sees any other message. Default implementation registers the callback with
     * {@link #completer()} and defers to {@link #startPersistEntry(ReplicatedLogEntry, RaftCallback)}.
     *
     * @param entry the journal entry to persist
     * @param callback the callback when persistence is complete
     */
    default void persistEntry(final ReplicatedLogEntry entry, final RaftCallback<Long> callback) {
        startPersistEntry(requireNonNull(entry), completer().syncWithCurrentMessage(callback));
    }
