 ...
 * <p>Leaders:
 * <ul>
 *   <li>Upon election: send initial empty AppendEntries RPCs (heartbeat) to each server; repeat during idle periods to
 *       prevent election timeouts (§5.2)</li>
 *   <li>If command received from client: append entry to local log, respond after entry applied to state machine
 *       (§5.3)</li>
 *   <li>If last log {@code index ≥ nextIndex} for a follower: send * AppendEntries RPC with log entries starting at
 *       nextIndex</li>
 *   <li>If successful: update nextIndex and matchIndex for * follower (§5.3)</li>
 *   <li>If AppendEntries fails because of log inconsistency: decrement nextIndex and retry (§5.3)</li>
...
public non-sealed class Leader extends AbstractLeader {
    ...
    Leader(final RaftActorContext context, final IsolatedLeader initializeFromLeader) {
        super(context, RaftRole.Leader, requireNonNull(initializeFromLeader));
    }
