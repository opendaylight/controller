...
public final class EntryJournalV1 {
     ...
     */
    private record MetaEntry(long replayFrom, long applyTo) {
        // nothing else
    }
    ...
    private static final FromByteBufMapper<MetaEntry> META_ENTRY_READER = (index, bytes) -> {
        try (var in = new ByteBufInputStream(bytes)) {
            ...
            return new MetaEntry(WritableObjects.readFirstLong(in, hdr), WritableObjects.readSecondLong(in, hdr));
        } catch (IOException e) {
    ...
    public EntryJournalV1(final String memberId, final Path directory, final CompressionType compression,
        ...
        LOG.info("{}: journal open: firstIndex={} lastIndex={}", memberId, entryJournal.firstIndex(),
            entryJournal.lastIndex());
    }
