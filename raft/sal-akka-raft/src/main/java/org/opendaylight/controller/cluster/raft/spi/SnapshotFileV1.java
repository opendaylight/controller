...
final class SnapshotFileV1 implements SnapshotFile {
    ...
    static <T extends Snapshot.State> SnapshotFileV1 create(final Path file, final EntryInfo lastIncluded,
            ...
            final CompressionSupport stateCompress, final RaftActorSnapshotCohort<T> stateSupport, final T state)
                throws IOException {
        final var ec = switch (entryCompress) {
            case NONE -> COMPRESS_NONE;
            case LZ4 -> COMPRESS_LZ4;
            default -> throw new UnsupportedOperationException("Unsupported entry compression " + entryCompress);
        };
        final var sc = switch (stateCompress) {
            case NONE -> COMPRESS_NONE;
            case LZ4 -> COMPRESS_LZ4;
            default -> throw new UnsupportedOperationException("Unsupported sate compression " + stateCompress);
        };

        // FIXME: Sanity check unapplies entries

        final var headerBytes = new byte[HEADER_SIZE];
        final long sso;
        final long limit;

        try (var fos = Files.newOutputStream(file, WRITE, CREATE, TRUNCATE_EXISTING, NOFOLLOW_LINKS)) {
            try (var cos = new CountingOutputStream(new Uncloseable(fos))) {
                // Zero-out the header first
                cos.write(headerBytes);
                ...
                // record SSO, emit state and record limit
                sso = cos.getCount();

                try (var eos = entryCompress.encodeOutput(cos)) {
                    stateSupport.serializeSnapshot(state, eos);
                }
                limit = cos.getCount();
            }
        }
        ...
        final var header = ByteBuffer.wrap(headerBytes).order(ByteOrder.LITTLE_ENDIAN);
        header.putInt(MAGIC_BITS)
            .put((byte) 0x00).put((byte) (ec & COMPRESS_MASK)).put((byte) (sc & COMPRESS_MASK)).put((byte) 0x00)
            .putLong(lastIncluded.index()).putLong(lastIncluded.term())
            .putLong(sso).putLong(limit)
        ...
        // write out header
        try (var fos = Files.newOutputStream(file, WRITE, SYNC, NOFOLLOW_LINKS)) {
            fos.write(headerBytes);
        }

        return new SnapshotFileV1(file, lastIncluded, timestamp, entryCompress, stateCompress, sso, limit);
    }

    static SnapshotFileV1 open(final Path file) throws IOException {
        try (var fc = FileChannel.open(file, READ, NOFOLLOW_LINKS)) {
    ...
    public ServerState readServerState() throws IOException {
        try (var dis = new DataInputStream(ByteStreams.limit(Files.newInputStream(file, READ, NOFOLLOW_LINKS), sso))) {
